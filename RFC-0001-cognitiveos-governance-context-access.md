...
以下伪 schema 表示规范语义。治理对象族的已登记机器 schema 见 [governed-object-contract](./docs/standards/governed-object-contract.md) 与 `specs/schemas/`；登记字段名与此处示意可能不同（如 `owner_ref`/`authority_ref`/`resource_scope_ref`），机器形状以登记 schema 为准，本文伪 schema 只约束语义。
...
[REQ-KNOW-004] 模型或生成内容 **MUST NOT** 绕过独立 admission/publish authority；生成内容、同一模型改写、循环引用或下游再次引用 **MUST NOT** 作为独立 corroboration 或直接发布。

[REQ-KNOW-007] `Query` **MUST** 在候选泄露前过滤，并在返回正文前逐对象重新授权。

[REQ-KNOW-008] `Lint` **MUST** 覆盖 deterministic、policy、semantic、security 四层并保存版本化结果。
...
[REQ-KNOW-005] 维护 loop **MUST** 有界并对超界、部分重编译与发布原子性给出机器状态。

[REQ-KNOW-006] source deletion/legal hold **MUST** 区分保留义务与读取授权，并证明派生 closure 或列出无法删除的边界。

### 9.5 持久投毒防御与符合性

实现 **MUST** 测试 ingestion sandbox、压缩/递归上限、MIME/签名、内容/控制隔离、跨源独立性、scope/sensitivity 继承、发布职责分离、删除传播和发布后异常监控。性能和质量按 architecture §19.4 的 BenchmarkManifest 报告 claim precision/recall、citation coverage、unsupported-claim、contradiction detection、stale exposure、invalidation latency、recompile、deletion closure 与 poison admission/escape（指标定义见 architecture §18.3），并给出分母、窗口、p50/p95/p99 和置信区间。
...
符合性声明 **MUST** 固定 RFC 版本、适用范围、实现版本、策略模型、迁移状态和测试证据。所有适用 MUST 必须通过，或逐项声明不支持；`planned`、`experimental` 和人工演示不算通过。声明 **MUST NOT** 把本文伪 schema 当作机器 schema，也只能引用实际登记的 registry 条目和 conformance vector；当前 performance 资产、治理对象 schema 与知识向量已登记，知识对象 schema 仍未登记。

## 18. 受治理 Memory 与 Cognitive Discovery 扩展
本节将既有统一治理字段应用到在线 memory 与资源发现；详细 lifecycle 由 Governed Memory/Cognitive Discovery companion 定义。

MemoryObject 至少固定 memory kind、governance domain、ResourceScope、owner/authority、principal/Conversation/Task/Episode、purpose/sensitivity/compartment、source evidence、valid/observed/ingest time、confidence method、conflict、retention/expiry、representation、admission 与 supersedes/invalidates。模型输出和 Agent 自述只能形成 MemoryCandidate。

`Observation → MemoryCandidate → Validate/Policy/Conflict → MemoryAdmissionDecision → MemoryObject`。跨 scope/kind 晋升创建新对象、版本和 decision；撤销/删除沿 lineage 使 embedding/index/summary/cache/view 失效。owner 或创建者不自动拥有修改其他 authority 对象的权限。

CognitiveResourceManifest 与 ContextViewDelta 绑定 ActivityContext、ActorChain、Conversation/ResourceScope、purpose、policy/membership/revocation、budget 和 expiry。discover 与 read 分离；获取 ref、标题或 OperationSummary 均不是正文/调用权限。安全拒绝不得区分隐藏对象不存在与不可发现。
