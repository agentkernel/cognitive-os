    ...
    S: AuthorityStore + HarnessStore + IntentChainStore + ProtocolStore,
{
    ...
    if contract.header.schema_version != TASK_CONTRACT_EXECUTION_SCHEMA_VERSION {
        return Err(SchedulerAuthorityError::LegacyContract(
            contract.header.schema_version,
        ));
    }

    let loop_object_id = ObjectId::parse(&contract.loop_object_id.0)
        .map_err(|error| SchedulerAuthorityError::MalformedContract(error.to_string()))?;
    let budget_id = BudgetId::parse(&contract.budget_id.0)
        .map_err(|error| SchedulerAuthorityError::MalformedContract(error.to_string()))?;
    ...
    Ok(SchedulerCeilingFacts {
        deadline: Some(contract.deadline),
        retry_count,
