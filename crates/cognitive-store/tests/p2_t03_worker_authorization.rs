...
#[test]
fn bound_wia_handoff_rejects_contract_epoch_mismatch_without_consuming_authority() {
    let temporary_directory = tempfile::tempdir().unwrap();
    let authority_database_path = temporary_directory.path().join("authority.db");
    let store = SqliteAuthorityStore::open(&authority_database_path)
        .expect("fresh authority database opens");
    let authorization_id = object_id(750);
    let task_ref = "task://personal/contract-epoch-mismatched-worker-handoff";
    let connection = Connection::open(&authority_database_path).expect("open authority fixture");
    connection
        .execute(
            "INSERT INTO worker_iteration_authorizations
               (authorization_id, worker_authorization_root_id, task_ref, contract_epoch,
                ...
                intent_id, effect_object_id, budget_id, budget_charge_json,
                action_fingerprint, issued_fencing_epoch, canonical_json)
             VALUES (?1, ?2, ?3, 2, ?4, 1, 1, ?5, ?6, ?7, ?8, ?9, ?10, 1, ?11)",
            rusqlite::params![
                authorization_id.as_str(),
                object_id(751).as_str(),
                task_ref,
                object_id(752).as_str(),
                object_id(753).as_str(),
                object_id(754).as_str(),
                object_id(755).as_str(),
                budget_id(756).as_str(),
                "{\"tool_calls\":1}",
                "contract-epoch-mismatched-worker-handoff",
                "{\"worker_authorization\":1}",
            ],
        )
        .expect("seed WIA from successor task contract epoch");
    connection
        .execute(
            "INSERT INTO scheduler_entries
               (task_ref, contract_epoch, state, lease_owner, lease_epoch, lease_expires,
        ...
        .expect("seed active scheduler lease from current task contract epoch");
    drop(connection);
    ...
    let error = store
        .consume_worker_iteration_authorization_bound_to_scheduler_lease(
            &BoundWorkerAuthorizationConsumption {
                consumption: WorkerIterationAuthorizationConsumptionRow {
                    authorization_id,
                    worker_attempt_id: object_id(757),
                    consumed_fencing_epoch: 1,
                    consumed_at: WallTimestamp::parse("2026-08-04T12:01:00Z").unwrap(),
                    canonical_json: "{\"worker_authorization_consumption\":1}".to_owned(),
                },
        ...
        .expect_err("a WIA from another contract epoch cannot receive the active lease");
    assert!(matches!(error, StorePortError::Conflict { .. }));
    assert!(
        store
            .list_consumed_worker_iteration_authorizations()
            .unwrap()
            .is_empty(),
        "an epoch-mismatched handoff must not persist a partial consumption"
    );
}

#[test]
fn bound_wia_handoff_rejects_runnable_work_without_consuming_authority() {
    let temporary_directory = tempfile::tempdir().unwrap();
    let authority_database_path = temporary_directory.path().join("authority.db");
    let store = SqliteAuthorityStore::open(&authority_database_path)
        .expect("fresh authority database opens");
    let authorization_id = object_id(760);
    let task_ref = "task://personal/runnable-worker-handoff";
    let connection = Connection::open(&authority_database_path).expect("open authority fixture");
    connection
        .execute(
            "INSERT INTO worker_iteration_authorizations
            ...
            rusqlite::params![
                authorization_id.as_str(),
                object_id(761).as_str(),
                task_ref,
                object_id(762).as_str(),
                object_id(763).as_str(),
                object_id(764).as_str(),
                object_id(765).as_str(),
                budget_id(766).as_str(),
                "{\"tool_calls\":1}",
                "runnable-worker-handoff",
                "{\"worker_authorization\":1}",
            ],
        )
        .expect("seed immutable WIA");
    ...
    let error = store
        .consume_worker_iteration_authorization_bound_to_scheduler_lease(
            &BoundWorkerAuthorizationConsumption {
                consumption: WorkerIterationAuthorizationConsumptionRow {
                    authorization_id,
                    worker_attempt_id: object_id(767),
                    consumed_fencing_epoch: 1,
                    consumed_at: WallTimestamp::parse("2026-08-04T12:01:00Z").unwrap(),
                    canonical_json: "{\"worker_authorization_consumption\":1}".to_owned(),
                },
        ...
        .expect_err("runnable scheduler work cannot authorize a worker handoff");
    assert!(matches!(error, StorePortError::Conflict { .. }));
    assert!(
        store
            .list_consumed_worker_iteration_authorizations()
...
#[test]
fn bound_wia_handoff_rejects_stale_fencing_epoch_without_consuming_authority() {
    let temporary_directory = tempfile::tempdir().unwrap();
    let authority_database_path = temporary_directory.path().join("authority.db");
    let store = SqliteAuthorityStore::open(&authority_database_path)
        .expect("fresh authority database opens");
    let authorization_id = object_id(780);
    let task_ref = "task://personal/stale-fencing-worker-handoff";
    let connection = Connection::open(&authority_database_path).expect("open authority fixture");
    connection
        .execute(
            "INSERT INTO worker_iteration_authorizations
            ...
            rusqlite::params![
                authorization_id.as_str(),
                object_id(781).as_str(),
                task_ref,
                object_id(782).as_str(),
                object_id(783).as_str(),
                object_id(784).as_str(),
                object_id(785).as_str(),
                budget_id(786).as_str(),
                "{\"tool_calls\":1}",
                "stale-fencing-worker-handoff",
                "{\"worker_authorization\":1}",
            ],
        )
        .expect("seed immutable WIA");
    ...
    assert_eq!(
        ...
        2,
        "the fixture advances the daemon fence after the lease was acquired"
    );
    ...
    let error = store
        .consume_worker_iteration_authorization_bound_to_scheduler_lease(
            &BoundWorkerAuthorizationConsumption {
                consumption: WorkerIterationAuthorizationConsumptionRow {
                    authorization_id,
                    worker_attempt_id: object_id(787),
                    consumed_fencing_epoch: 1,
                    consumed_at: WallTimestamp::parse("2026-08-04T12:01:00Z").unwrap(),
                    canonical_json: "{\"worker_authorization_consumption\":1}".to_owned(),
                },
        ...
        .expect_err("a stale daemon fence cannot consume a worker authorization");
    assert!(matches!(error, StorePortError::Conflict { .. }));
    assert!(
        store
            .list_consumed_worker_iteration_authorizations()
            .unwrap()
            .is_empty(),
        "a stale-fenced handoff must not persist a partial consumption"
    );
}
