...
fn current_context_source(identifier: &ObjectId) -> WorkspaceContextSourceRow {
    let payload = json!({
        "header": {
            "id": identifier.as_str(), "type": "WorkspaceContextSource", "schema_version": "cognitiveos.context/0.1", "object_version": 1,
            "scope_domain": "tenant", "tenant_id": "00000000-0000-7000-9000-0000000000f1", "resource_scope_ref": {"kind":"strong","id":"00000000-0000-7000-9000-000000000101","object_version":1,"content_digest":format!("sha256:{}", "a".repeat(64))}, "owner_ref": {"kind":"strong","id":"00000000-0000-7000-9000-000000000102","object_version":1,"content_digest":format!("sha256:{}", "a".repeat(64))}, "authority_ref": {"kind":"strong","id":"00000000-0000-7000-9000-000000000103","object_version":1,"content_digest":format!("sha256:{}", "a".repeat(64))}, "policy_refs": [], "purpose_constraints": ["task_execution"], "sensitivity": "internal", "compartments": [], "retention": {"policy":"standard","expires_at":null,"legal_hold":false}, "provenance": {"created_by":"principal://tenant-a/daemon","source_refs":[]}, "lineage": {"parents":[],"transform":"memory-store-test"}, "content_digest":format!("sha256:{}", "0".repeat(64)), "created_at":"2026-08-09T00:00:00Z", "valid_time":{"from":"2026-08-09T00:00:00Z","until":null}
        },
        "tenant_id":"tenant-a", "owner_ref":"principal://tenant-a/daemon", "resource_scope":"workspace://tenant-a/project", "conversation_ref":null, "role":"working", "trust_level":"verified", "representation":"text", "provenance_ref":"source://context/1", "content_bytes":12, "content_tokens":3, "body":{"text":"current source"}
    });
...
#[test]
fn admitted_current_source_creates_an_immutable_memory_object() {
    let (_directory, store) = fresh_store();
    let source_identifier = object_id(21);
    let source = current_context_source(&source_identifier);
    store.append_workspace_context_source(&source).unwrap();
    let mut candidate = memory_candidate(&object_id(22), &source_identifier);
    candidate.source_digest = source.source_digest.clone();
    let decision = MemoryAdmissionDecisionRow {
        decision_id: object_id(23),
    ...
    store
        .append_memory_admission(&candidate, &decision, Some(&memory_object))
        .unwrap();
    assert_eq!(
