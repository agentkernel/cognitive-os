...
use cognitive_domain::{
    BudgetId, EventId, LifecycleDomain, ObjectId, ReasonCode, StateName, UriRef, Version, WallTimestamp,
    table,
};
...
fn contract_cmd(contract_n: u64, task_ref: &str) -> Result<TaskContractCommand, ExecError> {
    Ok(TaskContractCommand {
        ...
        max_iterations: 8,
        max_retries: 3,
        deadline: WallTimestamp::parse("2026-08-02T12:00:00Z").map_err(|error| env_err(error.to_string()))?,
        loop_object_id: oid(contract_n + 100)?,
        budget_id: BudgetId::parse(&format!("00000000-0000-7000-b000-{contract_n:012x}")).map_err(|error| env_err(error.to_string()))?,
