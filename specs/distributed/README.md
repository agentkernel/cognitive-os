...
[REQ-CAP-003] 跨节点写操作 **MUST** 使用 lease/expiry、撤销 epoch 与 fencing。（复述；权威定义见 [Core §8](../core/README.md)）
...
[REQ-CAP-002] 派生 capability **MUST** 单调衰减。（复述；权威定义见 [Core §8](../core/README.md)）

[REQ-RES-002] 子预算 **MUST** 来自父级可用额度且不可放大。（复述；权威定义见 [Core §11](../core/README.md)）
