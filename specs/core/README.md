...
- `owner`（机器字段 `owner_ref`）：生命周期责任主体；

- `authority`（机器字段 `authority_ref`）：最终写入或仲裁主体/协议；
...
ContextView 包含 governed header（统一元数据，§3.2）、loaded、rejected、loss_declaration、pinned_versions、cost、lineage、complete 和 activity binding。
...
相对顺序稳定是必要而非充分条件：供应商缓存按**累计前缀字节**匹配，在既有条目之前插入任何新字节都会使其后全部缓存段失效。因此渲染器 **SHOULD** 把稳定内容组织为 immutable prefix segments 并只在尾部追加可变内容（append-only suffix），或声明显式 cache breakpoint 边界；同 Conversation 未变绑定下，已渲染前缀段的字节 **MUST NOT** 因新增无关对象而改变（前缀段字节稳定性，CTX-RENDER-001 断言之一）。
...
[REQ-EFF-002] 执行调用 **MUST** 携带稳定 idempotency key、参数 digest、授权摘要和适用的 fencing token；同一幂等键携带不同参数 digest 的请求 **MUST** 被拒绝并返回 `EFFECT_IDEMPOTENCY_CONFLICT`（见 [errors.yaml](../registry/errors.yaml)），不得去重为同一 Effect、静默改写参数或执行为新动作。
...
对账结果是 Effect 对象上的持久字段（[effect.schema.json](../schemas/effect.schema.json) `reconciliation_result`），与迁移表 guard `reconciliation_result_equals_*` 一一对应；`observed_outcome` 是观察事实，对账不得改写它——经 `OUTCOME_UNKNOWN` 对账确认已执行并最终提交的 Effect，其 `observed_outcome` 合法地保持 `unknown|not_observed`，提交判定依据是 `reconciliation_result=executed` 加当前验证证据，而不是观察字段被改成成功。

验证过期的唯一表达：VerificationReport 不可变（其 `status` 恒为 `passed|failed|indeterminate` 评定结论）；`PASSED→EXPIRED` 是 Verification 生命周期事实（[verification.transitions.json](../transitions/verification.transitions.json)），由失效事件承载，不改写既有 report。Effect 内嵌 `verification.status` 是当前 standing 投影，report 失效后投影为 `expired`，commit guard `verification_still_current` 拒绝非当前验证；重验产生新 VerificationReport，或按 `VERIFICATION_EXPIRED` 进入 `VERIFY_FAILED`。
...
版本与状态：`STATE_CONFLICT`、`STATE_STALE`、`STATE_AUTHORITY_UNAVAILABLE`、`STATE_STORE_UNAVAILABLE`（已登记：提交路径存储失败必须 fail closed，见 REQ-REC-003）、`SNAPSHOT_INVALID`。
...
效果：`EFFECT_DENIED`、`OUTCOME_UNKNOWN`、`VERIFY_FAILED`、`COMPENSATION_FAILED`、`QUARANTINED`、`EFFECT_IDEMPOTENCY_CONFLICT`（已登记：同键异参拒绝，见 REQ-EFF-002）。
...
[REQ-REC-003] 权威提交路径（事件日志、状态快照、Effect 记录）不可持久化时，受治理写入 **MUST** fail closed 并返回 `STATE_STORE_UNAVAILABLE`；实现 **MUST NOT** 把仅存于内存的缓冲当作已提交状态，**MUST NOT** 在 Intent 未持久化时分派新的 governed_external 执行。存储降级期间，只读检查、确定性停止/撤销路径与已提交历史的完整性必须保持可用；恢复遵循 recovery barrier 与重放顺序。
...
[REQ-PROFILE-CVM-001] 声明 `context_virtualization` Profile 的实现 **MUST** 覆盖 `REQ-CTX-001`–`REQ-CTX-012` 的全部适用要求，并提供下列负例的测试证据：required 超预算 fail closed、授权先于正文暴露（含 ranker/renderer）、缓存与派生视图不绕过撤销、注入内容不得提升为控制。
