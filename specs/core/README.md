...
规范状态为 `PROPOSED`、`AUTHORIZED`/`DENIED`、`EXECUTING`、`EXECUTED`/`NOT_EXECUTED`/`OUTCOME_UNKNOWN`、`VERIFIED`/`VERIFY_FAILED`、`COMMITTED`/`ABORTED`/`QUARANTINED`，以及可选 `COMPENSATING`。
