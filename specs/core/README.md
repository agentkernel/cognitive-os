...
| [RFC-0001：治理上下文、访问与知识编译](../../RFC-0001-cognitiveos-governance-context-access.md) | v0.2 Draft / Normative Companion RFC | Tenant、Membership、Principal、ActorChain、Conversation、ResourceScope、ShareGrant、短期 privileged management session、混合授权、认知准入、缓存/检索隔离与执行上下文 | 治理/知识对象仍为伪 schema；performance 与 management session/proposal/approval schema、registry 条目及 declarative vector 的实际登记各自决定机器边界，均不表示实现 |
...
- RFC-0001 的治理/知识对象代码块仍是伪 schema；实际提交的 performance schema、registry 与 vector 可被固定 digest 后引用，但不证明实现。
...
World State 是外部世界的受版本治理投影；传感器和工具默认仅为输入源。
...
规范状态为 `PROPOSED`、`AUTHORIZED`/`DENIED`、`EXECUTING`、`EXECUTED`/`NOT_EXECUTED`/`OUTCOME_UNKNOWN`、`RECONCILED`、`VERIFIED`/`VERIFY_FAILED`、`COMMITTED`/`ABORTED`/`QUARANTINED`，以及可选 `COMPENSATING`。
...
### 9.5 Effect 状态闭包
`RECONCILED` 是对执行事实完成查询/观察归并后的持久状态，不等于成功；其结果字段必须明确 `executed|not_executed|still_unknown`。合法主链为 `EXECUTING→EXECUTED→RECONCILED→VERIFIED|VERIFY_FAILED→COMMITTED|ABORTED`；`OUTCOME_UNKNOWN` 只能经对账进入 `RECONCILED`、独立补偿或 `QUARANTINED`。`NOT_EXECUTED` 可经 `RECONCILED` 证据终止为 `ABORTED`，不得验证为成功。
...
错误至少包含 `code`、`stage`、`retryable`、`details_ref`、`observed_versions`、`correlation_id` 和可选 `retry_after`。
...
本 v0.2 为 companion draft；performance schema/vector 已存在，治理与知识对象仍主要是伪 schema；registry、测试与实现状态由各自资产单独声明。
