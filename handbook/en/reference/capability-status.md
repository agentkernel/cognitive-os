...
| Governed tool execution (all six registered families) | partial | every family has an assembled executor, so the projection reports `execution_ready`; WorkspaceRead and query-bearing WorkspaceSearch now have the periodic production caller, while the other four still lack a production request carrier and remain test-called only |
| Workspace write/patch executors | partial | handle-relative no-follow traversal/publication, bounded preimages, target-locked CAS, workspace-external durable key-bound receipts and restart orphan recovery are tested on Linux/Windows; no production caller |
