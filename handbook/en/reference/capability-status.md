...
| Governed tool execution (all six registered families) | partial | every family has an assembled executor, so the projection reports `execution_ready`; WorkspaceRead, query-bearing WorkspaceSearch, and preimage-bearing WorkspaceWrite/Patch now have the periodic production caller, while ProcessCheck and HttpFetchReadOnly still lack a production request carrier and remain test-called only |
