...
| `crates/cognitive-store` | SQLite WAL 适配器：迁移 v1–v24（安装库 v1–v4）、调度 lease、Memory/Skill/Context/Artifact 存储、备份规划 | `sqlite/`、`migration.rs`、`personal_db.rs`（`prepare_personal_databases`）、`scheduler.rs` |
