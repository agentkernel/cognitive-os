...
1. **执行器接线仍为 partial**：六个已登记族都有已装配 sink（P2-T10），所以
   ...
   WorkspaceRead 与带 query 的 WorkspaceSearch（query 携带于持久 Intent）经持久
   Effect 协议从生产派发。WorkspaceWrite/Patch、ProcessCheck 与 HttpFetchReadOnly
   在生产尚无独立治理的 preimage/payload、受监督进程或已登记 origin 载体，因此在
   Effect 授权前失败；这些 sink 仍仅测试调用。
