...
| ProcessCheck 执行器 | implemented，仅测试调用 | 每个 sink 都重查不可变目录完全相等；有界 process-tree 监督仍仅测试调用 |
...
1. **执行器接线仍为 partial**：六个已登记族都有已装配 sink（P2-T10），所以
   ...
   WorkspaceRead、带 query 的 WorkspaceSearch 与带 preimage 的 WorkspaceWrite/Patch
   （query、payload 与期望 preimage 均携带于持久 Intent）经持久 Effect 协议从生产派发。
   ProcessCheck 与 HttpFetchReadOnly 在生产尚无独立治理的受监督进程或已登记 origin
   载体，因此在 Effect 授权前失败；这些 sink 仍仅测试调用。
