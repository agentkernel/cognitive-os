...
| `cognitive dsh apply` | 把 Cos dsh binding 发布为 Path B selected-model（`POST /personal/dsh/runtime` `op=apply`），并只重启 Cos 安装的原生 web 进程对，使对话与 Models 显示 Cos 指定模型。聊天走绑定账户（Cos 指定 grok 时绝不会发到 DeepSeek） |
