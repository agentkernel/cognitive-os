...
| SecretStore | implemented（Linux Secret Service） | headless vault 为 designed；其他 OS 不可用 |
...
| 受治理工具执行（workspace 读 / 进程检查） | partial | 执行器仅测试调用 |
...
| Windows/macOS 产品 | unavailable | 仅 Linux x86_64 |
