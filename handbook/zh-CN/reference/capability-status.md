...
| 受治理工具执行（全部六个已登记族） | partial | 六族都有已装配 executor，投影因此报告 `execution_ready`；WorkspaceRead、带 query 的 WorkspaceSearch 与带 preimage 的 WorkspaceWrite/Patch 现有周期生产调用者，ProcessCheck 与 HttpFetchReadOnly 仍缺生产请求载体且仅测试调用 |
