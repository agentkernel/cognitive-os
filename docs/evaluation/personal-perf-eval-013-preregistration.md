...
- Freeze checkpoint:
  [20260820-personal-perf-eval-013-preregistration.md](../checkpoints/20260820-personal-perf-eval-013-preregistration.md)
- Running report:
  [personal-performance-assessment-20260820-eval-013.md](personal-performance-assessment-20260820-eval-013.md)

This is a **new freeze**. Closed EVAL-002 and EVAL-004 through EVAL-012 are
never resumed.
...
1. Product follow-ups that would otherwise force B0 fail or C1/C2 `not-run`
   are closed: P2-T38, P9-T09, P9-T10, P9-T11. **pass.**
2. Owner sets the Current snapshot `Owner-directed campaign` row to
   `PERSONAL-PERF-EVAL-013`. **pass** (owner: continue the execution plan
   after P9-T11; 2026-08-20).
3. An evaluation lease `lease/personal/EVAL-013/<purpose>` owns only
   `docs/evaluation/`, `docs/checkpoints/`, and `docs/plan/PROGRESS.md`.
   **pass** (`lease/personal/EVAL-013/execution-plan-b0`).
...
| Root | `/home/hal9001/perfeval013-20260820` |
| Daemon | `127.0.0.1:48302` |
| P-arm broker | `127.0.0.1:48402` |
| SecretStore | new item `/25` (≠ `/12`–`/19` / `/24`) |
| Git revision | `6c415625c0ca12e9f8e51d139da601eb475ee231` (pushed `origin/main` after P9-T11 lease close). Product merge: PR [#251](https://github.com/agentkernel/cognitive-os/pull/251) at `49b66200`. |
...
Do not reuse closed EVAL ports `48286`–`48298` / `48300` / `48386`–`48398` /
`48383` / `48400`, SecretStore `/12`–`/19` / `/24`, or P2-T37 roots.
...
C1/C2 overlay packages 15–17 under EVAL-012 remain closed measurement and
must not be resumed. B0 on this freeze uses one qualification seed per class
C1, C2a, C2b, C2c, C2d; three warmups per arm; secret scan; tool-equivalence;
timeout; cleanup; no claim samples. `retry=0`. Missing runner or capability
is `not-run` / `not_available`. B5 24 h stays conditional. B6 stays later-only.
...
Counted C1/C2 envelope (same EVAL-012 style; new seeds/roots):
...
Stop campaign daemon `48302` and broker `48402` only. Clear only the
