...
- **激活条件**（全部满足才可规划实现里程碑）：依赖组 1/2/7 交付 + M5 出口评审通过 + [平台实现 gate](https://github.com/agentkernel/cognitiveos-clients/blob/main/governance/readiness-gates.md#console-实现-gate) 中目标平台 PoC 用**真实 API**（禁 mock 冒充）出具可复现实测报告。
