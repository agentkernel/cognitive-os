...
本目录记录 CognitiveOS Console 的 macOS 与受限 Linux 独立产品切片。它不新增或修改任何 CognitiveOS `REQ-*`、错误码、schema、transition table、conformance vector 或实现代码，也不表示 Console 实现已经启动。
...
- [平台产品决策记录](./platform-decision-log.md)
...
- runner：74 个向量均为 `not-run`；
- Console 平台端到端测试：未执行；
- macOS/Linux Console Profile：未符合。
