...
- **状态：** 以正式台账为准（`in-progress`，2026-09-03 领取 `P0-T09/D01`；lease `lease/personal/P0-T09/drift-checks`；[running report](../checkpoints/2026-09-03-personal-p0-t09-drift-checks-report.md)）。来源：`DOC-AGENT-RULES`（PR #306）复审「发现但未修改」第 1/4/6 项；`DOC-P13-DRIFT-FIX` 是其前置（先对齐再上校验；已 done，PR #309）。
