...
- **状态：** 以正式台账为准（2026-09-03 验收已满足，PR 合并后在 `main` 收口为 `done`；PR [#312](https://github.com/agentkernel/cognitive-os/pull/312)；[running report](../checkpoints/2026-09-03-personal-p0-t09-drift-checks-report.md)；[closure](../checkpoints/2026-09-03-personal-p0-t09-drift-checks-closure.md)）。来源：`DOC-AGENT-RULES`（PR #306）复审「发现但未修改」第 1/4/6 项；`DOC-P13-DRIFT-FIX` 是其前置（先对齐再上校验；已 done，PR #309）。
