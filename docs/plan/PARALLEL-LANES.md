...
|---|---|---|---|
| `crates/cognitive-contracts`、`packages/contracts-ts`、`tests/golden/`、`specs/schemas/`（迁移期） | Lane-CTR | `lane/ctr-v02-ordinary-core-refactor` | ADR-0014 Ordinary Core/High-Assurance split：五项 Core 与 in-process AUDIT lower tracer gate open；本批 docs-only，不登记 machine assets；合并后 Lane-RUN 实现内部 tracer types/tests |
...
| `crates/cognitive-runtime`、`cognitive-management`、`cognitive-akp`、`apps/kernel-server`、`apps/admin-cli` | Lane-RUN | `lane/run-v02-durable-audit-wiring` | Ordinary Core durable file AUDIT + `admin-cli inspect` mandatory release-gate wiring implemented/tested; future HTTP/API inspect must reuse gate; no registered assets or Profile claim |
