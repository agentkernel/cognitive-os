...
|---|---|---|---|---|---|---|---|
| `lease/personal/P2-T16/registered-check-run` | P2-T16/D01-D03 registered deterministic check execution | Lane-P2 | `personal/P2-T16-registered-check-run` | `apps/kernel-server/Cargo.toml`; `apps/kernel-server/src/main.rs`; `apps/kernel-server/src/personal/{mod.rs,server.rs,verification_executor.rs}`; `apps/kernel-server/src/personal/registered_check/**`; `apps/kernel-server/src/personal/scheduler_authority/{dispatch.rs,effect.rs,error.rs,tests.rs,types.rs,worker.rs}`; `apps/kernel-server/src/personal/tool_executor/{mod.rs,process.rs,router.rs,state.rs,tests.rs,types.rs,validate.rs}`; `apps/kernel-server/tests/p2_t16_registered_check.rs`; `crates/cognitive-kernel/src/{lib.rs,tool_registry.rs}`; `tests/fixtures/p2_t16_registered_check/**`; `tools/src/generate-handbook.mjs`; `docs/plan/{PARALLEL-LANES.md,PERSONAL-DEVELOPMENT-PLAN.md,PROGRESS.md}`; `docs/checkpoints/20260813-personal-p2-t16-registered-check-validation-report.md`; `handbook/{en,zh-CN}/developer/{architecture-overview.md,daemon-and-http.md,task-pipeline.md,execution-chain-status.md}`; `handbook/{en,zh-CN}/reference/{http-api.md,tool-catalog.md,capability-status.md}`; `handbook/{en,zh-CN}/user/{known-limitations.md,operations-and-recovery.md,six-resources.md,tasks-and-execution.md,what-is-personal.md}`; `handbook/{en,zh-CN}/ai/validation-commands.md`; `handbook/_meta/{annotations/tool-catalog.json,fingerprints.json}` | cursor-p2t16-checkrun-20260813 | 2026-08-13 / 2026-08-14 | active |
