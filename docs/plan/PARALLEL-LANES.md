...
|---|---|---|---|
| `crates/cognitive-contracts`、`packages/contracts-ts`、`tests/golden/`、`specs/schemas/`（迁移期） | Lane-CTR | `lane/ctr-v02-ordinary-core-audit-safe-reason-freeze` | ADR-0014 Ordinary Core/High-Assurance split：replacement review-only `status.inspect` AUDIT candidate closes `safe_reason` over registered public errors; new independent final-byte review pending; no machine assets registered |
...
| `crates/cognitive-runtime`、`cognitive-management`、`cognitive-akp`、`apps/kernel-server`、`apps/admin-cli` | Lane-RUN | `lane/run-v02-inspect-api-hardening` | Ordinary Core unaudited inspect is crate-private; external Rust consumers only see audited release-gate API; durable CLI path already wired; no registered assets or Profile claim |
