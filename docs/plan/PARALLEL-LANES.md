...
| P1-T09 implementation continuation | Lane-RUN | next task-correct `lane/personal-p1-t09-*` branch | declare exact paths when claimed; cohesive runtime, CLI, test and documentation paths are allowed when no active lease overlaps | unclaimed | — | available after governance closure |
