...
|---|---|---|---|
| `crates/cognitive-contracts`、`packages/contracts-ts`、`tests/golden/`、`specs/schemas/`（迁移期） | Lane-CTR | `lane/ctr-config-authority-spec-correction` | D-016/D-022 spec-correction **NO-GO / deferred-to-v0.2**；operation/payload/target、signature、audit 需新增 normative/wire surface，继续阻断 CA-1～CA-8；未改 normative 机器资产 |
