...
|---|---|---|---|---|---|---|---|
| `lease/personal/P2-T12/worker-loop-production-chain` | P2-T12/D02 zero-Intent first execution and per-row tick isolation | Lane-RUN | `personal/P2-T12-worker-loop` | `crates/cognitive-kernel/src/{engine.rs,intent_chain.rs,ports.rs}`; `crates/cognitive-management/src/task_application.rs`; `crates/cognitive-runtime/src/intent_flow.rs`; `crates/cognitive-runtime/tests/p2_t01_task_application_service.rs`; `crates/cognitive-store/src/scheduler.rs`; `crates/cognitive-store/src/sqlite/{intent_chain.rs,store.rs}`; `apps/kernel-server/src/personal/task_api.rs`; `apps/kernel-server/src/personal/server.rs`; `apps/kernel-server/src/personal/scheduler_authority/`; `apps/kernel-server/src/personal/tool_executor/`; `apps/kernel-server/tests/p2_t12_production_chain.rs`; `handbook/{en,zh-CN}/developer/{authority-kernel,context-and-artifact,execution-chain-status,management-plane,memory-and-skill,store-and-migrations,task-pipeline}.md`; `handbook/{en,zh-CN}/reference/{capability-status,cli-admin,errors,state-transitions}.md`; `handbook/{en,zh-CN}/user/{operations-and-recovery,tasks-and-execution}.md`; `docs/checkpoints/20260813-personal-p2-t12-worker-loop-report.md`; `docs/plan/PROGRESS.md`; `docs/plan/PERSONAL-DEVELOPMENT-PLAN.md` | `cursor-gpt-5.6-sol-20260813` | 2026-08-13 / 2026-08-13 | active |
