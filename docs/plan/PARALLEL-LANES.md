...
| `crates/cognitive-runtime`、`cognitive-management`、`cognitive-akp`、`apps/kernel-server`、`apps/admin-cli` | Lane-RUN | `lane/run-v02-ordinary-core-audit-tracer` | Ordinary Core `status.inspect` internal AUDIT port/receipt/release gate implemented and tested; durable adapter/product wiring pending; no registered assets or Profile claim |
