...
| P1-T09 | 首次安装到首次对话 route 与 B01 campaign | P1-T08 及现有 Secret/Provider/daemon/Pi contracts | deterministic binary Provider fixture、真实 pinned Pi Extension load、真实首个响应、native Secret Service smoke 与预注册 B01 runner；B01 仍由独立 formal campaign 决定 | in-progress | `experimental-local-only` / `tested-local`；deterministic binary Provider fixture 已在 supported Ubuntu/Windows CI **3/3** 通过，coherent bundle layout 与 route runner 已分别在 supported Ubuntu/Windows CI runs `30537927521`、`30543563029` 通过。2026-07-31，PR [#124](https://github.com/agentkernel/cognitive-os/pull/124) 后的 protected campaign `30592948805` 签名上传 `0.0.0-campaign.20260731.2`。在 SSH-qualified Linux x86_64/glibc `2.35`/native user-systemd host 上，SHA-256 固定 source bundle 经 `git fsck` 和 exact `main` commit `106789b1ba8eb70211f297ec31715d2ca44d611e` 核对后构建 host verifier；它独立接受 artifact、签名、expected Pi pin 和 key。verified installer 在确认无 kernel-server 后清理 crash-stale daemon lock 并重试，成功激活 `.2` single user service。只有 npm registry SRI 与 signed manifest 完全匹配后，才持久安装 `@earendil-works/pi-coding-agent@0.81.1`；其 absolute executable 返回 exact `0.81.1`。修正后的 bounded route 仍在 90 秒超时且无 response output；redacted trace 已确认 provider registration、selected model projection 和 initial-load `setModel` 调用，却未到 stream/completion，定位 initial-load deadlock。新增 deferred-session-start model-activation failure-first regression 通过，但尚未进入 signed bundle。Provider material、SecretRef、SQLite、Task、Effect、Verification、capability 与 authority data 均未输出，且未创建 authority side effect。B01、GMVP-LINUX、release 与 Profile 仍为 non-claim；下一步是将 deferred activation 修复合入 `main` 后重跑 protected campaign，再作独立 verify/install/route rerun。handoff：[20260730-personal-p1-t09-verified-experimental-deployment-handoff.md](../checkpoints/20260730-personal-p1-t09-verified-experimental-deployment-handoff.md)。 |
