...
| P2-T03 | durable scheduler、lease 与 timer | P2-T01, P1-T01 | crash/duplicate lease/clock/budget、durable stop、Effect closure 与 worker fencing 测试 | in-progress | 2026-08-02；repository/service/ceiling/authority/contract slices merged through PR #129 `main@7ea1cde`. A follow-up implementation-only daemon parsing regression centralizes v0.2 TaskContract admission and rejects v0.1 before execution-binding deserialization; `git diff --check` and `cargo fmt --all -- --check` passed, while the focused kernel-server test did not run to completion because the local Windows GNU linker exited 121 before crate compilation. Existing evidence covers durable scheduler rows, CAS lease, monotonic eligibility, higher-epoch takeover, inclusive deadline/retry/step/cost evaluation, finite TaskContract compatibility and daemon durable-fact loading. Worker-side durable stop persistence, dispatch quiescence/Effect closure integration, BoundedHarness wiring and P2 Gates remain not-run. See [handoff](../checkpoints/20260802-personal-p2-t03-daemon-authority-adapter-regression-handoff.md). |
