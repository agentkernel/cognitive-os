...
| Phase 2 - 单 Agent 任务闭环 | 9 | 8 | 1 | 0 | 0 | G2 / B02、B04、B05、B12 |
...
| P2-T09 | 已登记 Tool family 的 execution readiness 与 executor parity | P2-T05, P2-T06 | private Tool projection 必须区分 `registered/enabled` 与 `execution_ready`，composition root 只为已装配 executor 的 family 报 `execution_ready=true`；随后按 WorkspaceSearch → WorkspaceWrite/Patch → HttpFetchReadOnly 顺序补齐已登记 family 的真实 executor，每个都沿用 daemon authority、descriptor digest、persist-before-dispatch、幂等、fencing 与 reconcile；不得改弱 descriptor digest、不得新增未登记 family、不得把 registry availability 当作 execution readiness | in-progress | 2026-08-12 owner-authorized under the optimization proposal INV-2（由 P9-T04 `L4` `T1`/`T2` 能力缺口直接触发）。`lease/personal/P2-T09/tool-execution-readiness` on `personal/P2-T09-tool-execution-readiness`。无 Gate/release/Profile claim；不继承 P9-T04 证据。 |
