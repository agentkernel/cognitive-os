...
> **状态：in-progress（P0-T01..T07、P1-T01..T08、P2-T01..T02 已完成；P1-T09/P2-T03/P2-T04/P2-T07/P3-T01 in-progress；B01 running：固定 N=20 已记账 2 次，1 成功/1 失败；P2/B03/B09/GMVP-LINUX 正式验收尚未完成）**
> **最后更新：2026-08-05**
...
| Phase 2 - 单 Agent 任务闭环 | 8 | 2 | 3 | 0 | 3 | G2 / B02、B04、B05、B12 |
...
| P2-T04 | scheduler→Context→Pi sidecar→BoundedHarness worker | P2-T02, P2-T03 | TaskContract/lease 每轮重载；Context 经真实 port；Pi sidecar candidate-only；no-progress/budget/stale-lease fail-closed | in-progress | 2026-08-05: D01 is active on the Context branch. The scheduler accepts the Context-bound v0.4 TaskContract and resolves authorized Context through the durable metadata-first/body-after-authorization port, fenced to the current contract binding, principal, and revocation epoch. The existing WIA remains post-candidate-admission. The pinned Rust-to-Pi candidate bridge and daemon proposal persistence/admission composition remain to be implemented; Pi output must remain non-authoritative. |
