...
| P2-T03 | durable scheduler、lease 与 timer | P2-T01, P1-T01 | crash/duplicate lease/clock/budget 测试 | in-progress | 2026-08-01；repository slice merged as PR #128 `main@f3bacbe`. Current `lease/personal/P2-T03/scheduler-service` slice adds `crates/cognitive-runtime/src/scheduler_service.rs`: canonical wall-clock samples never regress within a worker, TTLs are computed deterministically, and only eligible rows or expired leases can be atomically acquired under a higher fence epoch. Failure-first focused test initially failed with missing `SchedulerService`, then Linux host passed `p2_t03_scheduler_lease_timer` 5/5, `cognitive-store` full suite, `cargo fmt --all -- --check`, and focused runtime/store clippy. Budget/deadline/step ceilings and worker BoundedHarness integration remain not-run; P2 Gate B02/B04/B05/B12 remains not-run. |
