...
| Phase 4 - Memory 与 Skill | 6 | 1 | 1 | 0 | 4 | G4 / B08 |
...
| P4-T02 | SQLite FTS5 + metadata filter baseline | P4-T01 | authority metadata pre-filter 后检索；unauthorized/stale/conflict/delete/rebuild 负例 | in-progress | 2026-08-09：`P4-T02/D01` 开始 daemon-private FTS5 authority-filtered retrieval slice；仅建立可重建 derived index 与 metadata-only candidate port，尚无 validation/Gate/release/Profile claim。 |
