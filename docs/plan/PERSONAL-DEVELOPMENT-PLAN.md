...
| P2-T03 | durable scheduler、lease 与 timer | P2-T01, P1-T01 | crash/duplicate lease/clock/budget、durable stop、Effect closure 与 worker fencing 测试 | in-progress | 2026-08-03；repository/service/ceiling/authority/contract slices merged through PR #129 `main@7ea1cde`. Commit `6ab740d` adds a runtime adapter that maps a reached fresh scheduler ceiling to the existing fenced kernel `STOP` transition before worker lease acquisition. Linux-native validation exposed an implementation defect in the earlier kernel STOP slice: `checkpoint_effects_are_closed` was called but not defined. Commit `d5e5024` restores that private fail-closed inventory evaluation; it accepts only terminal/reconciled Effect states and refuses malformed, unresolved or unknown inventory. Commit `a3cbea1` adds the daemon-owned admission ordering: each attempt reloads current authority facts, commits a reached ceiling STOP, and only then allows scheduler lease acquisition. Exact Linux revision `a3cbea10eb7c6ba4a6dcf8a9399ba97effd2e38d` passed `scheduler_authority::tests` 2/2, compiling the new composition boundary. The daemon-private release boundary forwards only a closed Effect's exact task/owner/epoch-fenced dispatch to durable scheduler release; stopped and reconciliation-pending attempts retain their state. The latest implementation-only slice adds a deterministic `TaskBinding -> Intent -> Effect` authority read: zero or multiple Intent bindings, inconsistent bindings, absent Effects and unknown states fail closed; only reconciliation/verification terminal states classify closed. The SQLite reverse lookup is ordered by Intent identity and focused storage coverage proves its epoch binding. Local Rust execution is blocked by the Windows GNU linker (exit 121), so this slice adds no implementation-evidence level. Existing evidence covers durable scheduler rows, CAS lease, monotonic eligibility, higher-epoch takeover, inclusive deadline/retry/step/cost evaluation, finite TaskContract compatibility, daemon durable-fact loading, and the kernel STOP edge. Concrete worker invocation of the Effect lookup/closure/release boundary, BoundedHarness integration and P2 Gates remain not-run. See [handoff](../checkpoints/20260803-personal-p2-t03-durable-task-effect-lookup-handoff.md). |
