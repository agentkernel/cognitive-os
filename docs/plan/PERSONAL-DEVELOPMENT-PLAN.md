...
| Phase 2 - 单 Agent 任务闭环 | 15 | 13 | 0 | 0 | 2 | G2 / B02、B04、B05、B12 |
...
| P2-T13 | 独立 verification 在生产中真正运行（`ACT -> VERIFY -> CONTINUE -> OBSERVE` 闭环） | P2-T12 | (1) daemon 在启动组合期构建 `ArtifactStore`（今天 `ArtifactStore::open` 只有测试与 benchmark 调用者，daemon 从不构建，故 verifier 即使被调用也无法解析证据）。(2) 在受治理 Tool Effect 到达终态且对账后，生产路径持久化 `FixedPostStateRow` 与 `VerificationRequestRow` 并提交 Loop `ACT -> VERIFY`；三者今天都无生产写入者。(3) `VerificationRequestRow.criteria_canonical_json` 必须由真实来源生成：筛选 `TaskContract.conditions` 中 `kind == Acceptance` 的项并序列化，`verifier_ref`/`verifier_version` 取自 `ContractCondition::verifier_ref`（该字段已在 admission 落库但从未被读回）。(4) 提供至少一个生产 `IndependentVerifier` 实现与 `verifier_ref` → 实例的解析表；今天仅存在两个测试替身。(5) 生产路径调用 `record_independent_verification`，并在报告 `passed` 后发出 checkpoint 与 `issue_continuation_authorization`；`ProtocolStore::append_checkpoint` 同样无生产调用者，而 `issue_continuation_authorization` 的 SQL 前置要求同 fencing epoch 的 checkpoint，故两者必须一起补。消费侧（`dispatch.rs:436` → `begin_verified_continuation_atomically`）**已接线**，无需新建。(6) 负例：Effect 未闭合或未对账即请求验证；stale epoch；缺失或 digest 不符的 CAS 证据引用；报告由 actor 自身身份产出；报告绑定到不同 fixed post-state；缺 checkpoint 时拒发 continuation authority。(7) 本任务**不**完成 Task：`CANDIDATE_COMPLETE -> COMPLETED` 属 P2-T14。 | not-started | 2026-08-12 registered，acceptance 同日据代码地图修正并收窄。它消除 wiring gap 4 的"验证从不运行"部分。评估 §14 将 "Independent verification / acceptance" 记为 **unreachable**；`UJ4` 独立验收 0/30。 |
