...
| P3-T06 | B03 Context correctness 与收益采集 | P3-T05 | MVP B03 uses the fixed 33-test authority-path matrix, native Linux/Clippy, Ubuntu/Windows CI, cleanup/redaction, and owner review under ADR-0040. B06/B07 only collect repeatable raw performance observations and never block correctness completion. | in-progress | D01-D02 evidence is complete at `76e3e1f`; ADR-0040 makes the focused matrix the MVP denominator. The task awaits final required CI and normal PR/lease/main closure. B03 status remains pending until that exact revision is validated. |
