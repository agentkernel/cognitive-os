...
|---|---|---|---|---|---|
| P0-T01 | 固定可复现基线与支持工具链 | — | Linux runner 与支持的 Windows 工具链结论可复现 | blocked | Started 2026-07-25; Lane-DOC branch `lane/personal-p0-t01-baseline`. Local GNU host build is blocked at the linker (`-lgcc_eh`, exit 101); the pinned CI run `30135181631` passed on both Ubuntu and Windows. Owner decision is required to define the supported Windows toolchain: MSVC-only, repair and support GNU, or no Windows support. See `tests/baseline/README.md`. |
| P0-T02 | 冻结 Personal 需求、追踪与架构边界 | P0-T01 | PERS-PR、任务、benchmark 无孤儿映射 | not-started | — |
