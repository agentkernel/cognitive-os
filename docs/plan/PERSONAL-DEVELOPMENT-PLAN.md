...
| P1-T08 | 可检查 Linux bundle installer 与 user service | P0-T03, P1-T01, P1-T04, P1-T06, P1-T07 | verifier、interruption、rollback 测试 | in-progress | 2026-07-28；`lane/personal-p1-t08-installer-orchestration`；`development_track: experimental-local-only`。P1-T07 已完成，依赖解除。第二个失败优先批在 `cognitive-runtime::linux_bundle` 固定 ADR-0028 的离线 Ed25519 detached signature、JCS canonical closed statement、产品拥有的 versioned keyring、revocation、strict HTTPS provenance 与 bounded regular-file inputs；staging 在写入前重新校验 artifact digest。focused WSL (`windows_wsl2_linux_guest`) attestation tests **14/14 passed**，strict Clippy 与 formatting 通过。第三个失败优先批新增官方本地安装编排入口：完整 verifier 成功前 deployment root 不得创建，随后固定 previous-active read → verified staging → 单次 caller-bounded health check → health-gated atomic activation → active pointer 复核 → non-secret typed receipt。orchestration focused WSL tests **10/10 passed**；覆盖 invalid signature/trust roots/tampering/Pi/platform 的 pre-mutation fail-closed、health failure retention、upgrade、same-version idempotency、information disclosure 与 post-verification artifact mutation。此批不含 production signing key/trust root、release attestation、downloader、inspected shell installer、systemd user service、cross-process lease、uninstall 或 Linux-native Gate；不得将本机或 fixture 结果写为 B01/Gate/Profile/release claim。 |
