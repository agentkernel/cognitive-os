...
| P2-T03 | durable scheduler、lease 与 timer | P2-T01, P1-T01 | crash/duplicate lease/clock/budget、durable stop、Effect closure 与 worker fencing 测试 | in-progress | 2026-08-02；repository/service/ceiling/authority/contract slices merged through PR #129 `main@7ea1cde`. The current implementation-only runtime slice turns a reached scheduler ceiling into the existing fenced kernel `STOP` transition before any worker lease acquisition: it maps only registered ceiling reasons and returns the committed transition instead of an in-memory stop flag. The new reason-mapping regression and focused runtime test did not compile locally because the Windows GNU linker exited 121 while building dependencies; `cargo fmt --all -- --check`, `git diff --check`, and `pnpm run check:consistency` passed. Existing evidence covers durable scheduler rows, CAS lease, monotonic eligibility, higher-epoch takeover, inclusive deadline/retry/step/cost evaluation, finite TaskContract compatibility, daemon durable-fact loading, and the kernel STOP edge. Linux-native focused execution requires a committed, pushed revision plus explicit user authorization for non-local SSH access. Durable fact reload at the caller, worker-side dispatch/Effect closure integration, BoundedHarness wiring and P2 Gates remain not-run. See [handoff](../checkpoints/20260802-personal-p2-t03-ceiling-stop-dispatch-adapter-handoff.md). |
