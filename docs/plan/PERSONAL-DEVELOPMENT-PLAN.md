...
| P1-T09 | 首次安装到首次对话 route 与 B01 campaign | P1-T08 及现有 Secret/Provider/daemon/Pi contracts | deterministic binary Provider fixture、真实 pinned Pi Extension load、真实首个响应、native Secret Service smoke 与预注册 B01 runner；B01 仍由独立 formal campaign 决定 | in-progress | `experimental-local-only`；deterministic binary Provider fixture 已在 supported Ubuntu/Windows CI **3/3** 通过，coherent bundle layout 与 route runner 已分别在 supported Ubuntu/Windows CI runs `30537927521`、`30543563029` 通过。2026-07-30 在 SSH-qualified `personal-linux-native-01`（`wuz@192.168.1.2`）以实际 `pi --version` 确认 exact Pi `0.81.1`；从该版本 Pi 执行最小 `--extension <absolute-path>` session-local observation。孤立 XDG child environment 未提供 Provider config、Provider/user secret、SecretRef、SQLite 路径、selected-model 或 authority state；built ESM import 和 Pi default-export invocation marker 均成功观察到。该 host 的隔离 non-secret source tree 已原生构建 `kernel-server`、`cognitive`、installer wrapper、campaign builder 和 Extension entry。新增可复现、脱敏的 product-route runner：它只配置 product Pi paths，拒绝不安全输入，检查 daemon readiness、exact Pi，并在清空 child environment 后执行单次 bounded response；focused positive/negative fixture 在本机和 Linux-native host 均通过。Environment-protected campaign `30553835734` 已签名并上传 coherent bundle，离线 verifier 和 host 端 verifier 均接受；但该 CI payload 需要 glibc `2.38/2.39`，在 glibc `2.35` host 上 fail-closed rollback。campaign `30562532613` 的 secret-free full validation 已通过，但 Ubuntu 22 signing job 在 signing 前因 `__isoc23_*` link failure 失败，未生成 artifact。现有 corrective workflow 以 pinned Zig 的 `x86_64-unknown-linux-gnu.2.35` target 生成四个 published Rust binaries，并在签名前拒绝 `GLIBC_2.36+` dynamic requirement。无 Task、Effect、Verification、capability 或 authority side effect；B01、GMVP-LINUX、release 与 Profile 仍为 non-claim。尚缺新 artifact 的验证 deployment、persistent exact Pi、真实首个响应、native Secret Service smoke、usability 与 formal B01。handoff：[20260730-personal-p1-t09-verified-experimental-deployment-handoff.md](../checkpoints/20260730-personal-p1-t09-verified-experimental-deployment-handoff.md)。 |
