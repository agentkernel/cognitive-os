...
> **状态：in-progress（P0-T01、P0-T02、P0-T04、P0-T05、P0-T07、P1-T01、P1-T02、P1-T03 已完成；P1-T04 等待 CI 执行证据；其余任务尚未开始）**
> **最后更新：2026-07-25**
...
| P1-T04 | 有界 Personal daemon 与本地认证 | P0-T07, P1-T01 | auth/size/timeout/concurrency/restart 测试 | in-progress | 2026-07-25；分支 `lane/personal-p1-t04-bounded-daemon-auth`，PR [#95](https://github.com/agentkernel/cognitive-os/pull/95)。`kernel-server --personal`：loopback-only listen、daemon single-instance lock、local bootstrap secret、channel-scoped bearer、ADR-0019 bounds、Host/Cookie fail-closed；ADR-0022。测试 `p1_t04_personal_daemon`（bad auth、wrong channel、oversized body、cookie/host reject、second-instance lock/restart）等待 CI Ubuntu/Windows-MSVC 执行证据。本机 Windows GNU linker exit 121 为非支持基线。非 G0/B01-B12/Profile；无 Task/Memory/MCP；无 registry/schema/vector 变更。handoff：[20260725-personal-p1-t04-bounded-daemon-handoff.md](../checkpoints/20260725-personal-p1-t04-bounded-daemon-handoff.md)。 |
