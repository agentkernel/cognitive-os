...
| Phase 2 - 单 Agent 任务闭环 | 15 | 14 | 1 | 0 | 0 | G2 / B02、B04、B05、B12 |
...
| P2-T14 | verified Task 完成与 acceptance authority | P2-T13 | (1) 实现 Task `ACTIVE -> CANDIDATE_COMPLETE`（`COMPLETION_CLAIMED`，guards `required_effects_closed`、`verification_requested_for_fixed_post_state`）与 `CANDIDATE_COMPLETE -> COMPLETED`（`ACCEPTANCE_GRANTED`，guards `acceptance_authority_matches`、`verification_passed_and_current`、`fixed_post_state_unchanged`）。(2) guard 在引擎中是 caller 提供的字符串集合（`engine.rs:189`/`:586`），因此生产代码必须从当前 TaskContract、Effect、fixed post-state、最新 verification report、fencing epoch 与 Artifact CAS 真正推导全部 guards，而不是断言它们；`verification_passed_and_current` 必须确认报告未被更新报告取代。(3) Owner 于 2026-08-13 决定沿用现有 Task transition table 已登记的 `acceptance_decision` evidence 槽位：它是 daemon acceptance authority 生成、以现有 `StrongReference` 绑定并存入 Artifact CAS 的最窄 canonical decision bytes；不新增 public schema、Rust public DTO 或专用表，也不得让私有结构改变已登记 transition contract。(4) acceptance authority 固定为 daemon-private `principal://personal/acceptance-authority` / `authority://personal/task-acceptance`，与 worker、Agent、Provider、Tool executor 和 independent verifier 身份分离，且不能由 caller 提供。(5) 负例：self-report、`agent_end`、Provider success、Tool exit 0、错误/陈旧 verifier、陈旧 state/epoch、缺失/篡改 evidence、open/unknown Effect 与重复 acceptance 均不得完成 Task。(6) 完成后 `verified completion` 在测试中成立不构成 Gate/release/Profile claim。 | in-progress | 2026-08-13 owner decision resolves the former blocker without changing the public transition contract. Claimed as `lease/personal/P2-T14/verified-task-completion` on separate branch `personal/P2-T14-verified-completion` from clean merged `main@326f97728ab6aaaacceaedd2156d953231b32e01`. D01 starts with the narrow acceptance evidence/authority boundary and failure-first production negatives; P2-T07 per-criterion reporting remains a separate governance observation and its `done` status is unchanged. |
