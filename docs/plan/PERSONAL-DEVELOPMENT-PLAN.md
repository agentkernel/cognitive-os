...
| P1-T09 | 首次安装到首次对话 route 与 B01 campaign | P1-T08 及现有 Secret/Provider/daemon/Pi contracts | deterministic binary Provider fixture、真实 pinned Pi Extension load、真实首个响应、native Secret Service smoke 与预注册 B01 runner；B01 仍由独立 formal campaign 决定 | in-progress | `experimental-local-only` / `tested-local`；deterministic binary Provider fixture 已在 supported Ubuntu/Windows CI **3/3** 通过，coherent bundle layout 与 route runner 已分别在 supported Ubuntu/Windows CI runs `30537927521`、`30543563029` 通过。2026-07-31，PR #125 后的 protected campaign `30595882821` 签名上传 `0.0.0-campaign.20260731.3`。在 SSH-qualified Linux x86_64/glibc `2.35`/native user-systemd host 上，SHA-256 固定 source bundle 经 `git fsck` 和 exact `main` commit `7bf69f35658489ea6141b3367989924d9049c6a8` 核对后构建 host verifier；它独立接受 artifact、签名、expected Pi pin 和 key。verified installer 在确认无 kernel-server 后清理 crash-stale daemon lock 并重试，成功激活 `.3` single user service。只有 npm registry SRI 与 signed manifest 完全匹配后，才持久安装 `@earendil-works/pi-coding-agent@0.81.1`；其 absolute executable 返回 exact `0.81.1`。原 90 秒 timeout 已确定为 route runner 向 Pi 传递未关闭 non-TTY stdin；显式 `/dev/null` 后 Pi 进入 agent lifecycle。installed `.3` route 在 2.9 秒 nonzero exit，并产生仅 redacted output、无 expected marker、无 authority side effect；均未输出 Provider material、SecretRef、SQLite、Task、Effect、Verification、capability 或 authority data。当前 implementation-only slice 由 exact installed Pi declarations/composer 确认 `setModel` 接受的 runtime model 必须携带 composed loopback `baseUrl`；focused regression 先在该字段缺失时失败，补齐后 package build/test 通过。修复未提交，尚未进入 protected campaign；真实 first response、B01、GMVP-LINUX、release 与 Profile 仍为 `not-run` / non-claim。下一步是 review、commit、merge to `main`，再独立 verify/install 和运行 redacted route。handoff：[20260730-personal-p1-t09-verified-experimental-deployment-handoff.md](../checkpoints/20260730-personal-p1-t09-verified-experimental-deployment-handoff.md)。 |
