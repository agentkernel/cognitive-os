...
| Phase 2 - 单 Agent 任务闭环 | 16 | 14 | 1 | 0 | 1 | G2 / B02、B04、B05、B12 |
...
| P2-T18 | 本地 bootstrap/session token OS CSPRNG 安全修正 | P1-T04、ADR-0019/0022；与 P2-T14 无依赖 | 生产 bootstrap 与 channel-scoped session bearer 只从 OS CSPRNG 取得至少 256 bit；熵失败、零/短输出或重复 block 在创建文件、session、token 前 fail closed，绝无 PID/时间/`DefaultHasher` fallback。保留 private runtime `0600`、opaque `prefix-group-group` 形状、constant-time bootstrap 比较、channel binding、expiry/revocation 与 Debug/log/evidence redaction。RNG 注入只存在于 private/test seam；负例覆盖静态 fallback 防回归、无文件/无 session、bounded uniqueness（无统计声明）及既有 auth 行为。 | in-progress | Owner-directed independent security corrective，基线 `origin/main@326f97728ab6aaaacceaedd2156d953231b32e01`；分支 `personal/P2-T18-local-token-csprng`，lease `lease/personal/P2-T18/local-token-csprng`。首个 failure-first source negative 已登记；required validation 为 exact-revision native Linux、GitHub Ubuntu/Windows CI、fmt/static/docs-sync/consistency、双语安全文档与独立 defect-first self-review。无 public contract、P2-T14、Gate/release/Profile/B01/campaign 语义变化。 |
