...
> **状态：in-progress（P0-T01、P0-T02、P0-T04、P0-T05、P0-T07、P1-T01、P1-T02、P1-T03、P1-T04 已完成；其余任务尚未开始）**
> **最后更新：2026-07-25**
...
| Phase 1 - 安装到首次对话 | 9 | 4 | 0 | 0 | 5 | G1 / B01 |
...
| P1-T04 | 有界 Personal daemon 与本地认证 | P0-T07, P1-T01 | auth/size/timeout/concurrency/restart 测试 | done | 2026-07-25；分支 `lane/personal-p1-t04-bounded-daemon-auth`。`kernel-server --personal`：loopback-only listen、daemon single-instance lock、local bootstrap secret、channel-scoped bearer、ADR-0019 bounds、Host/Cookie fail-closed；ADR-0022。测试 `p1_t04_personal_daemon`（bad auth、wrong channel、oversized body、cookie/host reject、second-instance lock/restart）。本机 Windows GNU linker exit 121 为非支持基线；执行证据以 CI Ubuntu/Windows-MSVC 为准。非 G0/B01-B12/Profile；无 Task/Memory/MCP；无 registry/schema/vector 变更。handoff：[20260725-personal-p1-t04-bounded-daemon-handoff.md](../checkpoints/20260725-personal-p1-t04-bounded-daemon-handoff.md)。 |
