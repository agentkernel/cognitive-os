...
> **状态：in-progress（P0-T01、P0-T02、P0-T05 已完成；其余任务尚未开始）**
> **最后更新：2026-07-25**
...
| P0-T05 | Linux Secret Service PoC | P0-T01 | set/get/rotate/delete 与泄漏负例通过 | done | 2026-07-25；Lane-DOC `lane/personal-p0-t05-secret-service-poc`。WSL2 Ubuntu 已改为 non-root `ubuntu` user（UID 1000）+ systemd user session；`/run/user/1000` 权限为 `700`，D-Bus socket activation 后 `org.freedesktop.secrets` 由 `gnome-keyring-daemon` 提供。仅用用户交互输入的非生产临时值执行 `secret-tool` set/get/rotate/delete；retrieval/rotation 与 delete verification 均通过。初始本地负例发现 Bash history 泄漏，已删除匹配行、重启终端并复验；最终输出 `NO_LEAK`：repository text、Bash history、environment、process arguments、user journal；`LEAK_CHECK_OK`。测试值未回传、未写入 Provider/config/SQLite/证据；无证据工件或 digest。此完成仅为 WSL2 环境 PoC，不是首发 Linux desktop 支持、正式 SecretStore 后端、G0、B01-B12 或 Profile 已符合声明。详见 [20260725-personal-p0-t05-complete-handoff.md](../checkpoints/20260725-personal-p0-t05-complete-handoff.md)。 |
