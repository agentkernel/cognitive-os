...
> **状态：in-progress（P0-T01、P0-T02、P0-T04 done；P0-T05 in-progress；其余尚未开始）**
> **最后更新：2026-07-25**
...
| P0-T05 | Linux Secret Service PoC | P0-T01 | set/get/rotate/delete 与泄漏负例通过 | in-progress | 2026-07-25 开始；分支 `lane/personal-p0-t05-secret-store-api`。已交付隔离 crate `cognitive-secret`：冻结 `SecretStore::{probe,put,get,delete}` + opaque `SecretRef`；模拟 backend 覆盖 put/get/rotate/delete、service-absent/locked/prompt-unavailable fail-closed、Debug/Display/env 泄漏负例；`LinuxSecretServiceProbe` 为 native probe-only（mutating D-Bus 归 P1-T02）；ADR-0018。本机 Windows GNU `cargo test -p cognitive-secret` linker exit 121（非支持基线）；`cargo check -p cognitive-secret` 通过。**行为测试待 CI Ubuntu/Windows-MSVC 真实执行后才可标 done**。无明文 fallback；无真实 Provider Key；无 registry/schema/vector 变更。见 handoff。 |
