...
> **状态：in-progress（P0-T01、P0-T02、P0-T04、P0-T05、P0-T07、P1-T01、P1-T02、P1-T03 已完成；P1-T04 timeout/concurrency 实现已提交、待 CI 行为证据；其余任务尚未开始）**
> **最后更新：2026-07-25**
...
| P1-T04 | 有界 Personal daemon 与本地认证 | P0-T07, P1-T01 | auth/size/timeout/concurrency/restart 测试 | in-progress | 2026-07-25；PR #95 已合入 `main@6fe21b3`（auth/size/host/cookie/restart）。本批 `lane/personal-p1-t04-timeout-concurrency`：header/body 读超时（`PERSONAL_REQUEST_READ_TIMEOUT`/408）、连接与 in-flight 上限（`CONNECTION_LIMIT_EXCEEDED` / `IN_FLIGHT_LIMIT_EXCEEDED`/429）、非 once 路径 worker 线程 accept、单元测试 `slow_header_read_*` / `slow_body_read_*` / `concurrent_connection_limit_*` / `in_flight_request_limit_*`；ADR-0022 同步。本机 Windows GNU linker exit 121 不可执行 cargo test（P0-T01 非支持基线）。**在 CI Ubuntu/Windows-MSVC 对上述测试给出 SUCCESS 前不得标 done，也不得解锁 P1-T05。** 非 G0/B01-B12/Profile。handoff：[20260725-personal-p1-t04-timeout-concurrency-handoff.md](../checkpoints/20260725-personal-p1-t04-timeout-concurrency-handoff.md)。 |
