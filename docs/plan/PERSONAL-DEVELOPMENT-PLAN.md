...
| P2-T03 | durable scheduler、lease 与 timer | P2-T01, P1-T01 | crash/duplicate lease/clock/budget、durable stop、Effect closure 与 worker fencing 测试 | in-progress | 2026-08-02；repository/service/ceiling/authority/contract slices merged through PR #129 `main@7ea1cde`. Commit `6ab740d` adds a runtime adapter that maps a reached fresh scheduler ceiling to the existing fenced kernel `STOP` transition before worker lease acquisition. Its Linux-native validation exposed an implementation defect in the earlier kernel STOP slice: `checkpoint_effects_are_closed` was called but not defined, so the focused runtime test could not compile. The current implementation-only kernel repair restores that private fail-closed inventory evaluation; it accepts only terminal/reconciled Effect states and refuses malformed, unresolved or unknown inventory. `cargo fmt --all -- --check`, `git diff --check`, and `pnpm run check:consistency` were run before the repair's Linux validation. Existing evidence covers durable scheduler rows, CAS lease, monotonic eligibility, higher-epoch takeover, inclusive deadline/retry/step/cost evaluation, finite TaskContract compatibility, daemon durable-fact loading, and the kernel STOP edge. Worker-side dispatch/Effect closure integration, BoundedHarness wiring and P2 Gates remain not-run. See [handoff](../checkpoints/20260802-personal-p2-t03-checkpoint-effect-closure-helper-handoff.md). |
