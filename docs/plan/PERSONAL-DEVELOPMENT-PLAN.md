...
> **状态：in-progress（P0-T01、P0-T02、P0-T04、P0-T05、P0-T07、P1-T01、P1-T02、P1-T03 已完成；P1-T04 的已合并安全前门仍待 timeout/concurrency 验收；其余任务尚未开始）**
> **最后更新：2026-07-25**
...
| Phase 1 - 安装到首次对话 | 9 | 3 | 1 | 0 | 5 | G1 / B01 |
...
| P1-T04 | 有界 Personal daemon 与本地认证 | P0-T07, P1-T01 | auth/size/timeout/concurrency/restart 测试 | in-progress | 2026-07-25；PR [#95](https://github.com/agentkernel/cognitive-os/pull/95) 已合入 `main@6fe21b3`。`kernel-server --personal` 已提供 loopback-only listen、daemon single-instance lock、local bootstrap secret、channel-scoped bearer、ADR-0019 bounds、Host/Cookie fail-closed；ADR-0022。CI run [30159744673](https://github.com/agentkernel/cognitive-os/actions/runs/30159744673) 的 Ubuntu/Windows-MSVC 均成功，且执行现有 `p1_t04_personal_daemon` auth/size/host/cookie/restart 覆盖。timeout 与 concurrency 行为测试尚未提供，故不得标为 done 或解锁 P1-T05。本机 Windows GNU linker exit 121 为非支持基线。非 G0/B01-B12/Profile；无 Task/Memory/MCP；无 registry/schema/vector 变更。handoff：[20260725-personal-p1-t04-bounded-daemon-handoff.md](../checkpoints/20260725-personal-p1-t04-bounded-daemon-handoff.md)。 |
