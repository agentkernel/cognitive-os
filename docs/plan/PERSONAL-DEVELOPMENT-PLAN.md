...
| Phase 2 - 单 Agent 任务闭环 | 13 | 11 | 0 | 0 | 2 | G2 / B02、B04、B05、B12 |
...
| P2-T12 | admitted Task 进入 worker loop（scheduler 引导与受治理 Tool 派发） | P2-T10 | (1) `task.admit` 在同一 authority 事务内为该 Task 持久化 scheduler work row：未准入的 Task 不得有 row，重复 admit 不得产生第二行，启动恢复必须修复"已准入但缺 row"而不是让 Task 静默停摆。(2) daemon 以有界周期 tick 取代单次启动 tick：单写者、不重入、可取消并在关闭时干净退出；一次 tick 失败只记录并在下个周期重试，不得毒化 store 或泄漏 scheduler lease。(3) worker attempt 依据已持久化 descriptor（`resolve_persisted_native_descriptor`）选择已装配 native executor 并经 Effect 协议派发，沿用 persist-before-dispatch、原键幂等、fencing 与 reconcile；未装配 family 必须 fail closed，不得静默把 Effect 记为已闭合。(4) 负例：未准入无 row；重复 admit 不重复建行；tick 自重入不得双重 lease；stale/replaced scheduler lease 不得派发；unsupported family fail closed；派发后崩溃经原键 reconcile 且不重复外部操作；STOP/budget ceiling 仍先于派发生效。(5) 本任务不产生 Task 完成，也不放松 A1/A2/A3；完成仍由 P2-T13 的独立验证把关。 | not-started | 2026-08-12 registered under the owner's delivery instruction to remove the `PERSONAL-PERF-EVAL-002` structural blockers. 它消除 `handbook/*/developer/execution-chain-status.md` 记录的 wiring gap 1 与 2，并把 P2-T10 装配的 sink 接到 daemon 自身 worker 路径。实测依据：`UJ4` 30/30 admitted、0 executed、0 verified。 |
| P2-T13 | 独立 verification 的生产调用者与 verified Task 完成 | P2-T12 | (1) daemon 自有生产路径在该 Task 的 Effect 全部 closed/reconciled 且 epoch 非 stale 时提交 verification request，复用 P2-T07 verifier 接缝与 P3-T03 Artifact CAS 证据引用。(2) verifier 相对 actor 可写面保持 out-of-band；verification report 仍为追加式，且是唯一可推进 Task 验收的输入。(3) Task 完成只在绑定 fixed post-state、当前 epoch 与已闭合 Effect 的肯定报告上发生；process exit、Pi `agent_end`、Provider 响应、外部 receipt 或 Tool receipt 单独都不得完成 Task。(4) 负例：Effect 未闭合或未对账即尝试完成；stale epoch；缺失或 digest 不符的 CAS 证据引用；报告由 actor 自身身份产出；报告绑定到不同 fixed post-state；以及 `agent_end` 单独不得完成的 false-completion 自检。(5) 证据为 exact-revision native Linux focused tests 与 required CI；测试中的 verified completion 不构成 Gate/release/Profile claim。 | not-started | 2026-08-12 registered under the same owner delivery instruction. 它消除 wiring gap 4。实测依据：`UJ4` 独立验收 0/30，评估 §14 将 "Independent verification / acceptance" 记为 **unreachable**。只有 P2-T12 与本任务都合并后，`C1`/`C2` 才可能端到端执行。 |
...
| `P2-T12/D01` | P2-T12 | make an admitted Task schedulable: `task.admit` persists the scheduler work row in the same authority transaction that admits the Task, and startup recovery repairs an admitted Task whose row is missing instead of letting it stall silently | completed P2-T01 `TaskApplicationService` and the durable P2-T03 scheduler repository | failure-first negatives proving an unadmitted Task has no row, a duplicate admit creates no second row, and a recovered row is `Runnable` at the current contract epoch and no other; exact-revision native Linux focused tests, Clippy, and required CI; a schedulable Task is not an executed or completed Task |
| `P2-T12/D02` | P2-T12 | replace the single startup tick with a bounded periodic scheduler tick that preserves the single-writer boundary: non-reentrant, cancellable, clean on shutdown, and a failed tick is recorded and retried on the next interval without poisoning the store or leaking a lease | `P2-T12/D01`; the existing `run_private_scheduler_tick_with_store` | failure-first negatives proving an overlapping tick cannot double-lease one work key, that shutdown leaves no running tick and no held lease, and that a tick failure neither poisons the store nor loses the row; exact-revision native Linux focused tests, Clippy, and required CI |
| `P2-T12/D03` | P2-T12 | dispatch the task's durable Tool Effect from the worker attempt through the assembled native executor selected by the persisted descriptor, so an admitted Task actually reaches a governed Tool execution | `P2-T12/D02`; the P2-T10 assembled sinks | failure-first negatives proving an unassembled family fails closed rather than closing the Effect, a stale or replaced scheduler lease cannot dispatch, a crash between dispatch and outcome recording reconciles through the original key without repeating the external operation, and a STOP/budget ceiling still stops before dispatch; exact-revision native Linux focused tests over real SQLite, Clippy, and required CI |
| `P2-T12/D04` | P2-T12 | complete the P2-T12 acceptance mapping and closure, and update the execution-chain-status gap record to whatever is then true | `P2-T12/D01-D03` | acceptance mapping, required CI, consistency, bilingual handbook sync, and closure; Task completion is explicitly still unreachable until P2-T13, and no Gate/release/Profile claim is created |
| `P2-T13/D01` | P2-T13 | add the daemon-owned production caller that submits a verification request once a Task's Effects are closed and reconciled under a non-stale epoch, reusing the P2-T07 verifier seam and P3-T03 content-addressed evidence | completed P2-T07 verifier persistence, P3-T03 Artifact CAS, and `P2-T12/D03` governed dispatch | failure-first negatives proving no request is submitted while any Effect is open or unreconciled, under a stale epoch, or with a missing/digest-invalid evidence reference; exact-revision native Linux focused tests, Clippy, and required CI; submitting a request is not completing a Task |
| `P2-T13/D02` | P2-T13 | advance Task acceptance only on an affirmative independent verification report bound to the fixed post-state, the current epoch and closed Effects, keeping the verifier out-of-band from the actor's writable surface | `P2-T13/D01` | failure-first negatives proving a report from the actor's own identity, a report bound to a different fixed post-state, a Tool receipt, a Provider response and Pi `agent_end` each fail to complete a Task, plus the existing false-completion self-check; exact-revision native Linux focused tests, Clippy, and required CI |
| `P2-T13/D03` | P2-T13 | complete the P2-T13 acceptance mapping and closure, and record which `PERSONAL-PERF-EVAL-002` capability classes become executable | `P2-T13/D01-D02` | acceptance mapping, required CI, consistency, bilingual handbook sync, and closure; a verified completion in tests is evidence of the mechanism, not a Gate, release, Profile or campaign result |
