...
| P3-T06 | B03 Context correctness 与收益采集 | P3-T05 | B03 correctness 通过；采集 B06/B07，收益未达或未运行不阻塞 1.0；一次 UCR-01 run 不自动通过多个 Gate | in-progress | D01 evaluator and exact native/CI validation are complete. Registered campaign `B03-personal-context-correctness-001` fixes the B03 assertion and failure matrix; D02 is executing the native evidence set under owner authorization. B03 remains `not-run` until independent-verifier review produces an explicit Gate state. |
