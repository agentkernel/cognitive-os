...
| Phase 2 - 单 Agent 任务闭环 | 11 | 10 | 1 | 0 | 0 | G2 / B02、B04、B05、B12 |
...
| P2-T10 | 已登记 Tool family 的 executor parity | P2-T09 | 按 WorkspaceSearch → WorkspaceWrite/Patch → HttpFetchReadOnly 顺序补齐已登记 family 的真实 executor，每个都沿用 daemon authority、descriptor digest、persist-before-dispatch、幂等、fencing 与 reconcile；mutation 另加 expected preimage、atomic publish、partial write、symlink/path race、duplicate dispatch 与 OUTCOME_UNKNOWN 负例；不得新增未登记 family，不得因"本地工具"绕过 Effect 语义 | in-progress | 2026-08-12 registered under optimization-proposal INV-TASK-2，并于同日按 owner delivery instruction 领取（移除 `PERSONAL-PERF-EVAL-002` §14/§16 Priority 6 记录的结构性阻塞）。本任务只消除 `handbook/*/developer/execution-chain-status.md` 记录的四个 wiring gap 中的第 3 个（四个已登记 family 没有已装配 executor）；gap 1（admission 不插入 scheduler row）、gap 2（无周期 tick）与 gap 4（verifier 无生产调用者）不在本任务范围，另行登记为后续任务。因此本任务完成后 executor 可从 daemon 自身 worker 路径到达，但仍不能由 Agent 到达。Kill criterion（提案 §4.3）：产品明确只读定位，或 mutation 无法可靠 query/reconcile。`ProcessRun` 与 Git 专用面属 INV-TASK-3，需 owner 先做 product-semantic 决策，不在本任务内。 |
