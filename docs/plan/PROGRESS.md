...
| Active task lease | `none` | P2-T02/D01 remains `in-progress`; its Lane-CTR public-contract prerequisite is validated by Linux and required Ubuntu/Windows CI at PR #138. Existing Task watch cursor-resume/dedup semantics remain unchanged. | merge PR #138, then claim a new Lane-RUN lease for daemon preview/admit composition |
