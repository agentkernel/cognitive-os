...
| Active task lease | `lease/personal/P2-T27/user-lifecycle-backup` | P2-T26 merged via PR #225 at `main@9e1404a1`. Unique in-progress slice `P2-T27/D02`. Draft PR [#226](https://github.com/agentkernel/cognitive-os/pull/226). D01 Ubuntu `31938000525` green at `6eea42c4`. D02 linux-002 `p2_t27_pi_lifecycle` 1/1 at `cf7dd501`. Windows `not-run by owner-directed Linux-only route`. `B01-Desktop-Linux-002` untouched (EVAL-004-only) | Ubuntu supporting CI on `cf7dd501`; then D03 matrix |
...
| `P2-T27/D02` | `in-progress` | Public `admin-cli activate-root`/`rollback` plus current-revision install→recover test with process-bound and stale-recover negatives. `DEV-LINUX-NATIVE-01` at `cf7dd501`: `p2_t27_pi_lifecycle` 1/1, fmt, `clippy -p admin-cli -D warnings`. Local Rust `not-run` (`RUST-LINK-DEV-WIN-GNU-01`) | Ubuntu supporting CI on `cf7dd501`; then D03 linux-002 matrix |
| `P2-T27/D03` | `ready` | Registered formal slice; implementation has not started | waits for D02 and owns whole-task validation/closure |
