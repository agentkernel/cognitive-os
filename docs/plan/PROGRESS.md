...
|---|---|---|---|
| `P9-T04/D04` | `in-progress` | the bounded resource sampler records CPU ticks, RSS, threads, descriptor count, and I/O bytes for declared daemon/Pi/adapter roles without opening `cmdline`/`environ` or resolving a descriptor target | run the sampler negatives on exact native Linux and required CI, then assemble the campaign evidence report |
