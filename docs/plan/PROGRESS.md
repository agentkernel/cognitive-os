...
| `P2-T03/D05` | `in-progress` | daemon-only candidate admission, sealed WIA issuance, exact scheduler lease-bound one-time consumption, and startup recovery are checkpointed. `4060eba` passed required Ubuntu/Windows CI; focused negatives reject cancelled/task/contract-epoch-mismatched scheduler work without consumption/binding, closed legacy unbound recovery retains its lease, and duplicate bound consumption preserves the original binding. Exact Linux candidate-persistence/conformance evidence remains scoped to `d872635`/`50d4fc6`; an exact `d329293` Linux test was `not-run` after the remote clean clone hit GitHub's low-throughput timeout. No worker execution, progress, evidence, Task acceptance, or Task completion is claimed. | add the remaining failure-injection and exact-revision worker integration validation; external executor dispatch remains fail-closed/reconciliation-only |
