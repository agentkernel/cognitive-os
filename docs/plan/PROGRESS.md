...
| P5-T02 Sidecar contract, registration, instance identity | `in-progress` | `P5-T02/D01` registration and staged `D02` activate/SidecarSession on `personal/P5-T02-sidecar-foundation` @ `8d5e4d27bbd553a73adb182d623c934224c992dd`. Exact native Linux passed focused runtime 8/8, admin registration/activation 1/1, and Clippy for runtime/store/admin-cli. Required CI pending on Draft PR #181. | close D01 after required CI on `8d5e4d2`, then mark D02 in-progress for formal validation closure |
...
| `P5-T02/D01` | `in-progress` | Daemon-private Agent registration from an active official Pi installation root. Exact native Linux at `8d5e4d27bbd553a73adb182d623c934224c992dd` passed focused runtime registration/activation suite 8/8 and admin caller 1/1; Clippy passed for runtime/store/admin-cli. Required CI pending on Draft PR #181. | wait for required Ubuntu/Windows CI on `8d5e4d2`, then close D01 |
| `P5-T02/D02` | `ready` | Activate registered instance into epoch-fenced SidecarSession without OS process spawn, capability, Effect, or Task completion; implementation validated on the same Linux revision behind D01 CI closure | enter in-progress validation immediately after D01 closes |
