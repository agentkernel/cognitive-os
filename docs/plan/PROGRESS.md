...
| Active task lease | `lease/personal/P7-T04/performance-governance` | `P7-T04/D01` is done at exact revision `e0c8c38265ac440e1043a4b03f0eba09625fac81` / Draft PR #179: seven-module hypothesis runner plus Memory FTS5 and Intent/Effect measurements. Exact native Linux passed bin tests, bounded run, Clippy/fmt at `c2aece5`; required Ubuntu/Windows CI run `31373702405` passed for the docs/lease repair HEAD. `P7-T04/D02` is now the unique in-progress Slice on the same branch/lease/PR. | implement the governed-path stage collector with warm/cold and omitted-stage negatives |
...
| `P7-T04/D02` | `in-progress` | Instrument one real governed daemon path with monotonic stage timing and bounded counters; do not fabricate tail statistics. | add focused warm/cold and omitted-stage negatives, then validate on exact native Linux and required CI |
