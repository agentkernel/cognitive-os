...
|---|---|---|---|
| Project focus | `cognitiveos-personal`: active and sole implementation project | CognitiveOS architecture assets remain reference/contract inputs; no second product backlog | EVAL-008 C1/C2 freeze in progress. P2-T32 remains in-progress pending Windows merge (lease closed). P7-T07 remains blocked on B01-W prerequisites |
| Owner-directed campaign | `PERSONAL-PERF-EVAL-008` **active**. Frozen product pin `fb85cfff25d8dd9fc5e3a8743ab9fdb3b3586630` (P2-T32 public launcher; unmerged OK). C1/C2 paired B0/B1/B2 `not-run`. Claim ceiling `hypothesis`; `not_reviewed`. Evaluation routing ON. `PERSONAL-PERF-EVAL-007`, `PERSONAL-PERF-EVAL-006`, `PERSONAL-PERF-EVAL-005`, and `PERSONAL-PERF-EVAL-004` remain **closed**. | Preregistration: [20260818-personal-perf-eval-008-preregistration.md](../checkpoints/20260818-personal-perf-eval-008-preregistration.md). Running: [personal-performance-assessment-20260818-eval-008.md](../evaluation/personal-performance-assessment-20260818-eval-008.md). Do not reopen EVAL-004/005/006/007. Do not reuse those roots, ports `48286`/`48288`/`48290`/`48292`/`48386`/`48388`/`48390`/`48392`, or SecretStore `/12`–`/16`. Stub-adapter pass ≠ real `pi-agent-adapter` | freeze: archive `fb85cfff`, guest root `/home/hal9001/perfeval008-20260818`, daemon `48294`, new SecretStore item `/17` via stdin |
...
| Owner-directed C1/C2 campaign | PERSONAL-PERF-EVAL-008 **active**; PERSONAL-PERF-EVAL-007 **closed**; PERSONAL-PERF-EVAL-006 **closed**; PERSONAL-PERF-EVAL-004 **closed**; PERSONAL-PERF-EVAL-005 **closed** | Product pin `fb85cfff` (public launcher + stub tests pass; real adapter is this campaign). Do not reuse EVAL-004/005/006/007 roots, ports, or SecretStore `/12`–`/16`. Claim ceiling hypothesis/`not_reviewed` | freeze guest root `/home/hal9001/perfeval008-20260818` daemon `48294` |
| Active task lease | `lease/personal/EVAL-008/c1-c2-paired-freeze` | EVAL-008 C1/C2 paired freeze after P2-T32 lease close; branch `evaluation/EVAL-008-freeze` | freeze archive `fb85cfff` + guest root `48294` + stdin SecretStore `/17`; measurement-only |
| Benchmark readiness product train | `P2-T21` **done** … `P2-T31` **done**; `P2-T32` **in-progress** (lease closed) at `fb85cfff` / PR [#239](https://github.com/agentkernel/cognitive-os/pull/239) | Public `cognitive daemon start` + stub Workspace* leaves `DRAFT` and acquires a lease (Ubuntu + linux-002). Real adapter is EVAL-008. Claim ceiling `hypothesis` | EVAL-008 freeze; Windows required-ci is merge-only for P2-T32; no Gate/B01 promotion |
...
| P2-T32 Public `cognitive daemon start` scheduler lease | **in-progress** (lease closed; pending Windows merge) | Ubuntu `32047487272` `verify (ubuntu-latest)` **pass** at `fb85cfff`; linux-002 focused `p2_t32` 1/1 in 3.33s + `daemon_log` 2/2. Windows `verify` fail is unrelated endpoint-publish flake (330/1). Draft PR [#239](https://github.com/agentkernel/cognitive-os/pull/239). Lease closed 2026-08-18 so EVAL-008 can own `PROGRESS.md`. Claim ceiling `hypothesis`. Stub ≠ real adapter. | rerun Windows required-ci before merge; do not treat stub pass or this EVAL as C1/C2 Agent-benefit |
...
| `P2-T32/D02` | `in-progress` | `daemon.log` + Unix `process_group(0)` landed; stub path proven. Lease closed 2026-08-18 for EVAL-008. Windows required-ci flake blocks merge only. | rerun Windows `32047487272`; do not treat as C1/C2 pass |
| `P2-T32/D03` | `ready` | handbook already in PR #239; full linux-002 workspace/Clippy matrix not required to start EVAL-008 measurement | P2-T32 merge is separate from EVAL-008 freeze |
