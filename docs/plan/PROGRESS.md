...
| Active task lease | `lease/personal/P5-T01/pi-acquisition` | `P5-T01/D03` is active on `personal/P5-T01-pi-acquisition`; D01 closed at `39b0e06e804947f9bd6c4a67295ffd4f4ccd3231`, D02 closed at `f63a5c8916089ac8dbcbc0be741ebd92c4fc28ef` with exact native Linux focused tests/Clippy and required Ubuntu/Windows CI run `31352954426` passed. Owned paths remain the private installation store/runtime installer, authenticated admin install caller, focused tests, and exact plan/progress/closure documents recorded in `PARALLEL-LANES.md`. | implement daemon-private uninstall/quarantine with stopped/absent lifecycle precondition and no data/secret deletion; preserve no AgentInstance, sidecar, process, Effect, Task-completion, B09, release, or Profile claim |
...
| P4-T05 Memory/Skill API and unified projection | `done` | Closure checkpoint `20260810-personal-p4-t05-memory-skill-api-closure.md`; D01-D05 provide task-bound projection, authority-backed explain reads, durable Memory forget/remember, Skill revoke/import/bind callers, and failure-first channel/payload/authority negatives. Required Ubuntu/Windows CI run `31335218082` passed; local fmt, diff, and consistency passed. | PR #176 must still be marked Ready, merged, and fully closed in the lease/branch/main reconciliation chain; public contract generation, B08, Gate, release, and Profile remain separate |
| P5-T01 Agent + sidecar package acquisition/install lifecycle | `in-progress` | `P5-T01/D01` has claimed the official exact-Pi acquisition-lock path. The intended daemon-private outcome is an authenticated, immutable acquisition lock atomically committed only after fixed package identity/origin, SRI/digest, dependency, Node, adapter, and signature/reference validation. | add failure-first origin/identity/integrity/dependency/Node/signature and no-commit negatives, then wire the authenticated official install caller; required Rust evidence routes to exact-revision native Linux and Ubuntu/Windows CI |
...
| Active task lease | `none` | P3-T06 is blocked after D01; its lease was closed with the campaign blocker record. The ignored local `/artifacts/` directory remains protected from modification, deletion, commitment, or implicit use. | await a newly preregistered supported B03 campaign before claiming a follow-up execution slice |
...
| `P5-T01/D03` | `in-progress` | The daemon-private uninstall slice now rejects active/implicit lifecycle, missing pointer, wrong root, and stale CAS fences; one SQLite transaction records the versioned binding/package reference in append-only quarantine and removes only the matching active pointer, preserving immutable acquisition evidence. Focused failure-first negatives cover lifecycle denial, missing/wrong root, data/secret preservation, unrelated roots, and no partial quarantine/success receipt. Local `cargo fmt --all` and `git diff --check` passed; Rust compile/test/Clippy are `not-run` on unsupported Windows GNU and require supported Linux/CI validation. | run exact-revision supported Linux focused runtime/store tests and Clippy; then reconcile D03 acceptance and continue the formal task |
