...
| Active task lease | `lease/personal/P2-T18/local-token-csprng` on `personal/P2-T18-local-token-csprng` | P2-T18 acceptance is complete on Draft PR [#215](https://github.com/agentkernel/cognitive-os/pull/215) exact `1e579c842585a3afc167769229870492109201b9`: native auth 15/15, source guard 1/1, kernel-server 226/226, Clippy pass; required CI `31732398077` Ubuntu/Windows 2/2 pass; PR mergeable/clean. The exact-path lease remains active through normal ready/merge closure and does not overlap P2-T14 | commit/push this acceptance ledger, wait for its docs-only CI, mark PR ready; do not merge or alter P2-T14 paths |
| P2-T18 Local bootstrap/session token OS CSPRNG corrective | **in-progress** | Acceptance complete / merge-ready. Both production generation points use `getrandom::fill` with 256-bit token payload plus independent probe; entropy failure/short/zero/repeated output creates no file/session/token. Persisted empty, legacy predictable, or malformed bootstrap fails closed. Static fallback, bounded uniqueness, 0600, channel/expiry/revoke, startup/restart, request/view/authority Debug and real front-door log/error negatives pass. Exact native/CI evidence is the active-lease row and running report. Status remains in-progress only because closure requires merge before lease release | ready PR #215 after the final ledger CI; no public contract, P2-T14, Gate/release/Profile/B01/campaign claim |
...
| `P2-T18/D02` | `in-progress` | Acceptance complete / merge-ready at exact `1e579c842585a3afc167769229870492109201b9`: session minting shares the OS CSPRNG; persisted legacy/empty bootstrap and request-Debug leaks found in final review are failure-first fixed. Native auth 15/15, source guard 1/1, kernel-server 226/226, all-target Clippy; required CI `31732398077` Ubuntu/Windows 2/2; fmt/consistency/handbook/generated/docs-sync and final call-site review pass. Active-lease consistency keeps this slice in-progress until merge | commit/push ledger, final CI, then mark PR #215 ready while retaining lease until merge |
