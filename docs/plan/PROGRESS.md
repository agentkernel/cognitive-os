...
| Owner-directed C1/C2 campaign | PERSONAL-PERF-EVAL-004 **active re-freeze**; prior 2026-08-15 lease closed and not reused | B0 on the old freeze found a product gap that BR-01..BR-08 closed on `main`. New freeze must use a new campaign root, port, SecretStore entry, broker, runner, and denominator. Claim ceiling remains hypothesis/non-claim. Source archive `sha256:a871f5d32…` and guest root `/home/hal9001/perfeval004-20260816` are pinned; campaign daemon is live on `127.0.0.1:48286`; old `48284`/`48181` untouched. Exact-source `kernel-server` `sha256:ecc1bf39…` and `cognitive` `sha256:760ad2c7…` are in the new root | new SecretStore item (do not reuse); pin Pi 0.81.1; do not start Provider samples |
