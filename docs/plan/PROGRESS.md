...
| Owner-directed C1/C2 campaign | PERSONAL-PERF-EVAL-004 **paused before B1; evaluation lease closed** | B0 target and runtime readiness were qualified without any B1/B2/B3/B4 sample. The campaign discovered a product gap: the private candidate carrier persists only parameters_digest, while the production WorkspaceSearch/WorkspaceWrite/WorkspacePatch router requires canonical governed parameters. EVAL-002 remains closed and unchanged; claim ceiling remains hypothesis/non-claim | P2-T25 D01 in progress on `personal/P2-T25-tool-lifecycle-invocation`. EVAL-004 re-freeze only after BR-02..BR-08 are merged; no prior campaign asset or SecretStore entry is reused |
| Benchmark readiness product train | `P2-T21` **done** at `5d1f5c2643d82807eb96f2c615d460dbdca749c3` / PR #220; `P2-T22` **done** at `03bc530d692649a00efa7918bfeadddf8ba0ebea` / PR [#221](https://github.com/agentkernel/cognitive-os/pull/221); `P2-T23` **done** at `795bfac8d8ee77b1b3be4137d33399fc817d20eb` / PR [#222](https://github.com/agentkernel/cognitive-os/pull/222); `P2-T24` **done** at `2b803e0fb955d03374e10569b4d9aecd0b4162d7` / PR [#223](https://github.com/agentkernel/cognitive-os/pull/223); `P2-T25` D01 in progress on `personal/P2-T25-tool-lifecycle-invocation`; `P2-T26..P2-T28` registered and not started | Owner instruction 2026-08-15 approved BR-01..BR-08 and the Linux-only product-validation route. Ubuntu is supporting CI; exact pushed-revision `DEV-LINUX-NATIVE-01` is required acceptance evidence for these product tasks; `B01-Desktop-Linux-002` remains owner-directed EVAL-004-only. Windows is `not-run by owner-directed Linux-only route`. This is product implementation evidence only, not EVAL/Gate/release/Profile/B01 evidence | finish P2-T25 D01 Ubuntu supporting CI, then D02 RegisteredCheckRun/pinned HTTPS. EVAL-004 re-freeze happens only after every BR product task is merged |
...
| Active task lease | `lease/personal/P2-T25/tool-lifecycle-invocation` | P2-T24 merged via PR #223 at `main@2b803e0f`. P2-T25 D01 authors public Tool lifecycle, Agent exposure, and bounded selection receipts. Unique in-progress slice `P2-T25/D01`. Branch `personal/P2-T25-tool-lifecycle-invocation`. Windows `not-run by owner-directed Linux-only route`. `B01-Desktop-Linux-002` untouched (EVAL-004-only) | commit/push Draft PR, then Ubuntu supporting CI |
...
| `P2-T25/D01` | `in-progress` | Public Tool lifecycle projection, Agent exposure, and bounded selection receipts: management enable/disable/quarantine/revoke; task channel denied mutations; stale/widened candidate digest and prompt restatement fail closed. Overlay never mutates descriptor digests. Local Rust tests `not-run` (`RUST-LINK-DEV-WIN-GNU-01`). Unique remaining D01 action is Ubuntu supporting CI | wait Ubuntu supporting CI, then D02 |
| `P2-T25/D02` | `ready` | Registered formal slice; implementation has not started | waits for D01 within the same future task branch and lease |
