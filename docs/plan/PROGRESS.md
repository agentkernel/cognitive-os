...
| P9-T05 Daemon integration-test readiness wait | **in-progress** | Owner-directed corrective delivery for a confirmed required-CI reliability defect, registered 2026-08-13 on `personal/P9-T05-ci-readiness-wait` from `origin/main@326f977`. `p1_t05_personal_readiness` gives the daemon a fixed 2 s (100 × 20 ms) budget to publish its bootstrap secret; a GitHub Windows runner that took ~2.2 s red-lighted the required check while all 151 unit tests in the same run passed, and re-running the identical revision passed. Independently confirmed pre-existing — not introduced or amplified by P2-T11 — and it will keep randomly failing the other in-flight task branches. The same copied budget sits in `p1_t07_pi_readiness`, `p4_t05_resource_api`, `p2_t02_task_api_watch`, `p2_t02_resource_projection`, `p1_t04_personal_daemon`, `p1_t07_provider_proxy` and `m5_http_sse`; three of those additionally wait for a connection in an unbounded `loop`, and two files had already been privately raised to 6 s and 10 s | D01 lands the shared wait carrying today's exact 2 s budget plus the failure-first case that budget cannot pass; D02 replaces it with a bounded readiness poll and moves every sibling onto it; D03 maps acceptance and closes. Test-harness reliability creates no Gate, release, Profile, or product claim |
| Active task lease | `lease/personal/P9-T05/daemon-readiness-wait` | Writable paths are `apps/kernel-server/tests/**`, `docs/plan/PERSONAL-DEVELOPMENT-PLAN.md`, `docs/plan/PROGRESS.md`, `docs/plan/PARALLEL-LANES.md` and `docs/checkpoints/**`. P2-T13 D01-D04 closed at exact `dab38cdf86c8e3486a913d85660593b6671bf114`; native scheduler authority 50/50, verification executor 12/12, `m5_harness` 7/7, three-package Clippy, and required Ubuntu/Windows CI `31707250244` passed, PR #210 merged, and that task lease is archived closed | complete P9-T05 D01-D03, then treat P2-T14 as a separate owner/Lane-CTR contract decision and implementation task |
...
| `P9-T05/D02` | `in-progress` | — | replace the fixed budget with a bounded readiness poll plus exited-daemon detection, and move every sibling wait in `apps/kernel-server/tests` onto it |
| `P9-T05/D03` | `ready` | — | acceptance mapping, required CI green on both platforms at the merge revision, and closure |
