...
| Active task lease | `lease/personal/P2-T02/private-resource-projection` | `P2-T02/D02` is implementing the private versioned six-family resource projection and management-bound cursor watch. This slice exposes daemon observations only; it does not fabricate missing family authority records. | validate the focused projection/channel-isolation process test on supported Linux and required CI before starting CLI parity |
