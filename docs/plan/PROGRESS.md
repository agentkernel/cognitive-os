...
| Active task lease | `lease/personal/P9-T04/comprehensive-performance-campaign` | P9-T04/D01 is the sole active delivery slice on `personal/P9-T04-comprehensive-performance-campaign`; the current checkpoint adds Pi-side fail-closed Provider usage availability parsing | extend opaque correlation and monotonic timing through a real daemon-owned caller without adding an authority writer |
| P9-T04 Comprehensive performance and real Task campaign | **in-progress** | ADR-0051 and preregistration `20260812-personal-p9-t04-performance-campaign-preregistration.md` are registered. D01's exact native Linux test passed 4/4 at `47629c40217ad039aededcb6a91207fe83f15b7a`; local fmt, consistency, and Pi client build/test passed. The pushed Pi usage checkpoint `038a194f959f59a5f5ce44cf299812bc282bceb`, transport-only timing checkpoint `54aad3159f209f0a3a34a05a127a004d08bc832a`, and zero-free Pi propagation checkpoint `7ab9d10569a7983d274e593bcd53b0bbac5a8ba5` each passed native Linux Pi build/test in disposable Git worktrees. Provider-network timing integration reached `8e0c6e3`, whose CI failed only because the compatibility `forward_chat_completion` helper became dead code under `-D warnings`; corrective checkpoint `1be6fcb` marks that helper test-only and has fresh Ubuntu/Windows CI pending. The Pi client now preserves complete, internally consistent Provider usage only; missing, partial, or inconsistent usage and all cost remain unavailable rather than synthetic zeroes, and it separately records monotonic Pi-to-daemon loopback HTTP duration without conflating Context or Provider stages. B01 guest remains shut off and no Provider import, guest mutation, benchmark execution, Gate, release, Profile, or Agent-benefit claim has occurred. | await corrective CI, then continue correlation/timing integration through the remaining daemon-owned lifecycle |
...
|---|---|---|---|
| `P9-T04/D01` | `in-progress` | secret-free correlation/timing/Provider-usage envelope negatives passed 4/4 on exact native Linux `47629c40217ad039aededcb6a91207fe83f15b7a`; required CI is pending on Draft PR #199 | extend the envelope through a real daemon-owned caller while preserving the non-authority and no-secret boundaries |
