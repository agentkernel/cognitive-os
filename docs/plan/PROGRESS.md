...
| P2-T03 scheduler/runtime | `in-progress` (D05 blocked) | `P2-T03/D01`-`D04` are done. D05 durable registration, recovery discovery, TaskBinding fencing, v2-to-v3 preservation, and stale-epoch negative are checkpointed. It cannot safely create a live `BoundedHarness` dispatch because no durable authority source defines the required iteration charge, progress/evidence observation, or selected authorized candidate. | owner decision: define the existing-contract authority source for scheduler worker inputs, or explicitly reorder/extend the formal plan; do not begin P2-T04 before D05 closes |
...
| Active task lease | `none` | P2-T03/D05 is blocked on a documented authority-source decision; B02/B04/B05/B12, release, and Profile remain separate and incomplete | obtain the bounded product/architecture decision recorded in the D05 handoff, then claim a new exact-path D05 lease |
...
| `P2-T03/D05` | `blocked` | exact Linux `cc006c1` passed scheduler-authority 14/14, including stale TaskContract epoch rejection before admission. Durable registration, discovery, composite TaskBinding fencing, and closed-Effect release exist, but a live worker lacks a canonical durable authority source for required `BoundedHarness` iteration inputs. | owner decision required: select the canonical persisted source for candidate, charge, progress/evidence, and expected loop version, or revise task order/scope; no worker may fabricate them |
