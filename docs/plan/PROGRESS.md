...
| Active task lease | `lease/personal/P2-T28/journey-capability-truth` | P2-T27 merged via PR [#226](https://github.com/agentkernel/cognitive-os/pull/226) at `main@70980131`. Unique in-progress slice `P2-T28/D01`. Windows `not-run by owner-directed Linux-only route`. `B01-Desktop-Linux-002` untouched (EVAL-004-only) | freeze UJ1..UJ6 matrix and missing-caller/oracle negatives |
...
| P2-T28 End-to-end journey and capability truth | **in-progress** | Unique slice `P2-T28/D01` UJ1..UJ6 freeze on `personal/P2-T28-journey-capability-truth`. Web UI/Multi-Agent stay scope-excluded. Windows `not-run by owner-directed Linux-only route`. Claim ceiling `hypothesis`; no Gate/release/Profile/B01/EVAL promotion | freeze the matrix and missing-caller/oracle negatives |
...
| `P2-T28/D01` | `in-progress` | Frozen UJ1..UJ6 capability-truth matrix with failure-first missing-caller/oracle/path/cleanup and scope-exclusion negatives. Local Node 6/6 pass. Web UI/Multi-Agent remain explicit `excluded`. Rust unit tests `not-run` (`RUST-LINK-DEV-WIN-GNU-01`) | Ubuntu supporting CI on the Draft PR head |
| `P2-T28/D02` | `ready` | Registered formal slice; implementation has not started | waits for D01 within the same task branch and lease |
