...
| Active task lease | `lease/personal/P2-T34/candidate-parameter-digest` | P2-T34 private-candidate parameters digest canonicalization; branch `personal/P2-T34-candidate-parameter-digest` | linux-002 empty-digest test then Ubuntu CI on the D02 head |
...
| Benchmark readiness product train | `P2-T21` **done** … `P2-T31` **done**; `P2-T32` **in-progress** (lease closed) at `a653dc7b` / PR [#239](https://github.com/agentkernel/cognitive-os/pull/239) CI `32049064078` Ubuntu+Windows SUCCESS; `P2-T33` **in-progress** (lease closed) at `9dc40417` / PR [#240](https://github.com/agentkernel/cognitive-os/pull/240) CI `32064313430` Ubuntu+Windows SUCCESS; `P2-T34` **in-progress** at `a7c7c2a5` / PR [#241](https://github.com/agentkernel/cognitive-os/pull/241) CI `32067650868` Ubuntu+Windows SUCCESS | Public launcher + stub Workspace* on a long root leaves DRAFT and acquires a lease. EVAL-010 long-root 真机 confirmed the UNIX_PATH_MAX bind; C1-search then skipped `candidate_has_missing_fields_or_an_invalid_parameters_digest`. Claim ceiling `hypothesis` | merge T32 then T33 after P2-T34 D02 linux-002; no Gate/B01 promotion |
...
| P2-T34 Private-candidate parameters digest | **in-progress** | EVAL-010 skip `candidate_has_missing_fields_or_an_invalid_parameters_digest`. Adapter JSON-fallback recomputes digest from parameters. D01 linux-002 at `936333ab` + fmt `a7c7c2a5`; PR [#241](https://github.com/agentkernel/cognitive-os/pull/241) CI `32067650868` Ubuntu+Windows SUCCESS. D02 empty-digest test added this window. Claim ceiling `hypothesis`. Stub ≠ real adapter. | linux-002 empty-digest test; then D03 closure. Keep Draft until T32/T33 merge order is honest |
| P2-T33 Private-candidate host path | **in-progress** (lease closed; merge-ready) | linux-002 at `289eebad`: focused `p2_t33` 2/2, host-path units, Clippy `-D warnings`. Ubuntu `32063236152` `verify` **pass**. PR [#240](https://github.com/agentkernel/cognitive-os/pull/240) CI `32064313430` Ubuntu+Windows+`required-ci` SUCCESS at `9dc40417`. Lease closed 2026-08-18 before EVAL-010. Claim ceiling `hypothesis`. Stub ≠ real adapter. | merge after P2-T32; do not treat stub pass as C1/C2 Agent-benefit |
| P2-T32 Public `cognitive daemon start` scheduler lease | **in-progress** (lease closed; merge-ready) | Ubuntu `32047487272` `verify (ubuntu-latest)` **pass** at `fb85cfff`; linux-002 focused `p2_t32` 1/1 in 3.33s + `daemon_log` 2/2. PR [#239](https://github.com/agentkernel/cognitive-os/pull/239) later run `32049064078` Ubuntu+Windows+`required-ci` SUCCESS at `a653dc7b` (supersedes the endpoint-publish flake on `32047487272`). Lease closed 2026-08-18 before EVAL-008. EVAL-008/009 are **closed**. Claim ceiling `hypothesis`. Stub ≠ real adapter. | mark ready and merge; do not treat stub pass as C1/C2 Agent-benefit |
...
| `P2-T34/D02` | `in-progress` | garbage-digest overwrite already proven at `936333ab`; empty-digest-with-parameters test added this window | linux-002 focused empty-digest test |
| `P2-T34/D03` | `ready` | handbook already synced at `936333ab`; Ubuntu+Windows CI green at `a7c7c2a5` | after D02 linux-002; keep Draft until T32/T33 merge |
...
| `P2-T33/D03` | `in-progress` | exact-revision linux-002 matrix recorded; Ubuntu `32063236152` pass at `289eebad`; PR [#240](https://github.com/agentkernel/cognitive-os/pull/240) CI `32064313430` Ubuntu+Windows SUCCESS at `9dc40417`. Lease closed 2026-08-18 so EVAL-010 can own `PROGRESS.md`. | merge after P2-T32; stub ≠ C1/C2; EVAL-010 remains closed |
...
| `P2-T32/D02` | `done` | `daemon.log` + Unix `process_group(0)` landed; stub path proven. Lease closed 2026-08-18 before EVAL-008. PR [#239](https://github.com/agentkernel/cognitive-os/pull/239) CI `32049064078` Ubuntu+Windows SUCCESS at `a653dc7b`. | consumed by D03 |
| `P2-T32/D03` | `in-progress` | handbook already in PR #239; EVAL-008 closed without waiting on this merge; Windows flake superseded | mark ready and merge #239; stub ≠ C1/C2 |
