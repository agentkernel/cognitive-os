...
| Active task lease | `lease/personal/P8-T09/dsh-akp-adapter` | owner-authorized DeepSeek Harness candidate-only AKP adapter on `personal/P8-T09-dsh-akp-adapter`; EVAL-015 remains closed and is not this task | remaining D03: real-dsh Workspace* startupEvents + retained Path B samples at pushed HEAD; then required CI and D04 merge |
| P8-T09 DeepSeek Harness AKP adapter | `in-progress` | linux-002 identity confirmed. Jump-host Path A `pong` **pass** (9.66 s, pin `528c682e`). Shim Path B Read/Search/Write `COMPLETED`. Path B real dsh→linux-002 daemon Flash **pass** (2/2 `pong`, 9566 ms / 9463 ms) at `bd8baf96`. SecretStore cleanup **pass**. Claim ceiling `hypothesis`. | re-run combined real-dsh Workspace* + Flash at the next pushed SHA; do not auto-claim P6/P7 |
...
| `P8-T09/D03` | `in-progress` | linux-002 identity pass. Shim Path B Read/Search/Write `COMPLETED`. Jump-host Path A **pass** (`pong`, 9.66 s) at dsh pin `528c682e`. Path B real dsh→linux-002 Flash **pass** 2/2 at `bd8baf96` (9566 ms, 9463 ms). Real-dsh Workspace* startupEvents **not-run**. Cleanup **pass**. | combined real-dsh Workspace* + Flash at pushed HEAD; retained denominator |
| `P8-T09/D04` | `ready` | docs-sync, CI, acceptance, merge/lease/branch/main | after D03 real-dsh retained denominator |
