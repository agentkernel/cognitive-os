...
| Console 产品车道 | **tracking-only（informative 文档例外）** | — | macOS/Linux/iPhone/Android phone 产品切片已记录；Agent Hub 已收敛为无 CognitiveOS 安全直连接管/完整治理两模式提示词，并加入 Paseo 类进程/会话/文件接管研究；客户端 implementation 均未启动，平台测试未执行，Profile 未符合 |
...
| Lane-CON Console | tracking-only 文档例外 | — | iPhone/Android phone 独立产品设计已记录；Agent Hub 安全直连接管/完整治理提示词已提供、产品决策尚未执行；实现 gate 未通过 |
