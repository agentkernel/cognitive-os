...
| Active task lease | `lease/personal/P8-T15/dsh-native-web-panel` on `personal/P8-T15-dsh-native-web-panel` | Writable: admin-cli dsh + client, dsh-akp-adapter scripts, kernel-server apply/remap + scheduler recovery, endpoint_trust, plan/PROGRESS, P8-T15 checkpoint, mapped handbook. Clients Apply is PR #4 (separate repo). | linux-002 daemon **515695** `:48681` (binary `p8t15-8b9d09cb`) until this overlay-sync revision replaces it. Cos `:3080` still on old helper until replaced. Do not auto-claim P6 / P7-T06 / P7-T07 |
| P8-T15 Native dsh Web UI | **in-progress** | D04: HTTP Apply previously wrote selected-model only (`restart_performed=false`) so Cos Models stayed stale; leftover grok stayed after unbind because overlay injected selected-model. **Coded:** daemon writes `dsh-web-home/control-plane-overlay.json` from the current dsh-bound account catalog on bind/remove/catalog/apply; helper reloads Cos chrome; unbound catalog is empty (no grok). Local `dsh-web-preflight.test.mjs` **5/5**. Linux kernel-server `p8_t13` overlay test **not-run**. Report: [P8-T15 report](../checkpoints/20260823-personal-p8-t15-dsh-native-web-panel-report.md). Claim ceiling `hypothesis`. | Linux `p8_t13` + Clippy; replace guest daemon **515695** and Cos helper; then bind/unbind/Apply show matching Models. Do not auto-claim P6 / P7-T06 / P7-T07 |
