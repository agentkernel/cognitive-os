...
## Current snapshot (2026-08-23 P8-T15 grok routing + Cos catalog overlay)
...
| Active task lease | `lease/personal/P8-T15/dsh-native-web-panel` on `personal/P8-T15-dsh-native-web-panel` | Writable: admin-cli dsh + client, dsh-akp-adapter scripts, kernel-server apply/remap + scheduler recovery, plan/PROGRESS, P8-T15 checkpoint, mapped handbook. Clients Apply is PR #4 (separate repo). | Jump-host exact-revision test then replace linux-002 daemon **494681**. Do not auto-claim P6 / P7-T06 / P7-T07 |
| P8-T15 Native dsh Web UI | **in-progress** | D04 routing+catalog overlay implemented: DeepSeek hosts refuse grok; Path B fail-closes leftover grok-on-DeepSeek with `PERSONAL_PROVIDER_BINDING_MISMATCH` (no POST to DeepSeek). Native settings/Models overlay Cos selected-model + account catalog. Path B launch ignores Pi `provider.json` blocked. linux-002 retest pending. Report: [P8-T15 report](../checkpoints/20260823-personal-p8-t15-dsh-native-web-panel-report.md). Claim ceiling `hypothesis`. | Jump-host test/clippy/build; replace **494681**; prove grok display + non-DeepSeek chat error; Flash regression. Do not auto-claim P6 / P7-T06 / P7-T07 |
...
| `P8-T15/D04` | `in-progress` | Routing+overlay coded: `PROVIDER_MODEL_ENDPOINT_MISMATCH` on add/bind; Path B leftover grok-on-DeepSeek → Cos 400 not DeepSeek allowlist; native settings/Models overlay Cos catalog; Path B launch no longer requires Pi provider.json. linux-002 retest pending. | Jump-host exact-revision tests; replace guest daemon; prove conversation+Models+routing. Do not auto-claim P6 / P7-T06 / P7-T07 |
