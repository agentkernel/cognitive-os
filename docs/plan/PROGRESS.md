...
| Active task lease | `lease/personal/P2-T16/registered-check-run` on isolated sibling worktree `D:\agent-kernel-checkrun` | P2-T16 is an owner-directed parallel implementation based on open Draft PR #210 head `b514d278ef4a3daafe9cceeb62ced2dc649d186b`; it does not edit the primary worktree or P2-T14 paths. The task is implementation/non-claim and cannot merge until #210 and P2-T14 are on `main` | D01: retain the first `check_id`-only failure-first test, implement the immutable registry boundary, then run required Ubuntu/Windows CI on an exact pushed revision |
...
| P2-T16 Registered deterministic check execution | **in-progress** | D01 began failure-first on isolated branch `personal/P2-T16-registered-check-run`: the desired daemon-private request accepts only `check_id`, but no registry/request API exists at base `b514d278ef4a3daafe9cceeb62ced2dc649d186b`. Scope is a fixed immutable registry plus the existing P2-T15 Tool/Effect authority path, bounded process-tree supervision, durable original-key reconciliation, Artifact/CAS evidence and P2-T13 verification. General ProcessRun, caller argv/env/cwd/shell/credential/network and exit-based completion remain forbidden | commit and push the red D01 checkpoint; capture Ubuntu/Windows failure evidence, then implement the registry without weakening the test |
...
| `P2-T16/D01` | `in-progress` | First failure-first Rust test names the absent production boundary: a caller must be able to request the fixed `c2a.repair.typescript` check using only `check_id`; local Rust execution is forbidden on `DEV-WIN-GNU-01`, so the expected compile failure will be retained through required CI | push the red checkpoint and append the exact CI result before implementing the immutable registry |
| `P2-T16/D02` | `ready` | not started | after D01: route resolved checks through persisted Intent/Effect, durable `EXECUTING`, bounded supervision and original-key reconciliation |
| `P2-T16/D03` | `ready` | not started | after D02: publish Artifact/CAS evidence, invoke independent verification and close C2a/A3/T6 negatives without Task completion |
| `P2-T16/D04` | `ready` | not started | after D03 and P2-T14 merge: rebase, supported CI/native validation, bilingual docs/fingerprints, self-review and deterministic closure |
