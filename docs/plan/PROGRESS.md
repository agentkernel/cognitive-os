...
| Benchmark readiness product train | `P2-T21` **done** at `5d1f5c2643d82807eb96f2c615d460dbdca749c3` / PR #220; `P2-T22` **done** at `03bc530d692649a00efa7918bfeadddf8ba0ebea` / PR [#221](https://github.com/agentkernel/cognitive-os/pull/221); `P2-T23` **done** at `795bfac8d8ee77b1b3be4137d33399fc817d20eb` / PR [#222](https://github.com/agentkernel/cognitive-os/pull/222); `P2-T24` D01 authored on `personal/P2-T24-effect-fault-reconciliation`; `P2-T25..P2-T28` registered and not started | Owner instruction 2026-08-15 approved BR-01..BR-08 and the Linux-only product-validation route. Ubuntu is supporting CI; exact pushed-revision `DEV-LINUX-NATIVE-01` is required acceptance evidence for these product tasks; `B01-Desktop-Linux-002` remains owner-directed EVAL-004-only. Windows is `not-run by owner-directed Linux-only route`. This is product implementation evidence only, not EVAL/Gate/release/Profile/B01 evidence | confirm Ubuntu supporting CI on the P2-T24 D01 head, then D02 original-key reconcile. EVAL-004 re-freeze happens only after every BR product task is merged |
...
| Active task lease | `lease/personal/P2-T24/effect-fault-reconciliation-observation` | P2-T23 merged via PR [#222](https://github.com/agentkernel/cognitive-os/pull/222) at `main@795bfac8d8ee77b1b3be4137d33399fc817d20eb`. P2-T24 D01 unique in-progress slice: default-off authorized fault profiles plus bounded `GET /task/effects` history. Windows `not-run by owner-directed Linux-only route`. `B01-Desktop-Linux-002` untouched (EVAL-004-only). Draft PR pending | push D01 head, confirm Ubuntu supporting CI, then D02 original-key reconcile |
...
| P2-T24 Effect fault and reconciliation observation | **in-progress** | D01 default-off campaign-authorized fault profiles (`POST/GET /management/resource/v1/fault-profile`) and bounded `GET /task/effects` history authored on `personal/P2-T24-effect-fault-reconciliation`; lease `lease/personal/P2-T24/effect-fault-reconciliation-observation`. Task-channel enablement and unauthorized campaigns fail closed; history omits receipts/raw parameters. Running report: [20260816-personal-p2-t24-effect-fault-reconciliation-report.md](../checkpoints/20260816-personal-p2-t24-effect-fault-reconciliation-report.md). Claim ceiling `hypothesis`; no Gate/release/Profile/B01/EVAL promotion | Ubuntu supporting CI on D01, then D02 original-key reconcile |
...
| `P2-T24/D01` | `in-progress` | Default-off authorized fault profiles and bounded `GET /task/effects` history; unauthorized/task-channel callers denied; no raw parameter/receipt in the history projection | Ubuntu supporting CI |
| `P2-T24/D02` | `ready` | Registered formal slice; implementation has not started | waits for D01 within the same future task branch and lease |
