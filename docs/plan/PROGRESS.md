...
| Benchmark readiness product train | `P2-T21` implementation complete, pending PR #220 merge; `P2-T22..P2-T28` registered and not started | Owner instruction 2026-08-15 approved BR-01..BR-08 and the Linux-only product-validation route. Ubuntu is supporting CI; exact pushed-revision `DEV-LINUX-NATIVE-01` is required acceptance evidence for these product tasks; `B01-Desktop-Linux-002` remains owner-directed EVAL-004-only. Windows is `not-run by owner-directed Linux-only route`. This is product implementation evidence only, not EVAL/Gate/release/Profile/B01 evidence | ready/merge PR #220, then claim `P2-T22` (BR-02 software repair journey) on its own branch/lease; EVAL-004 re-freeze happens only after every BR product task is merged |
...
| Active task lease | `none` | P2-T21 implementation and supported validation are complete on Draft PR [#220](https://github.com/agentkernel/cognitive-os/pull/220); the governed-candidate-parameters-terminal-evidence lease is recorded in Parallel Lanes 3.1 pending ready/merge. Ubuntu required CI green at `a942abd5` (run `31902392486`); `DEV-LINUX-NATIVE-01` exact-revision validation at `0404f0be` is in [P2-T21 running validation report](../checkpoints/20260816-personal-p2-t21-governed-candidate-terminal-evidence-report.md) | ready/merge PR #220, then claim `P2-T22` on its own branch/lease |
| P2-T21 Governed candidate parameters and terminal evidence | **in-progress** | D01 bounded candidate-parameter contract, D02 public Task admission → Pi candidate → daemon canonicalization/digest → governed Intent → scheduler → production Search/Write/Patch router/sink reachability, and D03 authority/CAS-rehydrated redacted terminal-evidence endpoint (`GET /task/evidence`) plus `admin-cli evidence` caller. Focused negatives: digest mismatch, preimage drift, family/epoch/action drift, unknown outcome / restart / missing-CAS, cross-task/wrong-channel query rejection. Ubuntu required CI green at `a942abd5` (run `31902392486`); `DEV-LINUX-NATIVE-01` (`wuz@192.168.1.2`, hal9000, Rust 1.97.1) at `0404f0be`: kernel-server bin tests 283/283, full workspace tests, `cargo clippy --workspace --all-targets --locked -- -D warnings` green, `cargo fmt --all -- --check` green. Windows is `not-run by owner-directed Linux-only route`. `B01-Desktop-Linux-002` untouched (EVAL-004-only guest). No generic tool parameter channel, authority-shaped candidate input, private candidate injection, raw SQLite, or secret-bearing output. Closure evidence: [running validation report](../checkpoints/20260816-personal-p2-t21-governed-candidate-terminal-evidence-report.md). Claim ceiling `hypothesis`; no Gate, release, Profile, B01, EVAL, or Agent-benefit promotion | ready/merge Draft PR [#220](https://github.com/agentkernel/cognitive-os/pull/220), then close the task branch and claim P2-T22 |
...
| `P2-T22/D01` | `ready` | Registered formal slice; implementation has not started | claim `P2-T22` only after PR #220 merges |
