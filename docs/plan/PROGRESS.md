...
| P9-T07 Pi route nested stage timing | **in-progress** | Correctively renumbered from the pre-merge P9-T05 registration after PR #213 made the daemon readiness repair the canonical P9-T05; P9-T06 is already a separate readiness/SecretStore workflow. Draft PR [#216](https://github.com/agentkernel/cognitive-os/pull/216) stays on legacy-named branch `personal/P9-T05-pi-nested-stage-timing` to avoid rewriting pushed history. Checkpoint `fd2f4e9a2a1f8ef135a19024dcea7cb8f1d1080d`: every authorized started attempt has an exact content-free terminal record; success joins seven stages, cancellation/error retain measured prefixes, concurrent correlation is isolated, and Provider usage is request-bound/immutable/single-publication. Local Pi suite 90/90; handbook, generator, consistency and docs-sync green; instrumentation-only local probe p50 80.0 µs/p95 327.4 µs (fixture non-claim). Rust daemon exact native proof remains `554c6cf9`: route observation 9/9, timing split 1/1, front door 2/2, fmt/Clippy pass | finish main reconciliation, push the merge revision, obtain required Ubuntu/Windows CI, then acceptance/closure; `EVAL-003/PI-NESTED` remains `not-run` because no frozen plan/runner/report is registered |
| Active task lease | `lease/personal/P9-T07/pi-nested-stage-timing` — `P9-T07/D04` on legacy-named branch `personal/P9-T05-pi-nested-stage-timing` | Exact paths are registered in Parallel Lanes; product code does not overlap P2-T13/P2-T14 scheduler/verification/store ownership. Shared plan files are reconciled only as the later merger | required CI on the pushed main-reconciled revision, then deterministic closure |
...
| `P9-T07/D04` | `in-progress` | failure-first terminal schema, cancellation/error retention, concurrent correlation, exact key-set validation and request-bound immutable single-publication usage provenance are implemented; local Node 90/90 and documentation/static gates pass | push the main-reconciled revision and obtain required Ubuntu/Windows CI before acceptance closure |
