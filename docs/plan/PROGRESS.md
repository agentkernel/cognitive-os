...
| Active task lease | `lease/personal/P7-T01/release-pipeline` (+ retained `lease/personal/P5-T05/b09-managed-pi`) | P7 owns release-manifest/lib/PROGRESS/plan/checkpoints paths; P5 retains B09 implementation paths without overlapping plan docs | await required CI on Draft PR #184 at `345b462`; then close D01 and start D02 |
...
| P7-T01 Release pipeline, six-resource manifest, SBOM & attestation | `in-progress` | `lease/personal/P7-T01/release-pipeline` on `personal/P7-T01-release-pipeline` / Draft PR #184. D01 signed six-family `release_manifest` authority path landed at `3108889e0f9ff9c8932e599d835a9085adc4ec69` (docs HEAD `345b462832a94e3fcda149256b1c31a1b8e2eec1`). Exact native Linux focused `release_manifest` 6/6 + Clippy `-D warnings` passed via git-bundle/SCP on DEV-LINUX-NATIVE-01. Local fmt + consistency passed. | await required Ubuntu/Windows CI on PR #184 at `345b462`; then close D01 and enter D02 SBOM digest binding |
