...
| Benchmark readiness product train | `P2-T21` **done** at `5d1f5c2643d82807eb96f2c615d460dbdca749c3` / PR #220; `P2-T22` in-progress D02; `P2-T23..P2-T28` registered and not started | Owner instruction 2026-08-15 approved BR-01..BR-08 and the Linux-only product-validation route. Ubuntu is supporting CI; exact pushed-revision `DEV-LINUX-NATIVE-01` is required acceptance evidence for these product tasks; `B01-Desktop-Linux-002` remains owner-directed EVAL-004-only. Windows is `not-run by owner-directed Linux-only route`. This is product implementation evidence only, not EVAL/Gate/release/Profile/B01 evidence | continue `P2-T22/D02` governed write → RegisteredCheckRun → verifier → acceptance; EVAL-004 re-freeze happens only after every BR product task is merged |
...
| Active task lease | `lease/personal/P2-T22/governed-software-repair` | P2-T21 closed (PR #220 merged at `5d1f5c2643d82807eb96f2c615d460dbdca749c3`). Active table owns `P2-T22/D02` production wiring of write → RegisteredCheckRun → independent verifier → acceptance on the same Task. Ubuntu supporting CI; exact-revision `DEV-LINUX-NATIVE-01`; Windows `not-run by owner-directed Linux-only route` | continue D02 governed journey wiring and focused negatives; then D03 linux-002 matrix |
...
| P2-T22 Governed software-repair journey | **in-progress** | D01 freeze complete at `4a737884d80a2a25b350f965ea9b92229e6ea356` (Ubuntu required CI run `31915159130` green; `DEV-LINUX-NATIVE-01` clippy `-D warnings` + fmt). D02 wires a closed intermediate WorkspaceWrite back to Loop `DECIDE` so a later tick admits `check_id`-only RegisteredCheckRun; only that check's independent verifier plus acceptance may complete the Task. Lease `lease/personal/P2-T22/governed-software-repair` on `personal/P2-T22-governed-software-repair` / Draft PR [#221](https://github.com/agentkernel/cognitive-os/pull/221). Running report: [20260816-personal-p2-t22-governed-software-repair-report.md](../checkpoints/20260816-personal-p2-t22-governed-software-repair-report.md). Windows `not-run by owner-directed Linux-only route`. Claim ceiling `hypothesis`; no Gate/release/Profile/B01/EVAL promotion | complete D02 production-path positives and focused negatives, then D03 linux-002 matrix |
...
| `P2-T22/D02` | `in-progress` | Production caller: closed WorkspaceWrite returns Loop to `DECIDE`; later tick admits `check_id`-only RegisteredCheckRun against the workspace capability; only that check's independent verifier plus acceptance may complete the Task. Focused negatives: hidden-test gutting, public-test weakening, out-of-scope write. Public C1 WorkspaceRead with the fixed-Effect verifier remains on `ACT -> VERIFY` | Ubuntu supporting CI and exact-revision `DEV-LINUX-NATIVE-01`; then D03 |
| `P2-T22/D03` | `ready` | Registered formal slice; implementation has not started | waits for D02 and owns whole-task validation/closure |
