...
| Active task lease | `lease/personal/P8-T09/dsh-akp-adapter` | owner-authorized DeepSeek Harness candidate-only AKP adapter on `personal/P8-T09-dsh-akp-adapter`; EVAL-015 remains closed and is not this task | remaining D03: linux-002 Path B real dsh + Provider proxy; then Windows/Ubuntu CI and D04 merge |
| P8-T09 DeepSeek Harness AKP adapter | `in-progress` | linux-002 identity confirmed. Jump-host Path A headless `pong` **pass** (9.66 s, pin `528c682e`). Shim Path B Read/Search/Write `COMPLETED`. Cordis `apply` plugin landed. Real dsh→daemon Flash **not-run** on linux-002 yet. Claim ceiling `hypothesis`. | linux-002 Path B `dsh --patch` through Provider proxy; do not auto-claim P6/P7 |
...
| `P8-T09/D03` | `in-progress` | linux-002 identity pass. Shim Path B Read/Search/Write `COMPLETED`. Jump-host Path A **pass** (`pong`, 9.66 s) at dsh pin `528c682e`. Cordis plugin + `dsh-real-process.mjs` landed. linux-002 real dsh→Flash **not-run**. | Path B on linux-002 at pushed HEAD; retained denominator |
