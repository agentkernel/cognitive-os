...
| P2-T06 Tool/process executor | `in-progress` | `P2-T06/D01` and `D02` are done at immutable `54bd1748e6a3611503242a0f458510eb84e52f67`: the descriptor-bound WorkspaceRead sink now runs through the durable Intent/Effect protocol, records `EXECUTING` before filesystem I/O, records explicit outcomes, and reconciles a simulated lost response through the original key without a second read. Exact native Linux focused tests/Clippy/fmt passed; required Ubuntu/Windows CI passed in Draft PR #162. This remains `experimental-local-only` / `tested-supported-ci` implementation evidence only. | implement `P2-T06/D03`: bounded process/check execution with supervisor lifetime, timeout/orphan handling, and the same durable Effect/reconciliation boundary |
...
| `P2-T06/D03` | `in-progress` | D02's durable Effect and reconciliation boundary is stable at `54bd1748e6a3611503242a0f458510eb84e52f67`. | add bounded process/check execution, supervisor timeout/orphan negatives, and restart-safe reconciliation |
