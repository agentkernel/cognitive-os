...
|---|---|---|---|
| Project focus | `cognitiveos-personal`: active and sole implementation project | CognitiveOS architecture assets remain reference/contract inputs; no second product backlog | `PERSONAL-PERF-EVAL-013` **closed**. Evaluation routing OFF. Do not claim a `P*-T*` task until the owner gives a fresh delivery instruction. P7-T07 remains blocked on B01-W prerequisites |
| Owner-directed campaign | `PERSONAL-PERF-EVAL-013` **closed** (2026-08-20) on `evaluation/EVAL-013-freeze`. Evaluation routing OFF. Product pin `6c415625c0ca12e9f8e51d139da601eb475ee231`. B0 **pass** (C1/C2a comparable + fairness 13/13). B1–B5/C0 `not-run` (no frozen live paired/soak/fault executor). Cleanup **pass** (daemon `48302` / broker `48402` stopped; SecretStore `/25` cleared). EVAL-002 and EVAL-004 through EVAL-012 remain **closed**. | Preregistration: [EVAL-013 freeze](../checkpoints/20260820-personal-perf-eval-013-preregistration.md), [EVAL-013 preregistration](../evaluation/personal-perf-eval-013-preregistration.md). Running report: [EVAL-013 assessment](../evaluation/personal-performance-assessment-20260820-eval-013.md). Claim ceiling `hypothesis`; `not_reviewed` | none for this campaign — wait for an explicit owner delivery instruction before claiming any implementation task |
| Owner-directed C1/C2 campaign | `PERSONAL-PERF-EVAL-013` closed. C1 and C2a B0 **pass** on pin `6c415625`. C2b–d split-score / `not-run`. B1/B2 not opened. | Plan: [PERSONAL-C1-C2-READINESS-DELIVERY-PLAN.md](PERSONAL-C1-C2-READINESS-DELIVERY-PLAN.md). Claim ceiling `hypothesis`; no Gate, release, Profile, B01, or Agent-benefit promotion. | none — campaign closed |
| Active task lease | `none` | `PERSONAL-PERF-EVAL-013` closed; evaluation lease closed | wait for owner delivery instruction |
| Benchmark readiness product train | `P2-T21` … `P2-T38` **done**; **P9-T08 done**; **P9-T09 done**; **P9-T10 done**; **P9-T11 done**; `PERSONAL-PERF-EVAL-012` **closed**; `PERSONAL-PERF-EVAL-013` **closed** | Navigation: [PERSONAL-C1-C2-READINESS-DELIVERY-PLAN.md](PERSONAL-C1-C2-READINESS-DELIVERY-PLAN.md). Packages 1–14 delivered. Live `--append-system-prompt` and comparable C2a unified-diff Patch are merged. EVAL-013 B0 C1/C2a **pass**. No Gate, release, Profile, B01, or Agent-benefit conclusion follows. | none — wait for owner delivery instruction |
