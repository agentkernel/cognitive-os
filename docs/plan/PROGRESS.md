...
| P2-T12 Admitted Task reaches the worker loop | **not-started** | Registered 2026-08-12 under the owner's delivery instruction. Removes wiring gaps 1 and 2 and connects the P2-T10 sinks to the daemon's own worker path: `task.admit` must persist the scheduler work row in the same authority transaction, the daemon must run a bounded non-reentrant periodic tick instead of one startup tick, and the worker attempt must dispatch the durable Tool Effect through the assembled executor selected by the persisted descriptor. Measured trigger: `UJ4` recorded 30/30 admitted, 0 executed, 0 verified | claim a non-overlapping lease and start `P2-T12/D01` (the durable scheduler row at admission) |
| P2-T13 Independent verification caller and verified completion | **not-started** | Registered 2026-08-12 under the same instruction. Removes wiring gap 4: the P2-T07 verifier is implemented but has only test callers, so "verified completion" is structurally impossible and every campaign must report 0 verified completions. Task completion must advance only on an affirmative out-of-band report bound to the fixed post-state, the current epoch and closed reconciled Effects — never on a Tool receipt, Provider response, process exit or Pi `agent_end` | after P2-T12: claim a lease and start `P2-T13/D01` (the production verification caller) |
...
| `P2-T12/D01` | `ready` | not started | `task.admit` persists the scheduler work row in the same authority transaction; startup recovery repairs an admitted Task whose row is missing |
| `P2-T12/D02` | `ready` | not started | after D01: the bounded, non-reentrant, cancellable periodic scheduler tick |
| `P2-T12/D03` | `ready` | not started | after D02: dispatch the durable Tool Effect from the worker attempt through the assembled executor chosen by the persisted descriptor |
| `P2-T12/D04` | `ready` | not started | after D03: acceptance mapping, handbook gap-record update, and closure |
| `P2-T13/D01` | `ready` | not started | the daemon-owned production caller that submits a verification request once the Task's Effects are closed and reconciled under a non-stale epoch |
| `P2-T13/D02` | `ready` | not started | after D01: advance Task acceptance only on an affirmative out-of-band report bound to the fixed post-state, current epoch and closed Effects |
| `P2-T13/D03` | `ready` | not started | after D02: acceptance mapping, the capability classes that become executable, and closure |
