...
| Active task lease | `lease/personal/P4-T03/memory-lifecycle` | P4-T03/D01 is the only active delivery slice on `personal/P4-T03-memory-lifecycle` | complete the private Memory lifecycle task under the same Draft PR |
...
| P4-T03 Memory lifecycle, retention and forget | `in-progress` | `P4-T03/D01` is the active implementation-only private forget/tombstone slice under `lease/personal/P4-T03/memory-lifecycle`; focused validation is not-run. It will append an auditable tombstone, atomically invalidate FTS, and prevent stale-index resurrection without changing public contracts. Version/update/conflict/expiry and public Memory API remain unclaimed. | implement failure-first lifecycle tests, then run exact native Linux and required CI validation |
...
| `P4-T03/D01` | `in-progress` | `lease/personal/P4-T03/memory-lifecycle` owns the implementation-only daemon-private forget/tombstone and FTS invalidation path at `34ee8ecd02525bdae8b294566b79175f1aef345f`; focused validation is not-run. | run exact-revision native Linux lifecycle/migration/FTS tests, then complete the remaining lifecycle acceptance items |
