...
| P1-T09 route implementation | `in-progress` | `experimental-local-only`; exact Pi `0.81.1`, native Secret Service initialization, and a bounded daemon-owned Provider-proxy response were observed on the SSH-qualified Linux-native host, with no authority side effect. The coherent-bundle implementation includes `kernel-server`, product `cognitive`, and the complete built Pi Extension distribution; supported CI passed Ubuntu and Windows. The protected experimental Environment and its signing secret were configured, PR #118 was merged to `main`, and campaign run `30550012325` passed reviewed inputs and Linux payload builds. Its builder/offline verifier succeeded, but its final public facts check used the wrong rendered bootstrap variable name, so artifact upload and all deployment/install/route evidence remain `not-run`. The corrective workflow branch changes only that public facts regex; it neither exposes nor changes the signing secret. No Task, Effect, Verification, capability or authority side effect was created. No product-Gate, release, B01, GMVP-LINUX, or Profile claim is made | merge the corrective workflow change, rerun a protected signed campaign, independently inspect the uploaded artifact, then begin separately leased controlled deployment |
...
| Active task lease | `lease/personal/P1-T09/experimental-campaign-artifact` | scoped to the public campaign facts-check correction and signed-artifact rerun; deployment paths remain unleased and untouched | merge the corrective workflow change and rerun the protected experimental campaign |
