...
| Lane-RUN 运行时与管理面 | **D-018 persisted governance-header consumption implemented** | `lane/run-d018-governance-port-consumption` | publication envelope resolves its header through the durable KRN port and fails closed when absent; this is Ordinary Core minimum audit plumbing, not a High-Assurance claim or D-018 closure. |
