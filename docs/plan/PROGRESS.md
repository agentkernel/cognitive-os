...
|---|---|---|---|
| Project focus | `cognitiveos-personal`: active and sole implementation project | CognitiveOS architecture assets remain reference/contract inputs; no second product backlog | continue `P7-T01/D02` on `personal/P7-T01-release-pipeline` / Draft PR #184 |
| Active task lease | `lease/personal/P7-T01/release-pipeline` | P7 owns release-manifest/lib/PROGRESS/plan/checkpoints/tools-test paths on `personal/P7-T01-release-pipeline` / Draft PR #184 | validate D02 on DEV-LINUX-NATIVE-01; then enter D03 toolchain pins |
...
| P7-T01 Release pipeline, six-resource manifest, SBOM & attestation | `in-progress` | `lease/personal/P7-T01/release-pipeline` on `personal/P7-T01-release-pipeline` / Draft PR #184. D01 closed (signed six-family manifest + CI `31425522168`). D02 implements SBOM/artifact digest binding and contaminated-inventory rejection on the D01-verified path. | exact native Linux focused `release_manifest` tests + Clippy; then D03 |
...
`P7-T01` is `in-progress` with unique slice `P7-T01/D02`. `P5-T05` is `done`
...
| `P7-T01/D02` | `in-progress` | SBOM/artifact digest binding + contaminated-inventory rejection on D01-verified manifests | exact native Linux focused module tests + Clippy; required CI |
