...
| Active task lease | `lease/personal/P2-T16/memory-skill-agent-consumer` | Isolated sibling worktree `D:\agent-kernel-msconsumer`, branch `personal/P2-T16-memory-skill-consumer`, based on unmerged P2-T13 head `b514d278ef4a3daafe9cceeb62ced2dc649d186b`; writable paths are the Memory/Skill stores, governed Context/resource API seams, focused tests, one running report and mapped bilingual handbook pages only | commit and remotely observe the D01 failure-first governed-consumption negative, then implement D02 without touching the main worktree or P2-T14 paths |
| P2-T16 Production governed Memory/Skill Agent consumer | **in-progress** (`D01`) | Owner-directed 2026-08-13 delivery allocated the next non-colliding Phase-2 ID after P2-T15. D01 starts failure-first on an isolated branch from unmerged P2-T13. Target acceptance is daemon-derived Memory/Skill authorization before body/ranking, exact digest/provenance binding in sealed ContextView, durable task/epoch/request/view consumption facts, restart/session-2 resume without chat/user restatement, immediate future-load blocking after forget/revoke/supersede with stable exact pins, candidate-only Pi, complete lifecycle client paths and the registered negative set. Change class is implementation-only unless a genuinely new public field/error proves necessary; no parallel store/client authority and no Gate/release/Profile/B08/EVAL claim | commit the expected-failing D01 test, push a Draft PR explicitly dependent on PR #210 and P2-T14, then use required Ubuntu/Windows CI to record the failure before implementing D02 |
...
| `P2-T16/D01` | `in-progress` | Failure-first test added on isolated branch from unmerged P2-T13; local Rust execution is not supported on `DEV-WIN-GNU-01`, so the expected baseline failure remains `not-run` until pushed CI | commit/push the expected-failing test and observe it on exact-revision required CI |
| `P2-T16/D02` | `ready` | not started | after D01 failure observation: add metadata-first eligibility, sealed exact bindings and durable consumption persistence |
| `P2-T16/D03` | `ready` | not started | after D02: wire daemon-approved fragments to candidate-only Pi and prove durable session-2 resume |
| `P2-T16/D04` | `ready` | not started | after D03: close documented public lifecycle cycles and channel/partial-residue negatives |
| `P2-T16/D05` | `ready` | not started | after D04 and P2-T14 merge: rebase, validate, review, sync bilingual handbook, then merge/reconcile |
