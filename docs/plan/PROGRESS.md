...
|---|---|---|---|
| `P9-T05/D01` | `in-progress` | Failure-first proof of the confirmed CI defect. `apps/kernel-server/tests/common/mod.rs` now holds the single daemon readiness wait, carrying today's exact budget (100 probes × 20 ms = 2 s) and today's semantics, so the budget is directly assertable. `p9_t05_daemon_readiness_wait.rs` adds two cases: a healthy start published after 2.5 s — longer than the ~2.2 s the Windows runner actually took — must still be waited for, and a readiness that never arrives must still fail inside its own budget with a diagnostic naming the missing subject. The first case fails against the 2 s budget by construction; that failure is the proof | commit the failing proof, then D02 |
| `P9-T05/D02` | `ready` | — | replace the fixed budget with a bounded readiness poll plus exited-daemon detection, and move every sibling wait in `apps/kernel-server/tests` onto it |
