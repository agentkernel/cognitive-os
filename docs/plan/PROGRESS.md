...
| Active task lease | `lease/personal/P9-T09/system-prompt-fairness` | P9-T09/D01 on `personal/P9-T09-system-prompt-fairness` | continue D01/D02: freeze shared prompt bytes; do not start a new EVAL |
...
| P9-T09 C1/C2 system_task_prompt_bytes | **in-progress** | EVAL-012 Priority 2. Dry-run no longer forges equality with `frozen-c1-c2-prompt-v1`; both arms observe `frozen-system-task-prompt.txt` byte length. Local `c1_c2_paired_p_arm` **15/15**. | D03 Linux/CI + live O-arm injection remaining; no Gate/EVAL claim |
...
|---|---|---|---|
| `P9-T09/D01` | `in-progress` | Local `c1_c2_paired_p_arm` **15/15** including placeholder rejection. | D02 freeze file already in this change set; Linux/CI on D03 |
| `P9-T09/D02` | `ready` | D01 names the placeholder defect | observe frozen prompt byte length on both dry-run arms |
| `P9-T09/D03` | `ready` | waits on D02 | Linux/CI, docs-sync, ready/merge/lease/branch/main |
