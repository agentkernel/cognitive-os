...
|---|---|---|---|
| Project focus | `cognitiveos-personal`: active and sole implementation project | CognitiveOS architecture assets remain reference/contract inputs; no second product backlog | EVAL-010 **closed**. P2-T32 **done** (PR [#239](https://github.com/agentkernel/cognitive-os/pull/239) `main@c7ce4e5f`), P2-T33 **done** (PR [#240](https://github.com/agentkernel/cognitive-os/pull/240) `main@09a44455`), and P2-T34 **done** (PR [#241](https://github.com/agentkernel/cognitive-os/pull/241) `main@106cfcc0`). P2-T35 **in-progress** (D01 failure-first). P7-T07 remains blocked on B01-W prerequisites |
...
| Active task lease | `lease/personal/P2-T35/candidate-digest-default` | P2-T35 private-candidate JSON-fallback digest default and adapter diagnostic separation; branch `personal/P2-T35-candidate-digest-default` | complete focused D01/D02 verification on exact pushed linux-002 revision, then required Ubuntu CI |
...
| Benchmark readiness product train | `P2-T21` … `P2-T34` **done** (`main@106cfcc0`); `P2-T35` **in-progress** | EVAL-011 evidence: a JSON fallback omitted `parameters_digest` (`missing field parameters_digest at line 1 column 118`), while adapter USAGE mixed with Pi runtime errors prevented attribution. T35 defaults the private candidate digest only for independently recomputed parameters and separates adapter diagnostics. Claim ceiling `hypothesis` | run exact-revision linux-002 validation and required Ubuntu CI; no Gate/B01 promotion |
...
| P2-T35 Private-candidate digest default and diagnostics | **in-progress** | EVAL-011 `private_pi_candidate_adapter_rejected_the_request`: omitted JSON `parameters_digest` yielded `missing field parameters_digest at line 1 column 118`; mixed USAGE and runtime stderr obscured cause. D01 adds omitted/missing/unknown-field negatives plus exit-class and redacted diagnostic-tail coverage. D02 defaults only the adapter response field, retains fail-closed daemon validation, makes runtime errors exit 3 without USAGE, and reports adapter exit code. Running report: [P2-T35 validation](../checkpoints/20260818-personal-p2-t35-candidate-digest-default-report.md). Windows Rust is `not-run by owner-directed Linux-only route`. Claim ceiling `hypothesis`; no Gate/release/Profile/B01/EVAL promotion. | push exact revision, run linux-002 focused tests/Clippy/fmt, then required Ubuntu CI; D03 records whether a new live evaluation is separately authorized and any residual skip subclass |
