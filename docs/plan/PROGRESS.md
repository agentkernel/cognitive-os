...
| P3-T01 Context source/retrieval port | `in-progress` | `experimental-local-only` / `tested-supported-ci`; `P3-T01/D01` now has TaskContract v0.4 ContextRequest binding, immutable Context request/view/source persistence, metadata-only source discovery, and daemon-admin admission routes for sealed authorization/revocation facts. Exact native Linux `cda31dc4ec74ae5faaf3d8d47ecb902e97dc8af3` passed Context-store 8/8, including a prior allowed body read denied after durable epoch advancement; required Ubuntu/Windows CI passed on the preceding authority-admission checkpoint. The owner-approved MVP path uses one owner-local management session and does not require advanced multi-principal policy administration before resolver integration. B03 and task completion remain absent. | wire the owner-local Context source path to the private candidate transport; keep Pi candidate-only |
...
| Active task lease | `lease/personal/GOV/task-atomic-delivery-rules`; `lease/personal/P2-T04/private-worker-composition`; `lease/personal/P2-T05/native-tool-registry-validation`; `lease/personal/P3-T01/context-request-binding` | The governance lease owns the whole-task delivery/MVP-first/closure rule refactor on `lane/governance-task-atomic-delivery-rules`. P2-T04/D01 is active under its narrow private-worker-composition lease, and P2-T05 owns the native catalog/validator review boundary. P3-T01/D01 owns the ContextRequest binding/persistence prerequisite. Pi remains candidate-only and scheduler/verified-continuation authority remains daemon-owned. B02/B03/B04/B05/B12, release, Profile, and Task completion remain separate/incomplete. | merge and close the governance refactor; then continue each active formal task atomically on its own branch/PR through acceptance and deterministic closure |
