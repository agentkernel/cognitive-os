...
| P1-T09 route implementation | `in-progress` | `experimental-local-only`; campaign `30553835734` built, signed, offline-verified, and uploaded the coherent bundle after protected Environment approval. Local and remote SHA-256 checks passed, and the host-built verifier independently accepted the signed archive, expected Pi pin, and public key. The qualified Linux host is glibc `2.35`; the signed CI payload requires glibc `2.38/2.39`, so verified installation published then compensated/rolled back before active-pointer promotion. The prior user service remains active and no product CLI/Extension is active. Campaign `30560351793` confirmed `ubuntu-22.04` is the correct payload baseline but failed before signing because current AWS-LC workspace tests reference `__isoc23_*` symbols unavailable on that runner. The workflow now gates the protected glibc-2.35 signing job on a separate secret-free `ubuntu-latest` full-validation job, preserving both full validation and compatible payload construction. Exact Pi is not persistently installed on the host; prior exact-Pi evidence was a pin-bound package invocation only. No Task, Effect, Verification, capability or authority side effect was created. No product-Gate, release, B01, GMVP-LINUX, or Profile claim is made | merge the split validation/signing workflow, issue a fresh protected signed campaign, then rerun verified installation and configure an exact Pi executable path before installed-route diagnostics |
...
| Active task lease | `lease/personal/P1-T09/verified-experimental-deployment` | scoped to the glibc baseline correction and the compatible signed-bundle deployment retry; Provider/secret configuration and B01 paths remain unleased | merge the corrective workflow change and rerun the protected experimental campaign |
