...
| Active task lease | `lease/personal/EVAL-20260813/performance-evaluation-002-recovery` | Narrow owner-directed evaluation recovery lease on `main`; writable paths are limited to the campaign plan/report/preregistration and this Current snapshot. It creates no implementation task or Slice status and does not authorize product, contract, negative, test, handbook, snapshot, power, P9-T04 residue, system, or network changes | continue the running `B5` 8 h denominator, then append result, cleanup, reconcile reports/capability truth, close this lease and campaign, and stop without resuming implementation |
