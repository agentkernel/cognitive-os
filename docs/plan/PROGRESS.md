...
| Active task lease | `lease/personal/P2-T27/user-lifecycle-backup` | P2-T26 merged via PR #225 at `main@9e1404a1`. Unique in-progress slice `P2-T27/D03`. Draft PR [#226](https://github.com/agentkernel/cognitive-os/pull/226). D03 linux-002 at `5a561fbf`: store 23/23, CLI 2/2, HTTP 1/1, Pi lifecycle 1/1, fmt, Clippy. Ubuntu `31939336791` in progress. Windows `not-run by owner-directed Linux-only route`. `B01-Desktop-Linux-002` untouched (EVAL-004-only) | wait Ubuntu supporting CI then ready/merge/lease/branch/main |
...
| P2-T27 User lifecycle and backup/restore | **in-progress** | D01 public backup/restore complete at `6eea42c4`. D02 Pi install→recover complete at `cf7dd501`. D03 linux-002 matrix **pass** at `5a561fbf` (store 23/23, CLI 2/2, HTTP 1/1, Pi lifecycle 1/1, fmt, Clippy `-D warnings`). Draft PR [#226](https://github.com/agentkernel/cognitive-os/pull/226). Ubuntu `31939336791` in progress. Windows `not-run by owner-directed Linux-only route`. Claim ceiling `hypothesis`; no Gate/release/Profile/B01/EVAL promotion | wait Ubuntu supporting CI then ready/merge/lease/branch/main |
...
| `P2-T27/D03` | `in-progress` | Exact-revision `DEV-LINUX-NATIVE-01` at `5a561fbf`: `personal_backup` 23/23, CLI backup 2/2, HTTP 1/1, `p2_t27_pi_lifecycle` 1/1, fmt, Clippy `-D warnings`. Ubuntu run `31939336791` in progress on Draft PR [#226](https://github.com/agentkernel/cognitive-os/pull/226). Windows `not-run by owner-directed Linux-only route` | wait for Ubuntu supporting CI, then ready/merge/lease/branch/main |
