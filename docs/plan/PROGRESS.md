...
| Active task lease | `lease/personal/P9-T09/system-prompt-fairness` | P9-T09/D03 on `personal/P9-T09-system-prompt-fairness` | D03 ready/merge: required CI `32335402680` passed; live O-arm injection remains a later task |
| Benchmark readiness product train | `P2-T21` … `P2-T38` **done**; **P9-T08 done**; `P9-T09` **in-progress**; `PERSONAL-PERF-EVAL-012` **closed** | Navigation: [PERSONAL-C1-C2-READINESS-DELIVERY-PLAN.md](PERSONAL-C1-C2-READINESS-DELIVERY-PLAN.md). Packages 1–14 delivered. EVAL-012 B0 fairness fail blocked B1/B2. P2-T38 closed the Patch post-state gap. No Gate, release, Profile, B01, or Agent-benefit conclusion follows. | implement P9-T09 prompt-byte freeze; do not activate a new EVAL until fairness can pass |
| P9-T09 C1/C2 system_task_prompt_bytes | **in-progress** | EVAL-012 Priority 2. Dry-run no longer forges equality with `frozen-c1-c2-prompt-v1`; both arms observe `frozen-system-task-prompt.txt` byte length. Local `c1_c2_paired_p_arm` **15/15**. Required CI `32335402680` passed Ubuntu, Windows, and required-ci at `e9bdd070`. | D03 ready/merge/lease/branch/main; live O-arm injection is a later task before a new EVAL |
...
| `P9-T09/D03` | `in-progress` | Required CI `32335402680` passed Ubuntu, Windows, and required-ci at `e9bdd070`. Live O-arm injection **not-run** (allowed later). | ready/merge/lease/branch/main |
