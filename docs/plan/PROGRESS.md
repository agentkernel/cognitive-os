...
|---|---|---|---|
| Project focus | `cognitiveos-personal`: active and sole implementation project | CognitiveOS architecture assets remain reference/contract inputs; no second product backlog | `PERSONAL-PERF-EVAL-012` is active. Execute package 15 (B0) on `B01-Desktop-Linux-002`. P7-T07 remains blocked on B01-W prerequisites. Do not claim a `P*-T*` task while this row is active. |
| Owner-directed campaign | `PERSONAL-PERF-EVAL-012` **active** (2026-08-20). Measurement-only. Evaluation routing ON. Packages 1–14 remain historical/readiness evidence. Package 15 (B0) is the unique next measurement step; B1/B2 wait on B0 pass. EVAL-002 and EVAL-004 through EVAL-011 remain **closed**. | Plan: [personal-perf-eval-012-preregistration.md](../evaluation/personal-perf-eval-012-preregistration.md). Freeze: [20260820-personal-perf-eval-012-preregistration.md](../checkpoints/20260820-personal-perf-eval-012-preregistration.md). Running report: [personal-performance-assessment-20260820-eval-012.md](../evaluation/personal-performance-assessment-20260820-eval-012.md). Product pin `370b26fcc05976c7c1c97e5510a99ed3ebc23f2c`. Isolation: root `/home/hal9001/perfeval012-20260820`, daemon `127.0.0.1:48300`, broker `127.0.0.1:48400`, SecretStore planned `/20` (≠ `/12`–`/19`). Do not reopen EVAL-004/005/006/007/008/009/010/011. Do not reuse those roots, ports `48286`–`48298` / `48386`–`48398` / `48383`, SecretStore `/12`–`/19`, or P2-T37 roots `p2-t37-c2a-write-20260820` / `p2-t37-c2a-patch-20260820`. Claim ceiling `hypothesis`; `not_reviewed` | unique next action: E9 `not-run` — guest Secret Service has 0 product items (EVAL-010 cleared `/19`). Owner graphical hidden-input into planned `/20` on `B01-Desktop-Linux-002`, then doctor and B0 (`retry=0`; retain every started sample). Do not reuse `/12`–`/19` or recapture from the owner keyfile. |
| Owner-directed C1/C2 campaign | Owner-activated `PERSONAL-PERF-EVAL-012`. Packages 1–14 delivered. Package 15 freeze/identity/Pi pin **pass**. E9 Secret bind `not-run`. | Plan: [PERSONAL-C1-C2-READINESS-DELIVERY-PLAN.md](PERSONAL-C1-C2-READINESS-DELIVERY-PLAN.md). Claim ceiling `hypothesis`; no Gate, release, Profile, B01, or Agent-benefit promotion. | Owner graphical hidden-input into planned `/20`; then B0 on `B01-Desktop-Linux-002`. |
| Active task lease | `lease/personal/EVAL-012/c1-c2-paired-b0` | `PERSONAL-PERF-EVAL-012` measurement-only; writable paths limited to EVAL docs, the freeze checkpoint, and `PROGRESS.md` | wait for owner SecretStore `/20` import; then doctor and B0 |
...
| Benchmark readiness product train | `P2-T21` … `P2-T37` **done**; **P9-T08 done** (`main@f24c282e`) | Navigation: [PERSONAL-C1-C2-READINESS-DELIVERY-PLAN.md](PERSONAL-C1-C2-READINESS-DELIVERY-PLAN.md). Packages 1–14 delivered. `PERSONAL-PERF-EVAL-012` now owns measurement. No Gate, release, Profile, B01, or Agent-benefit conclusion follows. | Continue EVAL-012 B0; do not claim a product task. |
...
| `P9-T08/D04` | `done` | B01 procedure, bind runbook, `cells.json`, EVAL-012 scaffolding. E7–E9 were `not-run` until EVAL activation. Merged PR [#247](https://github.com/agentkernel/cognitive-os/pull/247) at `main@f24c282e`. Required CI `32316092401` passed Ubuntu, Windows, and required-ci. | consumed; `PERSONAL-PERF-EVAL-012` is active |
| `EVAL-012/B0` | `in-progress` | Owner activated `PERSONAL-PERF-EVAL-012`. Lease `lease/personal/EVAL-012/c1-c2-paired-b0` on `evaluation/EVAL-012-freeze`. Product pin `370b26fc`. Guest identity/freeze/Pi pin **pass**. E9 Secret bind `not-run` (0 Secret Service items). No sample started. | Owner graphical hidden-input into planned `/20`; then doctor and B0 cells |
