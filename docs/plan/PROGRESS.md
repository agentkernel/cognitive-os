...
| P9-T05 Pi route nested stage timing | **in-progress** | Claimed 2026-08-13 on `personal/P9-T05-pi-nested-stage-timing` / Draft PR [#216](https://github.com/agentkernel/cognitive-os/pull/216), based on `origin/main@326f97728ab6aaaacceaedd2156d953231b32e01`. D01–D02 delivered the Pi-side joined seven-stage observation and Provider usage exposure (Node 81/81 plus 20/20 negatives). D03 adds the daemon nested split: config/selected-model/SecretStore preflight vs Provider network, an authorized correlation echo, and header-only negatives that leave the product body and authority store untouched. Measurement capability only; no Gate/release/Profile/B01/benchmark or overhead-attribution claim | finish D03 exact-revision Rust validation, then D04 acceptance/handbook/CI/closure |
| Active task lease | `lease/personal/P9-T05/pi-nested-stage-timing` — `P9-T05/D03` on `personal/P9-T05-pi-nested-stage-timing` | Draft PR [#216](https://github.com/agentkernel/cognitive-os/pull/216). Pi-side D01–D02 are on the branch; D03 daemon nested split and header-only negatives are written and awaiting exact-revision Rust validation. Writable paths are the Pi observation package, daemon `route_observation`/`provider_proxy`/`server`, mapped handbook pages, and this running report | exact-revision native Linux or required CI for `route_observation`, `provider_proxy` timing split, `p9_t05_route_observation`, and kernel-server Clippy; then D04 |
...
| `P9-T05/D01` | `done` | Failure-first proof plus the Pi-side nested observation core: opaque `campaign-<32 hex>` id, sequential recorder, strict validator, default-denied authorization, injected content-free sink. Local Node U01 expected-fail then U02 81/81 | consumed by D02 |
| `P9-T05/D02` | `done` | Real `daemon-provider`/`daemon-client`/Extension route publishes one observation per run; unechoed/mismatched/oversize nested daemon stages degrade to `not_available`. Local Node U03 20/20 covering all eight required negative families | consumed by D03 |
| `P9-T05/D03` | `in-progress` | Daemon splits preflight/SecretStore from Provider network, echoes a valid opaque correlation id only when `COGNITIVEOS_PI_ROUTE_OBSERVATION=enabled`, and refuses malformed/duplicate/zero-duration input without changing the product body. Front-door header-only negatives and the timing split are written; Rust validation is routed off `DEV-WIN-GNU-01` | exact-revision native Linux or required Ubuntu/Windows CI: `route_observation` unit tests, `provider_proxy` timing split, `p9_t05_route_observation` front door, kernel-server Clippy |
| `P9-T05/D04` | `ready` | not started | after D03: acceptance mapping, required CI, bilingual handbook sync, and deterministic closure |
