...
| P2-T03 scheduler/runtime | `in-progress` | `P2-T03/D01`-`D04` are done. D05 checkpoint `64015d2` passed required Ubuntu/Windows CI for sealed WIA handoff/recovery fences, including cancelled/task/contract-epoch/runnable/terminal/stale-fence rejection, duplicate bound-handoff preservation, legacy unbound recovery retention, and rollback when exact lease-binding persistence fails; exact Linux worker-recovery validation remains `not-run` after a clean clone was blocked by remote network throughput. Candidate WIA is now explicitly limited to its atomic `DECIDE -> ACT` handoff/recovery role: it cannot authorize harness continuation or a second budget debit. No worker execution evidence is claimed. | implement the owner-approved P2-T07/D01 private verification/checkpoint/continuation-authority prerequisite, then complete D05 private worker integration and exact-revision validation |
| P2-T07 verification prerequisite | `in-progress` | Owner-approved `P2-T07/D01` is limited to daemon-private fixed post-state, verification request/report, checkpoint, and continuation authority required to repair D05. It is not Provider/Tool execution, Artifact closure, Task acceptance, Task completion, a Gate, release, or Profile claim. | add durable verifier/currentness and continuation-authority paths; consume only the new authority at the bounded-harness entry |
...
| Active task lease | `lease/personal/P2-T07/d05-continuation-prerequisite` | P2-T07/D01 owns only the daemon-private verification/checkpoint/continuation authority that D05 requires; P2-T03 handoff/recovery remains its consuming boundary. B02/B04/B05/B12, release, Profile, and Task completion remain separate/incomplete. | implement durable verified continuation authority without treating a worker observation as evidence or a scheduler result as Task completion |
...
| `P2-T03/D05` | `in-progress` | daemon-only candidate admission, sealed WIA issuance, exact scheduler lease-bound one-time consumption, and startup recovery are checkpointed. `64015d2` passed required Ubuntu/Windows CI; focused negatives reject cancelled/task/contract-epoch-mismatched, runnable, terminal, or stale-fenced scheduler work without consumption/binding, and a binding-insert failure rolls the modern handoff transaction back without residue after reopen. Closed legacy unbound recovery retains its lease, and duplicate bound consumption preserves the original binding. Exact Linux candidate-persistence/conformance evidence remains scoped to `d872635`/`50d4fc6`; an exact `d329293` Linux test was `not-run` after the remote clean clone hit GitHub's low-throughput timeout. No worker execution, progress, evidence, Task acceptance, or Task completion is claimed. | add the remaining failure-injection and exact-revision worker integration validation; external executor dispatch remains fail-closed/reconciliation-only |
| `P2-T07/D01` | `in-progress` | owner-approved D05 prerequisite: private fixed-state verification request/report, independent currentness validation, checkpoint, and continuation authority. Its only permitted integration outcome is a legal `ACT -> VERIFY -> CONTINUE -> OBSERVE` sequence; it cannot create Task acceptance/completion or trust worker output as evidence. | establish durable record/port/migration boundaries and continuation negative/restart coverage before D05 consumes the authority |
