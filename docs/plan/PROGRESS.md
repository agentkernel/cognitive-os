...
| `EVAL-012/B0` | `in-progress` | Owner activated `PERSONAL-PERF-EVAL-012`. Lease `lease/personal/EVAL-012/c1-c2-paired-b0` on `evaluation/EVAL-012-freeze`. Product pin `370b26fc`. No sample started. | Confirm guest identity; freeze root/binaries; bind `--reuse-existing-secret-binding`; run B0 cells |
