...
| Active task lease | `lease/personal/P2-T03/daemon-ceiling-dispatch-admission` | P2-T03 implementation-only daemon composition slice in progress: reload current scheduler authority facts, persist a reached ceiling STOP through the kernel, and only then permit scheduler lease acquisition; no external worker or Effect dispatch | add focused admission-order regression coverage and validate the exact committed revision on the qualified Linux worktree; P2 Gates remain not-run |
