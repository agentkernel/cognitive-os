...
> 最后更新：2026-07-20（Lane-CTR F-011 批：R1 审批合同登记完成——approval-request schema 新增（60→61）+ decision R1 条件硬化 + 3 份负例向量（81→84）+ codegen 32 模块；**M5 入口 gate 达成，Lane-RUN 可启动**）
...
| 车道 | 状态 | 分支 | 当前任务 |
|---|---|---|---|
| Lane-CTR 契约与生成 | **F-011 批已交付**（本页所在 PR） | `lane/ctr` | R1 审批合同登记（M5 入口 gate 达成）：`management-approval-request.schema.json` 新增（分级确认面条件 fail-closed）+ `management-approval-decision` R1-approve 条件硬化（request_ref+single_use，既有形状零破坏）+ 3 份行为负例向量（缺确认/自批/疲劳，全用既有注册码，not-run 不虚报）+ registry 映射（REQ-MGMT-APPROVAL/GATE/AUTHZ、REQ-AKP-MGMT-002）+ 审批对入 codegen（32 模块，M5 RUN 即时消费）。**触碰通告**：计数钉扎同批调整（static_check 61/84、ci.yml 84/46/38、runner_execution.rs、生成模块计数 32、AGENTS/rules-12）；authn-authz §5 + conformance/README 登记指针。**遗留待 M5 窗口**：D-016、membership 绑定、D-018 实施 |
...
1. [20260720-lane-ctr-f011-handoff.md](../checkpoints/20260720-lane-ctr-f011-handoff.md)（Lane-CTR F-011 批：R1 审批合同登记、approval-request schema + decision 硬化、3 负例向量、codegen 32 模块、M5 gate 达成）
2. [20260720-lane-cfr-m4-handoff.md](../checkpoints/20260720-lane-cfr-m4-handoff.md)（Lane-CFR M4 故障注入向量执行批：7 向量行为执行、fencing 子集、反模式自检 27/27、F-006/F-010/F-014/F-023 台账、M4 出口评审）
3. [20260720-m4-milestone-review.md](../checkpoints/20260720-m4-milestone-review.md)（M4 出口评审：8 判据逐条证据 + tracer bullet 复现 + F-023 拒绝码确认，GO；M5 入口剩余项 = F-011 R1 登记）
