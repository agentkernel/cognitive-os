...
| Active task lease | `lease/personal/P2-T02/deterministic-cli-parity` | `P2-T02/D03` is implementing deterministic CLI access to the same daemon Task/resource read/watch surfaces with isolated Task/management tokens and cursors. | run daemon-plus-CLI parity and channel/cursor negatives on supported Linux and CI before starting sidecar parity |
...
| `P2-T02/D03` | `in-progress` | deterministic CLI implementation is adding separate Task/management tokens and resource/task watch cursor paths; process parity evidence remains pending | run daemon-plus-CLI parity and channel/cursor negatives on supported Linux and CI before starting sidecar parity |
| `P2-T02/D04` | `ready` | no Pi Shell private sidecar parity implementation or non-authority integration evidence yet | start only after D03 provides the shared deterministic client semantics |
