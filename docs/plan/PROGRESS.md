...
| Active task lease | `lease/personal/P2-T02/daemon-task-api-watch` | P2-T02/D01 remains `in-progress`. PR #140's bootstrap governance-root rule and PR #139's intent contracts are merged; the active Lane-RUN slice now composes them into the authenticated Personal Task API and process-lifetime watch. | implement the durable daemon composition and focused vertical/channel-isolation tests |
