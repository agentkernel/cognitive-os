...
|---|---|---|---|
| Project focus | `cognitiveos-personal`: active and sole implementation project | CognitiveOS architecture assets remain reference/contract inputs; no second product backlog | EVAL-010 is closed. P2-T32 through P2-T36 are done on `main`; P2-T36 closure reconciliation merged as PR [#245](https://github.com/agentkernel/cognitive-os/pull/245) at `08819a82`, with final required CI `32257350108` passing Ubuntu, Windows, and required-ci. P2-T37 is in progress for the demonstrated C2a public WorkspaceWrite/Patch gap. P7-T07 remains blocked on B01-W prerequisites |
| Owner-directed campaign | `PERSONAL-PERF-EVAL-010` **closed** (2026-08-18). Pin `289eebad` (P2-T33 host path). Long root `/home/hal9001/perfeval010-20260818` / daemon `48298` / SecretStore `/19`. UNIX_PATH_MAX product fix **confirmed** (short socket `$XDG_RUNTIME_DIR/cognitiveos/pc-287493-3.sock`). B0 C1-search **partial** (`candidate_has_missing_fields_or_an_invalid_parameters_digest`; Task `DRAFT`; `lease_acquired` 0; real adapter spawned). Evaluation routing OFF. `PERSONAL-PERF-EVAL-009`, `PERSONAL-PERF-EVAL-008`, `PERSONAL-PERF-EVAL-007`, `PERSONAL-PERF-EVAL-006`, `PERSONAL-PERF-EVAL-005`, and `PERSONAL-PERF-EVAL-004` remain **closed**. | Preregistration: [20260818-personal-perf-eval-010-preregistration.md](../checkpoints/20260818-personal-perf-eval-010-preregistration.md). Final: [personal-performance-assessment-20260818-eval-010.md](../evaluation/personal-performance-assessment-20260818-eval-010.md). Do not reopen EVAL-004/005/006/007/008/009/010. Do not reuse those roots, ports `48286`/`48288`/`48290`/`48292`/`48294`/`48296`/`48298`/`48386`/`48388`/`48390`/`48392`/`48394`/`48396`/`48398`, or SecretStore `/12`–`/19`. Claim ceiling `hypothesis`; `not_reviewed` | retarget and merge P2-T34 PR [#241](https://github.com/agentkernel/cognitive-os/pull/241) onto `main`; do not reopen EVAL-010 |
| Owner-directed C1/C2 campaign | PERSONAL-PERF-EVAL-010 **closed**; PERSONAL-PERF-EVAL-009 **closed**; PERSONAL-PERF-EVAL-008 **closed**; PERSONAL-PERF-EVAL-007 **closed**; PERSONAL-PERF-EVAL-006 **closed**; PERSONAL-PERF-EVAL-004 **closed**; PERSONAL-PERF-EVAL-005 **closed** | P2-T33 stub pass ≠ real adapter C1/C2. Long-root freeze confirmed the UNIX_PATH_MAX product fix. Claim ceiling hypothesis/`not_reviewed` | do not reuse EVAL-004/005/006/007/008/009/010 assets |
| Active task lease | `lease/personal/P2-T37/c2a-public-mutation-path` | P2-T37/D02 is in progress on `personal/P2-T37-c2a-public-mutation-path`. D01 failure-first public tool-surface and mutation-mapper negatives pass at the current working revision; the public launcher now exposes only the four daemon-governed Workspace tools. | Complete separate exact-revision non-B01 public WorkspaceWrite and WorkspacePatch lifecycles, then run supported validation and task closure. |
...
| Benchmark readiness product train | `P2-T21` … `P2-T36` **done** (`main@08819a82`); `P2-T37` **in progress** | The durable navigation is [PERSONAL-C1-C2-READINESS-DELIVERY-PLAN.md](PERSONAL-C1-C2-READINESS-DELIVERY-PLAN.md). EVAL-011 is closed and does not make readiness complete. P2-T37 addresses the demonstrated public C2a omission only; no C2 result, EVAL, Gate, release, Profile, B01, or Agent-benefit conclusion follows until separate public mutation Tasks complete supported validation. | Complete P2-T37/D02 separate public WorkspaceWrite and WorkspacePatch lifecycle validation on an exact pushed native-Linux revision. |
...
|---|---|---|---|
| `P2-T37/D01` | `in-progress` | Source inspection on exact merged `main@08819a82` proves the public launcher allowlist contains only WorkspaceRead/Search, while Extension and private adapter mutation support is not enough to establish a public C2a route. | Add failure-first public allowlist, mutation candidate mapping, schema, and built-in deny tests before implementation. |
| `P2-T37/D02` | `ready` | Formal acceptance and dependencies are registered; no public mutation implementation has been changed yet. | Implement the daemon-governed public WorkspaceWrite/Patch route after D01 negatives pass. |
...
| `P2-T36/D02` | `in-progress` | Exact pushed `979e52e4c6681d0fc8c6431c965e3267a7a0d917` ran public `cognitive daemon start` in a cleanable non-B01 runtime. Redacted doctor found system/database/secret/daemon/Pi ready but `provider_config_missing`, leaving `first_conversation_ready: false`; Pi launch and all C1 lifecycle stages are `not-run`. The disposable daemon was stopped through public CLI. The slice remains in progress with an external readiness blocker, not falsely complete. | owner/external environment: provision or identify an approved new non-EVAL opaque SecretRef and selected model through public configuration without exposing material, then rerun public readiness and WorkspaceRead |
| `P2-T36/D03` | `ready` | D02 has not established supported end-to-end C1 evidence. | complete supported validation, docs sync, task acceptance, and deterministic closure without starting an EVAL |
