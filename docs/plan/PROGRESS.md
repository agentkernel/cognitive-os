...
| Active task lease | `lease/personal/P2-T26/bounded-os-observation` | P2-T25 merged via PR #224 at `main@4b10db9a`. P2-T26 D01+D02 authored on `personal/P2-T26-bounded-os-observation`. Unique in-progress slice `P2-T26/D01` until Ubuntu CI. Windows `not-run by owner-directed Linux-only route`. `B01-Desktop-Linux-002` untouched (EVAL-004-only) | push Draft PR; Ubuntu supporting CI; then D03 exact-revision linux-002 |
...
| P2-T26 Bounded OS observation plane | **in-progress** | D01+D02 authored: O2/O3/O4 collectors plus O5 Effect-history reuse and O13 audit cursor/replay; empty windows return `observed_zero` plus a named control; stale cursor/digest break fail closed. Branch `personal/P2-T26-bounded-os-observation`; lease `lease/personal/P2-T26/bounded-os-observation`. Running report: [20260816-personal-p2-t26-bounded-os-observation-report.md](../checkpoints/20260816-personal-p2-t26-bounded-os-observation-report.md). Claim ceiling `hypothesis`; no Gate/release/Profile/B01/EVAL promotion | push Draft PR; Ubuntu supporting CI; then D03 linux-002 |
...
| `P2-T26/D01` | `in-progress` | Authenticated bounded O2/O3/O4 collectors with active negative controls; empty windows return `observed_zero` plus a named control; no body/capability leakage. Local Rust tests `not-run` (`RUST-LINK-DEV-WIN-GNU-01`) | Ubuntu supporting CI on the combined D01+D02 head |
| `P2-T26/D02` | `ready` | O5/O13 collectors are authored on the same task branch (Effect-history reuse, audit cursor/replay, stale-cursor and digest-break negatives). Slice stays unclosed until D01 CI | after D01 Ubuntu CI: record D02 evidence then D03 linux-002 |
| `P2-T26/D03` | `ready` | Registered formal slice; implementation has not started | waits for D02 and owns whole-task validation/closure |
