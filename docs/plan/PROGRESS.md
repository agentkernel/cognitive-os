...
|---|---|---|---|
| Project focus | `cognitiveos-personal`: active and sole implementation project | CognitiveOS architecture assets remain reference/contract inputs; no second product backlog | continue P8-T08 docs-sync enforcement delivery |
| Active task lease | `lease/personal/P8-T08/docs-sync-enforcement` | P8-T08/D01 is the sole active delivery slice on `personal/P8-T08-docs-sync-enforcement`: owner directed on 2026-08-12 that documentation-system synchronization must be an enforced obligation before every commit/push/merge, not advice. P7-T07 remains blocked on B01-W prerequisites with its lease released | land the docs-sync gate + hooks + tests, then register the canonical obligations in docs-sync-contract/rules/AGENTS and sync the affected handbook pages |
| P8-T08 Docs-sync enforcement | **in-progress** | Registered 2026-08-12 by owner direction (PR #202 follow-up). Scope: conditional `docs-sync-gate` (source-map routing; handbook check set on hit; fail-closed on unsynced mapped changes with explicit recorded `DOCS_IMPACT_NONE` escape), repo `.githooks/` pre-commit/pre-push + `hooks:install` + `check:docs-sync`, docs-sync-contract §2/§5/§6 handbook obligations, rule 10/20 + AGENTS checkpoint/closure wording, affected handbook pages synced bilingually. Obligation semantics stay owned by docs-sync-contract; no second workflow; no Gate/contract/release surface. | complete D01 (gate + hooks + tests green locally), then D02 canonical registration |
...
enforcement) was registered on 2026-08-12 by owner direction and is
`in-progress`; it is governance/tooling-only.

### Layer 2 — Current Delivery Slice queue

| Slice | Status | Actual evidence boundary | Executable next action |
|---|---|---|---|
| `P8-T08/D01` | `in-progress` | registered; gate/hooks/tests not yet landed in the task branch | implement `docs-sync-gate` routing + modes, `.githooks` pre-commit/pre-push, `hooks:install`/`check:docs-sync` scripts, and focused routing/negative tests; run tools tests + gate smoke locally |
| `P8-T08/D02` | `ready` | not started; depends on D01 | register obligations in docs-sync-contract §2/§5/§6, rule 10/20, AGENTS; sync affected handbook pages and refresh fingerprints |
| `P8-T08/D03` | `ready` | not started; depends on D01-D02 | acceptance mapping, closure checkpoint, deterministic ready/merge/lease/branch/main closure |
