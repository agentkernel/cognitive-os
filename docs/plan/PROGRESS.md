...
|---|---|---|---|
| Project focus | `cognitiveos-personal`: active and sole implementation project | CognitiveOS architecture assets remain reference/contract inputs; no second product backlog | P2-T32 public `cognitive daemon start` C1/C2 wiring in progress. Evaluation routing OFF. P7-T07 remains blocked on B01-W prerequisites |
| Owner-directed campaign | `PERSONAL-PERF-EVAL-007` **closed** (owner 2026-08-18). Public `cognitive daemon start` C1/C2 B0 stayed `DRAFT` (`lease_acquired` 0, no Pi child). Evaluation routing OFF. `PERSONAL-PERF-EVAL-006`, `PERSONAL-PERF-EVAL-005`, and `PERSONAL-PERF-EVAL-004` remain **closed**. | Do not reopen EVAL-004/005/006/007. Do not reuse those campaign roots, ports `48286`/`48288`/`48290`/`48292`/`48386`/`48388`/`48390`/`48392`, or SecretStore `/12`–`/16`. Claim ceiling `hypothesis`; `not_reviewed` | none for this campaign — product mutex is P2-T32 public daemon launcher |
...
| Owner-directed C1/C2 campaign | PERSONAL-PERF-EVAL-007 **closed**; PERSONAL-PERF-EVAL-006 **closed**; PERSONAL-PERF-EVAL-004 **closed**; PERSONAL-PERF-EVAL-005 **closed** | P2-T31 merged at `main@da6fca2e`. EVAL-007 public `cognitive daemon start` stayed `DRAFT`. Claim ceiling hypothesis/`not_reviewed` | none — product mutex is P2-T32; do not freeze EVAL in this window |
| Active task lease | `lease/personal/P2-T32/public-daemon-start-scheduler` | P2-T32/D02 on `personal/P2-T32-public-daemon-start-scheduler`; Draft PR [#239](https://github.com/agentkernel/cognitive-os/pull/239) at `b1f1d12e` | Ubuntu CI / linux-002 focused `p2_t32_*`; do not run EVAL |
| Benchmark readiness product train | `P2-T21` **done** … `P2-T31` **done** at `da6fca2e` / PR [#236](https://github.com/agentkernel/cognitive-os/pull/236); `P2-T32` **in-progress** | Live HTTP admit acquires a scheduler lease. Public `cognitive daemon start` remains the C1/C2 product mutex. Claim ceiling `hypothesis` | public launcher log + Unix detach; no EVAL/B01 |
...
| P2-T32 Public `cognitive daemon start` scheduler lease | **in-progress** | Owner 2026-08-18 after EVAL-007 close. Branch `personal/P2-T32-public-daemon-start-scheduler`; lease `lease/personal/P2-T32/public-daemon-start-scheduler`. Draft PR [#239](https://github.com/agentkernel/cognitive-os/pull/239) at `b1f1d12e`. Public launcher retains `daemon.log` and Unix process group. Claim ceiling `hypothesis`. | Ubuntu required CI / `DEV-LINUX-NATIVE-01` focused `p2_t32_*`; do not merge as C1/C2 proof; do not freeze EVAL-008 |
...
|---|---|---|---|
| `P2-T32/D01` | `done` | Failure-first Unix admin-cli test `p2_t32_public_daemon_start_scheduler` authored: public `cognitive daemon start` + stub Workspace* + HTTP admit. Local `cargo test` `not-run` (`RUST-LINK-DEV-WIN-GNU-01`). | consumed by D02 |
| `P2-T32/D02` | `in-progress` | CLI spawn appends stdout/stderr to layout `daemon.log` (0600 on Unix) and Unix `process_group(0)`. Stub Workspace* adapter allowed. Local cargo test `not-run`. | Ubuntu CI / linux-002 focused `p2_t32_*`; do not run EVAL |
| `P2-T32/D03` | `ready` | bilingual handbook + generated CLI usage in this change set; linux-002 matrix is the next window | after D02 CI/linux-002 |
