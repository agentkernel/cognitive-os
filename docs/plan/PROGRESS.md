...
| Active task lease | `lease/personal/P4-T03/memory-lifecycle` | P4-T03/D02 is the only active delivery slice on `personal/P4-T03-memory-lifecycle` | complete the private Memory lifecycle task under the same Draft PR |
...
| P4-T03 Memory lifecycle, retention and forget | `in-progress` | `P4-T03/D01` is complete at `e0b2c329a209fd9f92d4068a2e96226a0cd60d6d`; `P4-T03/D02` is active and adds an expiry-only lifecycle action guarded by the immutable retention deadline. Validation is not-run for D02. Version/update/conflict and public Memory API remain unclaimed. | run exact native Linux expiry/migration/FTS tests and required CI |
...
| `P4-T03/D02` | `in-progress` | `lease/personal/P4-T03/memory-lifecycle` owns the daemon-private retention-expiry lifecycle transition; the exact deadline boundary, duplicate sweep, and FTS invalidation tests are implemented but validation is not-run. | run exact-revision native Linux migration/FTS tests, then required Ubuntu/Windows CI |
