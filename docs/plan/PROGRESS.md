...
## Current snapshot (2026-08-24 P8-T15 closure)
...
| Active task lease | `none` | — | Wait for a fresh owner delivery instruction. Do not auto-claim P6 / P7-T06 / P7-T07 |
| Owner ops tooling (linux-002 boot) | **delivered** 2026-08-24 (owner-directed ops convenience; not a formal `P*-T*` task, not EVAL) | `scripts/linux002-boot-personal.ps1`: one local Windows PowerShell 5.1 entry point that syncs an exact pushed revision (default `origin/main`) via a jump-host build, installs digest-verified `cognitive`+`kernel-server` on linux-002, ensures the owner-ops daemon (`127.0.0.1:48681`, runtime `/home/hal9001/p8t13-owner-ops/runtime`; healthy daemon reused, never fought), configures installed Cos dsh Path B (`/home/hal9001/p8t10-a17edfad/dsh` + `p8t15-0376e942` adapter, pin `528c682e`) with the loopback web panel `:3080`, and leaves Pi configured/ready (interactive launch via `-LaunchPi` or the printed command). Secrets are never read, printed, or transferred. Local validation: pwsh AST parse, `bash -n` on all 13 embedded payloads, and fail-closed negatives pass; live linux-002 execution **not-run** (cloud agent cannot reach the private LAN). Ops lease `lease/personal/OPS-LINUX002-BOOT/owner-ops-boot-script` claimed and closed in the same delivery, then renewed and re-closed 2026-08-24 for the wrong-cwd follow-up (PARALLEL-LANES §3.1). Also corrective: Phase 8 / totals summary counts in the formal plan reconciled to the post-P8-T15 task rows (checker was red on `main`). Follow-up (same branch, PR [#268](https://github.com/agentkernel/cognitive-os/pull/268)) after the operator's first paste failed with `-File ... does not exist` from `C:\Windows\system32`: canonical cd-into-clone invocation examples with switches on the same command line, a TROUBLESHOOTING help section (wrong cwd vs checkout predating PR #268, with exact `git fetch`/`git checkout` commands), an early fail-closed guard when the code is not started as an on-disk .ps1, and a repo-root launcher `linux002-boot-personal.ps1` that forwards all parameters via `@args` and fails closed when `scripts/` is missing. SSH/guest defaults, secret boundary, and Path A non-start behavior unchanged. Branch `cursor/linux002-boot-personal-ops-script-ac2d` (script commit `fc0f980d`); the Draft PR is attached from this branch by the cloud-agent environment (this run's GitHub token is read-only for PR creation). Claim ceiling `hypothesis`; no Gate, release, Profile, B01, EVAL, or Agent-benefit promotion. | Operator: `cd <path-to-cognitive-os-clone>` then `powershell -ExecutionPolicy Bypass -File .\scripts\linux002-boot-personal.ps1 -EnsureGuestRunning -LaunchPi` (switches optional, same command line; until PR #268 merges first `git fetch origin cursor/linux002-boot-personal-ops-script-ac2d` and `git checkout cursor/linux002-boot-personal-ops-script-ac2d`). Otherwise wait for a fresh owner delivery instruction |
...
[#265](https://github.com/agentkernel/cognitive-os/pull/265). `P7-T05` is `done`
...
Do not auto-claim P6 / P7-T06 / P7-T07. Remaining items are blocked `P7-T07`
and owner-deferred P6 / P7-T06.
