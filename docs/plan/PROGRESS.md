...
| Personal 产品化计划 | **planning；P0-T01、P0-T02、P0-T05 done；无产品实现声明** | `01ceb93` 的 CI run 30140381194 在 Ubuntu 与 Windows/MSVC 均通过；P0-T02 专用映射核对、`pnpm run check:consistency`、`git diff --check` 均通过；P0-T05 在 WSL2 non-root systemd user session 中执行 `secret-tool` set/get/rotate/delete、删除验证和 repository text/Bash history/environment/process arguments/user journal 泄漏负例，最终 `LEAK_CHECK_OK`；Personal 功能/Gate 证据仍 `not-run` | 正式台账：[PERSONAL-DEVELOPMENT-PLAN.md](PERSONAL-DEVELOPMENT-PLAN.md)；[PERS-PR trace](personal-trace.yaml) 独立于 registry matrix，不新增 REQ/schema/vector；记录：[tests/baseline/README.md](../../tests/baseline/README.md)。P0-T05 仅为 WSL2 环境 PoC；Personal `done` 不代表首发 Linux desktop 支持、正式后端、G0、B01-B12 或 Profile 已符合。 |
...
1. [20260725-personal-p0-t05-complete-handoff.md](../checkpoints/20260725-personal-p0-t05-complete-handoff.md)（Personal：WSL2 non-root Secret Service PoC 完成；仅环境 PoC，非首发支持或产品实现声明）
2. [20260725-personal-p0-t05-handoff.md](../checkpoints/20260725-personal-p0-t05-handoff.md)（Personal：初始 WSL2 root 会话没有 Secret Service，P0-T05 曾 fail-closed blocked）
