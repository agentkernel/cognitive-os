...
|---|---|---|---|
| Project focus | `cognitiveos-personal`: active and sole implementation project | CognitiveOS architecture assets remain reference/contract inputs; no second product backlog | continue P8-T07 independent handbook delivery |
| Active task lease | `lease/personal/P8-T07/independent-handbook` | P8-T07/D01 is the sole active delivery slice on `personal/P8-T07-independent-handbook`; the handbook machine core (manifest/frontmatter schema/source-map/coverage, standalone checker + generator + negative fixtures) was validated in staging against reading baseline `9fbd3904a1f8e0893fcb7d8d2b434e636d546e8c` and is being landed against implementation baseline `6b637a5194dc56b568d2be25bb9ceb3bb24a0f72` | land the machine core and bilingual content in the task worktree, then run the full handbook/consistency check set and required CI |
| P8-T07 Independent bilingual handbook | **in-progress** | Owner-directed registration on 2026-08-12. Full-repo reading ledger closed at baseline `9fbd390` (1,148 tracked paths classified; live first-party and generated files fully read; only lock payloads and frozen `History/` excluded with reasons) plus the `9fbd390..6b637a5` increment. Deliverables: bilingual `handbook/` (en + zh-CN; user/developer/reference/AI), machine manifest/source-map/coverage/fingerprints, generated references, standalone `check-handbook` checker with focused negative fixtures, `.cursor/rules/20` sync adapter, root `llms.txt`, one AGENTS.md pointer. No second fact source; dynamic `PROGRESS.md`/Gate status linked, never copied; legacy documentation stays byte-identical except this minimal governance registration. | complete D01 (machine core + checks green locally), then D02 bilingual user/reference content |
