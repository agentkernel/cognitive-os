...
| P9-T05 Pi route nested stage timing | **in-progress** | Claimed 2026-08-13 on `personal/P9-T05-pi-nested-stage-timing` / Draft PR [#216](https://github.com/agentkernel/cognitive-os/pull/216), head `554c6cf9f69af836032af207eeb04a800ac55063`. D01–D03 delivered the joined seven-stage observation: Pi-side Node 81/81 plus 20/20 negatives; daemon nested preflight vs Provider network, authorized correlation echo, and header-only negatives. Exact native Linux at that revision: `route_observation` 9/9, timing split 1/1, `p9_t05_route_observation` 2/2, fmt and kernel-server Clippy pass. Measurement capability only; no Gate/release/Profile/B01/benchmark or overhead-attribution claim | D04: required Ubuntu/Windows CI on this head, then acceptance mapping and Draft-to-ready closure |
| Active task lease | `lease/personal/P9-T05/pi-nested-stage-timing` — `P9-T05/D04` on `personal/P9-T05-pi-nested-stage-timing` | Draft PR [#216](https://github.com/agentkernel/cognitive-os/pull/216) head `554c6cf9`. D01–D03 are implemented and Linux-validated; D04 is required CI plus acceptance/closure | wait for required Ubuntu/Windows CI on `554c6cf9`, then D04 acceptance mapping |
...
| `P9-T05/D03` | `done` | Daemon splits preflight/SecretStore from Provider network, echoes a valid opaque correlation id only when `COGNITIVEOS_PI_ROUTE_OBSERVATION=enabled`, and refuses malformed/duplicate/zero-duration input without changing the product body. Exact native Linux `554c6cf9`: `route_observation` 9/9, timing split 1/1, `p9_t05_route_observation` 2/2, fmt and kernel-server Clippy pass | consumed by D04 |
| `P9-T05/D04` | `in-progress` | handbook/docs-sync already in the D03 change set; required Ubuntu/Windows CI and formal acceptance mapping remain | wait for required CI on `554c6cf9`, then map acceptance and keep the PR Draft until closure |
