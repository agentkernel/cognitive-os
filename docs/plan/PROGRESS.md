...
| P1-T09 route implementation | `in-progress` | `experimental-local-only` / `tested-supported-ci`; Attempt 6 proved immutable installation and activation, but service readiness requires the normal Desktop login session rather than a transient SSH-only session. | obtain owner confirmation for a fresh Attempt 7 and establish the Desktop login session before installation, then continue the complete preregistered route |
| B01 first-install/first-conversation Gate | **running** | `B01-Desktop-Linux-002` is the sole active B01 guest. Attempt 6 installed and activated the independently verified replacement artifact, but the transient SSH user manager exited because no graphical Desktop session was present; cleanup restored the baseline. The ordered ledger has 6 of the formal minimum 20 attempts: 1 success / 5 failures, with no critical safety failure observed. | establish the normal Desktop login session in a fresh Attempt 7 before installation; do not retry Attempt 6 |
...
| Active task lease | `lease/personal/P1-T09/b01-attempt-7-execution` | P1-T09 is `in-progress` under the owner-confirmed complete Attempt 7. B01 remains `running` with 6 of its fixed 20 attempts recorded (1 success, 5 failures) until the fresh reset/start crosses the clean-reset checkpoint. | restore `b01-platform-qualified-baseline`, establish the Desktop session, and execute the entire preregistered Attempt 7 path without splitting or retrying it |
