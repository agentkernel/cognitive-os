...
|---|---|---|---|
| Project focus | `cognitiveos-personal`: active and sole implementation project | CognitiveOS architecture assets remain reference/contract inputs; no second product backlog | `PERSONAL-PERF-EVAL-012` **closed**. `P2-T38`, `P9-T09`, and `P9-T10` **done**. Next evidence-ranked item is comparable C2a Patch payload format. P7-T07 remains blocked on B01-W prerequisites |
| Owner-directed campaign | `PERSONAL-PERF-EVAL-012` **closed** (2026-08-20) on `evaluation/EVAL-012-freeze` @ `177669b7`. Evaluation routing OFF. B0 fairness **fail** (`system_task_prompt_bytes`) blocked B1/B2. C2a O Patch fail retained. Cleanup **pass**. EVAL-002 and EVAL-004 through EVAL-011 remain **closed**. | Report and freeze live on `evaluation/EVAL-012-freeze` (`docs/evaluation/personal-performance-assessment-20260820-eval-012.md`, `docs/checkpoints/20260820-personal-perf-eval-012-preregistration.md`). Product pin `370b26fcc05976c7c1c97e5510a99ed3ebc23f2c`. Claim ceiling `hypothesis`; `not_reviewed` | none for this campaign — product follow-up is comparable C2a Patch format |
| Owner-directed C1/C2 campaign | `PERSONAL-PERF-EVAL-012` closed. B0 did not pass fairness. C1 O/P and C2a Write ran; C2a Patch O fail retained. | Plan: [PERSONAL-C1-C2-READINESS-DELIVERY-PLAN.md](PERSONAL-C1-C2-READINESS-DELIVERY-PLAN.md). Claim ceiling `hypothesis`; no Gate, release, Profile, B01, or Agent-benefit promotion. | comparable C2a Patch format, then a new EVAL; do not reopen EVAL-012 |
| Active task lease | `none` | `P9-T10` merged; evaluation routing OFF | claim comparable C2a Patch payload format before a new EVAL |
| Benchmark readiness product train | `P2-T21` … `P2-T38` **done**; **P9-T08 done**; **P9-T09 done**; **P9-T10 done**; `PERSONAL-PERF-EVAL-012` **closed** | Navigation: [PERSONAL-C1-C2-READINESS-DELIVERY-PLAN.md](PERSONAL-C1-C2-READINESS-DELIVERY-PLAN.md). Packages 1–14 delivered. Live `--append-system-prompt` injection is merged. No Gate, release, Profile, B01, or Agent-benefit conclusion follows. | pin P/O C2a Patch to unified diff; do not activate a new EVAL until that can pass |
