...
| Active task lease | `lease/personal/P2-T08/runtime-spine-gates` | Active lease owns P2-T08 Runtime Spine Gate suite harness on `personal/P2-T08-runtime-spine-gates`. | implement and validate `P2-T08/D01` non-claim suite evaluator |
...
| P2-T08 Runtime Spine E2E Gate | `in-progress` | D01 non-claim Runtime Spine Gate suite harness is the unique in-progress slice. B02/B04/B05/B12 remain `not-run`. | close D01 after tools tests + required CI, then enter D02 ADR-0018 exception removal |
...
| `P2-T08/D01` | `in-progress` | Non-claim Runtime Spine Gate suite harness for fixed B02/B04/B05/B12 observations; evaluator cannot set Gate state | finish tools Node tests, consistency, and required CI; then enter D02 |
| `P2-T08/D02` | `ready` | Remove or replace ADR-0018 local-native Provider secret development exception with daemon-proxy-only path | enter after D01 closes |
| `P2-T08/D03` | `ready` | Authority-path focused Runtime Spine negatives for close/OUTCOME_UNKNOWN/false-completion | enter after D02 |
| `P2-T08/D04` | `ready` | Formal B02/B04/B05/B12 campaign preregistration and supported execution | enter after D03; may require owner if Provider/environment unavailable |
