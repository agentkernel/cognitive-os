...
| Active task lease | `lease/personal/P1-T09/windows-provider-fixture-timeout` | The narrow correction `main@0acf720` fixed its Clippy failure: the Linux CI job passed all checks. The replacement CI remains failed because the unrelated Windows `p1_t09_deterministic_provider_fixture::binary_fixture_drives_real_rustls_discovery_without_leaking_provider_material` timed out in Provider discovery. P2-T03 behavior and evidence remain unchanged | reproduce and repair the P1-T09 Windows fixture timeout; do not advance implementation evidence or P2 Gates |
