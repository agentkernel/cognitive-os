...
| Benchmark readiness product train | `P2-T21` **done** at `5d1f5c2643d82807eb96f2c615d460dbdca749c3` / PR #220; `P2-T22` **done** at `03bc530d692649a00efa7918bfeadddf8ba0ebea` / PR [#221](https://github.com/agentkernel/cognitive-os/pull/221); `P2-T23` D01 in progress on `personal/P2-T23-memory-skill-consumption-resume`; `P2-T24..P2-T28` registered and not started | Owner instruction 2026-08-15 approved BR-01..BR-08 and the Linux-only product-validation route. Ubuntu is supporting CI; exact pushed-revision `DEV-LINUX-NATIVE-01` is required acceptance evidence for these product tasks; `B01-Desktop-Linux-002` remains owner-directed EVAL-004-only. Windows is `not-run by owner-directed Linux-only route`. This is product implementation evidence only, not EVAL/Gate/release/Profile/B01 evidence | complete P2-T23 D01 public lifecycle + redacted consumption GET, then D02 session-2 resume. EVAL-004 re-freeze happens only after every BR product task is merged |
...
| Active task lease | `lease/personal/P2-T23/public-memory-skill-consumption-resume` | P2-T22 merged via PR [#221](https://github.com/agentkernel/cognitive-os/pull/221) at `main@03bc530d692649a00efa7918bfeadddf8ba0ebea`. P2-T23 D01 claimed: public remember/review/forget and import/inspect/bind plus `GET /task/resource/v1/consumption` redacted pins without query restatement; pre-rank cross-scope/forgotten/revoked negatives. Windows `not-run by owner-directed Linux-only route` | implement D01 public journey tests and redacted GET, then Ubuntu supporting CI |
...
| P2-T23 Public Memory/Skill consumption and resume | **in-progress** | D01 public remember/review/forget and import/inspect/bind plus daemon-authored `GET /task/resource/v1/consumption` redacted pins (no `query_text` restatement); pre-rank cross-scope/forgotten/revoked negatives. Branch `personal/P2-T23-memory-skill-consumption-resume`; lease `lease/personal/P2-T23/public-memory-skill-consumption-resume`. Running report: [20260816-personal-p2-t23-memory-skill-consumption-resume-report.md](../checkpoints/20260816-personal-p2-t23-memory-skill-consumption-resume-report.md). Claim ceiling `hypothesis`; no Gate/release/Profile/B01/EVAL promotion | D01 public journey + Ubuntu supporting CI, then D02 session-2 resume |
...
| `P2-T23/D01` | `in-progress` | Public remember/review/forget and import/inspect/bind plus `GET /task/resource/v1/consumption` redacted pins without query restatement; pre-rank cross-scope/forgotten/revoked negatives | Ubuntu supporting CI, then D02 session-2 resume |
| `P2-T23/D02` | `ready` | Registered formal slice; implementation has not started | waits for D01 within the same future task branch and lease |
