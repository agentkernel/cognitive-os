...
| Owner-directed C1/C2 campaign | PERSONAL-PERF-EVAL-004 **paused before B1; evaluation lease closed** | B0 target and runtime readiness were qualified without any B1/B2/B3/B4 sample. The campaign discovered a product gap: the private candidate carrier persists only parameters_digest, while the production WorkspaceSearch/WorkspaceWrite/WorkspacePatch router requires canonical governed parameters. EVAL-002 remains closed and unchanged; claim ceiling remains hypothesis/non-claim | P2-T22 merged at `03bc530d692649a00efa7918bfeadddf8ba0ebea` / PR #221. Continue P2-T23 (BR-03). EVAL-004 re-freeze only after BR-02..BR-08 are merged; no prior campaign asset or SecretStore entry is reused |
| Benchmark readiness product train | `P2-T21` **done** at `5d1f5c2643d82807eb96f2c615d460dbdca749c3` / PR #220; `P2-T22` **done** at `03bc530d692649a00efa7918bfeadddf8ba0ebea` / PR [#221](https://github.com/agentkernel/cognitive-os/pull/221); `P2-T23` D01 repair + D02 tests authored on `personal/P2-T23-memory-skill-consumption-resume`; `P2-T24..P2-T28` registered and not started | Owner instruction 2026-08-15 approved BR-01..BR-08 and the Linux-only product-validation route. Ubuntu is supporting CI; exact pushed-revision `DEV-LINUX-NATIVE-01` is required acceptance evidence for these product tasks; `B01-Desktop-Linux-002` remains owner-directed EVAL-004-only. Windows is `not-run by owner-directed Linux-only route`. This is product implementation evidence only, not EVAL/Gate/release/Profile/B01 evidence | confirm Ubuntu supporting CI on the D01 repair head, then record D02 and run D03 linux-002. EVAL-004 re-freeze happens only after every BR product task is merged |
...
| Active task lease | `lease/personal/P2-T23/public-memory-skill-consumption-resume` | P2-T22 merged via PR [#221](https://github.com/agentkernel/cognitive-os/pull/221) at `main@03bc530d692649a00efa7918bfeadddf8ba0ebea`. P2-T23 D01 unique in-progress slice: public consumption GET plus i64::MAX remember repair; D02 session-2/restart/forged-prompt tests authored in the same head. Ubuntu run `31921344719` failed 2 D01 tests. Windows `not-run by owner-directed Linux-only route` | push repaired head, confirm Ubuntu supporting CI, then D03 exact-revision linux-002 |
...
| P2-T23 Public Memory/Skill consumption and resume | **in-progress** | D01 public remember/review/forget and import/inspect/bind plus daemon-authored `GET /task/resource/v1/consumption` redacted pins (no `query_text` restatement); pre-rank cross-scope/forgotten/revoked negatives. D02 session-2 GET/restart/forged-prompt tests authored, slice not started until D01 CI. Branch `personal/P2-T23-memory-skill-consumption-resume`; lease `lease/personal/P2-T23/public-memory-skill-consumption-resume`; Draft PR [#222](https://github.com/agentkernel/cognitive-os/pull/222). Running report: [20260816-personal-p2-t23-memory-skill-consumption-resume-report.md](../checkpoints/20260816-personal-p2-t23-memory-skill-consumption-resume-report.md). Claim ceiling `hypothesis`; no Gate/release/Profile/B01/EVAL promotion | Ubuntu supporting CI on repaired D01 head, then D03 linux-002 matrix |
...
| `P2-T23/D01` | `in-progress` | Public remember/review/forget and import/inspect/bind plus `GET /task/resource/v1/consumption` redacted pins without query restatement; pre-rank cross-scope/forgotten/revoked negatives. Ubuntu `31921344719` failed 2 tests (`i64::MAX` in sealed MemoryCandidate); repair is in the current head | Ubuntu supporting CI on the repaired head |
| `P2-T23/D02` | `ready` | Session-2 GET/restart/forged-prompt tests and GET digest revalidation are authored in the D01 repair commit; slice stays ready until D01 Ubuntu CI is green | waits for D01 Ubuntu supporting CI |
| `P2-T23/D03` | `ready` | Registered formal slice; implementation has not started | waits for D02 and owns whole-task validation/closure |
