...
| `P2-T12/D01` | `in-progress` | Exact native Linux at `280cf2619470675a92880807f114f505eebb328e` confirmed the failure-first regression **red**: 0 passed / 1 failed / 4 filtered, at `admission must publish runnable scheduler work` after successful TaskContract admission and crash/reopen. The first stdin-script invocation was honestly `not-run` because CRLF produced an invalid `nocapture\r` option before test execution; the direct Bash rerun executed the intended test. Local Rust remains `not-run` by the registered Windows GNU restriction | implement one SQLite authority transaction publishing the TaskContract event, current-epoch `Runnable` scheduler row, contract-named Loop at `START`, and contract-named Budget; then rerun the unchanged native test |
