...
| P3-T01 Context source/retrieval port | `in-progress` | `experimental-local-only` / `tested-supported-ci`; `P3-T01/D01` now has TaskContract v0.4 ContextRequest binding, immutable Context request/view/source persistence, metadata-only source discovery, and daemon-admin admission routes for sealed authorization/revocation facts. Exact native Linux `cda31dc4ec74ae5faaf3d8d47ecb902e97dc8af3` passed Context-store 8/8, including a prior allowed body read denied after durable epoch advancement; required Ubuntu/Windows CI passed on the preceding authority-admission checkpoint. The owner-approved MVP path uses one owner-local management session and does not require advanced multi-principal policy administration before resolver integration. B03 and task completion remain absent. | reconcile P2-T04 lease and wire the owner-local Context source path to the private candidate transport; keep Pi candidate-only |
| P2-T04 private worker composition | `blocked` | D05's exact-revision native Linux closure is complete, but D01 cannot safely begin on its current lease: the existing WIA represents a candidate already admitted before worker execution, while P2-T04 requires Pi to propose before candidate admission. P3-T01 now supplies an MVP owner-local management admission boundary, immutable source persistence, and metadata-first discovery, but the P2-T04-owned resolver/scheduler/Pi bridge is still not wired. | reconcile the P2-T04 lease, then compose source metadata -> owner-local Context body load -> private Pi candidate proposal -> daemon admission; do not invoke Pi after WIA consumption or convert Pi text into authority |
