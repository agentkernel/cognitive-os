...
| Active task lease | `lease/personal/P8-T10/dsh-installed-agent` | P8-T10/D04 on `personal/P8-T10-dsh-installed-agent`. P8-T09 remains `done`. EVAL-015 remains closed. | clippy fix + required CI on PR [#256](https://github.com/agentkernel/cognitive-os/pull/256), then ready/merge |
| P8-T10 Install dsh as Personal agent path | **in-progress** | D02 linux-002 `cognitive dsh launch --print` Path B Workspace* `COMPLETED` + Flash `pong` at `4fbffc24`. D03 same-host n=5 Path A p50 10628 ms vs Path B p50 10800 ms (overhead 172 ms). Cleanup **pass**. Running report: [P8-T10 report](../checkpoints/20260822-personal-p8-t10-dsh-installed-agent-report.md). Claim ceiling `hypothesis`. | D04: clippy `expect_err` + required CI, then merge |
...
`P8-T10` is `in-progress`: install dsh as the Personal product agent path,
real-machine launch, and same-host Path A/B measurement. Do not auto-claim
...
| `P8-T10/D04` | `in-progress` | D02/D03 evidence recorded. Ubuntu clippy failed `unwrap_err` at `4fbffc24`. | land `expect_err` + required CI, then merge |
