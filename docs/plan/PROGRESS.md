...
| Active task lease | `none` | `P2-T02/D02` is done at `70f40a5aa0acdcb21dbbcac66797803ebc030d71`: private versioned six-family projection, management-bound process-lifetime watch, unavailable authority sources made explicit, and task/management isolation. Exact Linux focused process test and required CI passed. | claim an exact-path D03 lease for deterministic CLI parity; D02 creates no P2 Gate, release, or Profile claim |
...
| `P2-T02/D03` | `ready` | no deterministic CLI Task/resource application-service client or channel/cache/retry/cursor isolation evidence yet | claim the exact CLI client and process-test paths after D02 merges |
