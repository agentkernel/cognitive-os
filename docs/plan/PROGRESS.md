...
| `P2-T09/D01` | `in-progress` | not started; the daemon-private Tool projection currently reports registry availability with no execution-readiness fact, so a family with no assembled executor is indistinguishable from an executable one | add an `execution_ready` fact derived only from the assembled executor set, with failure-first negatives proving it cannot be forged from registry availability |
