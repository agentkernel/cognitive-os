...
| Active task lease | `lease/personal/P2-T18/local-token-csprng` on `personal/P2-T18-local-token-csprng` | P2-T18 is an owner-directed, independent local-auth security corrective with exact paths only; it does not overlap or depend on P2-T14 and does not touch B01/campaign state. Draft PR [#215](https://github.com/agentkernel/cognitive-os/pull/215) head `e65cb0d7`. Exact native Linux: source guard 1/1, auth 13/13 including zero-block, Clippy pass; required Ubuntu CI `31721293941` SUCCESS | wait for required Windows CI on the same head, then self-review and close |
| P2-T18 Local bootstrap/session token OS CSPRNG corrective | **in-progress** | Production `getrandom::fill` OS-entropy adapter, 64-byte sample / 32-byte payload plus independent probe, opaque `prefix-group-group`, fail-closed before file/session creation. Negatives: entropy failure/short/repeated/zero-block, no-session, bounded uniqueness, Unix 0600, static fallback, channel/expiry/revoke, Debug redaction. Exact native Linux `e65cb0d7` and Ubuntu CI green; Windows CI in flight | confirm Windows required check, then review/closure; no P2-T14/campaign guest mutation |
...
| `P2-T18/D01` | `in-progress` | Failure-first source negative failed 1/1 at `bc415f09`. Candidate `e65cb0d7` passed native source guard 1/1, auth units 13/13 (failure/short/repeated/zero-block no-file, Unix 0600), Clippy, and required Ubuntu CI `31721293941` | wait for required Windows CI, then close D01 |
| `P2-T18/D02` | `ready` | Session minting through the same OS RNG passed native auth units at `e65cb0d7` (no-session-on-entropy-failure, bounded uniqueness, channel/expiry/revoke, redaction). Ubuntu CI green; Windows in flight | after Windows CI: self-review and deterministic closure |
