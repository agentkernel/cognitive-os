...
|---|---|---|---|
| Project focus | `cognitiveos-personal`: active and sole implementation project | CognitiveOS architecture assets remain reference/contract inputs; no second product backlog | EVAL-006 closed. P2-T31 is the live-daemon HTTP scheduler-lease mutex (EVAL-006 skip). P7-T07 remains blocked on B01-W prerequisites |
| Owner-directed campaign | `PERSONAL-PERF-EVAL-006` **closed**. Frozen product `main@103fe776`. C1/C2 paired B0 **partial** (`scheduler_row_skip_before_lease`; Task `DRAFT`; `lease_acquired` 0/0; no Pi child). B1/B2 `not-run`. Cleanup **pass**. Claim ceiling `hypothesis`; `not_reviewed`. Evaluation routing OFF. `PERSONAL-PERF-EVAL-005` and `PERSONAL-PERF-EVAL-004` remain **closed**. | Final: [personal-performance-assessment-20260817-eval-006.md](../evaluation/personal-performance-assessment-20260817-eval-006.md). Preregistration: [20260817-personal-perf-eval-006-preregistration.md](../checkpoints/20260817-personal-perf-eval-006-preregistration.md). Do not reuse EVAL-004/005/006 roots, ports `48286`/`48288`/`48290`/`48386`/`48388`/`48390`, or SecretStore `/12`/`/13`/`/14`/`/15` | none for this campaign — live-daemon skip is a new formal P2 task after close; do not collide with P2-T29/T30 |
...
| Owner-directed C1/C2 campaign | PERSONAL-PERF-EVAL-006 **closed**; PERSONAL-PERF-EVAL-004 **closed**; PERSONAL-PERF-EVAL-005 **closed** | EVAL-006 B0 partial on live-daemon skip `scheduler_row_skip_before_lease`. Do not reuse EVAL-004/005/006 roots, ports, or SecretStore `/12`/`/13`/`/14`/`/15`. Claim ceiling hypothesis/`not_reviewed` | none — next product task after close |
| Active task lease | `lease/personal/P2-T31/live-daemon-scheduler-lease` | P2-T31 live `kernel-server` HTTP admit → scheduler lease; branch `personal/P2-T31-live-daemon-scheduler-lease` | D02: shared authority store + Unix stub live test; Ubuntu CI + linux-002 |
| Benchmark readiness product train | `P2-T21` **done** at `5d1f5c2643d82807eb96f2c615d460dbdca749c3` / PR #220; `P2-T22` **done** at `03bc530d692649a00efa7918bfeadddf8ba0ebea` / PR [#221](https://github.com/agentkernel/cognitive-os/pull/221); `P2-T23` **done** at `795bfac8d8ee77b1b3be4137d33399fc817d20eb` / PR [#222](https://github.com/agentkernel/cognitive-os/pull/222); `P2-T24` **done** at `2b803e0fb955d03374e10569b4d9aecd0b4162d7` / PR [#223](https://github.com/agentkernel/cognitive-os/pull/223); `P2-T25` **done** at `4b10db9a64584bda42ae249bd1df289475bd6324` / PR [#224](https://github.com/agentkernel/cognitive-os/pull/224); `P2-T26` **done** at `9e1404a1b262a0e8588dbc2d1daac08ce419bd33` / PR [#225](https://github.com/agentkernel/cognitive-os/pull/225); `P2-T27` **done** at `70980131c34e514c8bfba681ca0ee7c8a8f92fdc` / PR [#226](https://github.com/agentkernel/cognitive-os/pull/226); `P2-T28` **done** at `1e71344a7b2c4a443fd0581e7fd33f21e970efbd` / PR [#227](https://github.com/agentkernel/cognitive-os/pull/227); `P2-T29` **done** at `98cb23d1dc548929e727259fa4d3109cfe38f5ee` / PR [#230](https://github.com/agentkernel/cognitive-os/pull/230); `P2-T30` **done** at `b13655b9091174d6fbfb774159e68b43ea60b0cc` / PR [#233](https://github.com/agentkernel/cognitive-os/pull/233) | Owner 2026-08-17 delivery after EVAL-005 close filled the scheduler-lease mutex. Ubuntu/Windows CI `32020479890` at `85d6a2bb`; linux-002 bin 337/337. Product evidence only, not EVAL/Gate/release/Profile/B01 | retain closure; next measurement is EVAL-006 |
...
|---|---|---|---|
| `P2-T31/D01` | `in-progress` | EVAL-006 live daemon HTTP admit stayed `DRAFT` with `lease_acquired` 0/0. P2-T30 fixture tick is not this path. Failure-first live HTTP test plus Unix stub adapter authored. | Ubuntu/linux-002: prove skip on unfixed HEAD, then D02 shared-store + stub path |
| `P2-T31/D02` | `ready` | HTTP `TaskApi` reused a second SQLite writer per request; periodic tick did not share the daemon-owned handle. | bind `TaskApi::with_shared_store`; live test must leave DRAFT and `lease_acquired` ≥ 1 |
| `P2-T31/D03` | `ready` | exact-revision linux-002 matrix after D02 | run focused live test, bin, workspace, Clippy, fmt; then ready/merge |
