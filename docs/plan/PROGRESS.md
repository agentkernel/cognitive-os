...
| P2-T04 private worker composition | `in-progress` | P2-T04/D01 remains active on `lane/personal-p2-t04-context-view`. The daemon resolves and authorizes Context before accepting only untrusted candidate fields, then owns candidate sealing and admission. The next checkpoint persists the exact resolved ContextView before private Pi proposal, retaining metadata and strong references rather than source bodies. WIA remains post-admission and unconsumed; no executor, progress, evidence, verification, acceptance, or Task completion is added. | run supported CI and exact-revision native Linux validation for ContextView persistence; add the revoked-source integration negative proving no revoked body reaches Pi or ranking |
...
| Active task lease | `lease/personal/P2-T04/private-worker-composition`; `lease/personal/P2-T05/native-tool-registry-validation`; `lease/personal/P3-T01/context-request-binding` | P2-T04/D01 is active under `lane/personal-p2-t04-context-view` and owns the ContextView persistence continuation; P2-T05 owns the native catalog/validator review boundary. P3-T01/D01 owns the ContextRequest binding/persistence prerequisite. Pi remains candidate-only and scheduler/verified-continuation authority remains daemon-owned. B02/B03/B04/B05/B12, release, Profile, and Task completion remain separate/incomplete. | run supported CI and exact-revision native Linux validation for the ContextView-before-Pi boundary |
