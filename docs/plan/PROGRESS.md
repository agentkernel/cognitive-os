...
| P3-T01 Context source/retrieval port | `in-progress` | `experimental-local-only` / `provided`; `P3-T01/D01` has committed TaskContract v0.4 ContextRequest binding plus append-only ContextRequest/ContextView persistence/query groundwork at `43c1cbf9e5ba1e83572772926c681c2c30c5abfa`. Native Linux migration-layout 8/8, intent-chain 6/6, and workspace formatting passed for that exact revision. This does not yet provide real workspace/task/evidence sources, scope-before-ranking, revocation coverage, required CI, B03, or task completion. | add D01's durable Context admission integrity and mismatch negatives, then integrate real sources without overlapping the active P2-T04 lease |
...
| `P3-T01/D01` | `in-progress` | committed TaskContract v0.4 ContextRequest binding and append-only ContextRequest/ContextView persistence/query groundwork at `43c1cbf9e5ba1e83572772926c681c2c30c5abfa`; native Linux migration-layout 8/8, intent-chain 6/6, and fmt passed. D01 mismatch/integrity negatives, required CI, real workspace/task/evidence sources, scope-before-ranking, and revocation coverage remain incomplete. | add durable admission integrity and mismatch regressions, then attach real Context sources while preserving the P2-T04 runtime lease boundary |
