...
| Active task lease | `lease/personal/P2-T21/governed-candidate-parameters-terminal-evidence` | P2-T21 is `in-progress` on `personal/P2-T21-candidate-parameters-terminal-evidence` under Lane-CTR. It repairs EVAL-004's public candidate-parameter and terminal-evidence product gaps; no evaluation sample has started | complete D01 failure-first contract/binding work, then wire the real production caller and durable redacted task evidence before campaign re-freeze |
| P2-T21 Governed candidate parameters and terminal evidence | **in-progress** | D01 is active. Scope is strictly bounded canonical candidate parameters for WorkspaceSearch/WorkspaceWrite/WorkspacePatch plus an authenticated, task-channel, read-only terminal outcome surface rehydrated from authority/CAS. No generic tool parameter channel, authority-shaped candidate input, private candidate injection, raw SQLite, or secret-bearing output is permitted | D01 failure-first negatives and generated contract binding updates; task acceptance requires D02 real production reachability, D03 durable redacted CLI evidence, Ubuntu/Windows CI, disposable native Linux validation, and task closure. No Gate/release/Profile/B01/EVAL/Agent-benefit claim |
