...
| Active task lease | `none` | P2-T02/D01 remains `in-progress`. PR #140 validates the Personal local governance-root decision: the daemon may persist principal-bound local anchors when absent, while clients remain unable to provide authority facts or writer leases. | merge PR #140, then claim a Lane-RUN lease for daemon authority-context composition and the full Task API/watch vertical path |
