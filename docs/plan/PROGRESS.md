...
| Benchmark readiness product train | `P2-T21` **done** … `P2-T31` **done**; `P2-T32` **in-progress** (lease closed) at `fb85cfff` / PR [#239](https://github.com/agentkernel/cognitive-os/pull/239); `P2-T33` **in-progress** | Public `cognitive daemon start` + stub Workspace* leaves `DRAFT` and acquires a lease (Ubuntu + linux-002). Real adapter EVAL-008 B0 stayed `DRAFT` (`private_completion_socket_could_not_be_created`). EVAL-009 short-root B0 stayed `DRAFT` (`private_pi_candidate_adapter_rejected_the_request`; socket created; adapter spawned). Claim ceiling `hypothesis` | P2-T33/D01 host-path/stderr/proxy; P2-T32 Windows required-ci is merge-only; no Gate/B01 promotion |
...
| P2-T33 Private-candidate host path | **in-progress** | Owner 2026-08-18 after EVAL-009 close. Branch `personal/P2-T33-private-candidate-host-path`. Lease `lease/personal/P2-T33/private-candidate-host-path`. Failure-first: EVAL-008 UNIX_PATH_MAX; EVAL-009 adapter reject with stderr `/dev/null` and `content`-only proxy. Claim ceiling `hypothesis`. | D01 socket bind + public stderr + role/content proxy |
...
| `P2-T33/D03` | `in-progress` | exact-revision linux-002 matrix recorded; Ubuntu pass; Windows `verify` pending on `32063236152`. | wait Windows; do not treat stub as C1/C2; EVAL-010 is a separate campaign after this lease closes |
