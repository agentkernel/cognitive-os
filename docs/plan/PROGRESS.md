...
| Active task lease | `none` | the P2-T02/D01 continuation lease is closed after confirming that its missing server-issued preview/request and Task-watch wire semantics are explicit Lane-CTR public-contract prerequisites; no implementation path was started | Lane-CTR must register and freeze the narrow public request/result and watch-resume contract, then P2-T02 can claim a new implementation lease |
...
| `P2-T02/D01` | `blocked` | P2-T01 service and P1-T07 channel contracts are available, and transport preserves the watch envelope; however, the required server-issued preview/admit request/result and authoritative watch-resume wire semantics remain explicitly unregistered public-contract prerequisites | `blocked_paths`: `specs/`, generated bindings and the task/watch public wire contract; `blocked_task_ids`: P2-T02/D01; `blocked_gate_ids`: none; owner: Lane-CTR; next action: register and freeze the narrow contract, then open a new P2-T02 implementation lease |
