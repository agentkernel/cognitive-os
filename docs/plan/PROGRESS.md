...
| Active task lease | `lease/personal/P4-T04/skill-package` | P4-T04/D01 owns the daemon-private local Skill package/revision import and durable binding baseline on `personal/P4-T04-skill-package` | add failure-first local-import, revision, binding, and scope-negative coverage before the supported native Linux validation route |
...
| `P4-T04/D01` | `in-progress` | Immutable SQLite v21 package/revision/binding facts are implemented at `c50cffc9bd7624514d4a4ae69479244fd5823680`; unsafe local provenance, incompatible revision, cross-workspace binding, and revoked-binding persistence have focused regressions. Required validation is running. | confirm supported CI and exact native Linux focused store/Clippy validation; then add the next lifecycle or daemon-service integration slice without creating a public API/projection claim |
