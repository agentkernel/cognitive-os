...
| P2-T03 scheduler/runtime | `in-progress` | `P2-T03/D01`-`D04` are done. The approved D05 public worker-input authority contract is active; it will bind a TaskContract epoch to immutable pre-dispatch authorization without treating worker observations as authority. | implement the versioned contract, issuance/consumption fence, and D05 failure-injection evidence; do not begin P2-T04 before D05 closes |
...
| `P2-T03/D05` | `in-progress` | user approved public TaskContract worker-input authority. `cc006c1` exact Linux scheduler-authority 14/14 remains the prior fence baseline; no new worker execution evidence is claimed. | add the v0.3 contract authority root and per-iteration authorization; bind consumption to current contract/lease/loop/budget fences; then run failure injection |
