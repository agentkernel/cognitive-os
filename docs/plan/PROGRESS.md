...
| P8-T13 Provider Control Plane | **in-progress** | Branch `personal/P8-T13-provider-control-plane` from `main@2a5eaf49`. Lease `lease/personal/P8-T13/provider-control-plane`. Draft PR [#259](https://github.com/agentkernel/cognitive-os/pull/259). Required CI at `19cc7945` **fail** (`-D warnings`). Running report: [P8-T13 report](../checkpoints/20260822-personal-p8-t13-provider-control-plane-report.md). Claim ceiling `hypothesis`. | push `-D warnings` fix then exact-revision Linux; do not auto-claim P6/P7 |
...
|---|---|---|---|
| `P8-T13/D01` | `in-progress` | Failure-first tests written. Linux `p8_t13_endpoint_trust` 1/1 and store 8/8 at `f825d17f`; kernel-server compile **fail** at `77cb1410`; required CI **fail** at `19cc7945`. | push `-D warnings` fix then exact-revision Linux focused tests |
