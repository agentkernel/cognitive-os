...
| P2-T10 Registered Tool family executor parity | **not-started** | Optimization-proposal INV-TASK-2, registered 2026-08-12. Order is WorkspaceSearch, then WorkspaceWrite/Patch, then HttpFetchReadOnly, each through the existing Intent/Effect, idempotency, fencing and reconcile path. `ProcessRun` and Git-specific surfaces are INV-TASK-3 and need an owner product-semantic decision first. | claim a non-overlapping lease and start the WorkspaceSearch vertical slice |
