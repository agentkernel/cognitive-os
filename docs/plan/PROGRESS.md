...
| Benchmark readiness product train | `P2-T21` **done** … `P2-T31` **done**; `P2-T32` **in-progress** (lease closed) at `fb85cfff` / PR [#239](https://github.com/agentkernel/cognitive-os/pull/239) | Public `cognitive daemon start` + stub Workspace* leaves `DRAFT` and acquires a lease (Ubuntu + linux-002). Real adapter EVAL-008 B0 stayed `DRAFT` (`private_completion_socket_could_not_be_created`). EVAL-009 short-root B0 stayed `DRAFT` (`private_pi_candidate_adapter_rejected_the_request`; socket created; adapter spawned). Claim ceiling `hypothesis` | P2-T32 Windows required-ci is merge-only; next product work is P2-T33 after this campaign close; no Gate/B01 promotion |
