...
| `P3-T04/D01` | `in-progress` | Define a daemon-private governed Context cache that can reuse only stable-prefix/delta material whose source versions, governance facts, Task epoch, and Tool identity remain current. | add failure-first stale/revoked-cache and stable-prefix/delta tests, then wire the cache into the Context Builder |
