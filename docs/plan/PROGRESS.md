...
|---|---|---|---|
| `P9-T04/D06` | `in-progress` | the offline campaign executor runs L0–L2 on a registered non-Gate environment, records `L3`–`L5` as `not_run`, and cannot label its own output with the sole active B01 campaign guest | execute the exact revision on `DEV-LINUX-NATIVE-01`, record the redacted report facts, and register the remaining Provider-dependent layers |
