...
| Active task lease | `lease/personal/P4-T04/skill-package` | P4-T04/D03 owns revision lineage plus daemon-private management-session authorization for local Skill import on `personal/P4-T04-skill-package` | validate exact-pinned supersession and task-bearer denial at exact native Linux and required CI, then complete the task acceptance assessment without a public API/projection claim |
...
| P4-T04 Skill package, revision, local import, and binding | `in-progress` | `P4-T04/D01` is establishing daemon-private immutable local package/revision/digest/import facts and Agent/Task/workspace bindings. It must reject digest drift, unsafe paths, revision mismatch, revoked bindings, unauthorized imports, and cross-workspace leakage; a binding remains non-authoritative and grants no direct capability. | implement the smallest durable authority-store slice with focused negatives; public Skill API/projection, Context/Task consumption, B08, Gate, release, and Profile remain separate |
...
| Active task lease | `none` | P4-T02 acceptance, exact Linux focused evidence, required CI, and PR closure are complete; the ignored local `/artifacts/` directory remains protected from modification, deletion, commitment, or implicit use. | select the next ready formal task only after P4-T02 branch closure |
...
| `P4-T04/D03` | `in-progress` | SQLite v23 same-package revision supersession preserves old exact pins and rejects competing lineage. Import rejects digest drift when the immutable payload does not carry the recorded manifest/content digest. The same daemon-private slice requires the existing owner-local management channel before local import; a task bearer is rejected before persistence. Supported validation has not run for this revised scope. | run exact native Linux kernel-server/store tests and Clippy plus required CI, then map the completed package/revision/import/binding/supersede/revoke/explain acceptance |
