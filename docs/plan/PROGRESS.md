...
| Active task lease | `lease/personal/P2-T25/tool-lifecycle-invocation` | P2-T24 merged via PR #223 at `main@2b803e0f`. P2-T25 D01 on Draft PR [#224](https://github.com/agentkernel/cognitive-os/pull/224). Ubuntu `31928197414` failed at `6da631ed` (`ToolLifecycleResponse` missing `Debug`). Unique in-progress slice `P2-T25/D01`. Windows `not-run by owner-directed Linux-only route`. `B01-Desktop-Linux-002` untouched (EVAL-004-only) | push Debug derive, then Ubuntu supporting CI |
...
| `P2-T25/D01` | `in-progress` | Public Tool lifecycle projection, Agent exposure, and bounded selection receipts. Ubuntu supporting CI `31928197414` failed at `6da631ed` (`ToolLifecycleResponse` missing `Debug`). Compile-fix authored. Local Rust tests `not-run` (`RUST-LINK-DEV-WIN-GNU-01`). Draft PR [#224](https://github.com/agentkernel/cognitive-os/pull/224) | push Debug derive, wait Ubuntu supporting CI, then D02 |
