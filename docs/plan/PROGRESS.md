...
|---|---|---|---|
| Project focus | `cognitiveos-personal`: active and sole implementation project | CognitiveOS architecture assets remain reference/contract inputs; no second product backlog | continue P7-T04 within its single task branch, Draft PR, and active lease |
| Active task lease | `lease/personal/P7-T04/performance-governance` | `P7-T04/D01` is active on `personal/P7-T04-performance-governance` at exact pushed revision `e88ebf4`; Draft PR #179 is open and Ubuntu/Windows required CI passes. The task owns the performance campaign implementation, report-policy validator, exact plan/progress/lease updates, and governance source de-duplication required to make task/lease state machine-checkable. | continue D01 with Memory FTS5 and Intent/Effect persistence measurements; retain hypothesis-only output until a registered campaign runs |
...
| P7-T04 Performance campaign and regression floor | `in-progress` | `P7-T04/D01` is active at pushed revision `e88ebf4` in Draft PR #179 on `personal/P7-T04-performance-governance`; the deterministic runner compiles, tests, and passes Clippy/rustfmt on Ubuntu and Windows required CI. Governance corrections are scoped to removing workflow duplication and making current state machine-checkable. Performance scope is module regression, governed-stage measurement, B06/B07 non-claim observations, and release-level A/B non-inferiority. | complete D01-D05 on this branch and Draft PR; B06/B07 remain observations and do not become GMVP-LINUX hard gates |
...
weakening any criterion. P4-T05 and P5-T01 are fully closed through merged PR,
lease, branch, and local-main reconciliation. P7-T04 is the only active task:
...
| `P7-T04/D01` | `in-progress` | At pushed revision `e88ebf4` / Draft PR #179, the deterministic module runner measures Context resolution/filter-builder, full-key Context cache hit, Artifact CAS publish/readback, scheduler eligible-lease CAS, and canonical report serialization with raw samples, warmup, fixture digest, and hypothesis-only output. Required Ubuntu/Windows CI passed build, test, Clippy, rustfmt, consistency, and conformance. | P7-T04 task lease `lease/personal/P7-T04/performance-governance`; add Memory FTS5 and Intent/Effect persistence measurements, then run exact-revision supported benchmark validation before closing D01 |
| `P7-T04/D02` | `ready` | Instrument one real governed daemon path with monotonic stage timing and bounded counters; do not fabricate tail statistics. | depends on `P7-T04/D01` |
| `P7-T04/D03` | `ready` | Collect B06/B07 stable/changed raw observations against full replay with complete denominator and safety accounting; non-claim only. | depends on `P7-T04/D02` and UCR-01 |
| `P7-T04/D04` | `ready` | Add controlled module regression-floor workflow and shared performance policy validation; floating CI is not release hardware evidence. | depends on `P7-T04/D01` |
| `P7-T04/D05` | `ready` | Run fixed-environment A/B governance non-inferiority campaign and close the complete task acceptance. | depends on `P7-T04/D01-D04` and owner-approved preregistration |
