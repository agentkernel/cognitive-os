...
| P1-T09 route implementation | `in-progress` | `experimental-local-only`; retained campaign `001` is failed at 2 successes / 8 failures after 10 attempts. Under ADR-0039 successor `002` completed its fixed six counted outcomes: Attempts 1, 3, 4, 5, and 6 passed the full clean route, while Attempt 2 failed bounded graphical Desktop readiness before product activation. | produce the redacted aggregate and obtain independent verifier closure; Provider credential entry remains graphical hidden input only |
| B01 first-install/first-conversation Gate | **running** | Retained campaign `001` is `fail`; successor `B01-clean-linux-first-install-first-conversation-002` is at Attempt 6 of formal minimum 6, with 5 successes, 1 failure, zero recorded critical safety failures, and an independently verified artifact/signature. Owner-waived transition Attempt 7 is retained outside the denominator because no product operation occurred. | complete aggregate statistics and affirmative independent verifier disposition; no B01/G1/GMVP-LINUX/release/Profile claim before closure |
...
| Active task lease | `lease/personal/P1-T09/b01-successor-native-verification` | P1-T09 has a narrow Lane-RUN lease for successor verification, product-semantic policy synchronization, and campaign closure. The ignored local `/artifacts/` directory remains protected from modification, deletion, commitment, or implicit use. | complete the revised aggregate and independent verifier disposition |
