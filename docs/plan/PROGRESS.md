...
| B01 first-install/first-conversation Gate | **running** | Attempt 1 of the formally required minimum 20 attempts (current fixed `N=20`) passed on `B01-Desktop-Linux-002`: immutable artifact `0.0.0-campaign.20260801.1` from `main@0a5524b`, exact Pi `0.81.1`, bounded response in 6295 ms, `authority_side_effects:false`, and cleanup including secret deletion. The 2026-08-02 statistical addendum preserves attempt 1 and fixes complete-denominator/no-optional-stopping interpretation. The remaining attempts, success rate of at least 90%, zero critical safety failures, aggregate statistics and final independent verifier disposition are not yet present | continue with attempt 2 under the fixed-N addendum without editing, deleting or rerunning attempt 1 |
...
| Active task lease | `none` | P2-T03 daemon admission-order regression slice is closed at `main@ac12a04`: it adds focused STOP-before-lease and clear-path single-acquisition tests, but neither local Windows nor the approved Linux worktree test completed; implementation evidence remains `experimental-local-only` / `tested-local` from prior slices only | restore a reachable, disposable exact-revision Linux Git worktree and run `cargo test -p kernel-server scheduler_authority::tests`; then begin worker dispatch and Effect closure integration; P2 Gates remain not-run |
