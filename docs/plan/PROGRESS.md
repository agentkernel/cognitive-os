...
|---|---|---|---|
| Project focus | `cognitiveos-personal`: active and sole implementation project | CognitiveOS architecture assets remain reference/contract inputs; no second product backlog | development is active under the owner's 2026-08-12 delivery instruction to remove the structural blockers `PERSONAL-PERF-EVAL-002` found: the `C1` read-only and `C2` mutation capability classes are unreachable because four registered Tool families have no executor, an admitted Task never reaches the worker loop, and the verifier has no production caller. Executing P2-T10 (executor parity) first; the scheduler-bootstrap and verification-caller tasks are registered as they are claimed. P8-T08 is closed and P7-T07 is blocked on B01-W execution prerequisites |
...
| Active task lease | `lease/personal/P2-T10/tool-executor-parity` | Claimed 2026-08-12 on `personal/P2-T10-tool-executor-parity` for the registered Tool family executor parity task. Writable paths are the tool-executor module, the shared Rustls transport crate, the formal plan, this snapshot, the three source-mapped handbook page pairs and the task's closure checkpoint. All three `PERSONAL-PERF-EVAL-002` evaluation leases are now closed in the [Parallel Lanes](PARALLEL-LANES.md) archive; phase 3 closed with every cell `not-run`. P8-T08 closed on 2026-08-12 with its lease, branch and PR #203 reconciled; P7-T07 remains blocked on B01-W prerequisites with its lease released | drive P2-T10 to complete acceptance in this one branch, Draft PR and lease |
...
| P2-T10 Registered Tool family executor parity | **in-progress** | Optimization-proposal INV-TASK-2, registered 2026-08-12; claimed 2026-08-12 under the owner's delivery instruction to remove the `PERSONAL-PERF-EVAL-002` structural blockers. This is the first of three tasks needed before a `C1`/`C2` campaign cell can execute: it removes gap 3 of the four wiring gaps recorded on `handbook/*/developer/execution-chain-status.md` (four registered families with no assembled executor). Order is `WorkspaceSearch` (D01), `WorkspaceWrite`/`WorkspacePatch` (D02), `HttpFetchReadOnly` (D03), readiness-projection parity plus acceptance closure (D04); each reuses the existing daemon authority, descriptor digest binding, persist-before-dispatch Intent/Effect, original-key idempotency, fencing and reconcile path. Gaps 1, 2 and 4 (no scheduler bootstrap row, no periodic tick, no verifier caller) are **out of this task's scope** and are registered separately as they are claimed; until they land, these executors are reachable from the daemon's own worker path but not yet from an Agent. `ProcessRun` and Git-specific surfaces remain INV-TASK-3 and still need an owner product-semantic decision. | run D01 through D04 in this one branch/PR/lease, then close the task |
...
|---|---|---|---|
| `P2-T10/D01` | `in-progress` | claimed 2026-08-12; no executor code or focused negative has run yet | write the failure-first `WorkspaceSearch` negatives, then the bounded search executor, and validate on exact-revision native Linux |
| `P2-T10/D02` | `ready` | not started | after D01: the `WorkspaceWrite`/`WorkspacePatch` mutation executor with expected-preimage, atomic publish, partial write, symlink/path race, duplicate dispatch and OUTCOME_UNKNOWN negatives |
| `P2-T10/D03` | `ready` | not started | after D02: the read-only HTTP fetch executor over the single audited Rustls boundary, with a hermetic loopback HTTPS fixture |
| `P2-T10/D04` | `ready` | not started | after D03: extend `ASSEMBLED_EXECUTOR_FAMILIES`, prove the readiness projection follows it, and close the task acceptance |
