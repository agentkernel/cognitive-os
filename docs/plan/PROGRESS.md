...
| Active task lease | `lease/personal/P9-T08/c1-c2-paired-readiness` | P9-T08/D02 on `personal/P9-T08-c1-c2-paired-readiness`. Writable includes `tools/personal/c1-c2-paired/`, `tools/test/c1_c2_paired_p_arm.test.mjs`, and mapped handbook pages. | continue D02: Linux Secret Service get + live Pi still remaining; local Node 8/8 pass |
...
| `P9-T08/D02` | `in-progress` | Local Node focused tests `tools/test/c1_c2_paired_p_arm.test.mjs` **8/8 pass**: loopback-only broker, secret-shaped argv/env refuse, no retained material, Workspace* schema match, path-escape deny, C1 read/search + C2a write/patch with preimage CAS. Linux Secret Service `get` and live Pi remain. | exact-revision `DEV-LINUX-NATIVE-01` for SecretStore get; do not start B01 samples |
| `P9-T08/D03` | `ready` | not started | after D02: freeze runner/corpus/oracle/redactor/seeds/`retry=0` and prove fairness checker on non-B01 |
| `P9-T08/D04` | `ready` | not started | after D03: B01 procedure, bind runbook, cell manifests, checklist recording, leftover-branch delete, EVAL preregistration; no samples |
