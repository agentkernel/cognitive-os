...
## Current snapshot (2026-08-23 P7-T05 control-panel follow-on)
...
| Active task lease | `lease/personal/P7-T05/web-ui-control-panel` | Owner reopened P7-T05 after D01–D07 left live Provider/SecretStore and Agent/Task click-through `not-run`. Branch `personal/P7-T05-web-ui-control-panel`. Clients checkout `D:\cognitiveos-clients` (`pc/web/`). | D08: wire live key/bind/Task run against typed daemon routes; then D09 Linux UI driver |
| P7-T05 Non-blocking Web UI | **in-progress** (`P7-T05/D08`) | Owner 2026-08-23 delivery instruction. Prior D01–D07 shell remains: ADR-0053; kernel PR [#261](https://github.com/agentkernel/cognitive-os/pull/261); clients PR [cognitiveos-clients#1](https://github.com/agentkernel/cognitiveos-clients/pull/1). Live Provider/SecretStore and post-session Agent/Provider/Task click-through were `not-run`. Running report: [control-panel report](../checkpoints/20260823-personal-p7-t05-web-ui-control-panel-report.md). Claim ceiling `hypothesis`. | implement D08 then exact-revision Linux D09; do not claim P6 / P7-T06 / P7-T07 |
...
owner-deferred P6 / P7-T06. `P7-T05` is `in-progress` on the owner control-panel follow-on (`D08`–`D09`).
...
| `P7-T05/D08` | `in-progress` | Owner 2026-08-23: enter LLM key, bind Agent Provider, run Agent through the localhost SPA. Gaps vs D07: wrong `provider_kind` tokens, no trust confirmation, no Task preview/admit/watch, live SecretStore **not-run**. | implement SPA+daemon CAS, then D09 Linux UI driver |
| `P7-T05/D09` | `ready` | Exact-revision `DEV-LINUX-NATIVE-01` UI driver after D08 push. | wait for D08 commit/push |
