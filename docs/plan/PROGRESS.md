...
| P2-T04 private worker composition | `in-progress` | P2-T04/D01 remains active. The daemon resolves and authorizes Context before accepting only untrusted candidate fields, then owns candidate sealing and admission. `331a584` adds immutable v15 daemon-private execution-policy persistence, and `fe5eb33` makes v0.4 scheduler work reload and fence the policy before WIA lookup. Ubuntu passed the persistence checkpoint; its Windows leg was canceled by a subsequent push and is not a pass. The latest scheduler-fence CI is running. Policy creation at Task admission, a supported sessionless secret-free pinned Pi entrypoint, and scheduler bridge invocation remain absent. WIA remains post-admission and unconsumed; no executor, progress, evidence, verification, acceptance, or Task completion is added. | persist complete daemon-owned policy inputs at Task admission, then construct Context/admission commands and invoke the supported pinned Pi candidate transport before WIA lookup |
...
| Active task lease | `lease/personal/P2-T04/private-worker-composition`; `lease/personal/P3-T01/context-request-binding` | P2-T04/D01 is active under its narrow private-worker-composition lease, beginning with v0.4 scheduler compatibility. P3-T01/D01 owns the ContextRequest binding/persistence prerequisite. Pi remains candidate-only and scheduler/verified-continuation authority remains daemon-owned. B02/B03/B04/B05/B12, release, Profile, and Task completion remain separate/incomplete. | complete the daemon-private Context-to-private-candidate transport before P2-T04 completion |
...
| `P2-T04/D01` | `in-progress` | D05 native Linux closure is complete and the P2-T04 lease is on this branch. `1f932c3` accepts v0.4 contracts; `5e63581` implements metadata-first Context authorization/body resolution; `7def236` adds the private candidate-only proposer contract plus daemon sealing, persistence, descriptor validation, and atomic admission composition; `331a584` adds the daemon-private policy port/migration/store and `fe5eb33` adds the pre-WIA v0.4 policy fence. Ubuntu passed for the policy persistence checkpoint; its Windows leg was canceled by a subsequent push and is not a pass. The scheduler-fence checkpoint remains in CI. No supported Pi process transport, policy writer at Task admission, or scheduler caller has invoked the bridge yet, and WIA remains post-admission. | persist complete daemon-owned policy inputs at Task admission, then construct the Context/admission commands and invoke the supported pinned Pi candidate transport before WIA lookup; do not consume WIA merely to call Pi |
