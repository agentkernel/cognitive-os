...
| Owner-directed campaign recovery | `PERSONAL-PERF-EVAL-002` **active (superseding append-only phase-3 recovery, 2026-08-13)** under `lease/personal/EVAL-20260813/performance-evaluation-002-recovery`; implementation-task routing remains suspended | Retained guest evidence proved the earlier “no phase-3 cell executed” closure false. At exact post-P2-T11-only revision `158e9276e49573db84aeb6ab55012d314368a76c`, recovered `B6` completed 270/270 blocks (540/540 runs): B2→B6 completion-delta change +0.37 pp, CI [−3.33,+4.07], and median-overhead change −91.5 ms, CI [−219.9,+31.2], so no measurable optimization. Recovered/finished B3, UJ2, optional T3 and mixed B4 cells are appended in the single running preregistration; no product/P2-T10 evidence is claimed. `B5` 8 h remains **running**, last owner-observed at minute 414/480 with `local_rc=0`; no final B5 result is claimed | recover only after `B5_8H_DONE`; analyze 480 minute rows + 48 paired blocks + 8 restart rows, append the result, disposition 24 h, clean up, reconcile evidence/reports, close the lease/campaign, and do not resume implementation |
...
| Active task lease | `lease/personal/EVAL-20260813/performance-evaluation-002-recovery` | Narrow owner-directed evaluation recovery lease on `main`; writable paths are limited to the campaign plan/report/preregistration and this Current snapshot. It creates no implementation task or Slice status and does not authorize product, contract, negative, test, handbook, snapshot, power, P9-T04 residue, system, or network changes | recover only after `B5_8H_DONE`; analyze 480 minute rows + 48 paired blocks + 8 restart rows, append the result, disposition 24 h, clean up, reconcile evidence/reports, close the lease/campaign, and stop without resuming implementation |
