...
| Benchmark readiness product train | `P2-T21` in progress; `P2-T22..P2-T28` registered and not started | Owner instruction 2026-08-15 approved BR-01..BR-08 and the Linux-only product-validation route. Ubuntu is supporting CI; exact pushed-revision `B01-Desktop-Linux-002` is required acceptance evidence; Windows is `not-run by owner-directed Linux-only route`. This is product implementation evidence only, not EVAL/Gate/release/Profile/B01 evidence | finish `P2-T21/D02`, then D03 and whole-task closure; only after merge claim `P2-T22` on its own branch/lease |
...
| Active task lease | `lease/personal/P2-T21/governed-candidate-parameters-terminal-evidence` | P2-T21 is `in-progress` on `personal/P2-T21-candidate-parameters-terminal-evidence` under Lane-CTR. D01 passed the required Ubuntu/Windows CI after contract, canonical-digest, binding, and generated-drift repairs; no evaluation sample has started | D02 production scheduler reachability and D03 durable redacted task evidence remain; Linux validation is running on a disposable native worktree, while Windows validation is `not-run` per owner instruction |
| P2-T21 Governed candidate parameters and terminal evidence | **in-progress** | D01 is done: candidate parameters are strictly bounded to WorkspaceSearch/WorkspaceWrite/WorkspacePatch, daemon-canonicalized and digest-verified before sealed candidate persistence, then atomically bound into the governed Intent. D02 now preserves already-admitted WIA/continuation recovery without eagerly requiring Pi configuration; Linux native `DEV-LINUX-NATIVE-01` at task head `1223f9e2` content-equivalent validation passed the 67-test scheduler-authority suite, the public C1 verified-completion regression, full workspace Rust tests, workspace Clippy, and handbook checks. Windows validation is `not-run` per owner instruction. No generic tool parameter channel, authority-shaped candidate input, private candidate injection, raw SQLite, or secret-bearing output is permitted | prove the actual Task admission -> candidate -> governed Intent -> scheduler -> production router -> Search/Write/Patch sink path; task acceptance still requires D03 durable redacted CLI evidence, required CI policy resolution, disposable native Linux validation record and task closure. No Gate/release/Profile/B01/EVAL/Agent-benefit claim |
...
| `P2-T21/D02` | `in-progress` | Existing router tests prove persisted Intent parsing only; they do not prove public Task admission reaches Search/Write/Patch through the actual candidate/scheduler path | prove Search/Write/Patch reach their production sinks from the real admitted Task scheduler path while preserving Effect invariants |
...
| `P2-T22/D01` | `ready` | Registered formal slice; implementation has not started | claim only after P2-T21 whole-task merge/lease/branch/main closure |
