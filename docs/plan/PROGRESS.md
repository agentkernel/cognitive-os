...
| Active task lease | `lease/personal/P2-T20/mutation-carriers` — `P2-T20/D01` on `personal/P2-T20-mutation-carriers` | C2 parameter-bearing mutation request carriers; owner-directed 2026-08-14. First failure-first slice proves the production gap | implement the first failure-first test, then wire real request carriers per family; no Gate/release/Profile claim |
| P2-T20 Parameter-bearing mutation request carriers (C2) | **in-progress** | Owner-directed 2026-08-14. Production worker still only dispatches parameter-free WorkspaceRead; WorkspaceSearch/Write/Patch and HttpFetchReadOnly lack governed payload/preimage/origin carriers | write the D01 failure-first test, then wire the carriers through the hardened Effect/authority path |
...
|---|---|---|---|
| `P2-T20/D01` | `in-progress` | failure-first proof that a parameter-bearing mutation request (WorkspaceSearch query / WorkspaceWrite preimage / HttpFetchReadOnly origin) has no production carrier and fails closed before Effect authorization | write the failing test, then wire real carriers per family |
