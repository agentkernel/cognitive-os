...
| `P3-T03/D01` | `in-progress` | Failure-first Artifact CAS storage and access-control negative coverage is pending on the active task branch. Required Rust validation is pre-routed to exact-revision `DEV-LINUX-NATIVE-01` and required CI; local Windows use is limited to formatting, static, and diff checks. | add the daemon-owned filesystem CAS plus immutable metadata; prove digest mismatch, partial-write rollback, and unauthorized fetch fail closed |
