...
| Active task lease | `lease/personal/P7-T04/performance-governance` | `P7-T04/D01` is active on `personal/P7-T04-performance-governance` at exact pushed revision `c016b96`; Draft PR #179 is open. The task owns the performance campaign implementation, report-policy validator, exact plan/progress/lease updates, and governance source de-duplication required to make task/lease state machine-checkable. | inspect PR #179 checks; then route exact-revision supported validation and continue D01 without making a performance or Agent-benefit claim |
...
| P7-T04 Performance campaign and regression floor | `in-progress` | `P7-T04/D01` is active at pushed revision `c016b96` in Draft PR #179 on `personal/P7-T04-performance-governance`; governance corrections are scoped to removing workflow duplication and making current state machine-checkable. Performance scope is module regression, governed-stage measurement, B06/B07 non-claim observations, and release-level A/B non-inferiority. | complete D01-D05 on this branch and Draft PR; B06/B07 remain observations and do not become GMVP-LINUX hard gates |
...
| `P7-T04/D01` | `in-progress` | At pushed revision `c016b96` / Draft PR #179, the deterministic module benchmark/report-policy boundary is implemented without Provider calls or performance claims; local consistency, policy tests, targeted governance failure injection, tools build, and diff checks passed. | P7-T04 task lease `lease/personal/P7-T04/performance-governance`; inspect required CI and run exact-revision supported validation before closing D01 |
