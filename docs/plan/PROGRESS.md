...
| Owner-directed C1/C2 campaign | PERSONAL-PERF-EVAL-004 **paused before B1; evaluation lease closed** | B0 target and runtime readiness were qualified without any B1/B2/B3/B4 sample. The campaign discovered a product gap: the private candidate carrier persists only parameters_digest, while the production WorkspaceSearch/WorkspaceWrite/WorkspacePatch router requires canonical governed parameters. EVAL-002 remains closed and unchanged; claim ceiling remains hypothesis/non-claim | P2-T25 D02 in progress on `personal/P2-T25-tool-lifecycle-invocation`. EVAL-004 re-freeze only after BR-02..BR-08 are merged; no prior campaign asset or SecretStore entry is reused |
| Benchmark readiness product train | `P2-T21` **done** at `5d1f5c2643d82807eb96f2c615d460dbdca749c3` / PR #220; `P2-T22` **done** at `03bc530d692649a00efa7918bfeadddf8ba0ebea` / PR [#221](https://github.com/agentkernel/cognitive-os/pull/221); `P2-T23` **done** at `795bfac8d8ee77b1b3be4137d33399fc817d20eb` / PR [#222](https://github.com/agentkernel/cognitive-os/pull/222); `P2-T24` **done** at `2b803e0fb955d03374e10569b4d9aecd0b4162d7` / PR [#223](https://github.com/agentkernel/cognitive-os/pull/223); `P2-T25` D02 in progress on `personal/P2-T25-tool-lifecycle-invocation`; `P2-T26..P2-T28` registered and not started | Owner instruction 2026-08-15 approved BR-01..BR-08 and the Linux-only product-validation route. Ubuntu is supporting CI; exact pushed-revision `DEV-LINUX-NATIVE-01` is required acceptance evidence for these product tasks; `B01-Desktop-Linux-002` remains owner-directed EVAL-004-only. Windows is `not-run by owner-directed Linux-only route`. This is product implementation evidence only, not EVAL/Gate/release/Profile/B01 evidence | push P2-T25 D02 (Clippy fix + pinned HTTPS), then Ubuntu supporting CI and D03 linux-002. EVAL-004 re-freeze happens only after every BR product task is merged |
...
| Active task lease | `lease/personal/P2-T25/tool-lifecycle-invocation` | P2-T24 merged via PR #223 at `main@2b803e0f`. P2-T25 D02 on Draft PR [#224](https://github.com/agentkernel/cognitive-os/pull/224). Ubuntu `31928197414` failed at `6da631ed` (missing `Debug`); `31929145967` failed at `c1cfcc13` (`clippy::comparison_to_empty`). Unique in-progress slice `P2-T25/D02`. Windows `not-run by owner-directed Linux-only route`. `B01-Desktop-Linux-002` untouched (EVAL-004-only) | push D02 pinned HTTPS + Clippy fix, then Ubuntu supporting CI |
...
| P2-T25 Tool lifecycle and real invocation | **in-progress** | D01 authors public overlay lifecycle (`enabled`/`disabled`/`quarantined`/`revoked`), Agent exposure, and bounded selection receipts. D02 authors a campaign-scoped pinned HTTPS origin registry that production HttpFetchReadOnly consults; RegisteredCheckRun remains the only process family (no ProcessRun). Overlay never mutates descriptor digests. Task callers cannot mutate lifecycle or pin origins. Least-set selection fails closed on stale/widened digests and prompt restatement. Branch `personal/P2-T25-tool-lifecycle-invocation`; lease `lease/personal/P2-T25/tool-lifecycle-invocation`. Running report: [20260816-personal-p2-t25-tool-lifecycle-invocation-report.md](../checkpoints/20260816-personal-p2-t25-tool-lifecycle-invocation-report.md). Claim ceiling `hypothesis`; no Gate/release/Profile/B01/EVAL promotion | Ubuntu supporting CI for D02, then D03 linux-002 |
...
| `P2-T25/D02` | `in-progress` | Campaign-scoped pinned HTTPS origin registry (`personal-pinned-https.json`); production HttpFetchReadOnly consults `allowed_origins(task_ref)`; exact `host:port` is part of the origin; 3xx is `NotExecuted` and not followed. Negatives: descriptor drift, credential URL, redirect, oversize (existing), duplicate/restart original-key (existing). No ProcessRun family. Local Rust tests `not-run` | push D02, wait Ubuntu supporting CI, then D03 linux-002 |
| `P2-T25/D03` | `ready` | Registered formal slice; implementation has not started | waits for D02 and owns whole-task validation/closure |
