...
|---|---|---|---|
| `P9-T04/D05` | `in-progress` | the campaign report policy enforces full L0–L5 coverage, retained denominators, hard safety accounting, cleanup facts, and a claim ceiling bounded by independent verification | run the report-policy negatives on exact native Linux and required CI, then execute the supported layers and record the campaign disposition |
