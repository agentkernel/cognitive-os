...
| P2-T06 Tool/process executor | `in-progress` | `P2-T06/D01` and `D02` are done at immutable `54bd1748e6a3611503242a0f458510eb84e52f67`. D03 at immutable `300ef02bc223ca044309033bf5de5d30150b8964` adds a daemon-private bounded `ProcessCheck` sink with supervisor-owned registration, timeout/orphan fail-closed behavior, stale fencing, idempotent original-key results, redacted bounded output, and a real SQLite Effect integration regression proving `EXECUTING` before supervisor access while Task remains unchanged. Exact native Linux focused tests/Clippy/fmt passed; required Ubuntu/Windows CI passed in Draft PR #162. D03 remains in-progress because unknown-outcome/restart reconcile, before/mid/after fault coverage, and production supervisor wiring remain. This is `experimental-local-only` / `tested-supported-ci` implementation evidence only. | add ProcessCheck unknown-outcome/restart reconciliation and before/mid/after fault-boundary coverage before advancing D03 |
...
| `P2-T06/D03` | `in-progress` | D02's durable Effect and reconciliation boundary is stable at `54bd1748e6a3611503242a0f458510eb84e52f67`. At `300ef02bc223ca044309033bf5de5d30150b8964`, the private ProcessCheck adapter covers bounded target validation, supervisor registration, timeout/orphan failure, stale fencing, idempotency, redaction, output limits, and real SQLite `EXECUTING`-before-supervisor integration with unchanged Task state. Exact native Linux focused tests/Clippy/fmt and required Ubuntu/Windows CI passed. | add ProcessCheck unknown-outcome/restart reconcile, before/mid/after fault coverage, and production-supervisor integration before advancing D03 |
