...
| Active task lease | `lease/personal/P8-T14/provider-control-plane-docs` | P8-T14/D01 on `codex/P8-T14-provider-control-plane-docs`; Lane-DOC; handbook operator usage + plan/product/architecture pointers | finish docs-sync/consistency, push, Draft PR; do not auto-claim P6 / P7-T05 / P7-T06 / P7-T07 |
| P8-T14 Provider Control Plane operator docs | **in-progress** | Documentation-only follow-up; does not reopen P8-T13. Authored bilingual `user.provider-control-plane` plus related user/reference pointers. No Web/desktop panel. Claim ceiling `hypothesis`. | unique next: local handbook/docs-sync/consistency, then push Draft PR and wait required CI |
...
|---|---|---|---|
| `P8-T14/D01` | `in-progress` | Authored bilingual operator usage for shipped Provider Control Plane CLI/daemon API; related user/reference pointers; source-map/manifest. Live Secret Store/Provider/Pi/dsh **not-run**. | local `check-handbook`, generator `--check`, `check:consistency`, `docs-sync-gate --staged`; push Draft PR; wait required CI |
