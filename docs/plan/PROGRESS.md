...
| P2-T04 private worker composition | `in-progress` | P2-T04/D01 lease is now on the active Context branch. Scheduler execution parsing at `1f932c3` accepts the already-minted v0.4 Context-bound TaskContract schema while retaining v0.3 compatibility. The daemon-private resolver at `5e63581` fences the current TaskContract strong ContextRequest binding, request principal, durable current revocation epoch, metadata/body equality, and per-object body authorization before deterministic rendering. It does not yet call Pi or persist/admit a candidate. The existing WIA remains post-admission; no budget debit, Effect, progress, evidence, verification, acceptance, or Task completion is added. | add the pinned private Pi candidate transport and daemon proposal validation/admission composition; do not invoke Pi after WIA consumption or convert Pi text into authority |
...
| `P2-T04/D01` | `in-progress` | D05 native Linux closure is complete and the P2-T04 lease is now on this branch. Scheduler parsing accepts a v0.4 Context-bound contract at `1f932c3`, and `5e63581` implements the daemon-owned metadata-first Context authorization/body load/resolution boundary while keeping WIA post-admission. The Rust-to-Pi candidate bridge and candidate admission composition are not yet integrated. | add a pinned private candidate transport and daemon-side proposal validation/admission; do not consume WIA merely to call Pi |
