...
| Owner-directed C1/C2 campaign | PERSONAL-PERF-EVAL-007 **closed**; PERSONAL-PERF-EVAL-006 **closed**; PERSONAL-PERF-EVAL-004 **closed**; PERSONAL-PERF-EVAL-005 **closed** | P2-T32 Ubuntu + linux-002 public-launcher+stub **pass** at `fb85cfff`. EVAL-007 skip (stderr `/dev/null`) is lifted for that path. Real `pi-agent-adapter` remains a measurement cell, not a stub. | next: EVAL-008 freeze; do not reuse EVAL-004/005/006/007 assets |
| Active task lease | `lease/personal/P2-T32/public-daemon-start-scheduler` | P2-T32/D02 on `personal/P2-T32-public-daemon-start-scheduler`; Draft PR [#239](https://github.com/agentkernel/cognitive-os/pull/239) at `fb85cfff`; Ubuntu + linux-002 focused `p2_t32` **pass** | close this lease before EVAL-008 (PROGRESS overlap); Windows required-ci flake is merge-only |
| Benchmark readiness product train | `P2-T21` **done** … `P2-T31` **done**; `P2-T32` **in-progress** at `fb85cfff` / PR [#239](https://github.com/agentkernel/cognitive-os/pull/239) | Public `cognitive daemon start` + stub Workspace* leaves `DRAFT` and acquires a lease (Ubuntu + linux-002). Real adapter is EVAL-008. Claim ceiling `hypothesis` | EVAL-008 new freeze after P2-T32 lease close; no Gate/B01 promotion |
...
| P2-T32 Public `cognitive daemon start` scheduler lease | **in-progress** | Ubuntu `32047487272` `verify (ubuntu-latest)` **pass** at `fb85cfff`; linux-002 focused `p2_t32` 1/1 in 3.33s + `daemon_log` 2/2. Windows `verify` fail is unrelated endpoint-publish flake (330/1). Draft PR [#239](https://github.com/agentkernel/cognitive-os/pull/239). Claim ceiling `hypothesis`. Stub ≠ real adapter. | close lease before EVAL-008; rerun Windows required-ci before merge; do not treat stub pass as C1/C2 Agent-benefit |
...
| `P2-T32/D02` | `in-progress` | `daemon.log` + Unix `process_group(0)` landed; stub path proven. Windows required-ci flake blocks merge only. | close lease before EVAL-008; rerun Windows `32047487272` |
| `P2-T32/D03` | `ready` | handbook already in PR #239; full linux-002 workspace/Clippy matrix not required to start EVAL-008 measurement | EVAL-008 freeze is owner next window |
