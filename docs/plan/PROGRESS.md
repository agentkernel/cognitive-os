...
| `P2-T14/D01` | `in-progress` | Native `22c3f502` C1/Clippy pass; native `4b046d36` failed the three new D02 negatives because an extra Effect before the tick is an ambiguous binding | push the verify-then-mutate D02 rewrite, exact native scheduler/Clippy, then stale fixed-post-state |
