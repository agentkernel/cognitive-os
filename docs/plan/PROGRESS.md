...
| `P3-T01/D01` | `in-progress` | TaskContract v0.4 ContextRequest binding and append-only ContextRequest/ContextView persistence/query groundwork are protected by append-time sealed-content, row-metadata, and strong request-reference validation. Exact native Linux `ed30deecc5e33e281a15c7810966ec9c3ec8b5bf` passed Context integrity 4/4, migration-layout 8/8, intent-chain 6/6, and fmt. Required CI, real workspace/task/evidence sources, scope-before-ranking, and revocation coverage remain incomplete. | attach real Context sources and scope/revocation regressions while preserving the P2-T04 runtime lease boundary |
