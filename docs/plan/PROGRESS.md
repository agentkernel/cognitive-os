...
| Active task lease | `lease/personal/P0-T01/checkpoint-delivery-governance` | corrective governance work separates recoverable checkpoint persistence from Slice closure. The merged P2-T02 branch lease is closed, while P2-T02/D01 remains honestly `in-progress` and requires a new continuation lease before more implementation | make commit/push/Draft PR the recoverable checkpoint path and reserve ready/merge for satisfied Slice exits |
