...
## Current snapshot (2026-08-23 P8-T15 native dsh web panel in-progress)
...
| Active task lease | `lease/personal/P8-T15/dsh-native-web-panel` on `personal/P8-T15-dsh-native-web-panel` | Writable: admin-cli dsh, dsh-akp-adapter scripts, plan/PROGRESS/lanes, P8-T15 checkpoint, handbook pages routed by cli-product + dsh-akp-adapter. Not P7-T05 SPA/clients. | diagnose grok-4.6 vs `deepseek-v4-flash` after native web; retry Path B `dsh launch --print`; wait Windows CI `32637705326` |
| P8-T15 Native dsh Web UI | **in-progress** | Owner delivery 2026-08-23. Native panel is `dsh --profile web` default `http://127.0.0.1:3080`, not Personal `/ui/`. linux-002 GET `/` HTML **pass** at `0376e942`. Headless `--print` after panel **fail**. Running report: [P8-T15 report](../checkpoints/20260823-personal-p8-t15-dsh-native-web-panel-report.md). Claim ceiling `hypothesis`. | restore/confirm selected-model Flash; retry Path B print; do not auto-claim P6 / P7-T06 / P7-T07 |
...
|---|---|---|---|
| `P8-T15/D01` | `done` | Jump-host exact `0376e942`: admin-cli `dsh` tests **9/9**, `cargo fmt`, Clippy `-D warnings`. Node preflight 3/3. Required CI Ubuntu pass on `0376e942` (Windows pending in run `32637705326`). | consumed |
| `P8-T15/D02` | `in-progress` | linux-002 `ss` `127.0.0.1:3080`; `GET /` **200** HTML title `DSH Local Build`; JS asset 399361 B; `dsh status` ACTIVE pid 479898. `--host 0.0.0.0` rc 2; missing dist rc 1. Headless `--print` after panel **fail** (`selected_model` grok-4.6, `INVALID_REQUEST`). | diagnose live selected-model grok-4.6 vs file `deepseek-v4-flash`; retry Path B `--print` without native web running |
