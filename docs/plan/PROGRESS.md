...
| Active task lease | `lease/personal/P2-T20/mutation-carriers` — `P2-T20/D03` on `personal/P2-T20-mutation-carriers` | C2 parameter-bearing mutation request carriers; owner-directed 2026-08-14. D01 failure-first proof and D02 WorkspaceSearch carrier are complete (CI green at `2bde32ab`) | wire the next production request carrier (D03) per family; no Gate/release/Profile claim |
| P2-T20 Parameter-bearing mutation request carriers (C2) | **in-progress** | Owner-directed 2026-08-14. Production worker now dispatches parameter-free WorkspaceRead and query-bearing WorkspaceSearch; WorkspaceWrite/Patch, ProcessCheck, and HttpFetchReadOnly still lack governed preimage/payload, supervised-process, or registered-origin carriers. D01 failure-first proof and D02 WorkspaceSearch carrier are green on CI | wire the next production request carrier (D03) through the hardened Effect/authority path |
...
| `P2-T20/D03` | `in-progress` | wire the WorkspaceWrite/WorkspacePatch production request carrier: expected-preimage + payload flows through the production router into the mutation sink, reusing the hardened Effect/authority path | implement the durable payload/preimage carrier plus a focused negative, then required Ubuntu/Windows CI |
