...
| P3-T04 Context cache and telemetry | `in-progress` | `lease/personal/P3-T04/context-cache` owns the daemon-private delta/stable-prefix/cache/telemetry implementation on `personal/P3-T04-context-cache`. The first delivery adds failure-first stale/revoked-cache coverage; no B03, Gate, release, or Profile claim is made. | implement the governed cache key and integrate it with the existing Context Builder |
...
| Active task lease | `lease/personal/P3-T04/context-cache` | P3-T04 is active on `personal/P3-T04-context-cache`; the ignored local `/artifacts/` directory remains protected from modification, deletion, commitment, or implicit use. | implement and validate the daemon-private Context cache boundary |
...
| `P3-T04/D01` | `done` | At `3097faa91439bcb3e9b348c8f474f6184d5277b5`, a daemon-private digest-only cache key binds governance, ContextRequest, TaskContract, source, renderer, and daemon-known Tool facts. Every cache consultation repeats discovery, freshness, authorization, and body/digest validation before comparing metadata; stable/revoked negatives, exact native Linux, and required Ubuntu/Windows CI passed. | consumed by D02; no B03, Gate, release, or Profile claim |
| `P3-T04/D02` | `in-progress` | Derive bounded repeat/no-progress loop-control observations from existing daemon-issued action identities and append-only fenced progress facts, without changing public contracts or assigning Task completion authority. | add durable repeat-signature, changed-evidence, malformed-fact, and retry/stagnation ceiling negatives |
