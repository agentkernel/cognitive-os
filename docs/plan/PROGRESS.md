...
| P2-T03 scheduler/runtime | `in-progress` | `P2-T03/D01`-`D04` are done. D05 checkpoint `e1df4cb` passed required Ubuntu/Windows CI for sealed WIA handoff/recovery fences, including cancelled/task/contract-epoch/runnable/terminal-state rejection, legacy unbound recovery retention, and duplicate bound-handoff preservation; exact Linux worker-recovery validation remains `not-run` after a clean clone was blocked by remote network throughput. No worker execution evidence is claimed. | complete the remaining D05 failure-injection and supported exact-revision worker integration validation; do not begin P2-T04 before D05 closes |
...
| `P2-T03/D05` | `in-progress` | daemon-only candidate admission, sealed WIA issuance, exact scheduler lease-bound one-time consumption, and startup recovery are checkpointed. `e1df4cb` passed required Ubuntu/Windows CI; focused negatives reject cancelled/task/contract-epoch-mismatched, runnable, or terminal scheduler work without consumption/binding, closed legacy unbound recovery retains its lease, and duplicate bound consumption preserves the original binding. Exact Linux candidate-persistence/conformance evidence remains scoped to `d872635`/`50d4fc6`; an exact `d329293` Linux test was `not-run` after the remote clean clone hit GitHub's low-throughput timeout. No worker execution, progress, evidence, Task acceptance, or Task completion is claimed. | add the remaining failure-injection and exact-revision worker integration validation; external executor dispatch remains fail-closed/reconciliation-only |
