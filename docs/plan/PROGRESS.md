...
| Active task lease | `lease/personal/P2-T27/user-lifecycle-backup` | P2-T26 merged via PR #225 at `main@9e1404a1`. Unique in-progress slice `P2-T27/D02`. Draft PR [#226](https://github.com/agentkernel/cognitive-os/pull/226). D01 Ubuntu `31938000525` green at `6eea42c4`; `DEV-LINUX-NATIVE-01` store 22/22, CLI 2/2, HTTP 1/1, fmt, Clippy `-D warnings`. Windows `not-run by owner-directed Linux-only route`. `B01-Desktop-Linux-002` untouched (EVAL-004-only) | author and validate D02 managed Pi install→recover public callers |
...
| `P2-T27/D02` | `in-progress` | Public `admin-cli activate-root`/`rollback` plus current-revision install→recover test with process-bound and stale-recover negatives. Local Rust `not-run` (`RUST-LINK-DEV-WIN-GNU-01`) | Ubuntu supporting CI; exact-revision linux-002 `p2_t27_pi_lifecycle` |
