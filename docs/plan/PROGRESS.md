...
| Owner-directed C1/C2 campaign | PERSONAL-PERF-EVAL-004 **paused before B1; evaluation lease closed** | B0 target and runtime readiness were qualified without any B1/B2/B3/B4 sample. The campaign discovered a product gap: the private candidate carrier persists only parameters_digest, while the production WorkspaceSearch/WorkspaceWrite/WorkspacePatch router requires canonical governed parameters. EVAL-002 remains closed and unchanged; claim ceiling remains hypothesis/non-claim | P2-T21 must establish bounded candidate parameters and a task-bound redacted terminal-evidence surface, complete supported validation, and merge. Then re-freeze EVAL-004 at the new origin/main revision and restart affected B0 cells; no prior campaign asset or SecretStore entry is reused |
| Benchmark readiness product train | `P2-T21` **done** (BR-01 delivered); `P2-T22..P2-T28` registered and not started | Owner instruction 2026-08-15 approved BR-01..BR-08 and the Linux-only product-validation route. Ubuntu is supporting CI; exact pushed-revision `DEV-LINUX-NATIVE-01` is required acceptance evidence for these product tasks; `B01-Desktop-Linux-002` remains owner-directed EVAL-004-only. Windows is `not-run by owner-directed Linux-only route`. This is product implementation evidence only, not EVAL/Gate/release/Profile/B01 evidence | claim `P2-T22` (BR-02 software repair journey) on its own branch/lease; EVAL-004 re-freeze happens only after every BR product task is merged |
...
| Active task lease | none | `lease/personal/P2-T21/governed-candidate-parameters-terminal-evidence` closed on 2026-08-16 after PR [#220](https://github.com/agentkernel/cognitive-os/pull/220) merged at `main@<merge-sha>` with Ubuntu required CI green at head `a942abd5` and `DEV-LINUX-NATIVE-01` exact-revision validation at head `0404f0be` recorded in [P2-T21 running validation report](../checkpoints/20260816-personal-p2-t21-governed-candidate-terminal-evidence-report.md) | claim the next ready Personal task on its own branch/lease |
| P2-T21 Governed candidate parameters and terminal evidence | **done** | D01 bounded candidate-parameter contract, D02 public Task admission → Pi candidate → daemon canonicalization/digest → governed Intent → scheduler → production Search/Write/Patch router/sink reachability, and D03 authority/CAS-rehydrated redacted terminal-evidence endpoint (`GET /task/evidence`) plus `admin-cli evidence` caller. Focused negatives: digest mismatch, preimage drift, family/epoch/action drift, unknown outcome / restart / missing-CAS, cross-task/wrong-channel query rejection. Ubuntu required CI green at `a942abd5` (run `31902392486`); `DEV-LINUX-NATIVE-01` (`wuz@192.168.1.2`, hal9000, Rust 1.97.1) at `0404f0be`: kernel-server bin tests 283/283, full workspace tests, `cargo clippy --workspace --all-targets --locked -- -D warnings` green, `cargo fmt --all -- --check` green. Windows is `not-run by owner-directed Linux-only route`. `B01-Desktop-Linux-002` untouched (EVAL-004-only guest). No generic tool parameter channel, authority-shaped candidate input, private candidate injection, raw SQLite, or secret-bearing output. Closure evidence: [running validation report](../checkpoints/20260816-personal-p2-t21-governed-candidate-terminal-evidence-report.md). Claim ceiling `hypothesis`; no Gate, release, Profile, B01, EVAL, or Agent-benefit promotion | retain closure evidence; EVAL-004 re-freeze and BR-02 (P2-T22) delivery proceed on their own branches/leases |
...
| `P2-T21/D02` | `done` | Public Task admission → Pi candidate → daemon canonicalization/digest check → governed Intent → scheduler → production Search/Write/Patch router/sink reachability proved by `admitted_search_write_and_patch_reach_production_sinks_on_later_ticks` at head `0404f0be` on `DEV-LINUX-NATIVE-01` (283/283 kernel-server tests, full workspace tests, `-D warnings` Clippy). Focused negatives cover digest mismatch, preimage drift, family/action drift, unknown outcome, restart, and missing-CAS. Ubuntu required CI green at `a942abd5` | closed with the P2-T21 running validation report; consumed by BR-02..BR-08 delivery |
| `P2-T21/D03` | `done` | Authority/CAS-rehydrated redacted terminal evidence endpoint (`GET /task/evidence?task_ref=…`) + `admin-cli evidence` caller returns only bounded/redacted lifecycle, opaque refs, current verification/acceptance, and cursor. `classify_reconcile_state` now uses only durable effect states (RECONCILED/VERIFIED/VERIFY_FAILED); `evidence_query_rejects_missing_duplicate_and_malformed_values` covers cross-task/wrong-channel/malformed input | closed with the P2-T21 running validation report; consumed by EVAL-004 re-freeze and BR-02..BR-08 delivery |
| `P2-T22/D01` | `ready` | Registered formal slice; implementation has not started; **now unblocked** by P2-T21 whole-task closure | claim `P2-T22` on a new task branch/lease and start the first vertical implementation slice for BR-02 software repair journey |
| `P2-T22/D02` | `ready` | Registered formal slice; implementation has not started | waits for D01 within the same future task branch and lease |
