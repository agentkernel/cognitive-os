...
|---|---|---|---|
| `P9-T04/D10` | `in-progress` | not started; the remaining autonomous work is the `T2`-`T8` scenario definitions on top of the D08/D09 harness. `L3`/`L4` execution and `L5` stay owner-gated | define the `T2` controlled-repair scenario with a deterministic test oracle and a non-zero permitted-mutation budget |
