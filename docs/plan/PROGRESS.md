...
| P2-T03 scheduler/runtime | `in-progress` | `P2-T03/D01`-`D04` are done. D05 checkpoint `d872635` passed exact Linux candidate-persistence and migration tests; final conformance checkpoint `50d4fc6` passed 13/13 exact Linux runner tests. The append-only non-authority candidate boundary creates no Intent, Effect, WorkerIterationAuthorization, budget debit, scheduler dispatch, progress, or Task outcome. | implement daemon-only candidate admission and atomic authorization issuance/consumption; do not begin P2-T04 before D05 closes |
...
| `P2-T03/D05` | `in-progress` | user approved public TaskContract worker-input authority. `d872635` exact Linux candidate-persistence and migration commands passed; `50d4fc6` exact Linux conformance runner passed 13/13. Candidate rows are immutable observations only, and `cc006c1` scheduler-authority 14/14 remains the prior scheduler fence baseline. No worker execution evidence is claimed. | add daemon-only candidate admission and per-iteration authorization issuance/consumption bound to current contract/lease/loop/budget fences before failure injection |
