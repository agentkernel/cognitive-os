...
|---|---|---|---|
| `P9-T04/D08` | `in-progress` | not started; the L4 governed-Task scenario harness (`T1`-`T8`) is the next code slice and has no owner dependency for its structure | build the `T1` read-only task-admission scenario against the existing daemon authority path with a fixed oracle and no Provider dependency |
