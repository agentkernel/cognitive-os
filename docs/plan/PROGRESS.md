...
1. [20260725-personal-p0-t02-handoff.md](../checkpoints/20260725-personal-p0-t02-handoff.md)（Personal：PERS-PR/任务/Gate 可机读 trace；planning-only，所有 Personal evidence 仍 `not-run`）
2. [20260724-lane-run-admin-cli-custom-install-handoff.md](../checkpoints/20260724-lane-run-admin-cli-custom-install-handoff.md)（RUN：Custom 安装 CLI；durable source/ack query persistence 仍缺）
3. [20260724-lane-run-custom-user-provided-install-handoff.md](../checkpoints/20260724-lane-run-custom-user-provided-install-handoff.md)（RUN：Custom User-Provided digest-pinned local bundle 可零权限登记；官方供应链与执行门槛不变）
