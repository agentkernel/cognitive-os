...
| P3-T01 Context source/retrieval port | `in-progress` | `experimental-local-only` / `tested-local`; `P3-T01/D01` has committed TaskContract v0.4 ContextRequest binding plus append-only ContextRequest/ContextView persistence/query groundwork. A pending exact-revision checkpoint adds a daemon-admitted append-only WorkspaceContextSource record and metadata-only scope/conversation discovery; no discovery result contains a body. Exact native Linux `ed30deecc5e33e281a15c7810966ec9c3ec8b5bf` passed Context integrity 4/4, migration-layout 8/8, intent-chain 6/6, and formatting; exact native Linux `ad95f913423bbee661b28915ef8bb0381523094b` passed the kernel-server suite plus the Context/intent-store regressions and formatting after daemon-side request issuance. Reconstructable authorization/revocation facts, resolver integration, required CI, B03, and task completion remain absent. | validate the new immutable source checkpoint on exact Linux, then add a durable authorization snapshot source before Context body authorization/resolution |
| P2-T04 private worker composition | `blocked` | D05's exact-revision native Linux closure is complete, but D01 cannot safely begin: the existing WIA represents a candidate already admitted before worker execution, while P2-T04 requires Pi to propose before candidate admission. The initial TaskContract-to-ContextRequest binding and durable request/view persistence groundwork now exist in P3-T01/D01, but real Context source/retrieval and the daemon-private Rust-to-Pi candidate bridge remain absent. | complete the P3 Context source prerequisite and provide the private candidate transport; do not invoke Pi after WIA consumption or convert Pi text into authority |
...
| Active task lease | `lease/personal/P2-T04/private-worker-composition`; `lease/personal/P3-T01/context-request-binding` | P2-T04/D01 remains blocked under its narrow private-worker-composition lease. P3-T01/D01 owns only the non-overlapping ContextRequest binding/persistence prerequisite. Pi remains candidate-only and scheduler/verified-continuation authority remains daemon-owned. B02/B03/B04/B05/B12, release, Profile, and Task completion remain separate/incomplete. | complete durable Context integrity/source work and the private candidate transport before P2-T04 implementation |
...
| `P2-T04/D01` | `blocked` | D05 native Linux closure is complete, but the existing WIA is post-admission while P2-T04 needs Pi candidate proposal before admission. P3-T01/D01 now provides only initial durable ContextRequest binding/persistence groundwork; real Context source/retrieval and the Rust-to-Pi candidate bridge do not exist. | complete the daemon-owned Context source prerequisite and private candidate transport; do not consume WIA merely to call Pi |
| `P3-T01/D01` | `in-progress` | TaskContract v0.4 ContextRequest binding and append-only ContextRequest/ContextView persistence/query groundwork are protected by append-time sealed-content, row-metadata, and strong request-reference validation. The pending source checkpoint adds a daemon-admitted append-only WorkspaceContextSource table, sealed row/payload admission validation, and metadata-only tenant/scope/conversation discovery with a separate body-load port. It does not authorize access, invoke the resolver, or claim revocation handling. Exact Linux validation and required CI for this new checkpoint are not-run. | validate the immutable source checkpoint on exact Linux, then add the durable authorization snapshot source before any Context body authorization/resolution; preserve the P2-T04 runtime lease boundary |
