...
| Active task lease | `lease/personal/P4-T05/resource-api` | P4-T05/D01 owns the daemon-private Memory/Skill projection and task-bound application boundary on `personal/P4-T05-resource-api` | implement failure-first projection authorization and revoked/forgotten/stale negative paths before expanding the shared application service |
...
| P4-T05 Memory/Skill API and unified projection | `in-progress` | `P4-T05/D01` owns a daemon-private task-bound Memory/Skill projection and application boundary; public contract generation and B08 remain separate | add the failure-first projection authorization and revoked/forgotten/stale negatives, then wire the smallest real Memory/Skill authority-backed caller |
