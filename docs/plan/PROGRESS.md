...
| Active task lease | `lease/personal/P2-T18/local-token-csprng` on `personal/P2-T18-local-token-csprng` | P2-T18 is an owner-directed, independent local-auth security corrective with exact paths only; it does not overlap or depend on P2-T14 and does not touch B01/campaign state. Draft PR [#215](https://github.com/agentkernel/cognitive-os/pull/215) head `68f0f1b0`. Exact native Linux at `e65cb0d7`: source guard 1/1, auth 13/13 including zero-block, Clippy pass. Required CI green on implementation `e65cb0d7` (`31721293941`) and ledger HEAD `68f0f1b0` (`31722030482`) | keep Draft until owner merge timing vs sibling P2-T14; no campaign guest mutation |
| P2-T18 Local bootstrap/session token OS CSPRNG corrective | **in-progress** | Production `getrandom::fill` OS-entropy adapter, 64-byte sample / 32-byte payload plus independent probe, opaque `prefix-group-group`, fail-closed before file/session creation. Negatives: entropy failure/short/repeated/zero-block, no-session, bounded uniqueness, Unix 0600, static fallback, channel/expiry/revoke, Debug redaction. Exact native Linux `e65cb0d7` and required Ubuntu/Windows CI green | owner merge/lease/branch reconciliation vs active P2-T14; no Gate/release/Profile/B01 claim |
...
| `P2-T18/D02` | `in-progress` | Session minting through the same OS RNG passed native auth units at `e65cb0d7` and required CI on Draft PR #215 HEAD `68f0f1b0` (`31722030482`). Remaining: merge-time reconciliation with sibling P2-T14 lease/PROGRESS | keep Draft; do not merge over P2-T14 |
