...
| Owner-directed C1/C2 campaign | PERSONAL-PERF-EVAL-004 **paused before B1; evaluation lease closed** | B0 target and runtime readiness were qualified without any B1/B2/B3/B4 sample. The campaign discovered a product gap: the private candidate carrier persists only parameters_digest, while the production WorkspaceSearch/WorkspaceWrite/WorkspacePatch router requires canonical governed parameters. EVAL-002 remains closed and unchanged; claim ceiling remains hypothesis/non-claim | P2-T23 merged at `795bfac8` / PR #222. Continue P2-T24 (BR-04). EVAL-004 re-freeze only after BR-02..BR-08 are merged; no prior campaign asset or SecretStore entry is reused |
| Benchmark readiness product train | `P2-T21` **done** at `5d1f5c2643d82807eb96f2c615d460dbdca749c3` / PR #220; `P2-T22` **done** at `03bc530d692649a00efa7918bfeadddf8ba0ebea` / PR [#221](https://github.com/agentkernel/cognitive-os/pull/221); `P2-T23` **done** at `795bfac8d8ee77b1b3be4137d33399fc817d20eb` / PR [#222](https://github.com/agentkernel/cognitive-os/pull/222); `P2-T24` D02 in-progress on `personal/P2-T24-effect-fault-reconciliation` / Draft PR [#223](https://github.com/agentkernel/cognitive-os/pull/223); `P2-T25..P2-T28` registered and not started | Owner instruction 2026-08-15 approved BR-01..BR-08 and the Linux-only product-validation route. Ubuntu is supporting CI; exact pushed-revision `DEV-LINUX-NATIVE-01` is required acceptance evidence for these product tasks; `B01-Desktop-Linux-002` remains owner-directed EVAL-004-only. Windows is `not-run by owner-directed Linux-only route`. This is product implementation evidence only, not EVAL/Gate/release/Profile/B01 evidence | confirm Ubuntu supporting CI on the P2-T24 D02 head, then D03 linux-002 matrix. EVAL-004 re-freeze happens only after every BR product task is merged |
...
| Active task lease | `lease/personal/P2-T24/effect-fault-reconciliation-observation` | P2-T23 merged via PR [#222](https://github.com/agentkernel/cognitive-os/pull/222) at `main@795bfac8d8ee77b1b3be4137d33399fc817d20eb`. P2-T24 D02 unique in-progress slice: original-key reconcile across restart under persisted default-off profiles; production native dispatch consults the four fixed points. Windows `not-run by owner-directed Linux-only route`. `B01-Desktop-Linux-002` untouched (EVAL-004-only). Draft PR [#223](https://github.com/agentkernel/cognitive-os/pull/223) | push D02 head, confirm Ubuntu supporting CI, then D03 linux-002 matrix |
...
| P2-T24 Effect fault and reconciliation observation | **in-progress** | D01 default-off campaign-authorized fault profiles and bounded `GET /task/effects` history authored; D02 original-key restart reconcile consults the persisted profile and production native dispatch injects only at the four fixed points. Lease `lease/personal/P2-T24/effect-fault-reconciliation-observation`. Draft PR [#223](https://github.com/agentkernel/cognitive-os/pull/223). Running report: [20260816-personal-p2-t24-effect-fault-reconciliation-report.md](../checkpoints/20260816-personal-p2-t24-effect-fault-reconciliation-report.md). Claim ceiling `hypothesis`; no Gate/release/Profile/B01/EVAL promotion | Ubuntu supporting CI on the D02 head, then D03 exact-revision linux-002 matrix |
...
| `P2-T24/D02` | `in-progress` | Persisted authorized profiles authorize original-key restart reconcile (mutation count 0 or 1); replacement key and mismatched point fail closed; Indeterminate/open Effects keep `acceptance_ref` absent. Production native dispatch consults the four fixed points; missing/default-off/unauthorized files never inject | Ubuntu supporting CI, then D03 linux-002 matrix |
| `P2-T24/D03` | `ready` | Registered formal slice; implementation has not started | waits for D02 and owns whole-task validation/closure |
