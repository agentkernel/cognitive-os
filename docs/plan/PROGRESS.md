...
| Active task lease | `lease/personal/P4-T04/skill-package` | P4-T04/D02 owns append-only binding revocation facts and active-binding non-resurrection on `personal/P4-T04-skill-package` | validate the revocation slice at exact native Linux and required CI, then continue with supersede/explain integration |
...
| `P4-T04/D02` | `in-progress` | SQLite v22 append-only Skill binding revocation facts and active-binding non-resurrection are implemented locally; supported validation has not run for this revision. | complete focused native Linux/store and required CI validation, then assess the remaining supersede/explain acceptance without public API/projection claims |
