...
| P9-T05 Pi route nested stage timing | **in-progress** | Claimed 2026-08-13 on `personal/P9-T05-pi-nested-stage-timing`, based on `origin/main@326f97728ab6aaaacceaedd2156d953231b32e01`. Delivers the highest evidence-ranked `PERSONAL-PERF-EVAL-002` optimization prerequisite: the campaign's +1828.5 ms median OS-arm overhead (CI [1753.6, 1893.9]) has no measured stage attribution after Extension load (4.5 ms), streaming mode (−38.7 ms), daemon residual scaling (flat 128.1 ms) and output size were each directly excluded. Scope is measurement capability only: one opaque correlation id joining Pi-side and daemon-side observations, seven monotonic non-overlapping stages, and Provider usage exposure for a campaign runner. Instrumentation is denied by default, content-free, non-authoritative, and creates no Gate/release/Profile/B01/benchmark claim and no overhead attribution | D01 in progress: failure-first proof plus the Pi-side nested observation core | continue D01, then D02 real-route wiring, D03 daemon preflight/SecretStore stage, D04 acceptance/closure |
| Active task lease | `lease/personal/P9-T05/pi-nested-stage-timing` | P2-T13 D01-D04 closed at exact `dab38cdf86c8e3486a913d85660593b6671bf114`; native scheduler authority 50/50, verification executor 12/12, `m5_harness` 7/7, three-package Clippy, and required Ubuntu/Windows CI `31707250244` passed. The task lease is archived closed | ready/merge PR #210, reconcile branch/main, then treat P2-T14 as a separate owner/Lane-CTR contract decision and implementation task |
...
| `P9-T05/D01` | `in-progress` | Failure-first `pi-route-observation.test.ts` runs and fails on the missing capability: `openPiRouteObservationSession` is `undefined`, so one Pi run publishes no joined seven-stage observation and no runner-readable Provider usage. Local `DEV-WIN-GNU-01` Node run 0/1 as designed | build the Pi-side nested observation core (opaque correlation id, sequential stage recorder, strict validator, default-denied authorization, bounded content-free sink) and its eight negatives |
| `P9-T05/D02` | `ready` | not started | after D01: wire the real `daemon-provider`/`daemon-client`/Extension route and join the daemon-reported nested stages by correlation id |
| `P9-T05/D03` | `ready` | not started | after D02: add the daemon preflight/SecretStore stage and the authorized correlation echo without changing the product response body |
