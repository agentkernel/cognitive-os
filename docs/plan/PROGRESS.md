...
| Active task lease | `lease/personal/P2-T12/worker-loop-production-chain` — `P2-T12/D01` on `personal/P2-T12-worker-loop` | Claimed 2026-08-13 after confirming clean `main == origin/main == 028b245c492aa6d62f0d680489dc39abf3f84574`, PR #208 merged, no active lease, and the evaluation campaign closed/inactive. Writable paths are the exact P2-T12 admission/scheduler/executor/test, bilingual execution-chain handbook, running report, and current-plan files listed in the active lease table | push the failure-first admission-publication test as an immutable checkpoint, open one Draft PR, confirm the expected failure on `DEV-LINUX-NATIVE-01`, append it to the running report, then implement D01's atomic publication |
...
| P2-T12 Admitted Task reaches the worker loop | **in-progress** | Registered 2026-08-12; acceptance remains the corrected full production chain. On 2026-08-13 the task lease and branch were created from clean `origin/main@028b245`. The first D01 failure-first regression now requires a crash-reopened successful `admit` to expose one current-epoch `Runnable` scheduler row, the contract-named Loop at `START`, and the contract-named hard Budget. Local Rust execution is deliberately `not-run` under `RUST-LINK-DEV-WIN-GNU-01`; expected-red exact-revision native execution follows the first push. Existing facts remain: admission publishes none of the three prerequisites; zero-Intent work aborts before the pre-admission branch; the only production tick runs before bind and propagates a row failure into daemon startup | checkpoint and push the failure-first test, open the Draft PR, capture its expected native failure in the running report, then implement D01 atomically without weakening A1/A2/A3 |
...
| `P2-T12/D01` | `in-progress` | Failure-first red at `280cf2619470675a92880807f114f505eebb328e` is fixed at exact pushed `f8494c878e06920f7c9ffbc37b5dc05a577cadc4`: native `p2_t01_task_application_service` **8/8 pass**. Production admission now repeats fencing + contract epoch CAS in one SQLite transaction and publishes TaskContract/event + contract-named `START` Loop/event + hard Budget + current-epoch `Runnable` scheduler row together. Negatives prove unadmitted absence, crash/reopen complete publication, duplicate admission, and late-member rollback. Local Rust remains `not-run` by the registered Windows GNU restriction | add startup repair for an already-admitted current contract missing only Loop or Budget, validate exact native/Clippy, then close D01 and move directly to D02 |
