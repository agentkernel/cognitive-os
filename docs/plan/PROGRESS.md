...
| P1-T09 route implementation | `in-progress` | `experimental-local-only` / `tested-local`; campaign `.4` (`30603971105`) from `main@c044f2f` passed reviewed-input validation and protected signing/upload. Its independently verified installation on the qualified host reached the redacted expected first response in 4267 ms with `authority_side_effects:false`. A separate KVM Ubuntu 24.04/x86_64 B01 guest and clean reset snapshot now exist; no artifact, Pi, or Provider credential has entered that guest | product owner uses a guest graphical Secret Service prompt agent to create the encrypted persistent login collection, then the runner completes the start gate |
| B01 first-install/first-conversation Gate | `not-run` | new dedicated KVM guest passed non-secret OS, architecture, user-systemd, and clean-state checks. Its native Secret Service supports a non-sensitive transient-session set/get/delete probe, but Product-compatible persistent default storage remains unavailable: standard collection creation reaches a GUI prompt with no headless prompt agent. No B01 attempt has started | approve encrypted persistent login collection creation through a guest graphical prompt; then validate a no-`--collection` non-sensitive store/lookup/clear cycle |
...
| Active task lease | `none` | B01 clean-VM allocation and native non-secret qualification are recorded. Execution is blocked only on a graphical Secret Service prompt needed to create the encrypted persistent login collection | owner approves collection creation; runner then claims a new execution lease for the remaining start-gate and B01 attempt |
