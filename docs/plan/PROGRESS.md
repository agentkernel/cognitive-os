...
| P2-T03 scheduler/runtime | `in-progress` | `P2-T03/D01`-`D05` are done. Exact immutable `08932f7868d46f494aaa76835f4818fd7a1f2962` passed the D05 native Linux focused worker/recovery matrix, `cargo fmt --all -- --check`, `cargo build --workspace`, `cargo test --workspace`, and `cargo clippy --workspace --all-targets -- -D warnings`; required Ubuntu/Windows CI also passed. Candidate WIA remains limited to its atomic `DECIDE -> ACT` handoff/recovery role, and only independently verified continuation authority can enter `CONTINUE -> OBSERVE`. No Task acceptance or completion is claimed. | complete the formal P2-T03 acceptance assessment separately; P2-T04 may now begin on its own lease |
| P2-T07 verification prerequisite | `in-progress` | `P2-T07/D01` is done: exact immutable `08932f7868d46f494aaa76835f4818fd7a1f2962` passed its durable fixed-post-state, verification request/report, currentness, checkpoint, and continuation-authority coverage on native Linux and required Ubuntu/Windows CI. The path is limited to `ACT -> VERIFY -> CONTINUE -> OBSERVE`; it is not Provider/Tool execution, Artifact closure, Task acceptance, Task completion, a Gate, release, or Profile claim. | select the next P2-T07 slice only when its Artifact/evidence/acceptance dependency is ready |
| P3-T01 Context source/retrieval port | `in-progress` | `experimental-local-only` / `tested-supported-ci`; `P3-T01/D01` has TaskContract v0.4 ContextRequest binding, immutable Context request/view/source persistence, metadata-only source discovery, daemon-admin authorization/revocation facts, and the daemon-only Context-before-Pi candidate composition merged by PR #153 at exact revision `d3d9d295970964f9fe4055fdf4f6a29ab85a5e0b`. Native Linux Context-store 8/8 passed at `cda31dc4ec74ae5faaf3d8d47ecb902e97dc8af3`, including a prior allowed body read denied after durable epoch advancement; required Ubuntu/Windows CI passed on PR #153. B03, durable resolved-view emission, and task completion remain absent. | keep P3-T01 blocked until the next owned integration slice persists the resolved ContextView before candidate admission; the scheduler/Pi path remains owned by `lease/personal/P2-T04/private-worker-composition` |
| P2-T04 private worker composition | `in-progress` | P2-T04/D01 remains active. The daemon resolves and authorizes Context before accepting only untrusted candidate fields, then owns candidate sealing and admission. `331a584` adds immutable v15 daemon-private execution-policy persistence, and `fe5eb33` makes v0.4 scheduler work reload and fence the policy before WIA lookup. The latest Draft PR #152 Ubuntu and Windows required checks pass for the current documentation revision. Policy creation at Task admission, a supported sessionless secret-free pinned Pi entrypoint, and scheduler bridge invocation remain absent. WIA remains post-admission and unconsumed; no executor, progress, evidence, verification, acceptance, or Task completion is added. | establish the evidenced daemon-private candidate transport, then persist complete daemon-owned policy inputs and invoke the supported pinned Pi candidate transport before WIA lookup |
| P2-T05 native Tool registry | `in-progress` | The static daemon-owned six-family catalog, immutable digest binding, pre-admission persisted-descriptor verification, private Tool projection, and pre-executor workspace/process/HTTP validators are implemented. Revision `1ced059` closes validation gaps for expected state version, private Provider text-only/single-choice responses, disabled/quarantined resolver coverage, registered process identifiers, and workspace root containment. Exact native Linux focused suites pass; P2-T06 executor/supervisor, external I/O, mutation, and reconciliation remain absent. | complete supported CI and retain P2-T05 as a pre-executor boundary; do not claim Tool execution or Task completion |
...
| Active task lease | `lease/personal/P2-T04/private-worker-composition`; `lease/personal/P2-T05/native-tool-registry-validation` | P2-T04/D01 owns the scheduler/Pi Context-before-candidate integration path, and P2-T05 owns the native catalog/validator review boundary. P3-T01/D01's prerequisite and push-blocker documentation leases are closed after recording the merged PR #153 integration and local delivery failure. Its next durable ContextView emission slice is blocked on a non-overlapping P2-T04 continuation lease. Local closure checkpoint `fcb1708` is not remotely recoverable: `git push -u origin lane/ctr-p3-t01-context-request-binding` failed because the configured GitHub HTTPS endpoint could not connect. Pi remains candidate-only and scheduler/verified-continuation authority remains daemon-owned. B02/B03/B04/B05/B12, release, Profile, and Task completion remain separate/incomplete. | restore ordinary GitHub HTTPS connectivity, push `fcb1708` and the blocker-record checkpoint, then create a Draft PR for the unmerged closure commits; P2-T04 owner separately persists ContextView before private candidate admission |
...
P1-T09, P2-T03, and P3-T01 remain in progress; P2-T03/D05 and P2-T07/D01 are done with
exact-revision native Linux evidence, while their parent-task acceptance exits
remain separate. Formal task completion remains independent from every product
Gate.
...
| `P2-T04/D01` | `in-progress` | D05 native Linux closure is complete and the P2-T04 lease is on this branch. `1f932c3` accepts v0.4 contracts; `5e63581` implements metadata-first Context authorization/body resolution; `7def236` adds the private candidate-only proposer contract plus daemon sealing, persistence, descriptor validation, and atomic admission composition; `331a584` adds the daemon-private policy port/migration/store and `fe5eb33` adds the pre-WIA v0.4 policy fence. Required Ubuntu and Windows checks for the latest documentation revision pass. No supported Pi process transport, policy writer at Task admission, or scheduler caller has invoked the bridge yet, and WIA remains post-admission. | establish the evidenced daemon-private candidate transport, then persist complete daemon-owned policy inputs at Task admission and invoke the supported pinned Pi candidate transport before WIA lookup; do not consume WIA merely to call Pi |
| `P3-T01/D01` | `in-progress` | TaskContract v0.4 ContextRequest binding, immutable request/view/source persistence, metadata-only discovery, sealed durable authorization/revocation facts, and management-only daemon admission are implemented. Exact Linux `cda31dc4ec74ae5faaf3d8d47ecb902e97dc8af3` passed Context store 8/8 and formatting; preceding authority-admission CI passed on Ubuntu/Windows. MVP authorization uses the owner-local management session rather than requiring advanced policy administration; the durable ledger remains available for later hardening. No resolver, body-load caller, Pi invocation, candidate/WIA, budget, Effect, progress, evidence, or Task completion is claimed. | reconcile the P2-T04 runtime lease for the first real owner-local Context resolution and private candidate transport path |
