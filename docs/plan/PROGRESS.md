...
| P2-T06 Tool/process executor | `in-progress` | `experimental-local-only`; P2-T05 registry and validators are stable. No execution, mutation, cursor, supervisor, or reconcile evidence is claimed yet. | implement persist-before-dispatch execution with bounded cursor, fault/orphan/redaction/idempotency negatives, and unknown-outcome reconciliation |
