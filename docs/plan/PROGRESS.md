...
> 最后更新：2026-07-20（Lane-CTR 契约缺口批：TSC 7 项缺口收敛，schema 56→60、向量 76→81、codegen 0.2.0）
...
| 实现已提供（构建通过且有实现代码的 REQ） | 0（合同层 canonical/bundle/projection/生成绑定 + runner 静态合同门 + 客户端层 sdk-ts/agent-shell 骨架均已存在，未作 REQ 级实现声明；REQ-CONF-001/002/003 的 impl/impl_tests 已回填 matrix，其余待各车道证据口径回填） |
| 测试已执行（行为层，runner 真实执行并留证据） | **0（行为）**；静态合同层：**30/81** 向量 pass（traceability/schema/CAS/迁移表/性能合同/信任面静态门 + AKP 信封/流帧/控制 payload schema 负例，逐条 grounding+evidence；**不构成行为覆盖**，见 conformance-evidence §2）；TS 客户端 75 项包内单元测试为实现测试，不计向量执行 |
...
1. [20260720-lane-ctr-gaps-handoff.md](../checkpoints/20260720-lane-ctr-gaps-handoff.md)（Lane-CTR 缺口批：TSC 7 项逐项终态、AKP wire schema ×4、codegen 0.2.0、D-013~D-016）
...
3. [20260720-lane-tsc-handoff.md](../checkpoints/20260720-lane-tsc-handoff.md)（Lane-TSC 客户端骨架：sdk-ts envelope/双通道/watch/retry + agent-shell 会话层、契约缺口清单、M5 剩余工作）
