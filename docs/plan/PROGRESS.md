...
|---|---|---|---|
| `P9-T04/D10` | `in-progress` | B01 access probe complete: under the active preregistration the guest was reverted to `b01-platform-qualified-baseline`, started, observed at its NAT address, and returned to the recorded shut-off baseline with no product install, Provider config, benchmark, or retained state change. The guest refuses SSH for the host account and has no connected QEMU guest agent, so a B01 `L3`/`L4` attempt is operator-driven end to end rather than automated with one manual credential step | owner decision: run `L3`/`L4` on `DEV-LINUX-NATIVE-01` as `tested-local`, or schedule a fully operator-driven B01 attempt at the graphical console |
