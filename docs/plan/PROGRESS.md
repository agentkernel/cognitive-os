...
| P2-T04 private worker composition | `in-progress` | P2-T04/D01 lease is now on the active Context branch. Scheduler execution parsing at `1f932c3` accepts the already-minted v0.4 Context-bound TaskContract schema while retaining v0.3 compatibility; its focused regression is local-format checked only because Windows GNU Rust compilation is unsupported. The existing WIA remains post-admission, and no resolver, body load, Pi proposal, candidate admission, budget debit, Effect, progress, evidence, verification, acceptance, or Task completion has been added by this bridge checkpoint. | implement the daemon-private metadata -> owner-local body-load -> resolver -> pinned Pi proposal -> daemon admission composition; do not invoke Pi after WIA consumption or convert Pi text into authority |
...
| `P2-T04/D01` | `in-progress` | D05 native Linux closure is complete and the P2-T04 lease is now on this branch. Scheduler parsing accepts a v0.4 Context-bound contract at `1f932c3`, removing the immediate legacy-schema rejection without changing the post-admission WIA boundary. Real Context source/retrieval and the Rust-to-Pi candidate bridge do not yet exist. | complete the daemon-owned Context metadata/body/resolver prerequisite and private candidate transport; do not consume WIA merely to call Pi |
