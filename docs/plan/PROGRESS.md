...
| Active task lease | `lease/personal/P2-T02/task-preview-admit-contracts` | P2-T02/D01 remains `in-progress`; Lane-CTR is freezing the owner-approved narrow server-issued Task preview/admit request/result contract. Existing Task watch cursor-resume/dedup semantics remain unchanged. | register preview/admit schemas, generated bindings and negative vectors, then transfer daemon composition to a new Lane-RUN lease |
