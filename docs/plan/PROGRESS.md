...
|---|---|---|---|
| Project focus | `cognitiveos-personal`: active and sole implementation project | CognitiveOS architecture assets remain reference/contract inputs; no second product backlog | Owner delivery is P8-T12 Resource Manager. Do not auto-claim P6 / P7-T05 / P7-T06 / P7-T07 |
...
| Active task lease | `lease/personal/P8-T12/resource-manager` | Owner delivery of Resource Manager (`docs/product/personal/resource-manager-design.md` + architecture pair). Branch `personal/P8-T12-resource-manager`. | continue `P8-T12/D01` implementation and focused negatives |
...
| P8-T12 Resource Manager | **in-progress** | Owner delivery of the named product/architecture pairing. Implementation + focused tests on `personal/P8-T12-resource-manager`. Lease `lease/personal/P8-T12/resource-manager`. Unique next action: push and exact-revision Linux 真机实测. Claim ceiling `hypothesis`. | continue D01–D04 through ready/merge |
...
Path A/B. Owner delivery `P8-T12` (Resource Manager) is `in-progress`. Do not
...
|---|---|---|---|
| `P8-T12/D01` | `in-progress` | Failure-first negatives and implementation written on `personal/P8-T12-resource-manager`. Local Windows GNU Rust `not-run`. | exact-revision Linux focused tests + Clippy/fmt after push |
| `P8-T12/D02` | `ready` | List/inspect + mutating dispatcher + CLI caller are in the same change set as D01. | close after D01 Linux evidence |
| `P8-T12/D03` | `ready` | Native Linux 真机实测 not yet run. | push exact revision; run on `wuz@192.168.1.2` |
| `P8-T12/D04` | `ready` | Docs-sync/CI/merge remaining. | after D03 |
