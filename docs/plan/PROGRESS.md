...
| Active task lease | `lease/personal/P2-T14/verified-task-completion` — P2-T14/D01 on `personal/P2-T14-verified-completion` | P2-T18 lease closed after PR [#215](https://github.com/agentkernel/cognitive-os/pull/215) merged as `main@203b3fca69b9a879fcbb5e933a5305141fcfb442`; the closed row is archived in Parallel Lanes. P2-T14 is the sole active formal task: owner decision 2026-08-13 resolved the `acceptance_decision` evidence (daemon-authored canonical bytes in Artifact CAS via existing `StrongReference`) and the distinct `principal://personal/acceptance-authority`. D01 and D02 are implemented; exact native `95f402d3` passed scheduler authority 57/57 (including the stale-fixed-post-state negative), verification executor 12/12 and Clippy; required CI Ubuntu green, Windows flaky only on the pre-existing `p1_t07_pi_readiness` bootstrap-secret timing. Draft PR [#217](https://github.com/agentkernel/cognitive-os/pull/217) is being merged with main | merge main, confirm required CI on both platforms, review, then close P2-T14 |
...
| `P2-T14/D01` | `in-progress` | owner 2026-08-13 decision resolved the `acceptance_decision` evidence (daemon-authored canonical bytes in Artifact CAS) and the distinct acceptance-authority principal; `activate_task_for_worker_authorization` and `complete_task_from_persisted_verification` are implemented with the three acceptance guards genuinely derived from durable facts; exact native 57/0, Clippy, CI Ubuntu green | merge main, confirm required CI, review, then close |
