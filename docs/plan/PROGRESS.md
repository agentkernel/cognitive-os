...
| Active task lease | `lease/personal/P2-T28/journey-capability-truth` | P2-T27 merged via PR [#226](https://github.com/agentkernel/cognitive-os/pull/226) at `main@70980131`. Unique in-progress slice `P2-T28/D01`. Draft PR [#227](https://github.com/agentkernel/cognitive-os/pull/227). Ubuntu `31941480101` **fail** at `aafe45a5` (`clippy::expect_used` / `unwrap_used` in `capability_truth` tests). Local Node 6/6. Windows `not-run by owner-directed Linux-only route`. `B01-Desktop-Linux-002` untouched (EVAL-004-only) | Clippy-clean D01 tests and re-run Ubuntu supporting CI |
...
| P2-T28 End-to-end journey and capability truth | **in-progress** | Unique slice `P2-T28/D01` freeze at `aafe45a5` / Draft PR [#227](https://github.com/agentkernel/cognitive-os/pull/227). Local Node 6/6. Ubuntu `31941480101` fail (Clippy `expect_used`/`unwrap_used`). D02 public-caller smoke authored. Web UI/Multi-Agent stay scope-excluded. Windows `not-run by owner-directed Linux-only route`. Claim ceiling `hypothesis`; no Gate/release/Profile/B01/EVAL promotion | Clippy-clean D01 tests; Ubuntu supporting CI on PR [#227](https://github.com/agentkernel/cognitive-os/pull/227) |
...
| `P2-T28/D01` | `in-progress` | Frozen UJ1..UJ6 capability-truth matrix with failure-first missing-caller/oracle/path/cleanup and scope-exclusion negatives. Local Node 6/6 pass at `aafe45a5`. Draft PR [#227](https://github.com/agentkernel/cognitive-os/pull/227). Ubuntu `31941480101` fail at `aafe45a5` (Clippy). Rust unit tests `not-run` (`RUST-LINK-DEV-WIN-GNU-01`) | Clippy-clean tests; Ubuntu supporting CI on PR [#227](https://github.com/agentkernel/cognitive-os/pull/227) |
| `P2-T28/D02` | `ready` | Hermetic public-caller smoke authored in `p2_t28_end_to_end_journey.rs` (UJ1/UJ3/UJ4/UJ5/UJ6 backup+consumption). Local Rust `not-run` (`RUST-LINK-DEV-WIN-GNU-01`) | waits for D01 Ubuntu supporting CI on the same branch |
| `P2-T28/D03` | `ready` | Registered formal slice; implementation has not started | waits for D02 and owns whole-task validation/closure |
