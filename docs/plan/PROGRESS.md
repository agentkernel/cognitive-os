...
| P2-T03 scheduler/runtime | `in-progress` | `P2-T03/D01`-`D03` are done. D03 exact immutable Linux `7489f4c` passed scheduler authority 11/11 and durable reverse-index store chain 6/6. D04 implementation remains present but lacks its supported focused runtime/kernel validation | run D04 closure/release regressions at an immutable Linux revision; do not begin D05 before D04 closes |
