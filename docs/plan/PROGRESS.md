...
| Active task lease | `lease/personal/P9-T10/live-prompt-injection` | unique in-progress slice `P9-T10/D01`; evaluation routing OFF | finish live P/O prompt injection, then required CI and D03 closure |
| Benchmark readiness product train | `P2-T21` … `P2-T38` **done**; **P9-T08 done**; **P9-T09 done**; **P9-T10 in-progress**; `PERSONAL-PERF-EVAL-012` **closed** | Navigation: [PERSONAL-C1-C2-READINESS-DELIVERY-PLAN.md](PERSONAL-C1-C2-READINESS-DELIVERY-PLAN.md). Packages 1–14 delivered. EVAL-012 B0 fairness fail blocked B1/B2. Dry-run prompt bytes are frozen; live O-arm injection is this task. No Gate, release, Profile, B01, or Agent-benefit conclusion follows. | complete P9-T10; do not activate a new EVAL until live injection can pass |
| P9-T10 Live P/O `--append-system-prompt` | **in-progress** (`P9-T10/D01`) | Branch `personal/P9-T10-live-prompt-injection`. Lease `lease/personal/P9-T10/live-prompt-injection`. CLI forwards an existing absolute non-empty prompt file to Pi; paired live manifests share `frozen-system-task-prompt.txt`. Running report: [P9-T10 validation](../checkpoints/20260820-personal-p9-t10-live-prompt-injection-report.md). | continue D01/D02; Rust tests `not-run` on Windows GNU |
...
|---|---|---|---|
| `P9-T10/D01` | `in-progress` | Local `c1_c2_paired_p_arm` **16/16** including live P/O `--append-system-prompt` argv equality. CLI parse/launch-plan tests are written. Windows GNU Rust `not-run`. | record D01 and continue D02 |
| `P9-T10/D02` | `ready` | Forward `--append-system-prompt` after `--print` to Pi; implementation is on the task branch. | after D01 |
| `P9-T10/D03` | `ready` | Required CI, docs-sync, ready/merge/lease/branch/main. | after D02 |
