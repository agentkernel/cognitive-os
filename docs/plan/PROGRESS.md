...
| Active task lease | `lease/personal/P2-T38/workspace-patch-post-state` | P2-T38/D01 on `personal/P2-T38-workspace-patch-post-state` | continue D01/D02: raw-file SHA-256 preimage + skip VERIFY until RECONCILED |
...
| P2-T38 Public WorkspacePatch post-state | **in-progress** | Owner reauthorized development 2026-08-20 after EVAL-012. D01 failure-first tests: raw-file SHA-256 preimage and no VERIFY on unreconciled Effect. D02 implements equivalent CAS preimage + `AwaitingReconciliation`. Windows GNU Rust `not-run`. | exact-revision Linux focused tests + required CI; no Gate/EVAL claim |
...
|---|---|---|---|
| `P2-T38/D01` | `in-progress` | EVAL-012 closed with C2a O Patch fail `fixed post-state is unavailable`; tagged workspace-image digest still completes Patch. Failure-first tests added for raw-file SHA-256 preimage and VERIFY-skip on unreconciled Effect. | D02 implementation in the same turn; Linux/CI on D03 |
| `P2-T38/D02` | `ready` | D01 tests name the two EVAL-012 fail classes | accept raw-file SHA-256 CAS preimage; skip VERIFY until RECONCILED |
| `P2-T38/D03` | `ready` | waits on D02 | Linux/CI, docs-sync, ready/merge/lease/branch/main |
