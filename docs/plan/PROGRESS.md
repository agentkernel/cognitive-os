...
| P4-T02 Memory FTS5 retrieval baseline | `in-progress` | `P4-T02/D01` is implementing a daemon-private SQLite FTS5 derived index whose candidate query filters admitted decision, scope, purpose, retention, and current source binding before ranking. The index returns metadata-only candidates; source-body authorization and revalidation remain downstream requirements. No focused validation has run; this is `implementation_evidence: provided`, `development_track: experimental-local-only`, and a non-claim. | add focused stale/scope/retention/rebuild negatives, then route exact revision validation to native Linux and required CI |
