...
|---|---|---|---|
| Project focus | `cognitiveos-personal`: active and sole implementation project | CognitiveOS architecture assets remain reference/contract inputs; no second product backlog | owner-directed blocker removal has completed P2-T10 executor parity, P2-T11 public correctness, P2-T12 admitted-Task execution, and P2-T13 production independent verification. A `C1` WorkspaceRead now has a written production acceptance caller on `personal/P2-T14-verified-completion@14e71824`, but exact native validation of that revision is still in flight and D01/D02 failure-first negatives remain. The owner resolved P2-T14's former decision blocker on 2026-08-13: keep the existing Task transition evidence contract, represent `acceptance_decision` as daemon-authored canonical bytes in Artifact CAS through the existing `StrongReference`, and use a distinct daemon-private acceptance-authority principal. P2-T14 is the sole active formal task; `C2` parameter-bearing mutation request carriers remain separate. P8-T08 is closed and P7-T07 remains blocked on B01-W prerequisites |
...
| Active task lease | `lease/personal/P2-T14/verified-task-completion` — `P2-T14/D01` on `personal/P2-T14-verified-completion` | P2-T13 merged through PR #210 at `main@326f97728ab6aaaacceaedd2156d953231b32e01`. Native `14e71824` failed 7 fixture UNIQUE-constraint tests; DRAFT projection helper, missing-report/duplicate-acceptance negatives, and bilingual handbook sync are ready to commit | push the fixture-fix HEAD, exact native scheduler/verifier/Clippy, open Draft PR, remaining D02 currentness/CAS/open-Effect negatives, then required CI |
...
| P2-T13 Independent verification runs in production | **done** | D01-D04 close the unchanged acceptance at exact `dab38cdf86c8e3486a913d85660593b6671bf114`. Production WorkspaceRead reaches `ACT -> VERIFY -> CONTINUE -> OBSERVE` through one daemon ArtifactStore, atomic fixed state/request publication, current Acceptance criteria, registered independent fixed-Effect verifier, CAS-backed report, progress, checkpoint and one-time continuation authority. Issue/requeue crash recovery and all registered negatives fail closed. Native scheduler authority 50/50, verification executor 12/12, `m5_harness` 7/7 and Clippy passed; required Ubuntu/Windows CI `31707250244` and closure-head CI `31714966351` passed. PR #210 merged at `main@326f97728ab6aaaacceaedd2156d953231b32e01`; zero Tasks complete at that revision | consumed by separate P2-T14 |
| P2-T14 Verified Task completion and acceptance authority | **in-progress** | Owner decision 2026-08-13 resolves the former normative blocker narrowly: reuse the registered Task transition table's evidence slots and existing `StrongReference`; persist daemon-authored canonical `acceptance_decision` bytes in Artifact CAS; add no public schema, DTO or table. Acceptance authority is the non-caller-supplied daemon-private `principal://personal/acceptance-authority` / `authority://personal/task-acceptance`, separate from worker, Provider, Tool and verifier. The task must derive every candidate/acceptance guard from current durable contract, Effect, fixed state, latest report, epoch and retrievable CAS facts. Production caller and C1 expected-green test are written at `14e71824`; that revision failed native fixture UNIQUE-constraint tests; the DRAFT projection helper and first two D01 negatives are in the worktree. Remaining D02 negatives: open Effect, stale fixed post-state, superseded report, CAS missing/tampered. P2-T07 per-criterion reporting remains a separate observation | push fixture-fix HEAD, exact native scheduler/verifier/Clippy, open Draft PR, remaining D02 negatives, required CI, then close |
...
| `P2-T13/D04` | `done` | Acceptance mapped; exact `dab38cdf86c8e3486a913d85660593b6671bf114` passed native verification/scheduler/harness suites and Clippy plus required Ubuntu/Windows CI `31707250244` and closure-head CI `31714966351`; PR #210 merged at `main@326f97728ab6aaaacceaedd2156d953231b32e01` | retain closure evidence; P2-T14 consumes the verification loop |
| `P2-T14/D01` | `in-progress` | Owner selected the existing transition evidence contract. Native `14e71824` failed because the WorkspaceRead fixture admitted a second INITIAL event; DRAFT projection helper plus missing-report and duplicate-acceptance negatives are written and locally fmt/consistency/handbook-checked | push the fixture-fix HEAD, rerun exact native scheduler tests, then add remaining currentness/CAS/open-Effect negatives |
| `P2-T14/D02` | `ready` | production caller is already on the task branch at `14e71824` but D01 negatives and exact native proof are still open, so this slice stays unclosed | after D01 negatives: finish exact native/CI validation of the written caller, then review, docs and task closure |
