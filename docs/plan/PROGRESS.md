...
| Owner-directed C1/C2 campaign | `PERSONAL-PERF-EVAL-004` **active; B0 blocked on target-environment access** | Owner authorized a fresh campaign on 2026-08-15 to re-execute the C1/C2 cells that were `not-run` in EVAL-002, without rewriting the historical report. Execution addendum: [personal-c1-c2-benchmark-execution-plan.md](../evaluation/personal-c1-c2-benchmark-execution-plan.md); preregistration: [20260815-personal-perf-eval-004-c1-c2-preregistration.md](../checkpoints/20260815-personal-perf-eval-004-c1-c2-preregistration.md). Source `93dde21da1635329bd11949b265f205ead46186b`. The current session has no registered `virsh`, SSH, SCP, WSL, or equivalent route to `B01-Desktop-Linux-002`, so B0 cannot start; no B1/B2/B3/B4 sample has started. Claim ceiling `hypothesis`, verifier `not_reviewed`, no Gate/release/Profile/B01/Agent-benefit claim | restore the predeclared target control route, then freeze campaign-only corpus/broker/runner/oracle digests and execute B0 before any paired sample |
...
| Active task lease | `none` | P2-T19 implementation and acceptance are complete; no next formal task has been claimed. C2 carriers (P2-T20), A7 (P2-T17), RegisteredCheckRun (P2-T16), and P2-T14 are on `main` | claim the next C1/C2 production-caller task only with a new task branch, Draft PR, and lease |
