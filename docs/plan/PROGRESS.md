...
| Active task lease | `lease/personal/P2-T27/user-lifecycle-backup` | P2-T26 merged via PR #225 at `main@9e1404a1`. Unique in-progress slice `P2-T27/D01`. Windows `not-run by owner-directed Linux-only route`. `B01-Desktop-Linux-002` untouched (EVAL-004-only) | author D01 public backup/restore, push Draft PR, Ubuntu supporting CI, then exact-revision `DEV-LINUX-NATIVE-01` |
...
| P2-T27 User lifecycle and backup/restore | **in-progress** | D01 public backup/restore archive I/O and `cognitive backup`/`restore` plus management HTTP routes, excluding secret/bearer/provider-config/authority SQLite. Unique slice `P2-T27/D01` on `personal/P2-T27-user-lifecycle-backup`. Windows `not-run by owner-directed Linux-only route`. Claim ceiling `hypothesis`; no Gate/release/Profile/B01/EVAL promotion | push Draft PR and Ubuntu supporting CI; exact-revision `DEV-LINUX-NATIVE-01` for D01 cells |
