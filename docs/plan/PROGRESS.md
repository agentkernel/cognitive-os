...
|---|---|---|---|
| `P9-T04/D02` | `in-progress` | executable L0/L1 runner admits only registered measurement arguments, refuses secret-shaped process input, clears the child environment, and rejects unredacted or self-promoted observations | run the focused negatives on exact native Linux and required CI, then add the transport-only stage decomposition |
