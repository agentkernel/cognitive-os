...
| Active task lease | `none` | P2-T06 is merged and its task lease is closed. The fail-closed observation source is intentionally not a production arbitrary-PID integration. Pi remains candidate-only and scheduler/verified-continuation authority remains daemon-owned. B05/B12, release, Profile, and managed-Pi production supervision remain separate/incomplete. | select the next safe in-progress Personal task from the formal plan; do not expand the product claim |
