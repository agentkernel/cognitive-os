...
| Active task lease | `lease/personal/P7-T04/performance-governance` | `P7-T04/D01` is active on `personal/P7-T04-performance-governance`; the task owns the performance campaign implementation, report-policy validator, exact plan/progress/lease updates, and governance source de-duplication required to make task/lease/blocker state machine-checkable. | implement the deterministic module benchmark and failure-first report-policy negatives; keep all performance evidence non-claim until the registered campaign runs |
...
| `P7-T04/D01` | `in-progress` | Establish the deterministic module benchmark/report-policy boundary without Provider calls or performance claims. | P7-T04 task lease `lease/personal/P7-T04/performance-governance`; complete failure-first negatives and supported validation before closing D01 |
