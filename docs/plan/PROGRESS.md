...
|---|---|---|---|
| Project focus | `cognitiveos-personal`: active and sole implementation project | CognitiveOS architecture assets remain reference/contract inputs; no second product backlog | continue `P7-T02/D03` on `personal/P7-T02-lifecycle-backup` |
| Active task lease | `lease/personal/P7-T02/lifecycle-backup` | P7-T02 owns personal_backup/store lib + plan/checkpoint paths on Draft PR #185. | validate D03 on exact native Linux; then enter D04 transactional lifecycle + acceptance |
...
| P7-T02 Transactional lifecycle, Memory/Skill backup/restore | `in-progress` | `lease/personal/P7-T02/lifecycle-backup` on `personal/P7-T02-lifecycle-backup` / Draft PR #185. D01 done (Linux 3/3 + Clippy at `8750666`, required CI `31430152517`). D02 digest-bound export: Linux `personal_backup` 7/7 + Clippy at `6b6d245`. D03 restore preflight implemented; Linux validation pending. | exact native Linux for D03; then D04 |
...
`P7-T01` is `done` (PR #184 merged). `P7-T02` is `in-progress` with unique
slice `P7-T02/D03`. Formal task completion remains independent from every
...
| `P7-T02/D02` | `done` | digest-bound Memory/Skill/bindings export; Linux `personal_backup` 7/7 + Clippy at `6b6d245`; required CI pending on PR #185 head after D03 | consumed by D03 |
| `P7-T02/D03` | `in-progress` | restore preflight + schema/migration/incomplete negatives before mutation | exact native Linux focused `personal_backup` + Clippy; then D04 |
| `P7-T02/D03` | `ready` | not started | waits for D02 |
| `P7-T02/D04` | `ready` | not started | waits for D03; final acceptance |
