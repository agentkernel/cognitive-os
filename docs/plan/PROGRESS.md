...
|---|---|---|---|
| `P9-T04/D03` | `in-progress` | the real loopback front door records disjoint connection-admission/request-read/header-admission/route-dispatch/response-write stages that disclaim `effect_persistence`, `provider_network`, `pi_process_launch`, and `scheduler_wait` | run the focused transport negatives and the real credential-header request on exact native Linux and required CI |
