...
| Active task lease | `lease/personal/P2-T19/memory-skill-agent-consumer` | Isolated sibling worktree `D:\agent-kernel-msconsumer`, branch `personal/P2-T19-memory-skill-consumer`, created from P2-T13 head `b514d278ef4a3daafe9cceeb62ced2dc649d186b`; writable paths are the Memory/Skill stores, governed Context/resource API seams, focused tests, one running report and mapped bilingual handbook pages only | publish the renumbered D01 failure-first checkpoint, observe it on exact-revision CI, then implement D02 without touching the main worktree or P2-T14 paths |
| P2-T19 Production governed Memory/Skill Agent consumer | **in-progress** (`D01`) | Owner-directed 2026-08-13 delivery. The first checkpoint used P2-T16 because no collision existed at allocation time; before PR creation, concurrent sibling branches registered P2-T16, P2-T17 and P2-T18, so this branch was immediately renumbered to the next non-colliding ID P2-T19 without rewriting the pushed checkpoint. D01 is failure-first from exact P2-T13 head. Target acceptance is daemon-derived Memory/Skill authorization before body/ranking, exact digest/provenance binding in sealed ContextView, durable task/epoch/request/view consumption facts, restart/session-2 resume without chat/user restatement, immediate future-load blocking after forget/revoke/supersede with stable exact pins, candidate-only Pi, complete lifecycle client paths and the registered negative set. Change class is implementation-only unless a genuinely new public field/error proves necessary; no parallel store/client authority and no Gate/release/Profile/B08/EVAL claim | commit/push the P2-T19 renumbering, create a Draft PR dependent on P2-T14, then use required Ubuntu/Windows CI to record the expected D01 failure before implementing D02 |
...
| `P2-T19/D01` | `in-progress` | Failure-first test added on an isolated branch from exact P2-T13; the first pushed checkpoint used the now-colliding P2-T16 ID and is superseded by an append-only P2-T19 renumbering commit. Local Rust execution is not supported on `DEV-WIN-GNU-01`, so the expected baseline failure remains `not-run` until corrected-branch CI | push the P2-T19 checkpoint and observe the expected failure on exact-revision required CI |
| `P2-T19/D02` | `ready` | not started | after D01 failure observation: add metadata-first eligibility, sealed exact bindings and durable consumption persistence |
| `P2-T19/D03` | `ready` | not started | after D02: wire daemon-approved fragments to candidate-only Pi and prove durable session-2 resume |
