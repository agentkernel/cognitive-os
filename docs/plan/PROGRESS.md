...
| Active task lease | `lease/personal/P4-T05/resource-api` | P4-T05/D04 owns daemon-private remember admission and local Skill import/bind command callers on `personal/P4-T05-resource-api`; the authority module is now registered in daemon composition | validate source mismatch, digest drift, unsafe import, incompatible binding, duplicate identity, and no-partial-write command negatives through supported CI; public contract generation and B08 remain separate |
...
| P4-T05 Memory/Skill API and unified projection | `in-progress` | `P4-T05/D01-D03` provide task-bound projection, authority-backed explain reads, durable Memory forget, and durable Skill revoke; exact Ubuntu/Windows CI run `31333312264` passed | implement remember admission plus local Skill import/bind callers and their source/digest/scope/no-partial-write negatives; public contract generation and B08 remain separate |
...
| `P4-T05/D04` | `in-progress` | — | implement daemon-private remember admission plus local Skill import/bind callers and failure-first authority negatives |
