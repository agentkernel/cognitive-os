...
| Active task lease | `lease/personal/P2-T21/governed-candidate-parameters-terminal-evidence` | P2-T21 is `in-progress` on `personal/P2-T21-candidate-parameters-terminal-evidence` under Lane-CTR. D01 passed the required Ubuntu/Windows CI after contract, canonical-digest, binding, and generated-drift repairs; no evaluation sample has started | wire D02 through the real production scheduler caller, then add D03 durable redacted task evidence before campaign re-freeze |
| P2-T21 Governed candidate parameters and terminal evidence | **in-progress** | D01 is done: candidate parameters are strictly bounded to WorkspaceSearch/WorkspaceWrite/WorkspacePatch, daemon-canonicalized and digest-verified before sealed candidate persistence, then atomically bound into the governed Intent. D02 is active. No generic tool parameter channel, authority-shaped candidate input, private candidate injection, raw SQLite, or secret-bearing output is permitted | prove the actual Task admission -> candidate -> governed Intent -> scheduler -> production router -> Search/Write/Patch sink path; task acceptance still requires D03 durable redacted CLI evidence, Ubuntu/Windows CI, disposable native Linux validation, and task closure. No Gate/release/Profile/B01/EVAL/Agent-benefit claim |
