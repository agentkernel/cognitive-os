...
| Active task lease | `lease/personal/P4-T03/memory-lifecycle` | P4-T03/D03 is the only active delivery slice on `personal/P4-T03-memory-lifecycle` | run exact native Linux version/CAS/FTS validation and required CI for the private Memory lifecycle task |
...
| P4-T03 Memory lifecycle, retention and forget | `in-progress` | `P4-T03/D01` and `D02` are implemented; `D03` adds immutable version lineage, expected-version CAS replacement, and supersede lifecycle audit. Validation is not-run for D03. Public Memory API and B08 remain unclaimed. | run exact native Linux version/CAS/expiry/migration/FTS tests and required CI |
...
| `P4-T03/D02` | `done` | At `9feff6060271ca2f004ec918019a9889c18e7d2f`, the daemon-private retention-expiry lifecycle transition enforces the exact deadline boundary, rejects duplicate sweeps, and invalidates FTS rows. Required Ubuntu/Windows CI passed; exact native Linux validation remains to be rerun against the final task revision. | consumed by D03; retain public Memory API and B08 as separate non-claims |
| `P4-T03/D03` | `in-progress` | `lease/personal/P4-T03/memory-lifecycle` adds immutable version lineage, expected-version CAS replacement, supersede audit, and atomic derived-index movement. Focused negative coverage is implemented; validation is not-run. | run exact-revision native Linux version/CAS/expiry/migration/FTS tests, then required Ubuntu/Windows CI |
