...
| Active task lease | `lease/personal/P8-T15/dsh-native-web-panel` on `personal/P8-T15-dsh-native-web-panel` @ `da37ac1d` | Writable: admin-cli dsh + client, dsh-akp-adapter scripts, kernel-server apply/remap + scheduler recovery, plan/PROGRESS, P8-T15 checkpoint, mapped handbook. Clients Apply is PR #4 (separate repo). | linux-002: Cos dsh web **495523/495509** `:3080`, daemon **494681** `:48681`. D04 HTTP apply **pass**; CLI apply **fail** (doctor provider blocked). Do not auto-claim P6 / P7-T06 / P7-T07 |
| P8-T15 Native dsh Web UI | **in-progress** | D04 HTTP apply + Path B remap **pass** at `da37ac1d` (Flash native-id → Flash pong; grok honest 400). SPA Apply in clients `a191e79`. CLI apply/web blocked by doctor `provider=blocked` after daemon replace. Report: [P8-T15 report](../checkpoints/20260823-personal-p8-t15-dsh-native-web-panel-report.md). Claim ceiling `hypothesis`. | Desktop: `/ui/#/bindings` Apply to running dsh. Keep **495523/495509/494681**. Do not auto-claim P6 / P7-T06 / P7-T07 |
...
| `P8-T15/D04` | `in-progress` | Jump `da37ac1d`: dsh **10/10**, p8_t11 **1/1**, p8_t13 **4/4**, recovered-terminal **1/1**, Clippy. linux-002 HTTP apply grok/Flash **pass**; Path B native-id remaps (Flash **200** pong; grok honest **400**). SPA `index-B5jhUhi8.js`. CLI apply **fail** (`doctor.provider=blocked`). | Desktop Apply button; repair CLI apply/web when doctor provider is blocked. Do not auto-claim P6 / P7-T06 / P7-T07 |
