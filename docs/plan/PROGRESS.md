...
| Personal 产品化计划 | **planning；P0-T01/T02/T04 done；P0-T05 in-progress；无 Profile 声明** | P0-T05 新增隔离 crate `cognitive-secret`（ADR-0018）：冻结 SecretStore port、模拟 put/get/rotate/delete 与 fail-closed/redaction 负例；native mutating D-Bus 明确归 P1-T02。本机 `cargo check -p cognitive-secret` 通过；本机 Windows GNU `cargo test -p cognitive-secret` linker exit 121（非支持基线）。P0-T05 行为测试执行证据 **pending CI**。P0-T04 CI run 30150183941 仍为既有通过证据。Personal Gate/B01-B12 仍 `not-run`。 | 正式台账：[PERSONAL-DEVELOPMENT-PLAN.md](PERSONAL-DEVELOPMENT-PLAN.md)；[PERS-PR trace](personal-trace.yaml) 独立于 registry matrix，不新增 REQ/schema/vector。Personal `done` 不代表 G0、B01-B12 或 Profile 已符合。 |
...
1. [20260725-personal-p0-t05-secret-store-api-handoff.md](../checkpoints/20260725-personal-p0-t05-secret-store-api-handoff.md)（Personal：P0-T05 SecretStore API/PoC crate in-progress；行为测试待 CI）
