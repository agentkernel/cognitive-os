...
| Active task lease | `lease/personal/P2-T08/runtime-spine-gates` | Active lease owns P2-T08 Runtime Spine Gates on `personal/P2-T08-runtime-spine-gates` / Draft PR #182. | implement and validate `P2-T08/D03` authority-path negatives |
...
| P2-T08 Runtime Spine E2E Gate | `in-progress` | D01 closed at `38b45bd` (CI `31395283074`). D02 ADR-0018 local-native Provider secret exception expiry closed at `e2cee441b60774326fa224aec3fd8f779584d6f6` with Linux focused `pi-agent-adapter` tests/Clippy and required Ubuntu/Windows CI run `31400044908`. D03 authority-path negatives is the unique in-progress slice. B02/B04/B05/B12 remain `not-run`. | implement D03 shell/daemon close, OUTCOME_UNKNOWN, no-blind-retry, and false-completion floor |
...
| `P2-T08/D03` | `in-progress` | Authority-path focused Runtime Spine negatives for close/OUTCOME_UNKNOWN/false-completion | implement named authority-path negatives + exact Linux + required CI |
