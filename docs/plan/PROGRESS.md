...
|---|---|---|---|
| `P9-T04/D09` | `in-progress` | each retained scenario run now carries its daemon-observed safety facts, so every hard campaign safety counter is reachable from an L4 judgment; a counter the harness cannot produce is a failure the campaign cannot detect | run the safety-coverage negatives on exact native Linux and required CI |
