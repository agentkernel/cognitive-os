...
| P1-T09 route implementation | `blocked` | `experimental-local-only` / `tested-supported-ci`; counted B01 attempt 3 reached the guest SSH service but failed public-key authentication. The exact baseline was restored and the guest is shut off. | B01 desktop/operator owner must provide a pre-authorized non-secret SSH path or revise the preregistered procedure before attempt 4 |
| B01 first-install/first-conversation Gate | **running** | `B01-Desktop-Linux-002` is the sole active B01 guest. Attempt 3 is recorded after the clean-reset checkpoint; attempts 2 and 3 failed readiness before artifact, Pi, service, Provider, credential, prompt, or route use, and cleanup restored the baseline and left the guest shut off. The ordered ledger has 3 of the formal minimum 20 attempts: 1 success / 2 failures, with no critical safety failure observed. | resolve guest SSH authentication provisioning or formally revise the procedure before a fresh attempt 4; do not retry attempt 3 |
...
| Active task lease | `none` | P1-T09 is `blocked` after counted attempt 3; the execution lease is closed. B01 remains `running` with 3 of its fixed 20 attempts recorded (1 success, 2 readiness failures). | the B01 desktop/operator owner must prepare a non-secret authenticated readiness path or approve a procedure change before a new P1-T09 lease and attempt 4 |
...
weakening any criterion. P1-T09 is blocked on the B01 readiness authentication path.
