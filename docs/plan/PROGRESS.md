...
| P4-T01 Memory store, admission, and policy | `in-progress` | `lease/personal/P4-T01/memory-admission` owns a daemon-private append-only Memory proposal-to-deterministic-admission slice on `personal/P4-T01-memory-admission`. It consumes stable Context facts; no FTS/retrieval, lifecycle/forget, public API, B08, Gate, release, or Profile claim is in scope. | persist and test source-bound Memory proposals, reason-coded deterministic policy decisions, and admitted-object absence on rejection |
...
| Active task lease | `lease/personal/P4-T01/memory-admission` | P4-T01 is active on `personal/P4-T01-memory-admission`; the ignored local `/artifacts/` directory remains protected from modification, deletion, commitment, or implicit use. | implement the daemon-private Memory proposal-to-admission boundary |
...
| `P4-T01/D01` | `in-progress` | Build the daemon-private append-only Memory proposal-to-deterministic-admission path from stable Context source/version/provenance facts. | add failure-first admission and persistence negatives before supported validation |
