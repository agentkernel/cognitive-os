...
| `P2-T12/D02` | `in-progress` | D01's exact native and required CI exits are satisfied. Failure-first work now targets the circular zero-Intent precondition and per-row tick isolation | add focused real-SQLite negatives proving a fresh zero-Intent Task reaches candidate admission without consuming worker authority in the same pass, and one malformed row cannot abort the rest of the tick |
