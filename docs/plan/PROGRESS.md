...
| P7-T05 Non-blocking Web UI | **in-progress** (D01 complete in-repo; D02–D07 **blocked**) | ADR-0053 accepted React+TS+Vite, daemon same-origin `/ui/`, memory-only session, Origin/CSRF binding. Route inventory frozen; Node inventory negatives **8/8 pass** locally. `blocked_paths`: `cognitiveos-clients/pc/web/`. Remote repo exists; no approved local checkout. Do not recreate `clients/**` or implement SPA in `apps/cognitiveos-console`. Running report: [P7-T05 report](../checkpoints/20260823-personal-p7-t05-web-ui-report.md). Claim ceiling `hypothesis`; no Gate/release/Profile/B01/Agent-benefit promotion. | owner: provide approved `cognitiveos-clients` checkout; then D02 SPA shell |
...
|---|---|---|---|
| `P7-T05/D01` | `in-progress` | ADR-0053 accepted; route inventory frozen; local Node inventory negatives **8/8 pass**. SPA not implemented. | keep Draft PR; Origin/Referer daemon enforcement is an in-repo follow-on; D02 waits on checkout |
