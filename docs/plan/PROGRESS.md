...
| Active task lease | `lease/personal/P2-T16/registered-check-run` on isolated sibling worktree `D:\agent-kernel-checkrun` | Draft PR [#211](https://github.com/agentkernel/cognitive-os/pull/211). P2-T13 is now on `main@326f977`; this branch remains mergeable. Implementation of D01–D03 is on `c620a630`; required CI run `31719604296` failed at `cargo build` with `E0433` (`DurableExecutorStateStore` undeclared in `tool_executor/router.rs`). Local Rust link remains `not-run` per `RUST-LINK-DEV-WIN-GNU-01`. Merge still waits for P2-T14 | push the import-only compile fix, rerun required Ubuntu/Windows CI, then exact-revision native focused tests |
...
| P2-T16 Registered deterministic check execution | **in-progress** | D01–D03 are written on `personal/P2-T16-registered-check-run`: `check_id`-only request, immutable `c2a.repair.typescript` registry, supervised Effect dispatch, original-key reconcile, CAS evidence and independent verifier. Failure-first red CI `31716967382` then implementation `c620a630`. Required CI `31719604296` failed both platforms at compile (`E0433` missing `DurableExecutorStateStore` import in the production router). General ProcessRun, caller argv/env/cwd/shell and exit-based completion remain forbidden | push the router import fix, rerun required CI, then native focused `registered_check` / `tool_executor` validation |
...
| `P2-T16/D01` | `in-progress` | Failure-first red CI `31716967382` observed the absent API; implementation at `c620a630` adds `RegisteredCheckRunRequest`/`RegisteredCheckRegistry` and the `check_id`-only / unknown / shell-metacharacter / descriptor-drift negatives. Required CI `31719604296` did not reach tests (`E0433` in production router). Local Rust remains `not-run` | after the router import fix: required Ubuntu/Windows CI must compile and pass the focused registry negatives |
| `P2-T16/D02` | `ready` | production router, durable attempting state, fencing, duplicate absorption and mid-dispatch restart reconcile are already written in `c620a630` but remain unvalidated while D01 CI is red | after D01 CI green: native `registered_check` plus `tool_executor` parity |
| `P2-T16/D03` | `ready` | CAS evidence, independent RegisteredCheck verifier, C2a fixture oracle and exit-0-is-not-completion negatives are already written in `c620a630` but remain unvalidated | after D02: required CI/native for evidence/verifier negatives; D04 still waits for P2-T14 on `main` |
| `P2-T16/D04` | `ready` | bilingual handbook already synced in `c620a630`; merge mutex is P2-T14 | after required CI green and P2-T14 merge: rebase/merge `main`, native validation, ready/merge/lease/branch/main |
