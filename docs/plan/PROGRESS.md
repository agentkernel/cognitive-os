...
| Owner-directed campaign | `PERSONAL-PERF-EVAL-011` **paused for procedure amendment** (2026-08-18), frozen source `979e52e4c6681d0fc8c6431c965e3267a7a0d917` (pushed P2-T36 product branch). New B0-only C1/C2 qualification on `B01-Desktop-Linux-002`: root `/home/hal9001/perfeval011-20260818`, daemon `48300`, reserved broker `48400`, and one new SecretStore item only. A campaign-root script interface check unexpectedly initiated a Provider session mint and returned redacted `401`; it is retained as a failed started event with `retry=0`, not a valid B0 sample. The campaign-owned daemon is stopped. `PERSONAL-PERF-EVAL-010`, `009`, `008`, `007`, `006`, `005`, and `004` remain **closed** and are not inputs. Evaluation routing ON; P2-T36 implementation continuation is suspended. | Preregistration: [20260818-personal-perf-eval-011-preregistration.md](../checkpoints/20260818-personal-perf-eval-011-preregistration.md). Running report: [personal-performance-assessment-20260818-eval-011.md](../evaluation/personal-performance-assessment-20260818-eval-011.md). Claim ceiling `hypothesis`; `not_reviewed`; no Gate, release, Profile, B01, or Agent-benefit promotion. Never reuse EVAL-004 through EVAL-010 roots, ports, SecretStore items, runners, corpus, oracle, evidence, or denominator. | await owner approval of an amendment freezing the B0 runner, manifest, bounded input, oracle, output schema, lifecycle observation, and cleanup procedure; do not retry the failed instrument or execute a B0 sample before that amendment. |
...
| Active task lease | `lease/personal/EVAL-011/c1-c2-b0-qualification` | Owner-directed EVAL-011 supersedes P2-T36 continuation while active. It writes only the EVAL-011 preregistration, one running report, and this snapshot; no product task, code, contract, test, generated handbook source, or P2-T36 path is writable in the campaign. Source is immutable pushed `979e52e4c6681d0fc8c6431c965e3267a7a0d917`. The campaign is paused after the retained failed unfrozen instrument invocation. | await the owner-approved B0 procedure amendment; no sample retry or new B0 execution is authorized |
...
| Benchmark readiness product train | `P2-T21` … `P2-T35` **done** (`main@9cefd92c`); `P2-T36` **in-progress but suspended by EVAL-011** | The durable navigation remains [PERSONAL-C1-C2-READINESS-DELIVERY-PLAN.md](PERSONAL-C1-C2-READINESS-DELIVERY-PLAN.md). EVAL-011 measures only frozen `979e52e4…` and does not make P2-T36 complete, nor does P2-T36 unit/CI evidence become an end-to-end C1/C2 conclusion. | Do not resume P2-T36 or claim a product task while EVAL-011 is active; execute only its preregistered B0 cells. |
...
|---|---|---|---|
| `P2-T36/D01` | `in-progress` | Task is registered and leased; no C1 completion evidence exists. EVAL-004 through EVAL-010 remain closed and are not inputs or denominators. | add failure-first real-Pi WorkspaceRead/Search candidate and policy negatives, then wire the smallest public path |
| `P2-T36/D02` | `ready` | Existing scheduler, daemon workspace read executor, fixed-effect verifier, and acceptance authority are product paths; no real Pi C1 completion evidence exists yet. | after D01, exercise the public `cognitive daemon start` composition on exact pushed supported Linux |
