...
| Active task lease | `lease/personal/P5-T05/b09-managed-pi` | Active lease owns P5-T05 B09 managed Pi on `personal/P5-T05-b09-managed-pi` / Draft PR #183. | implement and validate `P5-T05/D01` process-bound SidecarSession; next: push D01 checkpoint for exact Linux/CI |
...
| P5-T05 B09 managed Pi + sidecar qualification | `in-progress` | Claimed after P2-T08 closure. D01 process-bound SidecarSession implementation is on the task branch: installation schema v4 fenced process-attempt binding; activate/resume/recover bind; pause/stop clear; health `process_bound=true` only with durable binding. Failure-first tests added. Validation `not-run` pending exact Linux/CI. B09 remains `not-run`. | push D01 checkpoint; run exact native Linux focused tests + Clippy; required Ubuntu/Windows CI |
...
| `P5-T05/D01` | `in-progress` | Process-bound SidecarSession fenced process-attempt binding implemented on branch; validation not-run | push checkpoint; exact native Linux focused + Clippy; required CI |
| `P5-T05/D02` | `ready` | upgrade/rollback and uninstall/quarantine with process-bound fencing | enter after D01 |
