...
| P1-T09 route implementation | `in-progress` | `experimental-local-only` / `tested-supported-ci`; Attempt 4 passed repaired guest SSH readiness but failed because the immutable preregistered artifact had expired before installation. Cleanup restored the shutoff baseline; replacement-artifact recovery is active. | produce and independently verify a newly available reviewed-main artifact, preregister its immutable identity and Provider/model support, then obtain owner authorization for counted Attempt 5 |
| B01 first-install/first-conversation Gate | **running** | `B01-Desktop-Linux-002` is the sole active B01 guest. Attempt 4 is recorded after the clean-reset checkpoint: SSH readiness passed, but workflow run `30687541828` reported the required artifact expired; no artifact, Pi, service, Provider, credential, prompt, or route was used. The ordered ledger has 4 of the formal minimum 20 attempts: 1 success / 3 failures, with no critical safety failure observed. | produce and independently verify a replacement reviewed-main campaign artifact before a fresh Attempt 5; do not retry Attempt 4 |
...
| Active task lease | `lease/personal/P1-T09/b01-artifact-recovery` | P1-T09 remains `in-progress` while the runner produces a replacement campaign artifact after counted Attempt 4 failed immutable artifact availability. B01 remains `running` with 4 of its fixed 20 attempts recorded (1 success, 3 failures). | make a replacement artifact independently verifiable and preregister it, then obtain owner confirmation for a fresh Attempt 5 |
