...
| Active task lease | `lease/personal/P7-T04/performance-governance` | `P7-T04/D01` is active on `personal/P7-T04-performance-governance` in Draft PR #179. The deterministic runner now measures Context resolution/cache, Artifact CAS, scheduler CAS, Memory FTS5 metadata-first retrieval, Intent/Effect persist-before-dispatch, and canonical report serialization with hypothesis-only output. Required Ubuntu/Windows CI previously passed at `d867b7b`; the Memory/Intent measurement extension awaits exact-revision native Linux validation and replacement required CI. | run exact-revision native Linux benchmark/bin tests for the Memory FTS5 and Intent/Effect measurements, then close D01 after required CI |
...
| P7-T04 Performance campaign and regression floor | `in-progress` | `P7-T04/D01` extends the deterministic module runner with Memory FTS5 retrieval and Intent/Effect persistence measurements on `personal/P7-T04-performance-governance` / Draft PR #179. Prior runner baseline required CI passed at `d867b7b`; the extension remains hypothesis-only and is not a release, Gate, or Profile claim. | validate the extension on exact-revision native Linux and required CI, then continue D02-D05 on the same branch/PR; B06/B07 remain observations |
...
| `P7-T04/D01` | `in-progress` | Draft PR #179 now includes Memory FTS5 metadata-first retrieval and Intent/Effect persist-before-dispatch measurements alongside Context, Artifact CAS, scheduler CAS, and canonical report serialization. Fixture digest covers the new modules; output remains hypothesis-only. Prior baseline required CI passed at `d867b7b`; extension validation is `not-run` until exact native Linux and replacement CI complete. | run exact-revision native Linux focused bin tests plus one bounded benchmark invocation, then require Ubuntu/Windows CI before closing D01 |
