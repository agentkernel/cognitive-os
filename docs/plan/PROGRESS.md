...
|---|---|---|---|
| Project focus | `cognitiveos-personal`: active and sole implementation project | CognitiveOS architecture assets remain reference/contract inputs; no second product backlog | EVAL-010 **closed**. P2-T32 **done** (PR [#239](https://github.com/agentkernel/cognitive-os/pull/239) `main@c7ce4e5f`). P2-T33 **done** (PR [#240](https://github.com/agentkernel/cognitive-os/pull/240) `main@09a44455`). P2-T34 **in-progress** (D03 retarget/merge). P7-T07 remains blocked on B01-W prerequisites |
...
| Active task lease | `lease/personal/P2-T34/candidate-parameter-digest` | P2-T34 private-candidate parameters digest canonicalization; branch `personal/P2-T34-candidate-parameter-digest` | retarget PR [#241](https://github.com/agentkernel/cognitive-os/pull/241) to `main` and merge after required CI |
...
| Benchmark readiness product train | `P2-T21` **done** … `P2-T33` **done** (`main@09a44455`); `P2-T34` **in-progress** at `a60ceed5` / PR [#241](https://github.com/agentkernel/cognitive-os/pull/241) | Public launcher + stub Workspace* on a long root leaves DRAFT and acquires a lease. EVAL-010 long-root 真机 confirmed the UNIX_PATH_MAX bind; C1-search then skipped `candidate_has_missing_fields_or_an_invalid_parameters_digest`. T34 recomputes JSON-fallback digests. Claim ceiling `hypothesis` | merge P2-T34; no Gate/B01 promotion |
...
| P2-T34 Private-candidate parameters digest | **in-progress** | EVAL-010 skip `candidate_has_missing_fields_or_an_invalid_parameters_digest`. Adapter JSON-fallback recomputes digest from parameters, including empty digest. D01/D02 linux-002 at `a60ceed5`: protocol **17/17**, Clippy `-D warnings`, fmt. PR [#241](https://github.com/agentkernel/cognitive-os/pull/241). Claim ceiling `hypothesis`. Stub ≠ real adapter. | retarget to `main` after T32/T33 merge; merge when required CI is green |
