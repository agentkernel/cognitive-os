...
|---|---|---|---|
| Project focus | `cognitiveos-personal`: active and sole implementation project | CognitiveOS architecture assets remain reference/contract inputs; no second product backlog | Owner delivery instruction 2026-08-20 executes [PERSONAL-C1-C2-READINESS-DELIVERY-PLAN.md](PERSONAL-C1-C2-READINESS-DELIVERY-PLAN.md) to completion. Evaluation routing remains OFF. P2-T36 is closed on `main@08819a82`. P2-T37/D02 is the current C2a public WorkspaceWrite/Patch product slice. P7-T07 remains blocked on B01-W prerequisites |
| Owner-directed campaign | C1/C2 readiness **delivery** is active under [PERSONAL-C1-C2-READINESS-DELIVERY-PLAN.md](PERSONAL-C1-C2-READINESS-DELIVERY-PLAN.md). This is not a new EVAL ID. `PERSONAL-PERF-EVAL-010` **closed** (2026-08-18). Pin `289eebad`. Long root `/home/hal9001/perfeval010-20260818` / daemon `48298` / SecretStore `/19`. UNIX_PATH_MAX product fix **confirmed**. B0 C1-search **partial** (`candidate_has_missing_fields_or_an_invalid_parameters_digest`; Task `DRAFT`; `lease_acquired` 0; real adapter spawned). Evaluation routing OFF. EVAL-004 through EVAL-010 remain **closed**. | Preregistration: [20260818-personal-perf-eval-010-preregistration.md](../checkpoints/20260818-personal-perf-eval-010-preregistration.md). Final: [personal-performance-assessment-20260818-eval-010.md](../evaluation/personal-performance-assessment-20260818-eval-010.md). Do not reopen EVAL-004/005/006/007/008/009/010. Do not reuse those roots, ports `48286`/`48288`/`48290`/`48292`/`48294`/`48296`/`48298`/`48386`/`48388`/`48390`/`48392`/`48394`/`48396`/`48398`, or SecretStore `/12`–`/19`. Claim ceiling `hypothesis`; `not_reviewed` | Push `--reuse-existing-secret-binding`, rebuild `cognitive` on exact native Linux, bind Patch `/home/wuz/p2-t37-c2a-patch-20260820` (`127.0.0.1:48446`) without recapturing key material, then admit/launch `task://personal/p2-t37-public-patch` |
| Owner-directed C1/C2 campaign | Owner-authorized C1/C2 readiness **delivery** is active. C1 public O-arm is complete (P2-T36 merged). C2a public WorkspaceWrite is complete on `P2-T37`; WorkspacePatch remains. C2b-d, P-arm, frozen assets, B0 fairness, and a new campaign assessment remain later plan packages. EVAL-004 through EVAL-010 remain **closed**. | This delivery is product readiness, not a paired benchmark. Claim ceiling `hypothesis`; no Gate, release, Profile, B01, or Agent-benefit promotion. | Complete the separate public WorkspacePatch Task, then D03; do not reuse closed EVAL assets |
| Active task lease | `lease/personal/P2-T37/c2a-public-mutation-path` | P2-T37/D02 is in progress on `personal/P2-T37-c2a-public-mutation-path` / Draft PR [#246](https://github.com/agentkernel/cognitive-os/pull/246). D01 negatives pass. Public Write lifecycle passed. Patch daemon listens on `127.0.0.1:48446` with `provider_config_missing`. `cognitive init --reuse-existing-secret-binding` is the non-copy bind path. | Bind Patch Provider via the new init flag on exact native Linux, then admit/launch the public WorkspacePatch Task. |
...
| Benchmark readiness product train | `P2-T21` … `P2-T36` **done** (`main@08819a82`); `P2-T37` **in progress** | The durable navigation is [PERSONAL-C1-C2-READINESS-DELIVERY-PLAN.md](PERSONAL-C1-C2-READINESS-DELIVERY-PLAN.md). EVAL-011 is closed and does not make readiness complete. P2-T37 public WorkspaceWrite completed the full authority chain; WorkspacePatch is still required before C2a is complete. No EVAL, Gate, release, Profile, B01, or Agent-benefit conclusion follows. | Complete the separate public WorkspacePatch lifecycle on exact native Linux, then D03. |
...
| `P2-T37/D02` | `in-progress` | Public launcher and Extension mapping expose WorkspaceWrite/Patch. Stale two-tool CI assertion repaired at `b043d430`. Exact Linux public Write Task `task://personal/p2-t37-public-write` reached `COMPLETED` with `lease_acquired: 1`, passed verification, and `ACCEPTANCE_GRANTED`. Patch runtime `/home/wuz/p2-t37-c2a-patch-20260820` / `127.0.0.1:48446` is started; doctor Provider is `provider_config_missing`. Required CI `32282577281` at `e53f72ad` passed Ubuntu, Windows, and required-ci. | Bind the Patch runtime opaque SecretRef without exposing material, then admit/launch the public WorkspacePatch Task. |
| `P2-T37/D03` | `ready` | Supported validation and task closure are not started. | Close only after separate public mutation lifecycle evidence, required CI, docs sync, acceptance mapping, and branch/main reconciliation. |
