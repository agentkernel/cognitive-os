...
| P1-T09 route implementation | `in-progress` | `experimental-local-only` / `tested-supported-ci`; Attempt 8 passed artifact activation and stable service readiness, but the exact Pi prerequisite exposed a Node.js engine mismatch. Baseline maintenance now provides Node.js `v22.23.2` and npm `10.9.8`, verified against the Pi `0.81.1` engine and SRI metadata. | continue with fresh Attempt 9; do not retry Attempt 8 |
| B01 first-install/first-conversation Gate | **running** | `B01-Desktop-Linux-002` is the sole active B01 guest. The ordered ledger has 8 completed attempts of the formal minimum 20: 1 success / 7 failures, with no critical safety failure observed. The clean baseline now includes Node.js `v22.23.2` and npm `10.9.8`; no Pi or CognitiveOS state was present. | start fresh Attempt 9 using the repaired baseline |
...
| P3-T02 Context Builder and budgets | `in-progress` | `P3-T02/D01` supplies daemon-owned required System/Task fragments before the existing sealed ContextView-before-Pi path; duplicate authorized content is rejected with explicit loss, and a required-fragment budget overflow fails closed. Exact `DEV-LINUX-NATIVE-01` revision `76ced01` passed focused Context pipeline, scheduler budget, full kernel-server and Clippy validation. No B03, UCR-01, release, or Profile claim is made. | complete source-family policy, freshness, durable trace/loss detail, required CI, and final acceptance mapping |
...
| Active task lease | `lease/personal/P3-T02/context-builder` | P3-T02 is active on its dedicated branch and will use `DEV-LINUX-NATIVE-01` only for `tested-local` validation. This does not alter B01's `running` state, ledger, or claim boundary. | implement and wire the real minimal Context Builder and budgets |
...
weakening any criterion. P1-T09 is in progress while a newly available and independently verified campaign artifact is prepared for the next counted B01 attempt.
...
| `P3-T02/D02` | `in-progress` | `experimental-local-only`; source-family policy, freshness, and durable trace/loss acceptance remain incomplete. | define and wire the remaining source-family/freshness boundary, add focused negatives, then complete supported validation and final task assessment |
