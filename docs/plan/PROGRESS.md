...
|---|---|---|---|
| Project focus | `cognitiveos-personal`: active and sole implementation project | CognitiveOS architecture assets remain reference/contract inputs; no second product backlog | After push: Linux Secret Service `get` into the P-arm broker on `DEV-LINUX-NATIVE-01`. Do not activate EVAL-012. Do not start B01 samples. P7-T07 remains blocked on B01-W prerequisites |
| Owner-directed campaign | C1/C2 readiness delivery **reopened / incomplete** (2026-08-20 owner instruction) under [PERSONAL-C1-C2-READINESS-DELIVERY-PLAN.md](PERSONAL-C1-C2-READINESS-DELIVERY-PLAN.md). This is **not** an EVAL ID. Packages 1–5 remain historical O-arm product evidence. Package 6 is in progress (local Node 14/14; Linux SecretStore `get` remaining). Packages 7–12 and 14 have local freeze/docs; leftover remote branch (13) and Linux evidence remain. Packages 15–17 are the future measurement campaign. `PERSONAL-PERF-EVAL-012` is **reserved, not active**. Evaluation routing OFF. EVAL-002 and EVAL-004 through EVAL-011 remain **closed**. | Amendment: [20260820-personal-c1-c2-readiness-programme-amendment.md](../checkpoints/20260820-personal-c1-c2-readiness-programme-amendment.md). Running report: [20260820-personal-p9-t08-c1-c2-paired-readiness-report.md](../checkpoints/20260820-personal-p9-t08-c1-c2-paired-readiness-report.md). Do not reopen EVAL-004/005/006/007/008/009/010/011. Do not reuse those roots, ports `48286`–`48298` / `48386`–`48398` / `48383`, SecretStore `/12`–`/19`, or P2-T37 roots `p2-t37-c2a-write-20260820` / `p2-t37-c2a-patch-20260820`. Claim ceiling `hypothesis`; `not_reviewed` | unique next action: push, then Linux SecretStore `get` via D-Bus into the P-arm broker. Do not start B01 samples. |
| Owner-directed C1/C2 campaign | Owner-authorized C1/C2 readiness **amended in progress**. C1 and C2a public O-arm remain merged. C2b–d remain reconfirmed product evidence. Packages 6–14 must be delivered before a new EVAL may start B0. EVAL-004 through EVAL-011 remain **closed**. | Plan: [PERSONAL-C1-C2-READINESS-DELIVERY-PLAN.md](PERSONAL-C1-C2-READINESS-DELIVERY-PLAN.md). Claim ceiling `hypothesis`; no Gate, release, Profile, B01, or Agent-benefit promotion. | Execute package 6. Do not reuse closed EVAL assets. |
| Active task lease | `lease/personal/P9-T08/c1-c2-paired-readiness` | P9-T08/D02–D04 on `personal/P9-T08-c1-c2-paired-readiness`. Writable includes `tools/personal/c1-c2-paired/`, EVAL-012 scaffolding, B01 procedure, bind runbook, and mapped handbook pages. | continue D02: Linux Secret Service get remaining; local Node 14/14 pass; EVAL-012 not active |
...
| Benchmark readiness product train | `P2-T21` … `P2-T37` **done** (`main@286f7538`); **P9-T08 in-progress** (C1/C2 paired readiness instruments) | Navigation: [PERSONAL-C1-C2-READINESS-DELIVERY-PLAN.md](PERSONAL-C1-C2-READINESS-DELIVERY-PLAN.md). Public C1 Read/Search and C2a Write/Patch completed the O-arm authority chain. Packages 6–14 remain. No EVAL, Gate, release, Profile, B01, or Agent-benefit conclusion follows. | Execute P9-T08/D02 package 6. Do not activate EVAL-012 until packages 6–14 complete. |
...
|---|---|---|---|
| `P9-T08/D01` | `done` | Amended programme, P9-T08 registration, reserved EVAL-012 isolation, and Draft PR [#247](https://github.com/agentkernel/cognitive-os/pull/247) at `6ceb8d29`. | consumed; continue D02 package 6 |
| `P9-T08/D02` | `in-progress` | Local Node focused tests `tools/test/c1_c2_paired_p_arm.test.mjs` **14/14 pass**: loopback HTTP broker injects upstream auth in memory, Secret Service get fails closed off Linux, Workspace* schema match, C1/C2a fixture calls, fairness/freeze/`retry=0`. Linux Secret Service `get` into the broker remains. | exact-revision `DEV-LINUX-NATIVE-01` `prove-linux-secret-get.mjs`; do not start B01 samples |
| `P9-T08/D03` | `ready` | Freeze/fairness assets authored in `tools/personal/c1-c2-paired/`; local dry-run pass. Not closed until Linux digest recompute. | after D02 Linux get: freeze ledger digests on `DEV-LINUX-NATIVE-01` |
| `P9-T08/D04` | `ready` | B01 procedure, bind runbook, `cells.json`, reserved EVAL-012 scaffolding authored. Leftover remote `personal/P2-T37-c2a-public-mutation-path` still advertised (`837f9a4c`). | after D03: leftover-branch delete; do not activate EVAL-012 |
