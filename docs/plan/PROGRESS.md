...
| Active task lease | `lease/personal/P2-T36/c1-public-production-path` | P2-T36/D01 owns only the registered adapter, Pi extension, scheduler, test, report, and current-plan paths on `personal/P2-T36-c1-public-production-path`; it is a product task, not an EVAL campaign. Local head `2dde1c9b` is unpushed after documented GitHub transport failures. | restore ordinary GitHub transport, push exact head `2dde1c9b`, then run the public production-path validation on `DEV-LINUX-NATIVE-01`; do not copy unpushed source or touch B01 |
