...
| Active task lease | `none` | The approved no-secret preflight on `wuz@192.168.1.2` returned `failed to get domain 'B01-Desktop-Linux-002'`; no guest, snapshot, artifact, service, Pi state, Provider, or credential was created or changed. B01 remains `running` with only attempt 1 recorded | product owner must provide the authorized KVM host where the preregistered B01 domain and `b01-platform-qualified-baseline` snapshot are currently registered; do not substitute another guest |
