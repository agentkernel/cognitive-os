...
| P3-T01 Context source/retrieval port | `in-progress` | `experimental-local-only` / `tested-supported-ci`; `P3-T01/D01` now has TaskContract v0.4 ContextRequest binding, immutable Context request/view/source persistence, metadata-only source discovery, daemon-admin admission routes for sealed authorization/revocation facts, and the real scheduler-to-private-Pi integration. Exact native Linux `cda31dc4ec74ae5faaf3d8d47ecb902e97dc8af3` passed Context-store 8/8, while the integrated ContextView/revocation matrix passed on the P2-T04 closure revisions `d3c6181d6e5bc892871aad0896006443b233ce61`, `8f2d6e53e37cacdc3572305718df6bb29be22bb3`, and `a8ef5c00654e1c05a4c30beb193b9c026654c2f1`; required Ubuntu/Windows CI passed for the integrated closure. B03 and task completion remain absent. | run the P3-T01 focused supported validation at this task branch revision, then map the unchanged acceptance and close the task if no gap remains; keep Pi candidate-only |
...
| Active task lease | `lease/personal/P3-T01/context-source-closure` | P3-T01/D01 owns the real Context source, ContextView persistence, scheduler integration, focused negatives, task-scoped documentation, and final acceptance closure on branch `personal/P3-T01-context-source-closure`. Pi remains candidate-only and scheduler/verified-continuation authority remains daemon-owned. B03, release, Profile, and Task completion remain separate/incomplete. | run supported exact-revision validation, repair any local gap, and complete P3-T01 acceptance/lease closure |
