...
| Benchmark readiness product train | `P2-T21` **done** at `5d1f5c2643d82807eb96f2c615d460dbdca749c3` / PR #220; `P2-T22` in-progress D01; `P2-T23..P2-T28` registered and not started | Owner instruction 2026-08-15 approved BR-01..BR-08 and the Linux-only product-validation route. Ubuntu is supporting CI; exact pushed-revision `DEV-LINUX-NATIVE-01` is required acceptance evidence for these product tasks; `B01-Desktop-Linux-002` remains owner-directed EVAL-004-only. Windows is `not-run by owner-directed Linux-only route`. This is product implementation evidence only, not EVAL/Gate/release/Profile/B01 evidence | continue `P2-T22/D01` freeze + failure-first tests; EVAL-004 re-freeze happens only after every BR product task is merged |
...
| Active task lease | `lease/personal/P2-T22/governed-software-repair` | P2-T21 closed (PR #220 merged at `5d1f5c2643d82807eb96f2c615d460dbdca749c3`). Active table owns `P2-T22/D01` freeze of TypeScript/Rust repair corpora and failure-first public caller tests. Ubuntu supporting CI; exact-revision `DEV-LINUX-NATIVE-01`; Windows `not-run by owner-directed Linux-only route` | continue D01 catalog freeze, hidden-test pin, and write-alone gap proof; then D02 governed journey |
...
| P2-T22 Governed software-repair journey | **in-progress** | D01 freeze of TypeScript/Rust repair corpora (`c2a.repair.typescript` descriptor_version 2 with hidden test; new `c2a.repair.rust`) plus failure-first digest/hidden-test/public-caller tests proving write-alone does not complete the journey. Lease `lease/personal/P2-T22/governed-software-repair` on `personal/P2-T22-governed-software-repair`. Running report: [20260816-personal-p2-t22-governed-software-repair-report.md](../checkpoints/20260816-personal-p2-t22-governed-software-repair-report.md). Windows `not-run by owner-directed Linux-only route`. Claim ceiling `hypothesis`; no Gate/release/Profile/B01/EVAL promotion | complete D01 freeze tests, then wire D02 read/search → write/patch → RegisteredCheckRun → verifier → acceptance |
...
| `P2-T22/D01` | `in-progress` | Freeze TypeScript/Rust repair corpora and failure-first public caller tests: hidden-test pin, deterministic broken reset, repaired/broken worker oracle, public/hidden gutting, rust check_id resolve, no secret-shaped bytes, and production write-alone gap (Task not COMPLETED, no registered-check evidence) | complete D01 catalog freeze + tests; Ubuntu supporting CI and exact-revision `DEV-LINUX-NATIVE-01` |
| `P2-T22/D02` | `ready` | Registered formal slice; implementation has not started | waits for D01 within the same task branch and lease |
