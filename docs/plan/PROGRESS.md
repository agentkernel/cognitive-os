...
| Active task lease | `lease/personal/P8-T09/dsh-akp-adapter` | owner-authorized DeepSeek Harness candidate-only AKP adapter on `personal/P8-T09-dsh-akp-adapter`; EVAL-015 remains closed and is not this task | remaining D03: real dsh process + Path A/B; then Windows CI and D04 merge |
| P8-T09 DeepSeek Harness AKP adapter | `in-progress` | linux-002 identity confirmed. Jump-host Rust akp 8/8, live 1/1, dsh_akp 2/2, fmt/Clippy at `9e239b75`. Guest shim E2E Read/Search/Write `COMPLETED`; SecretStore Flash bind+cleanup **pass**. Real dsh process and Path A **not-run**. Claim ceiling `hypothesis`. | real pinned dsh E2E + Path A/B; do not auto-claim P6/P7 |
...
| `P8-T09/D03` | `in-progress` | linux-002 identity pass. Shim Path B Read/Search/Write `COMPLETED` at harness `a33e58ad` / bins `9e239b75`. Real dsh process and Path A **not-run**. Secret cleanup **pass**. | install pinned dsh; paired Path A/B; restart/timeout on guest |
