...
| B01 first-install/first-conversation Gate | **running** | `B01-Desktop-Linux-002` is the sole active B01 guest. Attempt 5 reached clean reset and SSH readiness but failed due to a runner quoting defect before installer activation; cleanup restored the baseline. The replacement `0.0.0-campaign.20260808.1` artifact remains independently verified and is owner-authorized for Attempt 6 without the extra shell digest step. The ordered ledger has 5 of the formal minimum 20 attempts: 1 success / 4 failures, with no critical safety failure observed. | restore the baseline and run complete Attempt 6; do not retry Attempt 5 |
...
| Active task lease | `lease/personal/P1-T09/b01-attempt-6-execution` | P1-T09 is `in-progress` under the owner-confirmed complete Attempt 6. B01 remains `running` with 5 of its fixed 20 attempts recorded (1 success, 4 failures) until the fresh reset/start crosses the clean-reset checkpoint. | restore `b01-platform-qualified-baseline`, start the guest, and execute the entire preregistered Attempt 6 path without splitting or retrying it |
