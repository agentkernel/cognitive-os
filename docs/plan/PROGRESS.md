...
| P2-T08 Runtime Spine E2E Gate | `in-progress` | D01 non-claim suite harness closed at `38b45bd64ee898438addeab10f04d96bc80e34d3` with tools tests and required Ubuntu/Windows CI run `31395283074`. D02 ADR-0018 local-native Provider secret exception removal is the unique in-progress slice. B02/B04/B05/B12 remain `not-run`. | validate D02 fail-closed adapter expiry and required CI |
...
| `P2-T08/D02` | `in-progress` | Remove ADR-0018 local-native Provider secret development exception; run/evaluate/extension-load fail closed to daemon-candidate + daemon Provider proxy | finish focused adapter tests + required CI |
