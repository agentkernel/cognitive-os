...
| P2-T03 scheduler/runtime | `in-progress` | `P2-T03/D01`-`D04` are done. D05 has checkpointed an append-only, non-authority OperationCandidateProposal persistence boundary and migration v4; it creates no Intent, Effect, WorkerIterationAuthorization, budget debit, scheduler dispatch, progress, or Task outcome. | validate the exact checkpoint on Linux, then implement daemon-only candidate admission and atomic authorization issuance/consumption; do not begin P2-T04 before D05 closes |
...
| `P2-T03/D05` | `in-progress` | user approved public TaskContract worker-input authority. The pending candidate-persistence checkpoint appends immutable non-authority OperationCandidateProposal rows behind migration v4; `cc006c1` exact Linux scheduler-authority 14/14 remains the prior scheduler fence baseline. No worker execution evidence is claimed. | validate candidate persistence on exact Linux, then add daemon-only candidate admission and per-iteration authorization issuance/consumption bound to current contract/lease/loop/budget fences before failure injection |
