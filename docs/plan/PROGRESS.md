...
| Active task lease | `lease/personal/P7-T02/lifecycle-backup` | P7-T02 owns personal_backup/store lib + plan/checkpoint paths on Draft PR (pending). | implement/validate secret-excluding backup inventory; then D02 Memory/Skill export |
...
| P7-T02 Transactional lifecycle, Memory/Skill backup/restore | `in-progress` | `lease/personal/P7-T02/lifecycle-backup` on `personal/P7-T02-lifecycle-backup`. D01 adds secret-excluding Personal backup inventory planner. | exact native Linux focused tests + Clippy; required CI |
...
| `P7-T02/D01` | `in-progress` | secret-excluding Personal backup inventory planner in `cognitive-store::personal_backup` | exact native Linux focused tests + Clippy; required CI |
