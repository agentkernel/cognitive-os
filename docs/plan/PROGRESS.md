...
| Active task lease | `lease/personal/P8-T15/dsh-native-web-panel` on `personal/P8-T15-dsh-native-web-panel` @ `09163031` | Writable: admin-cli dsh + client, dsh-akp-adapter scripts, kernel-server apply/remap + scheduler recovery, endpoint_trust, plan/PROGRESS, P8-T15 checkpoint, mapped handbook. Clients Apply is PR #4 (separate repo). | linux-002 daemon **499615** `:48681`; Cos `:3080` Flash overlay. Do not auto-claim P6 / P7-T06 / P7-T07 |
| P8-T15 Native dsh Web UI | **in-progress** | D04 routing+overlay **pass** at `14aadf27`. Control-panel create HTTP 400 `PROVIDER_ENDPOINT_PATH_FORBIDDEN` on chat-completions URL pastes is the grok-account blocker; root normalize coded (xAI `/v1/chat/completions` → `/v1`). Flash Path B **200** pong. No grok-capable Cos account yet. Report: [P8-T15 report](../checkpoints/20260823-personal-p8-t15-dsh-native-web-panel-report.md). Claim ceiling `hypothesis`. | Push; Linux `p8_t13_endpoint_trust` + focused kernel-server create-account; owner adds grok-capable account then bind+Apply. Keep **499615**. Do not auto-claim P6 / P7-T06 / P7-T07 |
