...
| Active task lease | `lease/personal/P5-T01/pi-acquisition` | `P5-T01/D01` is active on `personal/P5-T01-pi-acquisition`; owned paths are the private installation store/runtime installer, authenticated admin install caller, focused tests, and exact plan/progress/closure documents recorded in `PARALLEL-LANES.md`. | implement the authenticated official-Pi acquisition-lock transaction with failure-first negatives; preserve no activation/capability/Effect/Task-completion/B09/release/Profile claim |
