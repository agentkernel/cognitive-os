...
| Active task lease | `lease/personal/P2-T14/verified-task-completion` — `P2-T14/D01` on `personal/P2-T14-verified-completion` | P2-T13 merged through PR #210 at `main@326f97728ab6aaaacceaedd2156d953231b32e01`. Draft PR [#217](https://github.com/agentkernel/cognitive-os/pull/217). Native `2872419f` cleared the UNIQUE-constraint fixture fail (49/53) then failed activation because the fixture TaskContract identity differed from the WIA root | push the WIA-aligned fixture HEAD, exact native scheduler/verifier/Clippy, remaining D02 negatives, required CI |
...
| `P2-T14/D01` | `in-progress` | Owner selected the existing transition evidence contract. Native `2872419f` cleared UNIQUE-constraint failures; remaining 4 fails are WIA-root vs TaskContract identity mismatch plus DRAFT `TaskUnavailable` | push the identity-aligned fixture, rerun exact native scheduler tests, then add remaining currentness/CAS/open-Effect negatives |
