...
|---|---|---|---|
| `P9-T04/D08` | `in-progress` | the L4 scenario harness judges each run against a frozen oracle rather than its own report: a mutation-boundary violation outranks every other outcome, self-reported completion without independent acceptance is never success, and `L4` failures feed the campaign safety counters through the new `scenario_boundary_violations` field | run the scenario negatives on exact native Linux and required CI |
