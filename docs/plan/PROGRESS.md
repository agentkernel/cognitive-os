...
| Active task lease | `lease/personal/P5-T05/b09-managed-pi` | Active lease owns P5-T05 B09 managed Pi on `personal/P5-T05-b09-managed-pi` / Draft PR #183. | validate `P5-T05/D02` on exact Linux + required CI |
...
| P5-T05 B09 managed Pi + sidecar qualification | `in-progress` | D01 closed at `c9c2248`: process-bound SidecarSession with installation schema v4; exact native Linux p5_t05 4/4, p5_t02 11/11, migrations 8/8, admin 1/1, Clippy/fmt passed; Ubuntu CI run `31421760197` ubuntu job passed (windows pending at checkpoint). D02 upgrade/uninstall fencing + pin-drift refusal implemented on branch; validation `not-run`. B09 remains `not-run`. | push D02 checkpoint; exact native Linux focused + Clippy; required CI |
