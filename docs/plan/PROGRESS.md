...
| Active task lease | `lease/personal/P2-T12/worker-loop-production-chain` — `P2-T12/D04` on `personal/P2-T12-worker-loop` | Owner-directed quiet-session takeover retained the existing lease and Draft PR #209. D01-D03 are complete. D03 exact `4f0d625b5e4476cfc1956088134b9a4e1afd3e08` passed native server 9/9, scheduler authority 43/43, kernel-server Clippy, and required Ubuntu/Windows CI run `31677480018`. D04 seam inspection confirms the scheduler currently consumes WIA and classifies Effect state but never selects or invokes an assembled executor | add failure-first production-caller negatives for persisted descriptor selection, durable `EXECUTING` before I/O, stale/unsupported fail-closed behavior, and original-key restart reconciliation; then wire the caller through the existing Effect/executor seams |
...
| P2-T12 Admitted Task reaches the worker loop | **in-progress** | D01-D03 are complete. Admission/startup repair publish the full scheduler bootstrap; zero-Intent rows reach candidate admission with per-row isolation; the post-bind 250 ms worker is single-owner, non-reentrant, pass-error tolerant, and cancellable. D03 exact `4f0d625b5e4476cfc1956088134b9a4e1afd3e08` passed native server 9/9, scheduler authority 43/43, Clippy, and required CI `31677480018`. The remaining task gap is D04: a consumed WIA still only classifies the durable Effect and has no production executor caller | implement D04 through persisted descriptor selection and the existing durable Effect/native-executor seams, including crash/duplicate/stale/budget/unsupported negatives |
...
| `P2-T12/D04` | `in-progress` | Seam inspection confirms WIA consumption, scheduler lease fencing, Effect persist-before-dispatch, and all six native sinks exist, but the scheduler caller stops at durable Effect classification | add focused production-caller negatives, then reload the selected candidate/descriptor and dispatch through the existing Effect protocol without a parallel authority path |
