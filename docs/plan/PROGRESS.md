...
| Active task lease | `lease/personal/P2-T17/a7-unknown-outcome-observation` | P2-T17/D01-D04 on isolated branch `personal/P2-T17-a7-unknown-outcome-observation`, Draft PR #212. P2-T13 remains closed on its task branch and PR #210 is an unmerged implementation dependency; P2-T14 remains a separate completion-authority dependency | retain the failure-first CI result, implement D01-D03 without touching another worktree/campaign root, and keep Draft until every merge dependency is on `main` |
...
| P2-T17 A7 external-mutation unknown-outcome observation | **in-progress** | Owner-directed isolated delivery claimed on `personal/P2-T17-a7-unknown-outcome-observation` with `lease/personal/P2-T17/a7-unknown-outcome-observation`. Base is unmerged P2-T13 PR #210 head `b514d278ef4a3daafe9cceeb62ced2dc649d186b`; P2-T16 is already allocated by concurrent sibling worktree `personal/P2-T16-local-token-csprng`. D01 starts with a failure-first restart test for mutation success followed by receipt-persistence loss, requiring one original-key query/reconcile and mutation count exactly one. Prepared EVAL-003 assets remain read-only and no B01 guest/campaign root is touched. | keep the PR Draft; implement D01-D03, append each validation unit to the single running report, and do not merge until P2-T13/P2-T14 plus any required mutation carrier are on `main` |
...
| `P2-T17/D01` | `in-progress` | Failure-first source is being added on the isolated branch; Rust execution is `not-run` locally because `DEV-WIN-GNU-01` is an unsupported linker host. The expected initial failure is the absent campaign observation module/API, not a relaxed assertion | commit the focused failure-first test, push the immutable revision, and retain the expected Ubuntu/Windows CI failure before implementing the fixture and product path |
| `P2-T17/D02` | `ready` | not started | after D01: persist the carrier/Effect before dispatch, add authorized default-off fault points and public bounded observation, then restart-query only the original key |
| `P2-T17/D03` | `ready` | not started | after D02: bind P2-T13 verification and close the exactly-once, ambiguity, fencing, evidence, duplicate-worker and cleanup negative matrix without Task completion |
| `P2-T17/D04` | `ready` | not started | after D03: complete docs/handbook, exact-revision supported validation and self-review; remain Draft until P2-T13/P2-T14 and any carrier dependency merge |
