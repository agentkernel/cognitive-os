...
| Active task lease | `lease/personal/P9-T06/readiness-secretstore-coalesce` — `P9-T06/D01` on `personal/P9-T06-readiness-secretstore-coalesce` | Isolated from sibling feature streams. One status/doctor evaluation binds one SecretStore; no secret-material cache and no stale-ready TTL | required Ubuntu/Windows CI after the coalesce negatives; no Gate/release/Profile claim |
| P9-T06 Readiness SecretStore coalesce | **in-progress** | Owner-directed corrective after the CI-startup flake was already leased (PR #213). `evaluate_personal_readiness` now binds one SecretStore per evaluation: secret probe and provider resolve share it, skip `get` when probe already proved the backend cannot answer, drop material immediately, and re-evaluate every request. Counting-store negatives cover once-per-evaluation, no TTL, skip-get, and no material leak | push Draft PR and watch required CI |
