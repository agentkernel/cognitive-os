...
| Active task lease | `lease/personal/P2-T34/candidate-parameter-digest` | P2-T34 private-candidate parameters digest canonicalization; branch `personal/P2-T34-candidate-parameter-digest` | failure-first adapter digest tests then linux-002 |
...
| Benchmark readiness product train | `P2-T21` **done** … `P2-T31` **done**; `P2-T32` **in-progress** (lease closed) at `fb85cfff` / PR [#239](https://github.com/agentkernel/cognitive-os/pull/239); `P2-T33` **in-progress** (lease closed) at `289eebad` / PR [#240](https://github.com/agentkernel/cognitive-os/pull/240) | Public launcher + stub Workspace* on a long root leaves DRAFT and acquires a lease. EVAL-010 long-root 真机 confirmed the UNIX_PATH_MAX bind; C1-search then skipped `candidate_has_missing_fields_or_an_invalid_parameters_digest`. Claim ceiling `hypothesis` | product follow-up for the digest/fields skip; P2-T32/T33 Windows required-ci is merge-only; no Gate/B01 promotion |
...
| P2-T34 Private-candidate parameters digest | **in-progress** | EVAL-010 skip `candidate_has_missing_fields_or_an_invalid_parameters_digest`. Adapter JSON-fallback recomputes digest from parameters. Claim ceiling `hypothesis`. Stub ≠ real adapter. | linux-002 `pi-agent-adapter` tests then Ubuntu CI |
| P2-T33 Private-candidate host path | **in-progress** (lease closed; pending Windows merge) | linux-002 at `289eebad`: focused `p2_t33` 2/2, host-path units, Clippy `-D warnings`. Ubuntu `32063236152` `verify` **pass**. Draft PR [#240](https://github.com/agentkernel/cognitive-os/pull/240). Lease closed 2026-08-18 before EVAL-010. Claim ceiling `hypothesis`. Stub ≠ real adapter. | rerun/wait Windows required-ci; do not treat stub pass as C1/C2 Agent-benefit |
| P2-T32 Public `cognitive daemon start` scheduler lease | **in-progress** (lease closed; pending Windows merge) | Ubuntu `32047487272` `verify (ubuntu-latest)` **pass** at `fb85cfff`; linux-002 focused `p2_t32` 1/1 in 3.33s + `daemon_log` 2/2. Windows `verify` fail is unrelated endpoint-publish flake (330/1). Draft PR [#239](https://github.com/agentkernel/cognitive-os/pull/239). Lease closed 2026-08-18 before EVAL-008. EVAL-008/009 are **closed**. Claim ceiling `hypothesis`. Stub ≠ real adapter. | rerun Windows required-ci before merge; do not treat stub pass as C1/C2 Agent-benefit |
...
| `P2-T33/D03` | `in-progress` | exact-revision linux-002 matrix recorded; Ubuntu `32063236152` pass at `289eebad`; Windows `verify` pending. Lease closed 2026-08-18 so EVAL-010 can own `PROGRESS.md`. | Windows required-ci; stub ≠ C1/C2; EVAL-010 is a separate freeze |
...
| `P2-T32/D02` | `in-progress` | `daemon.log` + Unix `process_group(0)` landed; stub path proven. Lease closed 2026-08-18 before EVAL-008. Windows required-ci flake blocks merge only. | rerun Windows `32047487272`; campaign closure does not resume development; do not treat as C1/C2 pass |
| `P2-T32/D03` | `ready` | handbook already in PR #239; EVAL-008 closed without waiting on this merge | P2-T32 merge is separate; campaign closure does not resume development |
