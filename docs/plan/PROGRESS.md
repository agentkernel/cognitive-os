...
| Active task lease | `lease/personal/P2-T18/local-token-csprng` on `personal/P2-T18-local-token-csprng` | P2-T18 is an owner-directed, independent local-auth security corrective with exact paths only; it does not overlap or depend on P2-T14 and does not touch B01/campaign state. Draft PR [#215](https://github.com/agentkernel/cognitive-os/pull/215) is open. Implementation candidate `72ca18c6` passed exact native Linux auth/full kernel-server suites; required CI failed solely on `clippy::expect_used` in the source guard. Local HEAD adds the Clippy-safe helper plus an explicit full-length zero-block negative | push the Clippy/zero-block candidate, re-run exact native Linux focused tests and Clippy, then confirm required GitHub Ubuntu/Windows CI |
| P2-T18 Local bootstrap/session token OS CSPRNG corrective | **in-progress** | Production code uses a private `getrandom::fill` OS-entropy adapter, a 64-byte sample with a 32-byte token payload plus independent repeat probe, lower-hex `prefix-group-group` opaque shape, and fail-closed error paths before bootstrap file/session creation. Coverage includes entropy failure/short/repeated/zero-block (full and half-zero), no-session, bounded uniqueness, Unix 0600, static fallback, wrong-channel/expiry/revoke and Debug/log redaction. Exact native Linux at `72ca18c6`: source guard 1/1, auth units 12/12, kernel-server 220/220; Clippy failed on the integration `expect`. Local unpushed work replaces that helper and adds the missing zero-block seam | push, then exact native Linux + required CI on the new HEAD; no P2-T14/campaign guest mutation |
...
| `P2-T18/D01` | `in-progress` | Failure-first source negative failed 1/1 as expected at `bc415f09`. Implementation at `72ca18c6` passed native source guard, auth units (failure/short/repeated, no-file, 0600) and full kernel-server 220/220; required CI Clippy failed on the integration `expect`. Local HEAD adds Clippy-safe `split_once` helper and an explicit full-length zero-block (all-zero / token-half / probe-half) no-file negative | push the candidate and re-run exact native Linux focused tests plus Clippy |
| `P2-T18/D02` | `ready` | Session minting through the same OS RNG passed native auth units at `72ca18c6` (no-session-on-entropy-failure, bounded uniqueness, channel/expiry/revoke, redaction). Required CI still red on the Clippy helper; zero-block session path is covered by the shared generator | after D01 Clippy/zero-block candidate is pushed: confirm native Clippy, required Ubuntu/Windows CI, self-review and closure |
