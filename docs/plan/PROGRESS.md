...
| Active task lease | `lease/personal/P2-T03/daemon-admission-order-regression` | P2-T03 implementation-only regression slice is active; it covers daemon admission ordering only and does not change the existing `experimental-local-only` / `tested-local` evidence level | prove terminal ceiling STOP skips lease acquisition while a clear decision acquires exactly one lease; worker dispatch, Effect closure and P2 Gates remain not-run |
