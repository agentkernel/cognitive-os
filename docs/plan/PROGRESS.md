...
| Active task lease | `lease/personal/P2-T02/bootstrap-governance-context` | P2-T02/D01 remains `in-progress`. PR #139 validates authenticated intent wire contracts, and this narrow governance decision permits the daemon to durably bootstrap its own principal-bound local governance root; clients still cannot submit authority facts or writer leases. | validate and merge the governance decision, then claim a Lane-RUN lease for daemon authority-context composition and the full Task API/watch vertical path |
