...
|---|---|---|---|
| Project focus | `cognitiveos-personal`: active and sole implementation project | CognitiveOS architecture assets remain reference/contract inputs; no second product backlog | close `P7-T02` after required CI on Draft PR #185 |
| Active task lease | `lease/personal/P7-T02/lifecycle-backup` | P7-T02 owns personal_backup/store lib + plan/checkpoint paths on Draft PR #185. | required CI → ready/merge → close lease/branch → claim P7-T03 |
...
| P7-T02 Transactional lifecycle, Memory/Skill backup/restore | `in-progress` | `lease/personal/P7-T02/lifecycle-backup` on `personal/P7-T02-lifecycle-backup` / Draft PR #185. D01–D04 authority path complete: inventory, digest-bound Memory/Skill/bindings export, restore preflight, transactional update/rollback/uninstall. Exact native Linux `personal_backup` 15/15 + Clippy at `68abc82`. | required Ubuntu/Windows CI → ready/merge/lease/branch/main closure |
...
slice `P7-T02/D04`. Formal task completion remains independent from every
...
| `P7-T02/D04` | `in-progress` | transactional update/rollback/uninstall + acceptance mapping; Linux `personal_backup` 15/15 + Clippy at `68abc82` | required CI, closure checkpoint, ready/merge/lease/branch |
