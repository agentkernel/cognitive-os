...
|---|---|---|---|
| Project focus | `cognitiveos-personal`: active and sole implementation project | CognitiveOS architecture assets remain reference/contract inputs; no second product backlog | development is active under the owner's 2026-08-12 delivery instruction to remove the structural blockers `PERSONAL-PERF-EVAL-002` found: the `C1` read-only and `C2` mutation capability classes are unreachable because four registered Tool families have no executor, an admitted Task never reaches the worker loop, and the verifier has no production caller. P2-T10 (executor parity) is **done** — all six Tool families now have a real sink. The remaining blockers are registered as P2-T12 (admitted Task reaches the worker loop), P2-T13 (independent verification actually runs) and P2-T14 (verified Task completion, which is `blocked` on an owner/Lane-CTR decision about the missing `acceptance_decision` evidence contract); until P2-T12 and P2-T13 land, `execution_ready` means only that a sink exists in the binary and no `C1`/`C2` capability class is executable end to end. P8-T08 is closed and P7-T07 is blocked on B01-W execution prerequisites |
...
| Active task lease | `lease/personal/P2-T15/executor-hardening-review-repair` | Owner's explicit parallel-delivery instruction resumed this corrective task after the evaluation recovery closed. One task branch/Draft PR owns the exact runtime, readiness, handbook, report and closure paths listed in [Parallel Lanes](PARALLEL-LANES.md); no campaign guest or evaluation report path is writable | at pushed candidate `5da160b553e380d4ef8e601b79031046988c26c3`, run exact-revision `DEV-LINUX-NATIVE-01` focused/full validation for P2-T15/D03, then close acceptance/lease/PR normally |
...
| P2-T12 Admitted Task reaches the worker loop | **not-started** | Registered 2026-08-12; **acceptance corrected the same day after a full code map, because the first registration was incomplete.** Three verified facts widened it. (a) Admission must create *three* things, not one: `SchedulerRepository::upsert`, `ProtocolStore::create_budget` and production `LifecycleDomain::Loop` creation all have **zero** production callers, and `load_scheduler_authority_snapshot` fails with `LoopUnavailable`/`BudgetUnavailable` without the latter two — so a scheduler row alone would still not let a tick run. (b) The tick has a circular precondition: `resolve_scheduler_work_for_task` (`dispatch.rs:527`) demands exactly one existing Intent, which is the row the pre-admission branch at `dispatch.rs:554` exists to create, so a fresh Task's first execution is unreachable and the `?` aborts the whole pass. (c) A tick **does** run in production at `server.rs:128` — but 32 lines before `TcpListener::bind` at `server.rs:160`, so it provably precedes any admission in that process, and its error propagates as `PersonalDaemonError::Io`, meaning one bad row prevents daemon startup. Measured trigger: `UJ4` recorded 30/30 admitted, 0 executed, 0 verified | claim a non-overlapping lease and start `P2-T12/D01` (scheduler row + Loop object + Budget row in the admission transaction) |
| P2-T13 Independent verification runs in production | **not-started** | Registered 2026-08-12; **acceptance corrected and narrowed the same day.** The gap is wider than "the verifier has no caller": nothing in production writes a `FixedPostStateRow`, `VerificationRequestRow` or `CheckpointRow`, nothing calls `issue_continuation_authorization`, there is no production `IndependentVerifier` implementation (only two test doubles) and no `verifier_ref` registry, `criteria_canonical_json` has no producer even though `ContractCondition::verifier_ref` is already persisted at admission, and **the daemon never constructs an `ArtifactStore`** — `ArtifactStore::open` has only test and benchmark callers — so the verifier could not resolve evidence even if it were called. One encouraging finding: the *consumption* half of continuation authority is already production-wired (`dispatch.rs:436` → `begin_verified_continuation_atomically`), so the loop is broken in the middle, not at the end. Scope narrowed to closing `ACT -> VERIFY -> CONTINUE -> OBSERVE`; Task completion moved to P2-T14 | after P2-T12: claim a lease and start `P2-T13/D01` (compose the `ArtifactStore`, pin the post-state, commit `ACT -> VERIFY`) |
...
| P2-T15 Independent-review executor hardening | **in-progress** | Owner-directed corrective delivery after merged P2-T10/P2-T11. D01/D02 close all confirmed review defects with failure-first Linux/Windows negatives, durable original-key state and receipt isolation, exact catalog equality, handle-relative no-follow workspace I/O, final-window CAS, bounded enumeration/preimages, orphan recovery, final-newline patch semantics and one-snapshot readiness. Draft PR [#208](https://github.com/agentkernel/cognitive-os/pull/208) is clean and required CI run `31664427142` passed at exact pushed `5da160b553e380d4ef8e601b79031046988c26c3`; final defect-first review reports no findings. Campaign recovery is closed and no evaluation evidence is inherited | P2-T15/D03: exact-revision native Linux focused/full tests + Clippy/fmt, then acceptance/closure docs, ready/merge and lease/branch/main reconciliation; no Gate/release/Profile/B01/benchmark claim |
...
| `P2-T15/D03` | `in-progress` | Required Ubuntu/Windows matrix is green; exact native Linux is not yet run on the post-campaign candidate. Running report: `docs/checkpoints/20260813-personal-executor-hardening-validation.md`; Draft PR [#208](https://github.com/agentkernel/cognitive-os/pull/208) | sync a pushed immutable revision including this task/lease registration to `DEV-LINUX-NATIVE-01`; run focused `tool_executor` and readiness tests, full `kernel-server`, targeted Clippy and fmt, then complete acceptance/closure and merge |
| `P2-T12/D01` | `ready` | not started | create all three scheduling prerequisites in the admission transaction: scheduler work row, contract-named Loop object in `START`, contract-named Budget row |
| `P2-T12/D02` | `ready` | not started | after D01: route a Task with zero Intents into the pre-admission candidate branch instead of aborting the pass, and isolate per-row tick failures |
| `P2-T12/D03` | `ready` | not started | after D02: replace the pre-bind startup tick with a bounded, non-reentrant, cancellable periodic tick that cannot prevent daemon startup |
| `P2-T12/D04` | `ready` | not started | after D03: dispatch the durable Tool Effect from the worker attempt through the assembled executor chosen by the persisted descriptor |
| `P2-T12/D05` | `ready` | not started | after D04: acceptance mapping, handbook gap-record update, and closure |
| `P2-T13/D01` | `ready` | not started | compose an `ArtifactStore` into the daemon, pin the `FixedPostStateRow` and `VerificationRequestRow`, and commit Loop `ACT -> VERIFY` |
