...
## Current snapshot (2026-08-24 P8-T15 provider endpoint root normalize)
...
| Active task lease | `lease/personal/P8-T15/dsh-native-web-panel` on `personal/P8-T15-dsh-native-web-panel` @ `8b9d09cb` | Writable: admin-cli dsh + client, dsh-akp-adapter scripts, kernel-server apply/remap + scheduler recovery, endpoint_trust, plan/PROGRESS, P8-T15 checkpoint, mapped handbook. Clients Apply is PR #4 (separate repo). | linux-002 daemon **515695** `:48681` (binary `p8t15-8b9d09cb`); Cos `:3080` Flash overlay. Do not auto-claim P6 / P7-T06 / P7-T07 |
| P8-T15 Native dsh Web UI | **in-progress** | D04 routing+overlay **pass** at `14aadf27`. Create-account HTTP 400 `PROVIDER_ENDPOINT_PATH_FORBIDDEN` **fixed** at `8b9d09cb`: chat-completions paste normalizes to API root. Linux endpoint_trust **8/8** + public **2/2** + kernel-server control-plane **6/6** + Clippy `-D warnings`. Guest daemon **515695** health **200**. Flash Path B overlay unchanged. Report: [P8-T15 report](../checkpoints/20260823-personal-p8-t15-dsh-native-web-panel-report.md). Claim ceiling `hypothesis`. | Owner adds grok-capable account (e.g. `https://api.x.ai/v1` or `…/v1/chat/completions`) then bind+Apply. Keep **515695**. Do not auto-claim P6 / P7-T06 / P7-T07 |
