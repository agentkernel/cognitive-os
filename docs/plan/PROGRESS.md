...
| Active task lease | `lease/personal/P9-T03/store-composition` | P9-T03 owns server + scheduler_authority long-lived store paths on Draft PR #193. | D01: daemon-startup recovery+tick share one SqliteAuthorityStore |
| P9-T03 Store access and composition-root optimization | `in-progress` | Draft PR #193. D01 shared-store recovery/tick APIs + daemon startup wiring at `54be4c1`. | exact native Linux focused shared-store test + Clippy |
