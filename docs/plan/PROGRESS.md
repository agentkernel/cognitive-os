...
| P2-T04 private worker composition | `in-progress` | P2-T04/D01 lease is active on the Context branch. Scheduler execution parsing accepts v0.4 Context-bound TaskContracts, and the daemon-private resolver fences the current strong ContextRequest binding, request principal, durable revocation epoch, metadata/body equality, and per-object authorization before deterministic rendering. At `7def236`, a narrow private proposer port accepts only candidate fields; the daemon revalidates descriptor/tool/action facts, creates all governed references and headers, seals/persists the candidate, and invokes the existing atomic admission path. Malformed parameter digests and non-positive state versions are rejected before persistence. Supported Ubuntu/Windows CI passed for the bridge checkpoint. A real pinned Pi stdio adapter and its pre-admission scheduler caller are still absent. WIA remains post-admission and unconsumed; no executor, progress, evidence, verification, acceptance, or Task completion is added. | implement the daemon-supervised pinned Pi transport and call the bridge from the pre-admission scheduler path; preserve candidate-only output |
...
| `P2-T04/D01` | `in-progress` | D05 native Linux closure is complete and the P2-T04 lease is on this branch. `1f932c3` accepts v0.4 contracts; `5e63581` implements metadata-first Context authorization/body resolution; `7def236` adds the private candidate-only proposer contract plus daemon sealing, persistence, descriptor validation, and atomic admission composition. Supported Ubuntu/Windows CI passed. No real Pi process transport or scheduler caller has invoked the bridge yet, and WIA remains post-admission. | add a pinned private Pi transport and invoke it only on the scheduler's pre-admission path; do not consume WIA merely to call Pi |
