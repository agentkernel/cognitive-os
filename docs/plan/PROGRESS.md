...
> 最后更新：2026-07-21（Lane-RUN M5 批 1：无模型确定性 admin CLI 四动词 inspect/stop/revoke/reconcile 实现已提供 + 车道 Rust 行为/e2e 测试已执行；management 向量保持 not-run 归 Lane-CFR。另：ADR-0008 自动提交/推送治理政策生效）
...
| M5 意图链/Harness/Shell/管理面 | **in-progress（RUN 批 1 已交付）** | — | 入口 gate 达成（M4 出口 + F-011 R1 登记）。**RUN 批 1（2026-07-21，PR 见车道表）：确定性管理 fallback——PrivilegedManagementSession 门（过期/撤销/scope/step-up 全 fail-closed）+ 无模型四动词 inspect/stop/revoke/reconcile（经中央 transition 门 / M3 revocation 算术 / M4 recovery 路径），验收判据 5 的车道侧证据；判据 1–4/6–8 与 Management API/AKP/Shell 待后续批**；建议提示词 docs/prompts/milestone-m5.md / lane-run.md |
...
1. [20260720-lane-run-m5-batch1-handoff.md](../checkpoints/20260720-lane-run-m5-batch1-handoff.md)（Lane-RUN M5 批 1：session 门 + 无模型四动词 inspect/stop/revoke/reconcile、16 项行为/e2e 测试、matrix +5 REQ 回填）
2. [20260720-lane-con-clients-root-integration-handoff.md](../checkpoints/20260720-lane-con-clients-root-integration-handoff.md)（clients 迁移 squash 集成远端 M5 gate 基线、D-019 闭合、独立博客历史排除）
