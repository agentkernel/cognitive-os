...
| Active task lease | `lease/personal/P4-T05/resource-api` | P4-T05/D03 owns the daemon-private lifecycle command seam for Memory remember/forget and Skill import/bind/revoke on `personal/P4-T05-resource-api` | implement malformed-payload, unauthorized-channel, duplicate, non-resurrection, and no-partial-write command negatives; public contract generation and B08 remain separate |
...
| P4-T05 Memory/Skill API and unified projection | `in-progress` | `P4-T05/D01-D02` provide task-bound projection plus authority-backed Memory-object and Skill-binding explain reads; exact Ubuntu/Windows CI run `31332620195` passed after the corrected domain identifier import | implement the lifecycle command seam and its failure-first malformed/duplicate/non-resurrection/no-partial-write negatives; public contract generation and B08 remain separate |
...
| `P4-T05/D03` | `in-progress` | — | implement the daemon-private lifecycle command seam and failure-first no-partial-write negatives |
