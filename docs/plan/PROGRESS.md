...
| P2-T03 scheduler/runtime | `in-progress` | `P2-T03/D01`-`D04` are done. D05 checkpoint `be2948f` has required Ubuntu/Windows CI green for daemon-only sealed WIA handoff/recovery fences, including cancelled lease rejection; exact Linux evidence remains limited to the earlier candidate-persistence/conformance revisions. No worker execution evidence is claimed. | complete the remaining D05 failure-injection and supported exact-revision worker integration validation; do not begin P2-T04 before D05 closes |
...
| `P2-T03/D05` | `in-progress` | daemon-only candidate admission, sealed WIA issuance, exact scheduler lease-bound one-time consumption, and startup recovery are checkpointed. `be2948f` passed required Ubuntu/Windows CI; the focused new SQLite negative rejects cancelled scheduler work without consumption/binding. Prior exact Linux candidate-persistence/conformance evidence remains scoped to `d872635`/`50d4fc6`, not this latest recovery checkpoint. No worker execution, progress, evidence, Task acceptance, or Task completion is claimed. | add the remaining failure-injection and exact-revision worker integration validation; external executor dispatch remains fail-closed/reconciliation-only |
