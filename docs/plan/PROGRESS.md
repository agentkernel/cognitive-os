...
| P1-T09 route implementation | `in-progress` | `experimental-local-only` / `tested-supported-ci`; the owner authorized fresh, counted B01 attempt 3 after the read-only diagnosis confirmed the registered baseline and active `192.168.123.0/24` network. | restore the registered baseline and start the guest as attempt 3; then record the bounded readiness result and, if ready, request the desktop operator's hidden-input Provider opt-in |
...
| Active task lease | `lease/personal/P1-T09/b01-attempt-3-execution` | P1-T09 is `in-progress` under the owner-authorized fresh attempt 3. B01 remains `running` with 2 of its fixed 20 attempts recorded (1 success, 1 readiness failure) until the reset/start crosses the clean-reset checkpoint. | restore `b01-platform-qualified-baseline`, start the guest, and record the bounded readiness result as attempt 3; do not split or retry it |
...
weakening any criterion. P1-T09 is in progress for the owner-authorized B01 attempt 3.
