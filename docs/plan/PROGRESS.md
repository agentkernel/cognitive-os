...
| P2-T03 scheduler/runtime | `in-progress` | `P2-T03/D01`-`D04` are done. D05 checkpoint `a1d1cc7` passed required Ubuntu/Windows CI for daemon-only sealed WIA handoff/recovery fences, including cancelled/task-mismatched rejection and legacy unbound recovery retention; exact Linux worker-recovery validation remains `not-run` after a clean clone was blocked by remote network throughput. No worker execution evidence is claimed. | complete the remaining D05 failure-injection and supported exact-revision worker integration validation; do not begin P2-T04 before D05 closes |
...
| `P2-T03/D05` | `in-progress` | daemon-only candidate admission, sealed WIA issuance, exact scheduler lease-bound one-time consumption, and startup recovery are checkpointed. `a1d1cc7` passed required Ubuntu/Windows CI; focused negatives reject cancelled/task-mismatched scheduler work without consumption/binding, and closed legacy unbound recovery retains its lease. Exact Linux candidate-persistence/conformance evidence remains scoped to `d872635`/`50d4fc6`; an exact `d329293` Linux test was `not-run` after the remote clean clone hit GitHub's low-throughput timeout. No worker execution, progress, evidence, Task acceptance, or Task completion is claimed. | add the remaining failure-injection and exact-revision worker integration validation; external executor dispatch remains fail-closed/reconciliation-only |
