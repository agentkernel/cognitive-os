...
| `P3-T01/D01` | `blocked` | TaskContract v0.4 ContextRequest binding and append-only ContextRequest/ContextView persistence/query groundwork are protected by append-time sealed-content, row-metadata, and strong request-reference validation. The daemon now issues a private request during Task admission; exact native Linux `ad95f913423bbee661b28915ef8bb0381523094b` passed the kernel-server suite, Context integrity 4/4, intent-chain 6/6, and fmt. The required source/retrieval exit is blocked: no governed workspace ingestion or reconstructable authorization/revocation source exists. See `20260805-personal-p3-t01-real-context-source-blocker.md`. | add a daemon-only workspace admission source and durable authorization snapshot source before any Context resolution caller; preserve the P2-T04 runtime lease boundary |
