...
| Active task lease | `lease/personal/P1-T09/b01-clean-vm-execution` | B01 VM allocation and platform qualification are active; no attempt can start while the persistent native Secret Service collection is absent | owner completes persistent collection initialization; runner records the Product-compatible non-secret probe |
