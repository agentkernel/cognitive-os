...
| Active task lease | `lease/personal/P2-T18/local-token-csprng` on `personal/P2-T18-local-token-csprng` | P2-T18 is an owner-directed, independent local-auth security corrective with exact paths only; it does not overlap or depend on P2-T14 and does not touch B01/campaign state. Failure-first commit `bc415f094d655c0dc30bd205c3e60dced5ec0fc8` was pushed and its 1/1 source negative failed as expected on an exact-revision disposable native-Linux clone | finish D01/D02 implementation, push one immutable candidate, run exact native Linux plus required GitHub Ubuntu/Windows CI, self-review, then close PR/lease/branch/main |
| P2-T18 Local bootstrap/session token OS CSPRNG corrective | **in-progress** | Production code now uses a private `getrandom::fill` OS-entropy adapter, a 64-byte sample with a 32-byte token payload plus independent repeat probe, lower-hex `prefix-group-group` opaque shape, and fail-closed error paths before bootstrap file/session creation. Failure/zero/short/repeated entropy, no-session, bounded uniqueness, 0600, static fallback, wrong-channel/expiry/revoke and Debug/log serialization redaction coverage is present. Local fmt, static source guard and handbook/fingerprint checks pass; Rust behavior remains untested until a pushed exact revision | complete Current snapshot consistency, commit/push the implementation candidate, then execute D01/D02 supported validation without touching P2-T14 or any campaign guest |
...
| `P2-T18/D01` | `in-progress` | Failure-first source negative is committed/pushed and failed 1/1 as expected at exact native-Linux `bc415f094d655c0dc30bd205c3e60dced5ec0fc8`; working tree replaces bootstrap derivation with the private OS-CSPRNG seam and no-file invalid-entropy negatives. Local fmt/static/handbook pass; supported Rust behavior not yet run on the uncommitted tree | commit/push the immutable implementation candidate, then run the focused integration/unit matrix and Unix 0600 assertion on its exact native-Linux clone |
| `P2-T18/D02` | `ready` | Session minting implementation and negatives are written but not yet supported-validated; no completion claim | after D01 validation: run existing channel/expiry/revoke regressions, bounded uniqueness and redaction tests, Clippy/full package, required Ubuntu/Windows CI, self-review and deterministic closure |
