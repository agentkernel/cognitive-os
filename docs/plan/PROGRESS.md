...
|---|---|---|---|
| Project focus | `cognitiveos-personal`: active and sole implementation project | CognitiveOS architecture assets remain reference/contract inputs; no second product backlog | `PERSONAL-PERF-EVAL-012` **closed**. `P2-T38` and `P9-T09` **done**. Next evidence-ranked item is live O-arm injection of `frozen-system-task-prompt.txt`. P7-T07 remains blocked on B01-W prerequisites |
| Owner-directed campaign | `PERSONAL-PERF-EVAL-012` **closed** (2026-08-20) on `evaluation/EVAL-012-freeze` @ `177669b7`. Evaluation routing OFF. B0 fairness **fail** (`system_task_prompt_bytes`) blocked B1/B2. C2a O Patch fail retained. Cleanup **pass**. EVAL-002 and EVAL-004 through EVAL-011 remain **closed**. | Report and freeze live on `evaluation/EVAL-012-freeze` (`docs/evaluation/personal-performance-assessment-20260820-eval-012.md`, `docs/checkpoints/20260820-personal-perf-eval-012-preregistration.md`). Product pin `370b26fcc05976c7c1c97e5510a99ed3ebc23f2c`. Claim ceiling `hypothesis`; `not_reviewed` | none for this campaign — product follow-up is P2-T38 |
| Owner-directed C1/C2 campaign | `PERSONAL-PERF-EVAL-012` closed. B0 did not pass fairness. C1 O/P and C2a Write ran; C2a Patch O fail retained. | Plan: [PERSONAL-C1-C2-READINESS-DELIVERY-PLAN.md](PERSONAL-C1-C2-READINESS-DELIVERY-PLAN.md). Claim ceiling `hypothesis`; no Gate, release, Profile, B01, or Agent-benefit promotion. | P2-T38 delivers EVAL-012 Priority 1; do not reopen EVAL-012 |
| Active task lease | `none` | `P9-T09` merged; evaluation routing OFF | claim live O-arm prompt injection before a new EVAL |
| Benchmark readiness product train | `P2-T21` … `P2-T38` **done**; **P9-T08 done**; **P9-T09 done**; `PERSONAL-PERF-EVAL-012` **closed** | Navigation: [PERSONAL-C1-C2-READINESS-DELIVERY-PLAN.md](PERSONAL-C1-C2-READINESS-DELIVERY-PLAN.md). Packages 1–14 delivered. EVAL-012 B0 fairness fail blocked B1/B2. Dry-run prompt bytes are frozen; live O-arm injection remains. No Gate, release, Profile, B01, or Agent-benefit conclusion follows. | inject `frozen-system-task-prompt.txt` on live P/O launches; do not activate a new EVAL until that can pass |
