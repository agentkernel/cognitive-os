...
| Active task lease | `lease/personal/P5-T02/sidecar-foundation` | P8-T01 PR #180 merged at `main@aa18ec2296cdf317ef0689365ea466652add816b`; lease/branch/main reconciliation complete. Active lease owns P5-T02 sidecar foundation on `personal/P5-T02-sidecar-foundation`. | continue `P5-T02/D01` Agent registration from active official Pi installation root; validate on exact native Linux and required CI |
...
| P5-T02 Sidecar contract, registration, instance identity | `in-progress` | `P5-T02/D01` registration and `D02` activate/SidecarSession are implemented on `personal/P5-T02-sidecar-foundation`. Exact native Linux at `4e5f8cb` passed D01 focused 4/4; D02 activation adds epoch-fenced SidecarSession without process/capability/Effect/Task claims. Required CI for HEAD pending. | finish D02 focused Linux + required CI, then enter pause/resume/stop/recover slice |
...
| `P5-T02/D01` | `in-progress` | Daemon-private Agent registration from an active official Pi installation root: inactive `registered` instance, fencing epoch seed, digest binding, and management-session caller. Exact native Linux at `4e5f8cb8753190b0f1fa8fe246d80ab8ac645c55` passed focused runtime 4/4 and admin registration 1/1; Clippy passed for runtime/store/admin-cli. Required CI pending on Draft PR #181. | wait for required CI on current HEAD, then close D01 and validate D02 |
| `P5-T02/D02` | `ready` | Activate registered instance into epoch-fenced SidecarSession without OS process spawn, capability, Effect, or Task completion; implementation is staged on the task branch behind D01 CI closure | enter validation immediately after D01 closes |
