...
| Owner-directed C1/C2 campaign | PERSONAL-PERF-EVAL-004 **paused before B1; evaluation lease closed** | B0 target and runtime readiness were qualified without any B1/B2/B3/B4 sample. The campaign discovered a product gap: the private candidate carrier persists only parameters_digest, while the production WorkspaceSearch/WorkspaceWrite/WorkspacePatch router requires canonical governed parameters. EVAL-002 remains closed and unchanged; claim ceiling remains hypothesis/non-claim | P2-T23 D01–D03 complete; ready/merge PR [#222](https://github.com/agentkernel/cognitive-os/pull/222). EVAL-004 re-freeze only after BR-02..BR-08 are merged; no prior campaign asset or SecretStore entry is reused |
| Benchmark readiness product train | `P2-T21` **done** at `5d1f5c2643d82807eb96f2c615d460dbdca749c3` / PR #220; `P2-T22` **done** at `03bc530d692649a00efa7918bfeadddf8ba0ebea` / PR [#221](https://github.com/agentkernel/cognitive-os/pull/221); `P2-T23` D01–D03 complete on `personal/P2-T23-memory-skill-consumption-resume`; `P2-T24..P2-T28` registered and not started | Owner instruction 2026-08-15 approved BR-01..BR-08 and the Linux-only product-validation route. Ubuntu is supporting CI; exact pushed-revision `DEV-LINUX-NATIVE-01` is required acceptance evidence for these product tasks; `B01-Desktop-Linux-002` remains owner-directed EVAL-004-only. Windows is `not-run by owner-directed Linux-only route`. This is product implementation evidence only, not EVAL/Gate/release/Profile/B01 evidence | ready/merge PR [#222](https://github.com/agentkernel/cognitive-os/pull/222), then claim P2-T24. EVAL-004 re-freeze happens only after every BR product task is merged |
...
| Active task lease | `lease/personal/P2-T23/public-memory-skill-consumption-resume` | P2-T23 D01–D03 complete at `79764387`. Ubuntu supporting CI `31922660543` green; `DEV-LINUX-NATIVE-01` kernel-server bin 299/299, `p4_t05_resource_api` 5/5, workspace tests 0-failed, Clippy `-D warnings`, fmt. Windows `not-run by owner-directed Linux-only route`. `B01-Desktop-Linux-002` untouched (EVAL-004-only). Draft PR [#222](https://github.com/agentkernel/cognitive-os/pull/222) | ready/merge PR #222, then close lease/branch/main |
...
| P2-T23 Public Memory/Skill consumption and resume | **in-progress** | D01–D03 complete. Public remember/review/forget and import/inspect/bind plus `GET /task/resource/v1/consumption` redacted pins; session-2 GET resume after restart; forged-prompt/digest fail closed. Ubuntu `31922660543` green at `79764387`; `DEV-LINUX-NATIVE-01` bin 299/299, p4_t05 5/5, workspace 0-failed, Clippy `-D warnings`, fmt. Branch `personal/P2-T23-memory-skill-consumption-resume`; lease `lease/personal/P2-T23/public-memory-skill-consumption-resume`; Draft PR [#222](https://github.com/agentkernel/cognitive-os/pull/222). Running report: [20260816-personal-p2-t23-memory-skill-consumption-resume-report.md](../checkpoints/20260816-personal-p2-t23-memory-skill-consumption-resume-report.md). Claim ceiling `hypothesis`; no Gate/release/Profile/B01/EVAL promotion | ready/merge PR #222 |
...
| `P2-T23/D03` | `in-progress` | Exact-revision `DEV-LINUX-NATIVE-01` at `79764387`: focused public_ 8/8 (recall=100%, restate=0, cross-scope/forgotten exposure=0), p4_t05 5/5, kernel-server bin 299/299, workspace tests 0-failed, Clippy `-D warnings`, fmt. Windows `not-run by owner-directed Linux-only route`. `B01-Desktop-Linux-002` untouched. Unique remaining D03 action is ready/merge/lease/branch/main | ready/merge PR #222 |
| `P2-T24/D01` | `ready` | Registered formal slice; implementation has not started | waits for P2-T23 closure and its own task lease |
