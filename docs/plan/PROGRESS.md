...
| Active task lease | `lease/personal/P2-T17/a7-unknown-outcome-observation` | P2-T17 implementation is on `personal/P2-T17-a7-unknown-outcome-observation` at `b122cce1`, Draft PR #212. Exact native Linux focused suite 10/10 plus Clippy/fmt at that revision. C2 carriers (P2-T20) are now on `main` | re-merge with C2, run required CI, then close A7 acceptance |
...
| P2-T17 A7 external-mutation unknown-outcome observation | **in-progress** | Isolated delivery on `personal/P2-T17-a7-unknown-outcome-observation` with `lease/personal/P2-T17/a7-unknown-outcome-observation`. D01-D03 land a campaign-owned idempotent loopback fixture, persist-before-dispatch Effect, default-off authorized fault points, original-key restart reconcile, P2-T13 independent verification, and focused negatives (crash mid-mutation, original-key replay, duplicate Effect). Prepared EVAL-003 assets remain read-only. No Task completion and no Gate/release/Profile/B01/EVAL-003 claim | keep Draft PR #212; exact-revision native Linux plus required CI; do not merge until P2-T14 and any required mutation carrier are on `main` |
