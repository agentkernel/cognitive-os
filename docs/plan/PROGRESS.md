...
| Active task lease | `lease/personal/P8-T15/dsh-native-web-panel` on `personal/P8-T15-dsh-native-web-panel` | Writable: admin-cli dsh, dsh-akp-adapter scripts, plan/PROGRESS, P8-T15 checkpoint, handbook pages routed by cli-product + dsh-akp-adapter. Not P7-T05 SPA/clients. Not `PARALLEL-LANES.md`. | linux-002 native web helper **487806** / node **487821** on `127.0.0.1:3080`; Firefox **471883** on :3080. Do not auto-claim P6 / P7-T06 / P7-T07 |
| P8-T15 Native dsh Web UI | **in-progress** | Owner 密钥 report: Models catalog still described `DEEPSEEK_API_KEY` while Path B only stored `DAEMON_BEARER`. Helper now persists settings overlay + official-ref alias to the daemon bearer (not SecretStore). Path B `POST /provider/v1/dsh/chat/completions` **200** `pong`. Personal `#/providers` is P7-T05 (lease closed): HashRouter URL **200**, `/ui/providers` **404**. Report: [P8-T15 report](../checkpoints/20260823-personal-p8-t15-dsh-native-web-panel-report.md). Claim ceiling `hypothesis`. | refresh native dsh Models on linux-002; open Personal Providers as `http://127.0.0.1:48681/ui/#/providers` (not `/ui/providers`). Do not auto-claim P6 / P7-T06 / P7-T07 |
...
| `P8-T15/D02` | `in-progress` | Relit native panel after Path B Models-key fix: node **487821** / helper **487806** on `127.0.0.1:3080`. `GET /` **200**. Credentials refs `DAEMON_BEARER`+`DEEPSEEK_API_KEY` (same management bearer). settings `llm-deepseek` Path B. Path B chat probe **200** pong. Firefox **471883** still on :3080. | refresh native Models; keep 487821/487806 running. Do not auto-claim P6 / P7-T06 / P7-T07 |
