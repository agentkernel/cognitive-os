...
## Current snapshot (2026-08-23 P7-T05 D01 in-progress; D02–D07 blocked)
...
|---|---|---|---|
| Project focus | `cognitiveos-personal`: active and sole implementation project | CognitiveOS architecture assets remain reference/contract inputs; no second product backlog | Owner-directed P7-T05 is `in-progress` on D01. Do not auto-claim P6 / P7-T06 / P7-T07 |
...
| Active task lease | `lease/personal/P7-T05/web-ui` | D01 in-repo readiness on `personal/P7-T05-web-ui`; D02–D07 blocked on missing `cognitiveos-clients` checkout | owner: clone or register `cognitiveos-clients` at an approved path, then continue D02 in `pc/web/`. Do not auto-claim P6 / P7-T06 / P7-T07 |
| P7-T05 Non-blocking Web UI | **in-progress** (D01 in-repo; D02–D07 **blocked**) | ADR-0053 accepted. Route inventory frozen; Node inventory negatives **8/8 pass**. Daemon Origin/Referer allowlist and `GET /ui` 503 `not_available` implemented (local Rust **not-run**). Draft PR [#261](https://github.com/agentkernel/cognitive-os/pull/261). `blocked_paths`: `cognitiveos-clients/pc/web/`. Do not recreate `clients/**` or implement SPA in `apps/cognitiveos-console`. Running report: [P7-T05 report](../checkpoints/20260823-personal-p7-t05-web-ui-report.md). Claim ceiling `hypothesis`; no Gate/release/Profile/B01/Agent-benefit promotion. | Linux/CI Origin/`/ui` validation; owner: provide approved `cognitiveos-clients` checkout; then D02 SPA shell |
...
auto-claim P6 / P7-T06 / P7-T07. Remaining items are in-progress `P7-T05`
(D02–D07 blocked on `cognitiveos-clients` checkout), blocked `P7-T07`, and
...
|---|---|---|---|
| `P7-T05/D01` | `in-progress` | ADR-0053 accepted; route inventory frozen; local Node inventory negatives **8/8 pass**. Daemon Origin/Referer allowlist + `GET /ui` missing-bundle `not_available` written; local Rust **not-run** (`RUST-LINK-DEV-WIN-GNU-01`). SPA not implemented. | Linux/CI focused kernel-server Origin/`/ui` negatives; D02 waits on checkout |
| `P7-T05/D02` | `blocked` | no approved `cognitiveos-clients` checkout | owner: clone/register checkout at `pc/web/` |
| `P7-T05/D03` | `blocked` | depends on D02 | owner checkout |
| `P7-T05/D04` | `blocked` | depends on D03 | owner checkout |
| `P7-T05/D05` | `blocked` | depends on D04 | owner checkout |
| `P7-T05/D06` | `blocked` | depends on D05 | owner checkout |
| `P7-T05/D07` | `blocked` | depends on D06 | owner checkout |
