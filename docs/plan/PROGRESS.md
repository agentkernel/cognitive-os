...
|---|---|---|---|
| Project focus | `cognitiveos-personal`: active and sole implementation project | CognitiveOS architecture assets remain reference/contract inputs; no second product backlog | EVAL-009 **active**. EVAL-008 **closed**. P2-T32 remains in-progress pending Windows merge (lease already closed). P7-T07 remains blocked on B01-W prerequisites. Do not claim any `P*-T*` while this campaign is active |
| Owner-directed campaign | `PERSONAL-PERF-EVAL-009` **active** (owner 2026-08-18). Same pin `fb85cfff` as EVAL-008; short root `/home/hal9001/e009` / daemon `48296` to test the UNIX_PATH_MAX hypothesis for skip class `private_completion_socket_could_not_be_created`. Evaluation routing ON. `PERSONAL-PERF-EVAL-008`, `PERSONAL-PERF-EVAL-007`, `PERSONAL-PERF-EVAL-006`, `PERSONAL-PERF-EVAL-005`, and `PERSONAL-PERF-EVAL-004` remain **closed**. | Preregistration: [20260818-personal-perf-eval-009-preregistration.md](../checkpoints/20260818-personal-perf-eval-009-preregistration.md). Running: [personal-performance-assessment-20260818-eval-009.md](../evaluation/personal-performance-assessment-20260818-eval-009.md). Do not reopen EVAL-004/005/006/007/008. Do not reuse those roots, ports `48286`/`48288`/`48290`/`48292`/`48294`/`48386`/`48388`/`48390`/`48392`/`48394`, or SecretStore `/12`–`/17`. Claim ceiling `hypothesis`; `not_reviewed` | freeze then B0 C1 WorkspaceSearch O-arm |
| Owner-directed C1/C2 campaign | PERSONAL-PERF-EVAL-009 **active**; PERSONAL-PERF-EVAL-008 **closed**; PERSONAL-PERF-EVAL-007 **closed**; PERSONAL-PERF-EVAL-006 **closed**; PERSONAL-PERF-EVAL-004 **closed**; PERSONAL-PERF-EVAL-005 **closed** | P2-T32 stub pass ≠ real adapter. EVAL-008 B0 partial on long root. EVAL-009 short-root re-measure of pin `fb85cfff`. Claim ceiling hypothesis/`not_reviewed` | freeze `/home/hal9001/e009` / `48296`; do not reuse EVAL-004/005/006/007/008 assets |
| Active task lease | `lease/personal/EVAL-009/c1-c2-paired-freeze` | EVAL-009 C1/C2 paired freeze after EVAL-008 close; branch `evaluation/EVAL-009-freeze` | freeze then B0 C1-search |
...
| P2-T32 Public `cognitive daemon start` scheduler lease | **in-progress** (lease closed; pending Windows merge) | Ubuntu `32047487272` `verify (ubuntu-latest)` **pass** at `fb85cfff`; linux-002 focused `p2_t32` 1/1 in 3.33s + `daemon_log` 2/2. Windows `verify` fail is unrelated endpoint-publish flake (330/1). Draft PR [#239](https://github.com/agentkernel/cognitive-os/pull/239). Lease closed 2026-08-18 before EVAL-008; EVAL-008 is now **closed**. Claim ceiling `hypothesis`. Stub ≠ real adapter. | rerun Windows required-ci before merge; campaign closure does not resume development; do not treat stub pass as C1/C2 Agent-benefit |
