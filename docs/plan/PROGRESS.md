...
| Active task lease | `lease/personal/P9-T06/readiness-secretstore-coalesce` — `P9-T06/D01` on `personal/P9-T06-readiness-secretstore-coalesce` | P9-T05 closed first through separate PR #213. P9-T06 owns only the readiness/handbook/report paths listed in Parallel Lanes; P2-T14 scheduler/verification/store paths remain untouched | obtain reproducible production-backend baseline/post measurements, then exact native/required CI and closure; no Gate/release/Profile claim |
| P9-T06 Readiness SecretStore coalesce | **in-progress** | `evaluate_personal_readiness` binds one SecretStore per evaluation: secret probe and provider resolve share it, skip `get` when probe already proved the backend cannot answer, drop material immediately, and re-evaluate every request. Counting-store negatives cover once-per-evaluation, one config snapshot, no TTL, skip-get and no material leak. Earlier required CI `31721844753` passed before P9-T05 merged; PR #218 remains separate | measure the old and new exact revisions on the same supported native host, then revalidate the integrated head |
...
|---|---|---|---|
| `P9-T06/D01` | `in-progress` | One readiness evaluation binds one SecretStore: secret probe and provider resolve share it, skip `get` when probe already proved the backend cannot answer, drop material immediately, and do not cache ready across requests. Counting-store negatives cover once-per-evaluation, no TTL, skip-get, and no material leak | required Ubuntu/Windows CI; no Gate/release/Profile claim |
