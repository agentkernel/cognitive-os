...
| Active task lease | `lease/personal/P4-T03/memory-lifecycle` | P4-T03 acceptance is complete on `personal/P4-T03-memory-lifecycle`; the lease remains active only for deterministic PR and branch closure | commit/push closure evidence, confirm required CI, merge PR #174, close lease, and reconcile local `main` |
...
| P4-T03 Memory lifecycle, retention and forget | `done` | `P4-T03/D01-D03` satisfy the unchanged acceptance at exact immutable revision `8f9250dcd4cbcd8f15867e7a0f45165032e26c9d`: forget, expiry-boundary, and version/update/conflict lifecycle facts preserve immutable admission history, enforce expected-version CAS lineage, and atomically invalidate/move derived FTS rows. Exact native Linux focused tests passed 16/16, native Clippy passed, and required Ubuntu/Windows CI passed in PR #174. | complete task closure: acceptance mapping, final handoff, PR ready/merge, lease close, branch cleanup, and main reconciliation; no public Memory API, B08, Gate, release, or Profile claim |
