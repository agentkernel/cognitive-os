...
| Active task lease | `none` for closed P9-T07 | The closed lease is in Parallel Lanes recent history. Product paths never overlapped P2-T13/P2-T14 scheduler/verification/store ownership; no B01 guest state was touched | unique next action: register and freeze `EVAL-003/PI-NESTED` as an owner-directed campaign before any Provider execution, otherwise retain `not-run` |
