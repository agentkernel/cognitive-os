...
| P2-T03 scheduler/runtime | `in-progress` | `P2-T03/D01`-`D04` are done. D04 exact immutable Linux `b396d32` passed scheduler authority 13/13, scheduler lease/recovery 6/6, Clippy and fmt; required Ubuntu/Windows CI passed. D05 remains the only next P2-T03 slice | run the scheduler-to-BoundedHarness worker/recovery slice at an immutable Linux revision; do not begin P2-T04 before D05 closes |
...
| Active task lease | `lease/personal/P2-T03/scheduler-bounded-harness-worker-recovery` | P2-T03/D05 scheduler-to-BoundedHarness worker and restart-safe recovery is the sole active slice; B02/B04/B05/B12, release, and Profile remain separate and incomplete | establish a daemon-owned restart-safe scheduler worker with failure-first recovery evidence |
...
| `P2-T03/D05` | `in-progress` | D03/D04 durable binding and fenced closure exits are done; failure-first scheduler-to-BoundedHarness worker/recovery work is active | establish restart, duplicate, clock and ceiling recovery evidence; no additional helper-only slice |
