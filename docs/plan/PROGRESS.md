...
| Active task lease | `lease/personal/P1-T09/b01-attempt-2-execution` | The product owner verified that `B01-Desktop-Linux-002` is running under system libvirt on `wuz@192.168.1.2` and that snapshot `b01-platform-qualified-baseline` exists. The scoped `sudo virsh -c qemu:///system` reset/control authorization is active; no reset or attempt has yet occurred | run the authorized system-libvirt preflight, then reset exactly to the preregistered snapshot before attempt 2 |
