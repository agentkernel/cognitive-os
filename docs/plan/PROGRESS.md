...
| P2-T04 private worker composition | `in-progress` | P2-T04/D01 remains active. The daemon resolves and authorizes Context before accepting only untrusted candidate fields, then owns candidate sealing and admission. `331a584` adds immutable v15 daemon-private execution-policy persistence, and `fe5eb33` makes v0.4 scheduler work reload and fence the policy before WIA lookup. The persistence checkpoint passed supported Ubuntu/Windows CI; the scheduler-fence checkpoint is running. Policy creation at Task admission, a supported sessionless secret-free pinned Pi entrypoint, and scheduler bridge invocation remain absent. WIA remains post-admission and unconsumed; no executor, progress, evidence, verification, acceptance, or Task completion is added. | persist complete daemon-owned policy inputs at Task admission, then construct Context/admission commands and invoke the supported pinned Pi candidate transport before WIA lookup |
