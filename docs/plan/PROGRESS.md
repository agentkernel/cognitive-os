...
| P1-T09 route implementation | `in-progress` | `experimental-local-only` / `tested-supported-ci`; replacement artifact `0.0.0-campaign.20260808.1` is independently verified, its DeepSeek V4 Flash configuration is preregistered, and the owner authorized complete counted Attempt 5. | restore the registered baseline, start the guest, and execute the complete preregistered Attempt 5 route with the Desktop hidden-input credential step and independent verifier |
| B01 first-install/first-conversation Gate | **running** | `B01-Desktop-Linux-002` is the sole active B01 guest. Attempt 4 is recorded after the clean-reset checkpoint: SSH readiness passed, but workflow run `30687541828` reported the required artifact expired; no artifact, Pi, service, Provider, credential, prompt, or route was used. The replacement `0.0.0-campaign.20260808.1` artifact is independently verified and owner-authorized for Attempt 5. The ordered ledger has 4 of the formal minimum 20 attempts: 1 success / 3 failures, with no critical safety failure observed. | restore the baseline and run complete Attempt 5; do not retry Attempt 4 |
...
| Active task lease | `lease/personal/P1-T09/b01-attempt-5-execution` | P1-T09 is `in-progress` under the owner-confirmed complete Attempt 5. B01 remains `running` with 4 of its fixed 20 attempts recorded (1 success, 3 failures) until the fresh reset/start crosses the clean-reset checkpoint. | restore `b01-platform-qualified-baseline`, start the guest, and execute the entire preregistered Attempt 5 path without splitting or retrying it |
