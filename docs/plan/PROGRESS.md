...
| Active task lease | `lease/personal/P2-T16/registered-check-run` on isolated sibling worktree `D:\agent-kernel-checkrun` | Draft PR [#211](https://github.com/agentkernel/cognitive-os/pull/211). Native focused suites passed at `c75749cc` and exact `26a58365` (16/16); Windows `remove_dir_all` fix plus C2/A7 re-merge in progress | re-merge latest main (C2 carriers + A7), run required CI, then flip Draft and merge |
...
| P2-T16 Registered deterministic check execution | **in-progress** | D01–D03 implementation is on `personal/P2-T16-registered-check-run`. Exact native `c75749cc`: `registered_check` 16/16, `p2_t16_registered_check` 2/2, `tool_registry` 11/11, `tool_executor` 76/76, Clippy/fmt pass. Required Ubuntu on `31721644351` passed; Windows failed four scheduler cleanup tests (Win32 32) after those assertions. General ProcessRun, caller argv/env/cwd/shell and exit-based completion remain forbidden | push Windows router-drop cleanup; rerun required CI; D04 merge mutex remains P2-T14 |
