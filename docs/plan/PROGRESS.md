...
| Active task lease | `lease/personal/P3-T06/context-correctness` | ADR-0040 defines the MVP B03 fixed 33-test denominator. D01-D02 native evidence, cleanup, owner review, and required CI evidence are complete through `76e3e1f`; PR #171 remains Draft pending final CI and normal task closure. | confirm the final CI revision, record the bounded B03 MVP decision, then complete PR/lease/main closure |
...
| P3-T06 B03 Context correctness and benefit collection | `in-progress` | ADR-0040 defines MVP B03 as the fixed 33-test authority-path/evaluator matrix plus native Linux/Clippy, required Ubuntu/Windows CI, cleanup/redaction, and owner review. D01-D02 meet the evidence conditions through `76e3e1f`; B06/B07 remain optional raw performance observations. | wait for final CI on the policy revision, record B03 MVP disposition, and complete PR/lease/main closure |
...
| `P3-T06/D02` | `in-progress` | ADR-0040 fixes the MVP B03 denominator at 22 Rust authority-path tests plus 11 evaluator/tooling tests. Native Linux, focused Clippy, cleanup/redaction, and owner review passed; required CI is pending for the policy revision. | record the bounded B03 MVP disposition after required CI, then complete task closure |
