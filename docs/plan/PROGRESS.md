...
| `P2-T14/D01` | `in-progress` | Native `a38a27a0` scheduler 53/53 and verification 12/12 pass, including public C1 `COMPLETED`; Clippy failed on `expect_used` in the new tests | push the Clippy assert fix, rerun exact native Clippy, then add remaining currentness/CAS/open-Effect negatives |
