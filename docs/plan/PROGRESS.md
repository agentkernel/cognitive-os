...
| Owner-directed C1/C2 campaign | PERSONAL-PERF-EVAL-004 **paused before B1; evaluation lease closed** | B0 target and runtime readiness were qualified without any B1/B2/B3/B4 sample. The campaign discovered a product gap: the private candidate carrier persists only parameters_digest, while the production WorkspaceSearch/WorkspaceWrite/WorkspacePatch router requires canonical governed parameters. EVAL-002 remains closed and unchanged; claim ceiling remains hypothesis/non-claim | P2-T24 D01–D03 complete; ready/merge PR [#223](https://github.com/agentkernel/cognitive-os/pull/223). EVAL-004 re-freeze only after BR-02..BR-08 are merged; no prior campaign asset or SecretStore entry is reused |
| Benchmark readiness product train | `P2-T21` **done** at `5d1f5c2643d82807eb96f2c615d460dbdca749c3` / PR #220; `P2-T22` **done** at `03bc530d692649a00efa7918bfeadddf8ba0ebea` / PR [#221](https://github.com/agentkernel/cognitive-os/pull/221); `P2-T23` **done** at `795bfac8d8ee77b1b3be4137d33399fc817d20eb` / PR [#222](https://github.com/agentkernel/cognitive-os/pull/222); `P2-T24` D01–D03 complete on `personal/P2-T24-effect-fault-reconciliation` / Draft PR [#223](https://github.com/agentkernel/cognitive-os/pull/223); `P2-T25..P2-T28` registered and not started | Owner instruction 2026-08-15 approved BR-01..BR-08 and the Linux-only product-validation route. Ubuntu is supporting CI; exact pushed-revision `DEV-LINUX-NATIVE-01` is required acceptance evidence for these product tasks; `B01-Desktop-Linux-002` remains owner-directed EVAL-004-only. Windows is `not-run by owner-directed Linux-only route`. This is product implementation evidence only, not EVAL/Gate/release/Profile/B01 evidence | ready/merge PR [#223](https://github.com/agentkernel/cognitive-os/pull/223), then claim P2-T25. EVAL-004 re-freeze happens only after every BR product task is merged |
...
| Active task lease | `lease/personal/P2-T24/effect-fault-reconciliation-observation` | P2-T24 D01–D03 complete at `8b7dea7a`. Ubuntu supporting CI `31925698730` green; `DEV-LINUX-NATIVE-01` kernel-server bin 309/309, `p2_t24_d02` 3/3, `fault_profile` 5/5, P2-T17 15/15, workspace tests 0-failed, Clippy `-D warnings`, fmt. Windows `not-run by owner-directed Linux-only route`. `B01-Desktop-Linux-002` untouched (EVAL-004-only). Draft PR [#223](https://github.com/agentkernel/cognitive-os/pull/223) | ready/merge PR #223, then close lease/branch/main |
...
| P2-T24 Effect fault and reconciliation observation | **in-progress** | D01–D03 complete. Default-off campaign-authorized fault profiles and bounded `GET /task/effects` history; original-key restart reconcile under persisted profiles; production native dispatch injects only at the four fixed points. Ubuntu `31925698730` green at `8b7dea7a`; `DEV-LINUX-NATIVE-01` bin 309/309, `p2_t24_d02` 3/3, `fault_profile` 5/5, P2-T17 15/15, workspace 0-failed, Clippy `-D warnings`, fmt. Branch `personal/P2-T24-effect-fault-reconciliation`; lease `lease/personal/P2-T24/effect-fault-reconciliation-observation`; Draft PR [#223](https://github.com/agentkernel/cognitive-os/pull/223). Running report: [20260816-personal-p2-t24-effect-fault-reconciliation-report.md](../checkpoints/20260816-personal-p2-t24-effect-fault-reconciliation-report.md). Claim ceiling `hypothesis`; no Gate/release/Profile/B01/EVAL promotion | ready/merge PR #223 |
...
| `P2-T24/D03` | `in-progress` | Exact-revision `DEV-LINUX-NATIVE-01` at `8b7dea7a`: `p2_t24_effect_fault` 1/1, `p2_t24_d02` 3/3, `fault_profile` 5/5, P2-T17 15/15, kernel-server bin 309/309, workspace tests 0-failed, Clippy `-D warnings`, fmt. Residue `/tmp/cos-p2t24-*` and the LAN bundle removed. Windows `not-run by owner-directed Linux-only route`. `B01-Desktop-Linux-002` untouched. Unique remaining D03 action is ready/merge/lease/branch/main | ready/merge PR #223 |
| `P2-T25/D01` | `ready` | Registered formal slice; implementation has not started | waits for P2-T24 closure and its own task lease |
