...
| Active task lease | `lease/personal/P2-T27/user-lifecycle-backup` | P2-T26 merged via PR #225 at `main@9e1404a1`. Unique in-progress slice `P2-T27/D03`. Draft PR [#226](https://github.com/agentkernel/cognitive-os/pull/226). D01 Ubuntu `31938000525` at `6eea42c4`. D02 Ubuntu `31938624514` and linux-002 `p2_t27_pi_lifecycle` 1/1 at `cf7dd501`. Windows `not-run by owner-directed Linux-only route`. `B01-Desktop-Linux-002` untouched (EVAL-004-only) | D03 equality/migration/RTO/RPO/lifecycle/cleanup matrix and task close |
...
| P2-T27 User lifecycle and backup/restore | **in-progress** | D01 public backup/restore complete at `6eea42c4` (Ubuntu `31938000525`; linux-002 store 22/22, CLI 2/2, HTTP 1/1, fmt, Clippy). D02 Pi install→recover complete at `cf7dd501` (Ubuntu `31938624514`; linux-002 `p2_t27_pi_lifecycle` 1/1). Unique slice `P2-T27/D03` on `personal/P2-T27-user-lifecycle-backup` / PR [#226](https://github.com/agentkernel/cognitive-os/pull/226). Windows `not-run by owner-directed Linux-only route`. Claim ceiling `hypothesis`; no Gate/release/Profile/B01/EVAL promotion | D03 equality/migration/RTO/RPO/lifecycle/cleanup matrix and task close |
...
| `P2-T27/D03` | `in-progress` | D01/D02 public backup/restore and Pi install→recover are on Draft PR [#226](https://github.com/agentkernel/cognitive-os/pull/226). D03 owns equality/migration/RTO/RPO/lifecycle/cleanup matrix and task close. Windows `not-run by owner-directed Linux-only route` | author and run the linux-002 matrix at an exact pushed revision |
