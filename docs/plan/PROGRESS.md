...
| Personal 产品化计划 | **planning；P0-T01、P0-T02 done；P0-T05 blocked；无产品实现声明** | `01ceb93` 的 CI run 30140381194 在 Ubuntu 与 Windows/MSVC 均通过；P0-T02 专用映射核对、`pnpm run check:consistency`、`git diff --check` 均通过；P0-T05 WSL2 fail-closed 环境探针确认 `org.freedesktop.secrets` 不可用，故操作与泄漏负例 `not-run`；Personal 功能/Gate 证据均 `not-run` | 正式台账：[PERSONAL-DEVELOPMENT-PLAN.md](PERSONAL-DEVELOPMENT-PLAN.md)；[PERS-PR trace](personal-trace.yaml) 独立于 registry matrix，不新增 REQ/schema/vector；记录：[tests/baseline/README.md](../../tests/baseline/README.md)。Personal `done` 不代表 G0、B01-B12 或 Profile 已符合。 |
...
1. [20260725-personal-p0-t05-handoff.md](../checkpoints/20260725-personal-p0-t05-handoff.md)（Personal：WSL2 root 会话没有 Secret Service，P0-T05 fail-closed blocked；未创建测试 secret 或执行 PoC）
