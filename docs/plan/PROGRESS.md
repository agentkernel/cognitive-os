...
| P3-T03 Artifact CAS | `in-progress` | `P3-T03/D01` is implementing the single daemon-owned filesystem CAS and authority metadata port. This begins with digest/partial-write/access negative coverage; required Rust validation is routed to an exact committed native-Linux revision and required CI. | implement the narrow Artifact CAS storage boundary; no Artifact closure, Gate, release, or Profile claim |
...
| Active task lease | `lease/personal/P3-T03/artifact-cas` | P3-T03/D01 on `personal/P3-T03-artifact-cas`; the ignored local `/artifacts/` directory remains protected from modification, deletion, commitment, or implicit use. | implement and validate the single Artifact CAS authority port |
...
| `P3-T03/D01` | `done` | At `f626d107232e7861148e5894be52547c4d24cc99`, the daemon-owned bounded filesystem CAS publishes digest-addressed bytes and immutable metadata atomically, rejects malformed/digest-mismatched references, rejects unauthorized reads, and removes only abandoned staging writes. Exact native Linux focused test/Clippy/fmt and required Ubuntu/Windows CI passed. | consumed by D02; no Artifact closure, Gate, release, or Profile claim |
| `P3-T03/D02` | `in-progress` | P2-T07 verifier currently accepts only syntactically valid artifact references. D02 must resolve each reference through the D01 CAS before append-only report persistence. | add missing/tampered CAS-reference negatives and wire the real verifier boundary without granting Task-completion authority |
