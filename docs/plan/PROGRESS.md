...
| Active task lease | `lease/personal/P2-T27/user-lifecycle-backup` | P2-T26 merged via PR #225 at `main@9e1404a1`. Unique in-progress slice `P2-T27/D01`. Draft PR [#226](https://github.com/agentkernel/cognitive-os/pull/226). Windows `not-run by owner-directed Linux-only route`. `B01-Desktop-Linux-002` untouched (EVAL-004-only) | wait Ubuntu supporting CI on `6eea42c4`, then D02 managed Pi install→recover |
...
| P2-T27 User lifecycle and backup/restore | **in-progress** | D01 public backup/restore archive I/O and `cognitive backup`/`restore` plus management HTTP routes, excluding secret/bearer/provider-config/authority SQLite. Unique slice `P2-T27/D01` on `personal/P2-T27-user-lifecycle-backup` / PR [#226](https://github.com/agentkernel/cognitive-os/pull/226). `DEV-LINUX-NATIVE-01` at `6eea42c4`: store 22/22, CLI 2/2, HTTP 1/1, fmt, Clippy `-D warnings`. Windows `not-run by owner-directed Linux-only route`. Claim ceiling `hypothesis`; no Gate/release/Profile/B01/EVAL promotion | Ubuntu supporting CI on `6eea42c4`; then D02 managed Pi lifecycle |
...
| `P2-T27/D01` | `in-progress` | Public backup archive write + transactional restore overlay excluding secrets/SQLite; `cognitive backup`/`restore`; management HTTP backup/restore; tamper/missing-part/schema/partial-state negatives. `DEV-LINUX-NATIVE-01` at `6eea42c4`: store 22/22, CLI 2/2, HTTP 1/1, fmt, Clippy `-D warnings`. Local Rust `not-run` (`RUST-LINK-DEV-WIN-GNU-01`) | Ubuntu supporting CI on `6eea42c4`; then D02 |
| `P2-T27/D02` | `ready` | Registered formal slice; implementation has not started | waits for D01 within the same future task branch and lease |
