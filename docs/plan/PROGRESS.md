...
## Current snapshot (2026-08-23 P7-T05 D10 sidebar navigation fix)
...
|---|---|---|---|
| Project focus | `cognitiveos-personal`: active and sole implementation project | CognitiveOS architecture assets remain reference/contract inputs; no second product backlog | Owner review P7-T05/D10 Draft PRs. Do not auto-claim P6 / P7-T06 / P7-T07 |
...
| Active task lease | `lease/personal/P7-T05/web-ui-sidebar-fix` | D10 live linux-002: sidebar 8/8 pass; Provider key SecretStore pass; dsh binding `deepseek-v4-flash` digest `binding`. Report: [sidebar-fix report](../checkpoints/20260823-personal-p7-t05-web-ui-sidebar-fix-report.md). Claim ceiling `hypothesis`. | owner review Draft PRs; keep Draft until acceptance; do not auto-claim P6 / P7-T06 / P7-T07 |
| P7-T05 Non-blocking Web UI | **in-progress** (D10 live-proven; D01–D09 remain delivered; Draft until acceptance) | Root cause: unauthenticated `RequireSession` `Navigate replace` to `/session` bounced every sidebar hash. Fix: in-place session gate. linux-002 PID 465376 `http://127.0.0.1:48681/ui/`: sidebar 8/8; session pass; Provider key SecretStore pass; binding selected-model `deepseek-v4-flash` digest `binding`. Tasks preview/admit this slice **not-run**. Report: [sidebar-fix report](../checkpoints/20260823-personal-p7-t05-web-ui-sidebar-fix-report.md). Claim ceiling `hypothesis`. | owner review Draft PRs; do not auto-claim P6 / P7-T06 / P7-T07 |
