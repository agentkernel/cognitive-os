...
| `P2-T12/D01` | `in-progress` | Failure-first regression added: after successful `admit`, crash/reopen must reveal the TaskContract, one current-epoch `Runnable` scheduler row, the contract-named Loop at `START`, and the contract-named hard Budget as one durable publication. Local Rust execution is `not-run` by the registered Windows GNU restriction | push the immutable failure-first checkpoint, run it on `DEV-LINUX-NATIVE-01` to confirm expected red, append the result immediately, then implement atomic publication |
