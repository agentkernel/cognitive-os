...
| Owner-directed C1/C2 campaign | PERSONAL-PERF-EVAL-004 **paused before B1; evaluation lease closed** | B0 target and runtime readiness were qualified without any B1/B2/B3/B4 sample. The campaign discovered a product gap: the private candidate carrier persists only parameters_digest, while the production WorkspaceSearch/WorkspaceWrite/WorkspacePatch router requires canonical governed parameters. EVAL-002 remains closed and unchanged; claim ceiling remains hypothesis/non-claim | P2-T26 **done** at `main@9e1404a1` / PR [#225](https://github.com/agentkernel/cognitive-os/pull/225). Unique next: P2-T27/D01 public backup/restore. EVAL-004 re-freeze only after BR-02..BR-08 are merged; no prior campaign asset or SecretStore entry is reused |
| Benchmark readiness product train | `P2-T21` **done** at `5d1f5c2643d82807eb96f2c615d460dbdca749c3` / PR #220; `P2-T22` **done** at `03bc530d692649a00efa7918bfeadddf8ba0ebea` / PR [#221](https://github.com/agentkernel/cognitive-os/pull/221); `P2-T23` **done** at `795bfac8d8ee77b1b3be4137d33399fc817d20eb` / PR [#222](https://github.com/agentkernel/cognitive-os/pull/222); `P2-T24` **done** at `2b803e0fb955d03374e10569b4d9aecd0b4162d7` / PR [#223](https://github.com/agentkernel/cognitive-os/pull/223); `P2-T25` **done** at `4b10db9a64584bda42ae249bd1df289475bd6324` / PR [#224](https://github.com/agentkernel/cognitive-os/pull/224); `P2-T26` **done** at `9e1404a1b262a0e8588dbc2d1daac08ce419bd33` / PR [#225](https://github.com/agentkernel/cognitive-os/pull/225); `P2-T27` D01 in progress; `P2-T28` registered and not started | Owner instruction 2026-08-15 approved BR-01..BR-08 and the Linux-only product-validation route. Ubuntu is supporting CI; exact pushed-revision `DEV-LINUX-NATIVE-01` is required acceptance evidence for these product tasks; `B01-Desktop-Linux-002` remains owner-directed EVAL-004-only. Windows is `not-run by owner-directed Linux-only route`. This is product implementation evidence only, not EVAL/Gate/release/Profile/B01 evidence | continue P2-T27/D01 public backup/restore. EVAL-004 re-freeze happens only after every BR product task is merged |
...
| P2-T27 User lifecycle and backup/restore | **in-progress** | D01 public backup/restore complete at `6eea42c4` (Ubuntu `31938000525`; linux-002 store 22/22, CLI 2/2, HTTP 1/1, fmt, Clippy). Unique slice `P2-T27/D02` public Pi install→recover callers on `personal/P2-T27-user-lifecycle-backup` / PR [#226](https://github.com/agentkernel/cognitive-os/pull/226). Windows `not-run by owner-directed Linux-only route`. Claim ceiling `hypothesis`; no Gate/release/Profile/B01/EVAL promotion | D02 `admin-cli activate-root`/`rollback` plus current-revision lifecycle test; Ubuntu + linux-002 |
