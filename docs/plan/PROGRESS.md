...
|---|---|---|---|
| `P9-T04/D07` | `in-progress` | the L3 Provider-route policy fixes `retry=0`, retains every started request as a classified outcome, and rejects fabricated TTFT, fabricated cost, network time without dispatch, and measured usage on a failed request | run the route-policy negatives on exact native Linux and required CI, then execute L3 once the B01 start-gate and operator Provider import are available |
