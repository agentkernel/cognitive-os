...
| Active task lease | `none` | P9-T01 closure is ready to merge; no active writable task lease remains | merge P9-T01, reconcile `main`, then claim the next ready task |
| P9-T01 Async event foundation decision gate | **done** | D01's exact native Linux observation at `826745c` passed `perf::tests` 5/5 and its deterministic hypothesis-only rule selected `conservative-no-migration`; required Ubuntu/Windows CI `31510283352` passed on closure head `98896cf`. Aggregate `effect_persistence` is not transport evidence; no async migration, Gate, release, or Profile claim. Closure: `20260811-personal-p9-t01-async-decision-gate-closure.md`. | retain evidence; any reconsideration needs separately measured HTTP/watch/sidecar transport evidence |
