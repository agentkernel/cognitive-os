...
|---|---|---|---|
| Project focus | `cognitiveos-personal`: active and sole implementation project | CognitiveOS architecture assets remain reference/contract inputs; no second product backlog | EVAL-007 closed. Next product mutex is public `cognitive daemon start` C1/C2 path. P7-T07 remains blocked on B01-W prerequisites |
| Owner-directed campaign | `PERSONAL-PERF-EVAL-007` **closed** 2026-08-17. Frozen product target `main@2a8d4d2f`. Freeze pass (root `/home/hal9001/perfeval007-20260817`, daemon `48292`, SecretStore `/16`, Pi `0.81.1`). C1/C2 B0 **partial** (public `cognitive daemon start`: Task `DRAFT`, `lease_acquired` 0, no Pi child). B1/B2 `not-run`. Cleanup **pass**. Claim ceiling `hypothesis`; `not_reviewed`. Evaluation routing OFF. `PERSONAL-PERF-EVAL-006`, `PERSONAL-PERF-EVAL-005`, and `PERSONAL-PERF-EVAL-004` remain **closed**. | Final: [personal-performance-assessment-20260817-eval-007.md](../evaluation/personal-performance-assessment-20260817-eval-007.md). Do not reuse EVAL-004/005/006/007 roots, ports `48286`/`48288`/`48290`/`48292`/`48386`/`48388`/`48390`/`48392`, or SecretStore `/12`/`/13`/`/14`/`/15`/`/16` | none for this campaign — public daemon-launcher skip is a product mutex after this close |
...
| Owner-directed C1/C2 campaign | PERSONAL-PERF-EVAL-007 **closed**; PERSONAL-PERF-EVAL-006 **closed**; PERSONAL-PERF-EVAL-004 **closed**; PERSONAL-PERF-EVAL-005 **closed** | P2-T31 merged at `main@da6fca2e` / closure `2a8d4d2f`. EVAL-007 B0 on public `cognitive daemon start` stayed `DRAFT`. Do not reuse EVAL-004/005/006/007 roots, ports, or SecretStore `/12`–`/16`. Claim ceiling hypothesis/`not_reviewed` | none — product mutex is public daemon launcher after this close |
| Active task lease | `none` | EVAL-007 closed 2026-08-17 | next ready Personal task after evaluation routing OFF |
| Benchmark readiness product train | `P2-T21` **done** … `P2-T31` **done** at `da6fca2e` / PR [#236](https://github.com/agentkernel/cognitive-os/pull/236) | Live HTTP admit acquires a scheduler lease. CI `32038273275` at `49cdfc0c`; linux-002 focused 1/1, bin 337/337. Product evidence only, not EVAL/Gate/release/Profile/B01 | EVAL-007 closed; public `cognitive daemon start` path is the remaining product mutex |
...
| P2-T31 Live-daemon HTTP scheduler lease | **done** | Merged PR [#236](https://github.com/agentkernel/cognitive-os/pull/236) at `main@da6fca2e`. Exact `49cdfc0c` linux-002: focused live HTTP 1/1, kernel-server bin 337/337, workspace 0-failed, Clippy `-D warnings`, fmt. CI `32038273275` Ubuntu/Windows/`required-ci` green. Claim ceiling `hypothesis`. | retain closure; next measurement is EVAL-007 |
