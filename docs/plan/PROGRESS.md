...
| P3-T02 Context Builder and budgets | `in-progress` | `experimental-local-only`; P3-T02 is implementing the daemon-owned System/Shell/Task/Working/Evidence builder over P3-T01's stable source port and P2-T04's sealed ContextView-before-Pi path. No B03, UCR-01, release, or Profile claim is made. | add failure-first fragment, required-budget, deduplication, and explicit-loss coverage, then wire the minimal builder into the real scheduler path |
