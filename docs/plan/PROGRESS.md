...
| Active task lease | `lease/personal/P2-T20/mutation-carriers` — `P2-T20/D02` on `personal/P2-T20-mutation-carriers` | C2 parameter-bearing mutation request carriers; owner-directed 2026-08-14. D01 failure-first proof is complete (both-platform CI green at `d5a18a22`) | wire the first production request carrier (D02) per family; no Gate/release/Profile claim |
| P2-T20 Parameter-bearing mutation request carriers (C2) | **in-progress** | Owner-directed 2026-08-14. Production worker still only dispatches parameter-free WorkspaceRead; WorkspaceSearch/Write/Patch and HttpFetchReadOnly lack governed payload/preimage/origin carriers. D01 failure-first proof (fail closed before Effect authorization) is green on both platforms | wire the first production request carrier (D02) through the hardened Effect/authority path |
...
| `P2-T20/D02` | `in-progress` | wire the first parameter-bearing production request carrier (WorkspaceWrite with expected-preimage + payload) through the hardened Effect/authority path so an admitted Task reaches the mutation sink | implement the carrier plus a focused failure-first/negative test, then required Ubuntu/Windows CI |
