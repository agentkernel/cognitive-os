...
| Active task lease | `lease/personal/P2-T03/checkpoint-effect-closure-helper` | P2-T03 implementation-only kernel repair in progress: Linux-native validation found that the registered ceiling STOP path called an undefined private checkpoint-effect closure helper; the repair must retain fail-closed handling for malformed or unresolved inventory | run the focused kernel STOP regression from the exact committed Linux worktree, then re-run the runtime adapter regression; no new implementation-evidence, Gate, release or Profile claim |
