...
| P3-T01 Context source/retrieval port | `in-progress` | `experimental-local-only` / `tested-local`; `P3-T01/D01` has committed TaskContract v0.4 ContextRequest binding plus append-only ContextRequest/ContextView persistence/query groundwork. Exact native Linux `ed30deecc5e33e281a15c7810966ec9c3ec8b5bf` passed Context integrity 4/4, migration-layout 8/8, intent-chain 6/6, and formatting; exact native Linux `ad95f913423bbee661b28915ef8bb0381523094b` passed the kernel-server suite plus the Context/intent-store regressions and formatting after daemon-side request issuance. This does not yet provide real workspace/task/evidence sources, scope-before-ranking, revocation coverage, required CI, B03, or task completion. | add an admission-to-v0.4 binding assertion, then integrate real sources and scope-before-ranking/revocation negatives without overlapping the active P2-T04 lease |
...
| `P3-T01/D01` | `in-progress` | TaskContract v0.4 ContextRequest binding and append-only ContextRequest/ContextView persistence/query groundwork are protected by append-time sealed-content, row-metadata, and strong request-reference validation. The daemon now issues a private request during Task admission; exact native Linux `ad95f913423bbee661b28915ef8bb0381523094b` passed the kernel-server suite, Context integrity 4/4, intent-chain 6/6, and fmt. A focused admission-binding assertion, required CI, real workspace/task/evidence sources, scope-before-ranking, and revocation coverage remain incomplete. | assert the daemon admission binding, then attach real Context sources and scope/revocation regressions while preserving the P2-T04 runtime lease boundary |
