...
| Active task lease | `lease/personal/P5-T01/pi-acquisition` | `P5-T01/D02` is active on `personal/P5-T01-pi-acquisition`; D01 closed at immutable `39b0e06e804947f9bd6c4a67295ffd4f4ccd3231` with required Ubuntu/Windows CI run `31351532417` passed. Owned paths remain the private installation store/runtime installer, authenticated admin install caller, focused tests, and exact plan/progress/closure documents recorded in `PARALLEL-LANES.md`. | implement a durable acquisition-lock-only versioned activation, upgrade, and rollback transaction with failure-first negatives; preserve no AgentInstance, sidecar, process, Effect, Task-completion, B09, release, or Profile claim |
...
| `P5-T01/D02` | `in-progress` | D01's authoritative immutable lock is available at `39b0e06e804947f9bd6c4a67295ffd4f4ccd3231`. The local D02 worktree now adds daemon-private durable versioned installation-root bindings with CAS publication, lock-only admission, and failure-first uncommitted/no-lock, compatibility/health, upgrade/conflict, and incomplete-rollback negatives. Local `cargo fmt --all` and diff check passed; Rust tests/Clippy are `not-run` because `DEV-WIN-GNU-01` is unsupported for linking. | route the exact uncommitted D02 revision to supported Linux/CI validation; retain no AgentInstance, sidecar, process, Effect, Task-completion, B09, release, or Profile claim |
