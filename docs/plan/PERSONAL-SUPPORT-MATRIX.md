...
| OPC Control Plane | Today / Projects / Knowledge; Settings bottom; right Personal Assistant; Project briefing; HITL on canvas; all states. Team/Inbox are not first-level | **Requires-backend**; current daemon-served Web UI evidence does not establish the target IA or Windows packaging |
| Project/digital employee model | Project/Charter/Goal/Plan/Routine/Task/Attempt plus Blueprint/Assignment/Employee; one current manager; authority id = Employee | **Requires-backend**; no current generic Project support claim |
...
| Hidden hosted DSH engine | Exact audited artifact, isolated child/stdio broker, daemon Provider proxy, Personal-owned Conversation/Memory/Task, health/update/rollback; not visible Installed Agent chrome | **Requires-backend + Requires-environment**; current dsh Path B is not Windows managed-engine/sandbox/supply-chain qualification |
...
| Routine / HITL recovery | manual/schedule/qualified-event, no-overlap, queue-latest, missed/catch-up; HITL on project canvas + Today deep link (not a first-level Inbox) | **Requires-backend**; no 24/7/offline-host claim |
| Provider/honest usage | global→Project→employee→Task binding; actual usage; unknown cost ≠ 0; member-level budget is 2.1 / Deferred | **Requires-backend** beyond current fixed binding/advisory-budget foundation |
