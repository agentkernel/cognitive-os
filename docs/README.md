...
- [plan/agent-hub-development-plan.md](plan/agent-hub-development-plan.md)：Agent Hub Master Development Plan（受 gate 阻断，未激活实现车道）；子计划见 `plan/agent-hub/`。

## docs/clients/（informative canonical 目录索引）

- [clients/README.md](clients/README.md)：PC 客户端、手机 remote companion、共享 SDK/契约、平台设计与 Agent Hub 目录的唯一 canonical 索引、边界和阅读顺序。
...
- [platforms/README.md](platforms/README.md)：Console 跨平台入口、激活前文档例外与实现 gate。
...
- [platforms/ios-product-design.md](platforms/ios-product-design.md)：iPhone-only v1 范围、APNs/设备绑定/R1、要求、PoC 与 Apple 来源。
- [platforms/android-product-design.md](platforms/android-product-design.md)：列名 Android phone v1 范围、FCM/Keystore/Play、要求、PoC 与 Google 来源。
...
- [platforms/mobile-parity-matrix.md](platforms/mobile-parity-matrix.md)：Windows/macOS/Linux 到 iPhone/Android 的复用、适配、替换、不提供和阻断矩阵。
...
- [platforms/mobile-platform-decision-log.md](platforms/mobile-platform-decision-log.md)：`CONSOLE-IOS-V1-DEC-*` / `CONSOLE-AND-V1-DEC-*` canonical 产品决策。
- [platforms/agent-hub-platform-parity.md](platforms/agent-hub-platform-parity.md)：Agent Hub Direct Takeover 接管能力的跨平台差异（canonical 在 `apps/cognitiveos-console/docs/agent-hub/`）。
...
- [prompts/console-agent-hub-direct-mode-product-design.md](prompts/console-agent-hub-direct-mode-product-design.md)：生成无 CognitiveOS 安全接管与完整治理两种部署下，PC/手机统一管理第三方 Agent 的多代理产品设计和开发任务编排提示词（**已执行**，canonical 见 `apps/cognitiveos-console/docs/agent-hub/`）。
- `prompts/agent-hub/`：Agent Hub 12 个宏车道 + 6 个 Tier 1 Adapter 的自包含接续提示词（全部 `blocked`，未满足 gate 前不得启动编码）。
...
- `apps/cognitiveos-console/docs/agent-hub/`：Agent Hub / 直连接管 canonical 文档（informative，`not-implemented`）；两部署模式、接管层级、Adapter 研究、威胁模型、决策与追踪。
