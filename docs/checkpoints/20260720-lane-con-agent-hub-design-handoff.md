...
>
> 集成说明：原分叉分支把 74→76 计数修正登记为 D-012，与远端 Lane-CFR canonical D-012 撞号；合入时由 D-019 记录编号冲突并对齐当前 61 schema / 84 vectors。本文件其余计数保留为当时快照。
...
- 计数漂移修正（集成后登记 D-019）：`AGENTS.md`、`docs/plan/DEVELOPMENT-PLAN.md`、findings-ledger IMP-17 三处 74→76，对齐当时 `check:consistency` 实测。
- 关联条目：`CONSOLE-AGENTHUB-V1-*`（产品 informative，不进 registry）；D-019（分支集成漂移）；IMP-01（规范表面冻结遵守）。
...
- 漂移：原 74→76 向量计数修正的编号冲突已在 D-019 闭合（closed-by-branch-integration）。
...
- 本次提交列表：batch1 `03250e1b3e3249d2cc600346f2861886a351a079`（Agent Hub canonical 文档 + Master plan + 车道计划 + 提示词）；batch2 `8e1de44150c3ded0a7561fd69a7cbf1f7df89ad4`（仓库联动 + 旧计数修正，集成改号 D-019）；batch3 本提交（handoff + 目录索引会话提示词）。
