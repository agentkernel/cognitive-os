...
- Content head: `f17f558` (registration head `b4d3b83`); closure head is this commit
- Reading baseline: `9fbd3904a1f8e0893fcb7d8d2b434e636d546e8c`; increment `9fbd390..6b637a5` read before authoring; implementation baseline `6b637a5194dc56b568d2be25bb9ceb3bb24a0f72` (recorded in `handbook/_meta/source-set.json`, 1,150 tracked entries, digest `sha256:714ab852…ad159b2`)
...
| Required Ubuntu/Windows CI on content head `f17f558` | run `31572262460`: every pre-existing step green (TS, Rust build/test/clippy/fmt, codegen drift, consistency, traceability); the new handbook step failed only on HB013 `git ls-tree` against the baseline object absent from the shallow CI clone — fixed in the closure head (HB013 skips digest reproducibility with a notice when the revision is unavailable; full clones still verify) |
| Required Ubuntu/Windows CI on the closure head | gates the merge via PR #202 checks (governance closure + HB013 shallow-clone fix over `f17f558`) |
