...
- Status at handoff: owner approved merge; governance exit complete and lease
  closed; final closure revision awaits required GitHub CI before merge
...
| Pre-closure PR HEAD | `28252a5b8734ab1f6db51056fe3b01b24d672bc6` |
...
| Worktree | closure metadata pending commit at this record |
| Lease | `lease/personal/P0-T01/checkpoint-delivery-governance`, closed in this PR |
...
| `git diff --check` | pass |
| PR #136 Ubuntu CI before closure commit | pass |
| PR #136 Windows CI before closure commit | superseded while in progress by closure commit |
...
- Single next action: push this closure commit, observe all required GitHub CI
  on its exact revision, then mark PR #136 ready and merge. The user explicitly
  approved merge; no further routine confirmation is required. Do not merge if
  any required check fails or remains pending.
