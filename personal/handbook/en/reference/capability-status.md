...
| Current Web UI / Console | partial | same-origin daemon-served `/ui/` Dual Track L1 (Today/Projects/Knowledge + Settings + rail) exists at `clients/pc/web/`; empty home is only-create (`#/projects/new` five-step wizard) on `main` (`P12-T02`); packets/HITL Confirm/ingest/connections/rail-write remain later Phase 12 cards, not complete IA; Linux 1.0 six-family remains Advanced/secondary; not Windows OPC |
