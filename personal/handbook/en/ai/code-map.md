...
| `crates/cognitive-store` | SQLite WAL adapter: migrations v1–v31 (+installation v1–v4), scheduler leases, Memory/Skill/Context/Artifact stores, Provider Control Plane (v25 labelled usage read), secret-excluding backup archives, hidden hosted DSH child (v31) | `sqlite/`, `migration.rs`, `personal_db.rs` (`prepare_personal_databases`), `scheduler.rs`, `provider_control_plane.rs`, `personal_backup.rs` |
