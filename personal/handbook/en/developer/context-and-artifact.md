...
correctly. RegisteredCheckRun success evidence is CAS-backed; pinned HTTPS
origins are a separate data-dir registry and never enter Artifact CAS. The Personal daemon now opens one process-lifetime instance at
...
enter `VERIFY -> CONTINUE`. A closed intermediate mutation that is not yet a
RegisteredCheckRun on a RegisteredCheck-terminated Task returns the Loop to
`DECIDE` without writing a verification report into CAS. P2-T14 re-reads every report Artifact URI before
