...
fingerprint: "sha256:8624e10485d3bd1b36247c169a7fa9d4b90dcc587e6a75e779eea602dc035a40"
non_claims:
...
Personal-private Project 聚合路由（`/management/project/v1/{list,detail,axis,roster,employee.catalog,pending-previews,preview-detail,draft.apply,preview.request,preview.reject,preview.narrow,confirm,standing-policies,standing-policy.create,standing-policy.revoke,roster.register,employee.seat.request,employee.seat.confirm,employee.runtime.bind,speech.candidate,conversation.append,conversation.archive,conversation.record,handoff.record,assistant.turn,assistant.status,dsh.hosted.start,dsh.hosted.observe-exit,vault.import,vault.index.rebuild,vault.index,vault.conflicts,vault.apply-authority,routine.revision,routine.trigger,routine.ledger,routine.checkpoint,routine.resume,capability.discover,capability.acquire,capability.compat-test,capability.rollback}`）需要 management bearer。它们投影 v26 `p11_*` Project 表、v27 Employee/Blueprint/Assignment/Grant 表、v28 `p11_conversation_archive`（标识 `cognitiveos.personal.conversation-archive/0.1`，不重解释 ADR-0058 `conversation-projection/0.1`）、v29 ApprovalPreview `superseded_by`（HITL reject/narrow）、v30 `grant-expansion` 与 StandingApprovalPolicy 时间盒（`expires_at` 必填、≤7 天；Settings 列表/撤销），v31 隐藏托管 DSH 子进程身份（`p11_hosted_dsh_child`；`dsh.hosted.start` 把 `runtime_binding_ref` 绑到 `hosted-dsh:<digest>:<child_id>`；Windows GNU isolated spawn 失败闭合；Windows OPC E2E 为 `not-run`；不是 Installed Agent chrome；Pi 不是 Member 执行引擎），v32 Markdown Vault（`p11_vault_document` / 可重建 `p11_vault_index_entry` / `p11_vault_conflict`，标识 `cognitiveos.personal.markdown-vault/0.1`；文件不是 Project 权威；Memory FTS 不是 Vault 索引；无冲突记录的 last-write-wins 被拒绝；宿主文件系统 E2E 为 `not-run`），以及 v33 Routine/Trigger（`p11_routine` / `p11_routine_revision` / `p11_routine_occurrence`，标识 `cognitiveos.personal.routine/0.1`；no-overlap-queue-latest；missed/coalesced 可见；复用 `scheduler_entries`；checkpoint 不是完成；无 Temporal；clock/sleep/restart E2E 为 `not-run`），不是 Task 行冒充，也不动 P7-T05 冻结 inventory。空列表无假按钮；未知费用字面量是 `unknown`，序列化里不出现 `0`。P14-T03 Dual Track Write Project：Owner 标题加 process 环的 confirm 铸造带 PlanRevision 轴的 `active` Project，列表/详情 `title_summary` 为该标题；空标题 / `unknown` / 空 process 拒绝。无 `process:` 节的 G1 仍铸造 `creating`。P14-T04 Dual Track write-join 把 PlanRevision `responsible_slot` 落在 process 环 id（`collect` / `analyze` / `draft`）；`rights=` 是 Owner 访问，不是花名册槽。轴上没有的槽返回 422；`/task/project/v1/roster.register` 仍为 403。空花名册使用 `authority_note: empty-roster`；已就位成员按 `employee_id` 列出。Blueprint 行无 Provider binding。白名单投递发言落档案行；owner `conversation.append` 写入 `note` 等档案 kind；chatter 仅审计。档案索引用 `limit` 1..=32 且只返回引用；`include_bodies` 与缺省 limit 失败闭合。单条正文走 `conversation.record`。档案行只是观察，不是完成。HITL confirm/reject/narrow 与 standing-policy 签发/撤销仅限 management；聊天/task 别名失败闭合（`PROJECT_AGGREGATE_CHANNEL_FORBIDDEN`），不能完成批准。stale 只按机械 `base_state_digest` 不等判定，不是墙钟新鲜度。`preview.request` 返回供画布使用的 `preview_digest`。P13-T10 Skill/MCP 获取复用 v27 InstallFact/Grant 与 v30 grant-expansion：management `capability.discover` / `.acquire` / `.compat-test` / `.rollback` 从不写 Grant；install 阶段 confirm 只写 InstallFact（`granted: false`）；聊天/task 别名为 403。供应链宿主 E2E 在 P13-T13 前记 `not-run`。这不是 Today 页，不是 Inbox 一级，也不是完整 `/ui/` IA。

隐藏 Pi Assistant 真实推理（P13-T03）：`POST assistant.turn` 不再回显客户端 payload。daemon 先受理该 turn（封闭 turn/object kind、typed owner 出处、default-deny 工具——ambient tool 在任何 Pi 进程存在前即 403），再从 daemon 事实解析 `agent://personal/pi` 的 Provider 绑定（P8-T13 binding，否则旧 `provider.json` + selected model 载体），按 T10 注入顺序组装有界 Context（`task-contract` 草稿事实 → `fixed-decision` Project 事实 → `sourced-excerpt` research 抓取 → `summary` 档案索引引用；超过 16 KiB 从尾部整层丢弃），经 `pi-agent-adapter assistant-turn` 真实运行 exact pinned Pi 一次（无工具、除 daemon 私有 completion provider 外无扩展、无 session），把 Pi 的唯一一次 completion 经一次性私有 completion socket 转发到 Provider，再把 Pi 最终文本解析为封闭候选对象链（`business-brief → research-run → charter/axis/roster/recipe`），每个字段都带 `sources` | `owner-stated` | `assistant-assumption` 出处。只有此时才经 v26 候选路径登记（research/propose 另发 preview 宣布）。所有拒绝都不登记：Provider 未绑 → 409 `ASSISTANT_PROVIDER_UNBOUND`（指路 Settings，`chat_input: false`）；本 daemon 未配置 Pi → 503 `ASSISTANT_PI_UNAVAILABLE`；adapter/Pi/proxy 失败 → 502 `ASSISTANT_INFERENCE_FAILED`；纯文本、空链、无出处字段、伪造 `sources[]` uri（既未抓取也非 owner 提供）、乱序/重复/未知 kind、封闭 schema 字段或零 Provider 往返 → 422（`ASSISTANT_CANDIDATE_REFUSED` / `PROJECT_INVALID`）。research turn 接受 `research_targets[]`（≤4）；每个目标先过既有 `HttpFetchReadOnly` 预校验器对 `task://personal/assistant-research` 的 pinned origins（默认为空，未 pin 的目标只报告在 `research.refused`，绝不抓取）。`GET assistant.status` 告诉创建页聊天可渲染什么：`ready`、`provider_unbound` 或 `pi_unavailable`。daemon 与 adapter 之间的帧是 `cognitiveos.personal.assistant-inference/0.1`，不携带 bearer、bootstrap、凭据、capability 或权威事实。Linux Pi 证据不资格化 Windows；Windows Pi 路由在 P13-T13 前保持 `not-run`。
