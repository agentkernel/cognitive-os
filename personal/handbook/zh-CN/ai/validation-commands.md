...
`DEV-LINUX-NATIVE-01`）。host/DSH 原生 E2E 走已指定的
`DEV-WINDOWS-NATIVE-OPC-01`（`Requires-environment` / `not-run` 直到 `P13-T13` 资格化）。
...
`P13-T02`–`T13` done + `P13-T13` 资格化本机 Windows 宿主。Phase 12 Dual Track UI 用
`DEV-WIN-GNU-01` TS 加 required CI。Phase 13 卡的验证路由见
`PERSONAL-TEST-ENVIRONMENTS.md` §5.2（Linux native 只关闭"实现已存在"，Windows
原生格由 P13-T13 回填）。本机宿主已指定（2026-09-05）；B01-W 尚未 provision；`B01-DESKTOP-002` 不是
2.0 日常默认机。本机 cargo、WSL、ordinary CI 与 Canvas 都不能替代
Gate/release；`P13-T13` 执行前 native cell 记 `not-run`。`not-run` 永远不是 pass。
