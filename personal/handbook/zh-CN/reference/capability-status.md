...
| 当前 Web UI / Console | partial | daemon 同源 `/ui/` Dual Track L1（Today/Projects/Knowledge + Settings + 右栏）位于 `clients/pc/web/`；空 Home 只创建（`#/projects/new` 五段向导）已在 `main`（`P12-T02`）；Project 四子菜单（`#/projects/:id` 及 members/runs/outputs）已在 `main`（`P12-T03`）；先选后配 + 加成员已在 `main`（`P12-T04`）；Today 决策包（未验收只继续创建 / 已上线才 pending-previews）为 `P12-T05` done pending merge；HITL Confirm/ingest/连接表/右栏写画布仍是后续 Phase 12 卡，不是完整 IA；Linux 1.0 六族仍在高级/二级；不是 Windows OPC |
