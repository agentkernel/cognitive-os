...
- Status: **informative** judgement sheet / 非实现 / 非 Gate. Written by
  `P13-T12/D01`; **filled in by `P13-T12/D02`** on one exact daemon `/ui/`
  revision. Until D02 runs, every judgement in this file is `not-run`.
...
|---|---|---|---|---|---|---|---|---|---|
| `today` | `not-run` | `not-run` | `not-run` | `not-run` | `not-run` | `not-run` | `not-run` | `not-run` | `not-run` |
| `create` | `not-run` | `not-run` | `not-run` | `not-run` | `not-run` | `not-run` | `not-run` | `not-run` | `not-run` |
| `projects` | `not-run` | `not-run` | `not-run` | `not-run` | `not-run` | `not-run` | `not-run` | `not-run` | `not-run` |
| `members` | `not-run` | `not-run` | `not-run` | `not-run` | `not-run` | `not-run` | `not-run` | `not-run` | `not-run` |
| `runs` | `not-run` | `not-run` | `not-run` | `not-run` | `not-run` | `not-run` | `not-run` | `not-run` | `not-run` |
| `outputs` | `not-run` | `not-run` | `not-run` | `not-run` | `not-run` | `not-run` | `not-run` | `not-run` | `not-run` |
| `hitl` | `not-run` | `not-run` | `not-run` | `not-run` | `not-run` | `not-run` | `not-run` | `not-run` | `not-run` |
| `knowledge` | `not-run` | `not-run` | `not-run` | `not-run` | `not-run` | `not-run` | `not-run` | `not-run` | `not-run` |
| `settings` | `not-run` | `not-run` | `not-run` | `not-run` | `not-run` | `not-run` | `not-run` | `not-run` | `not-run` |
...
|---|---|---|---|---|---|---|
| `today` | `not-run` | `not-run` | `not-run` | `not-run` | `not-run` | `not-run` |
| `create` (wizard tablist, step dots, confirm dialogs) | `not-run` | `not-run` | `not-run` | `not-run` | `not-run` | `not-run` |
...
| `knowledge` (segmented tabs, filters, form) | `not-run` | `not-run` | `not-run` | `not-run` | `not-run` | `not-run` |
| `settings` (select, password, disclosure) | `not-run` | `not-run` | `not-run` | `not-run` | `not-run` | `not-run` |
| shell | S1 `not-run` | S2 `not-run` | S3 `not-run` | — | — | — |
...
|---|---|---|---|---|
| `today` | `not-run` | `not-run` | `not-run` | `not-run` |
| `create` | `not-run` | `not-run` | `not-run` | `not-run` |
| `projects` | `not-run` | `not-run` | `not-run` | `not-run` |
...
| `knowledge` | `not-run` | `not-run` | `not-run` | `not-run` |
| `settings` | `not-run` | `not-run` | `not-run` | `not-run` |
...
| `knowledge` | `not-run` | `not-run` | `not-run` | `not-run` |
| `settings` | `not-run` | `not-run` | `not-run` | `not-run` |
| shell (strip, nav, rail, palette, dialog) | `not-run` | `not-run` | `not-run` | `not-run` |
...
| Field | Value |
|---|---|
| Exact `/ui/` revision (pushed SHA) | — |
| Guest daemon environment | — (`DEV-LINUX-NATIVE-01` expected) |
| Host browser + version (registered host) | — |
| NVDA version | — |
...
Spec-level drift is [spec §13](personal-2.0-opc-visual-ui-spec.md) (a–l).
...
## 9. Summary counters (D01 baseline) / 计数

| Grid | Cells | pass | fail | partial | not-run | not_available |
|---|---:|---:|---:|---:|---:|---:|
| §1 modules | 19 | 0 | 0 | 0 | 19 | 0 |
| §2 nine × nine | 81 | 0 | 0 | 0 | 81 | 0 |
| §3 keyboard / focus | 57 | 0 | 0 | 0 | 57 | 0 |
| §4 200 % / narrow | 36 | 0 | 0 | 0 | 36 | 0 |
| §5 themes | 40 | 0 | 0 | 0 | 40 | 0 |
| §6 NVDA paths | 10 | 0 | 0 | 0 | 10 | 0 |

End of checklist. Informative only. Canvas v9 ≠ product. Every judgement
above is `not-run` until `P13-T12/D02` records it on an exact revision.
