...
| [Personal 2.0.0 OPC Visual UI specification](personal-2.0-opc-visual-ui-spec.md) | `P13-T12/D01`: Apple-led visual rules for daemon `/ui/` against the existing `tokens.css` names — typography, spacing, light/dark/high-contrast roles, locked three columns with narrow horizontal scroll, 200%, focus/keyboard, motion, nine states × nine surfaces, component states, proposed tokens, drift observed; IA unchanged; not a second design system; informative |
| [Frozen v9 ↔ `/ui/` comparison checklist](personal-2.0-opc-v9-ui-comparison-checklist.md) | `P13-T12/D01`: 19/19 maintenance-index modules mapped to v9 scenes, `/ui/` routes/selectors, daemon facts and owning cards, plus the D02 cell grids (nine × nine, keyboard/focus, 200%/narrow, themes, NVDA); every judgement `not-run` until `P13-T12/D02` |
