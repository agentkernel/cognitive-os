...
fn candidate_prompt(request: &DaemonCandidateRequest) -> String {
    format!(
        "Call exactly one of the CognitiveOS WorkspaceRead, WorkspaceSearch, WorkspaceWrite, or WorkspacePatch tools. WorkspaceRead arguments are target (string). WorkspaceSearch arguments are query (string) and target (string). WorkspaceWrite and WorkspacePatch arguments are input_b64 (canonical base64), preimage (\"absent\" or digest:sha256:<64 lowercase hex>), and target (string). Do not call bash, edit, write, read, or any other Pi built-in. A JSON object with only tool_ref, action, target, parameters, parameters_digest, expected_state_version, and operation_descriptor_id remains an accepted fallback; parameters_digest may be omitted because the adapter recomputes it from parameters. WorkspaceRead carries no parameter object. The only parameter objects are {{\"family\":\"WorkspaceSearch\",\"query\":string}}, {{\"family\":\"WorkspaceWrite\",\"input_b64\":canonical_base64,\"preimage\":\"absent\" or \"digest:sha256:<64 lowercase hex>\"}}, and {{\"family\":\"WorkspacePatch\",\"input_b64\":canonical_base64,\"preimage\":\"absent\" or \"digest:sha256:<64 lowercase hex>\"}}. Context follows:\n{}",
