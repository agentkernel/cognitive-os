...
- [9. Context Engineering：Context Virtual Memory 与 Resolution](#9-context-engineeringcontext-virtual-memory-与-resolution)（`context-resolution-protocol`、`context-engineering`、`context-virtualization-profile`）
  - [9.9 Cognitive Resource Namespace 与多轮解析](#99-cognitive-resource-namespace-与多轮解析)
- [10. Harness 与 Loop Engineering](#10-harness-与-loop-engineering)
- [11. AKP：Agent Kernel Protocol](#11-akpagent-kernel-protocol)
- [12. OperationDescriptor 与 AuthorizationCapability](#12-operationdescriptor-与-authorizationcapability)（`capability-protocol`、`core-effect-protocol`）
  - [12.7 Operation Catalog Service](#127-operation-catalog-service) · [12.8 Intelligent Shell 的短期特权管理会话](#128-intelligent-shell-的短期特权管理会话)
...
## 12. OperationDescriptor 与 AuthorizationCapability
### 12.1 两个经常混淆的概念
...
### 决策二十二：Shell 是参考客户端
Shell 可调用 SMS 和 Management API，但不承载全局 memory、catalog、调度、capability authority 或 commit；无 Shell、无模型时管理面仍可运行。
---
## 24. 结论
