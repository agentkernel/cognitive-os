...
3. **凭证与回复截获**：聊天流中输入的密码会进入 Agent 进程、会话存储与通道服务商；静态口令可重放且无法绑定 proposal（NIST SP 800-63B 将记忆型秘密列为最弱验证器）。
...
33. `[STD]` IETF, **draft-oauth-ai-agents-on-behalf-of-user**（Draft，act 链委派）：https://datatracker.ietf.org/doc/draft-oauth-ai-agents-on-behalf-of-user/
34. `[STD]` SPIFFE 与 IETF WIMSE（工作负载身份）：https://spiffe.io/ 、https://datatracker.ietf.org/wg/wimse/about/
35. `[STD]` IETF, **RFC 8693 — OAuth 2.0 Token Exchange**：https://www.rfc-editor.org/rfc/rfc8693
36. `[STD]` NIST, **SP 800-63B — Digital Identity Guidelines: Authentication**：https://pages.nist.gov/800-63-3/sp800-63b.html
37. `[STD]` CISA, **Implementing Number Matching in MFA Applications / Phishing-Resistant MFA**：https://www.cisa.gov/resources-tools/resources/implementing-number-matching-mfa-applications
38. `[STD]` MITRE ATT&CK, **T1621 — Multi-Factor Authentication Request Generation**：https://attack.mitre.org/techniques/T1621/
39. `[STD]` EBA, **PSD2 RTS on Strong Customer Authentication（动态链接）**, Regulation (EU) 2018/389：https://eur-lex.europa.eu/legal-content/EN/TXT/?uri=CELEX%3A32018R0389
