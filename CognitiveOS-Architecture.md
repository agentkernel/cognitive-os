...
v1.0.0 是本白皮书的**架构基线定稿**：在 v0.8 基础上完成一轮独立架构评审（第一性推导核验、OpenClaw/Hermes 类成熟 Agent 兼容负载分析、审批协议评价、性能基建研判）并采纳其结论（IMP-01–18，追踪见仓库评审结论文档）。本版核心增改：确定性渲染与前缀稳定成为显式要求（§9.3/§9.4）；记忆/知识准入异步化与 read-your-write 工作集语义（§9.5/§18.3）；治理开销自身纳入性能契约（§19.4 Governance overhead 指标族）；新增审批分发与分级确认子系统（§12.12、决策二十五）；R0 最小合法实现形态（§20.5）；验证带宽升格为部署一级约束（§2.2、§19.7）；带外修改对账与已登记批量代理语义（§5.3）；路线图写入实现优先与规范表面冻结原则（§21）；附录 A 重构为最小核心对象集 + 扩展集 + Context/Binding/Scope 判定表；附录 C 补全治理与安全证据链。§14 的 CIM 实现细节下沉至 Heterogeneous companion。

**定稿不等于实现**：1.0.0 冻结的是架构语义与评审闭环；全部 companion/schema/registry/向量仍为 Draft 且未经实现验证，任何能力声明以机器可读 manifest 为准。在参考实现跑通现有声明式向量之前，规范表面按 §21 原则冻结。
...
8. **性能与治理不是两套基建**：记忆/工具调取准确度、任务完成准确率、token 经济性与多 Agent 协作的承载对象就是治理层定义的同一批对象；性能必须从受治理表面内部长出来，不允许为性能开绕过治理面的旁路。
...
**实现验证状态**：截至本版本，上表全部 companion 及其 REQ/schema/向量均处于"已登记、未经实现验证"状态——仓库尚无参考实现或可执行符合性 runner，任何条目都不得被引用为实现证据。该维度独立于 companion 自身的版本/状态字段；单节点参考实现跑通现有声明式向量之前，规范表面按 §21 路线图原则冻结，仅接受实现反馈或安全事故驱动的修正型变更。
...
其中**验证带宽是部署的一级约束**：完成语义依赖独立 verifier 对固定后态给出证据（§6.3、§10.8），可确定验证的域（代码+测试、交易+对账）收益极高；开放式知识工作缺确定性 verifier，完成判定退化为人工验收，而人审吞吐与疲劳直接限制系统能安全承载的任务量。架构的采用价值分布由目标域的可验证性决定——部署方应在 Readiness Case（§19.7）中声明验收吞吐预算（人审容量、verifier 容量）并据此选择任务域，而不是假设验证带宽无限。
...
| 审批服务（§12.12） | 审批请求排队、分发、去重、限流、分级确认与审批指标 | capability 签发（属 authority）、代替确认人决定、绕过风险分级 | Core 服务面；R0 可降级为策略自动批准 |
| SMS/CRB | 候选解释、匹配、排序、预算内资源建议 | hard gate、事实、授权、验收 | 可选 Profile |
| Runtime/Operation | 模型计算、工具与技能执行 | 用户意图解释权、最终提交 | Core adapter 或原生 |

最小可用 CognitiveOS 闭环是：意图固定、TaskContract、持久执行身份、状态/事件、Context 门禁、Operation/Capability、Intent/Effect、验证/验收、watch 与恢复。多 Agent、SMS/CRB、CIM、受控学习和具身安全不是所有部署的 Core 前提；只有声明相应 Profile 时才进入符合性边界。低风险档位（R0）下这些组件存在合法的降级实现形态，实现者无需为个人助理/检索场景实例化全部十个组件——最小合法映射见 §20.5"R0 最小合法实现形态"。
...
需要正视的生态现状：对成熟 Agent 负载的兼容分析表明，现实落点是 C0/C1——C2 所需的 checkpoint、cancel 与 pending-effect 暴露在业界普遍缺失，且没有外部自然动力促使 Agent 作者补齐。部署方可以用策略性激励换取 C2 采纳（informative 建议）：对达到 C2 的安装授予更高自动化配额、更低审批频度或更长无人值守窗口——这些激励只能收窄治理成本，不得放宽 R0—R3 门禁本身。
...
两个常见负载形态需要显式语义而不是一概拒绝：

- **带外修改（file-as-truth）**：部分成熟 Agent 允许用户直接手改其记忆/配置文件。C1 适配后这些文件成为受治理对象的投影，带外编辑必须按"外部观察"处理：检测（digest 漂移）→ ingest 为候选 → 按策略准入或标记冲突；任何一侧都不得被静默覆盖（`REQ-AGENT-OOB-001`，详见 agent-compatibility companion §4）。
- **已登记批量代理**：脚本经 RPC 批量调用工具是高效且应保留的模式。其合法形态是经**已登记的 proxy 端点**（即 Tool Adapter 自身暴露的批量接口）：逐调用保留授权判定与审计，批量性只体现在传输与上下文成本上，不体现在门禁豁免上；未登记的 tool proxy 仍属绕过，安装测试必须拒绝。
...
澄清义务允许按风险档裁剪（见 §20.5）：R0 与可逆的 R1 场景下，实现可采用"默认解释 + 事后可纠正"策略——把逐次澄清收敛为预览义务与低摩擦撤销，避免消费级体验被澄清仪式淹没；个人场景中验收 authority 常塌缩回用户本人，这是合法形态。该放宽不适用于高风险、不可逆或跨信任边界的操作，也不改变"解释是候选、原文不可覆盖"的对象语义。
---
## 7. 认知微内核
...
8. `render` 按 target profile 形成结构化输入；渲染策略不能扩大数据 audience。渲染必须是确定性的：相同输入对象版本、相同 target profile、相同 renderer 版本下，输出字节级一致；分区顺序与分区内条目排序必须稳定，新增无关对象不得改变已有条目的相对顺序。前缀稳定性是下游推理缓存（供应商 prompt/KV cache）能否跨轮复用的前提，实现不得以逐 Activity 重排渲染的方式破坏它（`REQ-CTX-012`）。
...
该绑定语义的设计意图包含性能承诺的另一半：同一 Conversation 内绑定均未变化时，实现应保持渲染前缀稳定以维持供应商推理缓存命中（§9.3 第 8 阶段）。兼容负载分析表明，治理门禁的机械开销相对模型推理可忽略，而缓存失效会造成数倍的成本与首 token 延迟退化——缓存绑定用于隔离安全边界，不是逐 Activity 重排渲染的许可。
...
同一幂等键携带不同参数是必须拒绝的冲突（`EFFECT_IDEMPOTENCY_CONFLICT`），不得去重为同一 Effect 或静默执行为新动作；timeout 后重试不得更换幂等键（Core `REQ-EFF-002`，向量 EFF-IDEM-CONFLICT-001）。
...
§2.2 把验证带宽（含审批拥塞）列为稀缺资源；本节给出对应的机制设计。审批是 **OS 级服务**而不是各 Agent/Shell 的私有交互：集中审批队列才能实现同类请求批处理与去重、限流、按风险路由、审批健康指标（§19.4）与超时升级。审批请求（proposal 引用、风险档、期限、发起链）可以分发到用户所在的任意通道——包括插入 Agent 会话流——但**通知面与确认面必须区分强度**：对话通道适合作为通知面与低风险确认面；作为高风险审批的完成面则同时踩中三个已实证的攻击面：

1. **主持人伪造**：同一聊天窗口内"OS 主持"只是文本声明，被注入的 Agent 可以渲染像素级相同的假审批对话（Windows UAC secure desktop / secure attention sequence 防御的正是此类界面伪造）；
2. **审批疲劳轰炸**：高频重复推送配合社会工程劝诱批准（MITRE ATT&CK T1621 的 MFA fatigue 形态）；
3. **凭证与回复截获**：聊天流中输入的密码会进入 Agent 进程、会话存储与通道服务商；静态口令可重放且无法绑定 proposal（NIST SP 800-63B-4 将记忆型秘密列为最弱验证器）。

自然语言在审批中承担**协商与解释层**：用户可询问、要求解释、修改提案范围——修改产生带 `supersedes` 的收窄新 proposal 并重新进入审批；**批准行为本身**必须是对固定 proposal digest 的结构化动作，与 §12.3"自然语言批准可以触发 authority 签发 capability，自然语言本身不是 capability"一致。分级确认矩阵如下（与 §20.5 风险分级对应）：

| 风险档 | 通知面 | 确认面与凭证 |
|---|---|---|
| R0 | 不打扰，事后可查 | 策略自动批准 |
| R1 | 插入 Agent 对话流，由 **OS 独立通道身份**发出 | 聊天内结构化确认：回显 proposal digest 短码（number matching 的对话版），一次性、限时、精确匹配 |
| R2 | 对话流仅通知 + 深链 | 离开对话通道：Shell 可信表面或系统级确认面，以 passkey/FIDO2 类凭证对 proposal digest 签名（PSD2 动态链接原则：凭证密码学绑定审批内容） |
| R3 | 同 R2 | 独立 principal 双人审批 + step-up 认证 |

机制约束：OS 必须持有**独立于被审 Agent 的通道身份**（独立 bot/号码/进程边界），审批卡片的真实性由被审对象无法预知的一次性挑战或不可写入的渲染表面建立；聊天内审批不建立常驻 `PrivilegedManagementSession`，每次确认对应一次性的 `ManagementApprovalDecision`（§12.8）；防疲劳机制包括同类请求聚合、每来源频控、重复重发限流与连续拒绝升级告警。审批延迟、吞吐、橡皮图章率与拒绝后重试率按 §19.4 Governance overhead 指标族报告。本节的分级矩阵与通道要求为架构设计基线，其 REQ/向量登记按规范变更流程另行提交（遵循 §21 冻结原则的修正型通道）。
---
## 13. 慢认知、技能与硬实时回路
### 13.1 三时间尺度
...
| 提交路径存储失败（磁盘满、只读、日志不可写） | 受治理写入 fail closed（`STATE_STORE_UNAVAILABLE`），停止新 governed dispatch，保持只读检查与确定性停止/撤销 | 已提交历史的完整性、未决 Effect 清单 | 内存中未落盘的"提交"；存储恢复前的新写入结果 |
...
提交路径本身的持久化失败（数据库只读、磁盘满、日志设备不可写）是独立于消费者背压的故障类别：受治理写入必须 fail closed 并返回 `STATE_STORE_UNAVAILABLE`，不得把仅存于内存的缓冲当作已提交状态，也不得在 Intent 未持久化时分派新的 governed_external 执行；只读检查、确定性停止/撤销与审计导出路径必须保持可用，存储恢复后按 §16.6 恢复顺序重入（规范条文见 Core companion `REQ-REC-003`，向量 STATE-STORE-DEGRADE-001）。
### 16.6 恢复顺序
典型恢复流程：
...
#### Effect 门禁与验证
先持久 Intent，再授权、执行、对账、验证和提交。 未知结果隔离，补偿独立授权。 高风险审批的确认面必须渲染在被审对象不可写入的表面，或使用被审对象无法预知的一次性挑战；确认凭证绑定固定 proposal digest（§12.12），防疲劳限流与连续拒绝告警属于同一道防线。
#### 审计与不可抵赖证据
...
命名操作为：`Ingest(evidence, profile)` 建立隔离候选和谱系；`Query(request)` 在对象级授权、freshness 与 conflict policy 后返回 claim；`Lint(candidate, profile)` 执行四层检查：deterministic lint（schema、digest、引用闭合）、policy lint（scope、purpose、许可、retention）、semantic lint（主张-证据蕴含、矛盾、时效）、security lint（注入、恶意链接、隐写、秘密、跨域与持久投毒）。语义模型只能提出 lint 发现，最终 admission/publish gate 由确定性策略和 authority 控制。与 §9.5 的工作集语义一致，ingest 隔离区中的候选对其提交者立即可见（read-your-write），lint 与准入可以异步进行；隔离区内容不得跨 scope 泄露，也不进入其他主体的生产 ContextView。
...
**R0 最小合法实现形态**：若低风险档位的实现成本远高于朴素方案（文件 + git + 进度日志式 harness），实践者将绕开规范——安全规范被绕开的主因从来是成本。R0 部署允许下列降级映射，且无需阅读全部 Core 细节即可据此构建：

| 组件 | R0 合法降级形态 |
|---|---|
| 事件日志 | 本地追加式文件（保留顺序与不可原地修改语义） |
| Context 门禁 | 确定性选择（固定白名单/规则），无需语义检索与 SMS |
| TaskContract | 静态模板（按任务类别预定义目标/边界/出口） |
| 验收 | 按策略自动化（用户本人即验收 authority，事后可纠正） |
| Checkpoint | 进度文件 + 已消费事件位置 |
| 审批 | 策略自动批准，事后可查（§12.12 R0 行） |

以下边界**即使在 R0 也不可降级**：tenant/scope 隔离与检索前过滤（存在多主体数据时）；注入内容不得提升为控制（§9.2 分区语义）；缓存按授权绑定隔离；无外部写入（R0 定义本身）。降级形态必须在 profile manifest 中如实声明，不得包装成完整 Core 符合性。
---
## 21. 路线图
### Phase 0：对象与治理基线
...
### Phase 4：生命周期与恢复适配
交付 C2、checkpoint、pending Effect、unknown outcome、Conversation 切换和恢复负例；C3 保持为原生集成目标。同期交付 Effect/恢复/fencing 状态机子集的 TLA+ 或 Alloy 形式模型与不变量证明——该子集状态空间小、安全收益大，形式模型可作为 §19.5 第 3、5 层的补充证据；不承诺全系统形式化。需要正视的现状是：成熟 Agent 生态普遍缺失 checkpoint/cancel/pending-effect 暴露，C2 采纳没有外部自然动力，路线图不得隐含"生态自然到位"假设（采纳激励见 §5.2）。
### Phase 5：分布式与多 Agent
...
**实现优先与规范表面冻结**：在单节点 R0/R1 参考实现交付并跑通仓库现有声明式向量之前，冻结新对象族、新 Profile 与新 REQ 域的引入；仅允许实现反馈或安全事故证据驱动的修正型规范变更（既有域内的修正、澄清与登记不受限）。版本史 0.2→0.8 每版新增整个子系统、且 v0.4 曾纠正四项过度设计的事实表明，规范表面积增速超过验证速度是已发生的真实风险；后续版本说明中新增规范对象须引用实现反馈或安全事故证据。
---
## 22. 开放研究问题
...
它以虚拟化、保护、并发、持久性、资源管理、设备中介和故障语义对应严格 OS 判据（§3.1）。该命题当前在规范层成立、待实现证明：判据 9（可观察一致性）与判据 10（参考监视器不可绕过）的最终证据是行为测试，在参考实现与宿主级中介证据交付前，OS 定位是架构命题而非已验证能力。 它不以“包含 LLM”作为 OS 依据。
### 决策二：LLM 是协处理器
...
### 决策二十五：审批是 OS 服务
审批请求可全通道分发（含插入 Agent 对话流），确认强度按 R0—R3 风险分级；自然语言可协商、解释和收窄提案，但不构成批准行为本身——批准是对固定 proposal digest 的结构化、一次性动作，高风险确认面必须离开被审对象可写入的通道。
---
## 24. 结论
...
全文约 80 个命名对象。为避免实现者混用，本附录分为两层：**A.1 最小核心对象集**是任何 Core 实现都必须区分的对象宪法（14 项）；**A.2 扩展对象集**按主题/Profile 归组，仅在声明相应能力时进入符合性边界；**A.3** 为最易混淆的 Context/Binding/Scope 族提供"何时用哪个"判定表。
...
- **Task**：一个以 TaskContract 为契约、由 acceptance authority 判定完成的受治理目标单元（生命周期见 §6.3）。
- **TaskContract**：目标、约束、验收、预算、等待和出口的版本化契约。
- **Intent**：在执行前持久化的受治理副作用提案。
- **Effect**：Intent 的执行、回执、对账、验证和提交记录。
- **Event**：不可变的已发生事实通知，构成追加式提交历史；不是命令，也不取代权威状态。
- **StateSnapshot**：某状态域固定版本的不可变读视图。
- **ContextRequest / ContextView**：按目的、视角、预算、权限和新鲜度请求上下文的对象，及其解析产物——短期、固定版本、非权威的工作视图。
- **AuthorizationCapability**：可验证、可衰减、可撤销的本地授权对象。
- **OperationDescriptor**：操作接口、语义、效果、资源和验证特征的描述；描述不是权限。
- **Checkpoint**：通用恢复封装，可承载 LoopCheckpoint 等 payload。

### A.2 扩展对象集（按主题/Profile 归组）
...
**意图与 Agent Shell（Core / Intelligent Shell Profile）**：
...
**Context Engineering 扩展（Core / Cognitive Discovery）**：
...
**安装、目录与语义中介（Agent Compatibility / Operation Catalog / Semantic Mediation）**：
...
**知识、记忆与学习（RFC-0001 / Governed Memory / Learning）**：
...
**资源与评估（Heterogeneous / Conformance）**：
...
### A.3 Context/Binding/Scope 族判定表

该族语义区分细微、最易混用。判定规则：先问"这个对象回答什么问题"，再选类型。

| 你要表达的事实 | 应使用的对象 | 不要误用 |
|---|---|---|
| 这次判定发生在哪个租户、哪套策略/成员/撤销版本下 | TenantContext | 不要用 Tenant 字符串代替版本化上下文 |
| 该对象属于 tenant 域还是 platform 域 | GovernanceDomainContext（含 `scope_domain`） | 不要让 platform 对象隐含 tenant 正文权限 |
| 这个逻辑执行身份长期绑定了谁、哪条委派链 | ExecutionContext（AgentExecution 级） | 不要把单次活动的窄化绑定塞进执行级 |
| 这一次工作单元允许的 purpose/scope/预算/ContextView | ActivityContext | 不要复用上一 Activity 的绑定 |
| 执行与某个 Conversation 及其参与关系的关联 | ConversationBinding | 不要用"最近对话"隐式继承 |
| 执行身份与治理上下文/版本/fencing 的固定关系 | AgentExecutionBinding | 不要以进程/连接身份代替 |
| 对象可见、派生、保留、晋升的边界 | ResourceScope | 不要与授权（capability）混同 |
| 把受限访问显式授予另一主体或另一 tenant | ShareGrant | 不要直接复用远端 capability 或放宽原 scope |
---
## 附录 B：规范套件导航
| 主题 | 规范入口 | 版本 | 状态 | 主文档关系 |
|---|---|---|---|---|
...
## 附录 C：参考资料与证据等级
### C.1 证据等级
`[STD]`：已发布标准或官方稳定规范；`[DRAFT]`：标准组织在议草案，内容可能实质变化；`[REG]`：具有法律约束力的法规/监管技术标准；`[GUIDE]`：官方指南、框架或威胁知识库，非规范性标准；`[PEER]`：同行评审研究；`[PRE]`：预印本，结论仍待更广验证；`[IND]`：厂商或开源项目工程经验；`[HYP]`：本文提出、尚待系统实验验证的架构假设。
预印本、厂商经验和单一硬件论文不构成普遍性能保证。 任何数字都应在其任务、模型、设备、软件栈和实验条件内解读。草案、指南与法规不得被当作同等稳定度的互操作标准引用。
### C.2 标准与协议
...
6. `[GUIDE]` NIST, **AI Risk Management Framework 1.0**（自愿性框架）：https://www.nist.gov/itl/ai-risk-management-framework
...
### C.5 治理、安全与系统基础（1.0.0 增补）
本组条目由架构评审（IMP-10）补录，分别支撑注入防御（§9.2、§17.3）、双内核（§4.2、§13）、CVM（§9）、微内核最小性（§7）、授权模型（§12.3）、租约与 fencing（§16.3）、身份联邦（§6.2）、审批确认强度（§12.12）与 Effect 协议（§12.5、§16）。
23. `[PRE]` Debenedetti et al., **Defeating Prompt Injections by Design**（CaMeL）, arXiv:2503.18813：https://arxiv.org/abs/2503.18813
24. `[PEER]` Sha, **Using Simplicity to Control Complexity**（Simplex 架构）, IEEE Software 18(4), 2001：https://doi.org/10.1109/MS.2001.936213
25. `[PRE]` Packer et al., **MemGPT: Towards LLMs as Operating Systems**, arXiv:2310.08560：https://arxiv.org/abs/2310.08560
26. `[PRE]` Sumers et al., **Cognitive Architectures for Language Agents**（CoALA）, arXiv:2309.02427：https://arxiv.org/abs/2309.02427
27. `[PEER]` Klein et al., **seL4: Formal Verification of an OS Kernel**, CACM 53(6), 2010：https://doi.org/10.1145/1743546.1743574
28. `[PEER]` Heiser & Elphinstone, **L4 Microkernels: The Lessons from 20 Years of Research and Deployment**, ACM TOCS 34(1), 2016：https://doi.org/10.1145/2893177
29. `[PEER]` Pang et al., **Zanzibar: Google's Consistent, Global Authorization System**, USENIX ATC 2019：https://www.usenix.org/conference/atc19/presentation/pang
30. `[IND]` AWS, **Cedar policy language**：https://www.cedarpolicy.com/
31. `[PEER]` Gray & Cheriton, **Leases: An Efficient Fault-Tolerant Mechanism for Distributed File Cache Consistency**, SOSP 1989：https://doi.org/10.1145/74850.74870
32. `[IND]` Kleppmann, **How to do distributed locking**（fencing token 论述）：https://martin.kleppmann.com/2016/02/08/how-to-do-distributed-locking.html
33. `[DRAFT]` IETF, **draft-oauth-ai-agents-on-behalf-of-user**（在议草案，act 链委派）：https://datatracker.ietf.org/doc/draft-oauth-ai-agents-on-behalf-of-user/
34. `[STD]`/`[DRAFT]` SPIFFE（已发布规范）与 IETF WIMSE（工作组在议，工作负载身份）：https://spiffe.io/ 、https://datatracker.ietf.org/wg/wimse/about/
35. `[STD]` IETF, **RFC 8693 — OAuth 2.0 Token Exchange**：https://www.rfc-editor.org/rfc/rfc8693
36. `[STD]` NIST, **SP 800-63B-4 — Digital Identity Guidelines: Authentication and Authenticator Management**（2025 年第 4 修订版，取代旧 SP 800-63B）：https://csrc.nist.gov/pubs/sp/800/63/B/4/final
37. `[GUIDE]` CISA, **Implementing Number Matching in MFA Applications / Phishing-Resistant MFA**（安全指南）：https://www.cisa.gov/resources-tools/resources/implementing-number-matching-mfa-applications
38. `[GUIDE]` MITRE ATT&CK, **T1621 — Multi-Factor Authentication Request Generation**（威胁知识库条目）：https://attack.mitre.org/techniques/T1621/
39. `[REG]` EBA, **PSD2 RTS on Strong Customer Authentication（动态链接）**, Regulation (EU) 2018/389（欧盟监管技术标准）：https://eur-lex.europa.eu/legal-content/EN/TXT/?uri=CELEX%3A32018R0389
40. `[IND]` Temporal, **Durable execution / saga / idempotency 文档**：https://docs.temporal.io/
41. `[IND]` HumanLayer（全通道人审 API）与 Teleport 聊天审批插件：https://humanlayer.dev/ 、https://goteleport.com/docs/
### C.6 阅读原则
...
### 1.0.1（证据闭环修订）
本版由第二轮独立架构审查驱动（审查报告见 [CognitiveOS-Review-Conclusions.md](./CognitiveOS-Review-Conclusions.md) v2.0），在 1.0.0 架构语义不变的前提下修复五类已证实缺口。本版不新增对象族、Profile 或 REQ 域，符合 §21 冻结条款（既有域内的修正、澄清与登记）。
...
5. **规范权威落点修正（P1）**：18 条 REQ 的 `owner_spec` 从本 informative 白皮书锚点（及 1 条 schema 落点）改指 normative companion 承载文本：REQ-STATE-003/004、REQ-CTX-004/008、REQ-CAP-002/003、REQ-EFF-004/006、REQ-RES-001、REQ-SEC-002、REQ-PROFILE-CORE/CVM-001 → Core companion；REQ-PROFILE-DIST/EMB/LEARN/HET-001 → 各自 Profile companion；REQ-PERF-002/004 → 符合性索引（承接规范条文）。旧→新映射：白皮书锚点保留为主题检索入口，规范义务文本以 companion 为唯一权威（形状/行为分离，见 normative-source-and-versioning §2）。heterogeneous/learning companion 补未登记错误名免责行；semantic-mediation companion 补机器 schema 链接。
6. **机器合同封闭性加固（第二批，采纳独立审查 F-004/005/006/007/009/010/012/016/018/019/021/022/024 并逐条复核）**：effect schema 增加条件闭包（`COMMITTED` 必须 `succeeded+passed+commit`、`OUTCOME_UNKNOWN`/`VERIFY_FAILED`/`COMPENSATING`/`QUARANTINED` 的合法组合封闭，非法组合不再 schema-valid）；effect 迁移表升 v0.2——dispatch 与 commit 增加 `capability_and_revocation_current` guard（撤销竞态闭合）、dispatch 增加 `intent_durably_persisted`、`VERIFIED/VERIFY_FAILED→ABORTED` 要求 `residual_effect_disposition`（已执行外部效果的残留处置显式化）；ActivityContext 改两阶段创建（去除 `context_view_ref` 必填，绑定经 CAS 恰好一次，消除 ContextView↔ActivityContext 循环依赖，REQ-GOBJ-BIND-004 条文同步）；TaskContract 的 `allowed_state_domains` 与开放状态域注册表对齐（不再闭合四值枚举）并要求至少一条 acceptance 条件；ContextView loaded item 增加 trust/role 条件约束（`untrusted` 不得渲染为 `control`/`authoritative_state`）；MemoryCandidate 增加可选 `pending_visibility`（read-your-write 的机器表达）；LoopCheckpoint 增加可选 `event_high_watermark`/`fencing_epoch`/`pending_effects`/`pinned_environment_versions`（安全恢复所需稳定事实的机器表达）；REQ-CTX-012 补前缀段字节稳定语义（相对顺序不变不足以保持供应商累计前缀缓存），CTX-RENDER-001 升 v0.2；登记 `REQ-GOBJ-REF-004`（digest 自引用默认投影：`digest_excluded` 恰为对象自身 `content_digest` 与 `signature` 路径）；`slo_profile` 增加可选 `thresholds`（阈值/分层/release gate 机器落点）；conformance README 收紧 `implemented` 语义（degradation 不保 implemented，必须缩 scope 或降 experimental）；§4.7 责任矩阵补审批服务行。

机器资产净变化：REQ 270→273（+REQ-PERF-005、+REQ-REC-003、+REQ-GOBJ-REF-004），错误码 53→55（+`EFFECT_IDEMPOTENCY_CONFLICT`、+`STATE_STORE_UNAVAILABLE`），向量 68→74（+6），4 个既有向量升级（EFF-CRASH-001/002/003、CTX-RENDER-001 至 v0.2）、1 个扩展（PERF-REPORT-CONTRACT-001）；effect 迁移表 0.1→0.2；effect/task-contract/context-view/memory-candidate/loop-checkpoint/activity-context/performance-report 七个 schema 修订（多数为加法式；activity-context 必填集缩小与 task-contract 枚举开放属 draft 期修正型变更，随附迁移说明：曾依赖 `context_view_ref` 必填的消费者改为两阶段绑定，曾依赖四值枚举的消费者改查状态域注册表）。仓库处于 0.x draft 期且无已发布 pin，不触发新 specification set。

### 1.0.0（架构基线定稿）
本版为白皮书首个正式版本，采纳独立架构评审结论（改进项 IMP-01–18，逐项追踪见仓库评审结论文档），完成从 v0.2 起累积的架构语义收敛。定稿仅冻结白皮书架构基线：companion/schema/registry/向量保持各自 Draft 版本，全部处于"已登记、未经实现验证"状态；实现状态只能由机器可读 manifest 声明。

本版改动分五类：

1. **性能前提契约（P0）**：§9.3 第 8 阶段与 §9.4 确立确定性渲染、字节级前缀稳定与供应商缓存保持语义（对应新登记 `REQ-CTX-012` 与向量 CTX-RENDER-001）；§9.5/§18.3 确立准入异步化 + read-your-write + 跨 scope 不可见三要素（`REQ-MEM-ADMIT-002`、MEM-RYW-001）；§19.4 新增 Governance overhead 指标族（`REQ-PERF-004`，performance-report schema 加法式扩展）。
2. **审批子系统**：新增 §12.12"审批分发与确认强度"（审批即 OS 服务、R0—R3 分级确认矩阵、NL 协商不构成批准）；§17.1 增补 approval bombing 与审批界面伪造威胁；§17.2 注明确认面渲染约束；§23 新增决策二十五。其 REQ/向量登记按冻结原则递延为独立草案。
3. **落地性与部署指引**：§20.5 新增 R0 最小合法实现形态与不可降级边界；§6.9 澄清义务按风险档裁剪；§2.2/§19.7 验证带宽升格为部署一级约束并进入 Readiness Case（新增第 12 条验收容量）；§5.2 承认 C0/C1 现实落点并给出 C2 采纳激励；§5.3 定义带外修改对账（`REQ-AGENT-OOB-001`、AGENT-OOB-001）与已登记批量代理合法形态；§21 路线图写入实现优先冻结条款、Phase 1 适配性能对齐 A/B 报告与 Phase 4 形式化验证交付项。
4. **结构重组**：附录 A 分层为最小核心对象集（14 项）、按主题/Profile 归组的扩展集与 Context/Binding/Scope 判定表；§14.4—14.6 压缩为边界声明，误差分解与编译栈细节下沉 Heterogeneous companion；附录 C 新增 C.5 治理、安全与系统基础引用组（CaMeL、Simplex、MemGPT、CoALA、seL4/L4、Zanzibar/Cedar、Leases/fencing、IETF on-behalf-of/SPIFFE/WIMSE/RFC 8693、NIST SP 800-63B、CISA、ATT&CK T1621、PSD2 SCA、Temporal、HumanLayer/Teleport）。
5. **配套 companion 同步**：core（§6.6 确定性渲染）、governed-memory（异步准入）、agent-compatibility（read-your-write 默认语义、带外对账、批量代理）、operation-catalog（批量代理澄清）、heterogeneous（§6 误差分解、§8a 编译栈参考）同步修订；registry 新增 4 条 REQ 与 3 个向量（合计 270 REQ / 68 向量），无新增错误码、对象族或 Profile。
