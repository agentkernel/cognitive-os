...
## ——以可验证治理与上下文工程承载持续认知
- **版本**：1.0.0
- **上一版本**：0.8 Draft（见附录 D）
- **状态**：总体架构白皮书 · 正式版本（架构基线定稿）
- **发布日期**：2026-07-19
- **语言**：中文
- **文档性质**：Informative Architecture Whitepaper
- **适用对象**：Agent 平台、自治系统、机器人、边缘智能与异构计算架构师
- **定稿边界**：1.0.0 仅表示本白皮书的架构基线冻结与评审闭环；各 companion 规范、schema、registry 保持各自 Draft 版本，全部规范资产仍处于"已登记、未经实现验证"状态（§1.2），实现状态以机器可读 manifest 为准
...
[REQ-PROFILE-CVM-001] 声明 `context_virtualization` Profile 的实现 **MUST** 覆盖 `REQ-CTX-001`–`REQ-CTX-012` 的全部适用要求，并提供本节所列负例（required fail-closed、授权先于正文、缓存不绕过撤销、注入隔离）的测试证据。
...
### Phase 1：最小可安装系统
交付 AgentInstallation、C0/C1、sandbox、Identity/Memory/Tool adapters、静态 Catalog、确定性 Context Resolution、legacy Agent 参考适配器与不可绕过负例。legacy 参考适配器交付时，发布以 BenchmarkManifest 固定输入的适配前后 A/B 性能对齐报告（任务成功率、延迟、token/费用、缓存命中与 §19.4 Governance overhead 指标族），证明治理不劣化基线，作为后续语义优化的对照基线。
### Phase 2：认知发现与受治理记忆
