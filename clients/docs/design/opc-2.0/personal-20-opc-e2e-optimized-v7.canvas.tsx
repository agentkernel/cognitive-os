/**
 * PERSONAL 2.0 OPC INTERACTION PROTOTYPE — optimized v7
 *
 ...
 *
 * v7 delta (baseline = unapproved v6; last owner-approved chrome remains v5):
 * 全中文可见文案。项目 L1 下四个固定子菜单：项目列表 / 成员管理 / 运行管理 /
 * 产出管理（不是项目名称列表）。列表行含四跳转；成员 / 运行 / 产出用项目选择器
 * 切换当前项目。样品覆盖经营闭环与现场周检，无固定产出模板。无 Approve 在聊天。
 * 保留创建 ①–⑤、成员顺序紧凑就位、就位测试门、画布-聊天确认协议。
 *
 * v7 成员管理迭代（同基线，owner 2026-08-29 要求）：成员管理改为「左侧选人 +
 * 右侧标签页」的成员配置面板。标签页：概览 / 职责 / 输入输出 / 工作说明 /
 * 工具 / 技能 / 周期与触发 / 连接与权限。不再整页平铺每个人的职责。
 * 项目详情补「流程环节 / 成员」两行；子菜单高亮覆盖项目详情下钻。
 *
 * Design artifact:
 * d:\agent-kernel\clients\docs\design\opc-2.0\personal-20-opc-e2e-optimized-v7.canvas.tsx
 * Cursor-openable copy (IDE detection path; not a second product baseline):
 * C:\Users\wuron\.cursor\projects\d-agent-kernel\canvases\personal-20-opc-e2e-optimized-v7.canvas.tsx
...
const SCENES: ReadonlyArray<{ id: Scene; label: string }> = [
  ...
  { id: "project-members", label: "成员管理 · 选中成员看配置" },
...
const PROJECT_SUBNAV: ReadonlyArray<{ id: Scene; label: string }> = [
  ...
  { id: "project-members", label: "成员管理" },
  { id: "project-runs", label: "运行管理" },
  { id: "project-outputs", label: "产出管理" },
];
...
function runtimeSlotTab(slotId: RuntimeSlotId): MemberConfigTab {
  if (slotId === "prompt") return "prompt";
  if (slotId === "tools") return "tools";
  if (slotId === "skills") return "skills";
  if (slotId === "loop") return "loop";
  return "perms";
}
...
> = {
  ...
  members: {
    ...
    source: "当前项目成员投影",
    firstAction: "查看就位状态，或改职责后在对话里确认",
    native: "就位、模型待选、无安装。六项执行方式在配置页。",
  },
  runs: {
    label: "运行管理",
...
const RUNTIME_SLOT_DEFS: ReadonlyArray<{
  ...
  { id: "skills", businessLabel: "能力包", runtimeLabel: "Skill" },
  { id: "loop", businessLabel: "周期与触发", runtimeLabel: "进阶也称 loop" },
  { id: "mcp", businessLabel: "外部连接", runtimeLabel: "MCP · 精确授权，无安装" },
...
function currentInitHeadline(
  ...
  if (member.initStatus === "generating" && generatingIndex !== null) {
    ...
    return { kicker: "正在生成…", title: def.businessLabel, hint: def.runtimeLabel };
  }
  ...
  return (
    <div className="empty-home">
      <div>
        ...
        <h2>创建项目</h2>
        <p>用业务语言办一件长期的事。右侧对话在创建页才会打开。知识此时锁定。</p>
  ...
  return (
    <div className="scene-stack">
      ...
      <section className="work-surface">
        <Heading title="业务流程" meta="只读示意图。改环节请回 ②。" />
        ...
        {members.length === 0 ? (
          ...
          <>
            ...
            <div className="staff-table-wrap" tabIndex={0} aria-label="岗位名单">
              <table>
                <caption>按流程生成的岗位。就位状态不是 daemon 权威。</caption>
                <thead>
                  <tr>
                    <th scope="col">岗位</th>
                    <th scope="col">做什么</th>
                ...
                <tbody>
                  {members.map((member, index) => (
                    <tr key={member.id}>
                      <th scope="row">
                        <button className="text-button" type="button" onClick={() => setActiveMemberId(member.id)}>
                          {index + 1}. {member.name}
                        </button>
                      </th>
                      <td>
    ...
    <div className="scene-stack">
      <section className="setup-header">
        <div>
          ...
          <p>
            {fromLive ? "上线后维护同一份配置。" : "创建时与上线后看同一页。"}业务说法在前。回车同步到对话。无安装按钮。
          </p>
        </div>
        <div className="header-actions">
          <Tag tone={initStatusTone(member.initStatus)}>{initStatusLabel(member.initStatus)}</Tag>
          <button className="secondary-button" type="button" onClick={onBack}>
            {fromLive ? "返回项目" : "返回成员初始化"}
          </button>
        </div>
      </section>
      <section className="work-surface">
        <Heading title="职责" meta="做什么 / 交出什么。改完回车告知助手。" />
        <label className="field">
          <span>做什么</span>
          <SyncedField
            field={`member:${member.id}:duty`}
      ...
      <section className="work-surface">
        <Heading title="执行方式" meta="第二层才是提示词 / Skill / MCP。周期与触发默认不叫 loop。" />
        {member.runtime.map((slot) => (
          <label key={slot.id} className="field">
            <span>
              {slot.businessLabel}
              <small> · {slot.runtimeLabel} · {slotStatusLabel(slot.status)}</small>
            </span>
            <SyncedField
              field={`runtime:${member.id}:${slot.id}`}
              label={`${member.name} · ${slot.businessLabel}`}
              value={slot.value}
              onChange={(next) => setSlotValue(member.id, slot.id, next)}
              onCommit={onCommitField}
              onFocusField={onFocusField}
              multiline
              rows={3}
      ...
      <section className="work-surface">
        ...
        {selected ? (
          <div className="stage-detail">
            <Heading title={`${selected.label} · ${selected.owner}`} meta="今日成功 / 失败 / 平均时长 · 均为样品" />
            <dl className="ledger-facts">
              <div>
                <dt>成功次数</dt>
                <dd>{selected.todayOk}</dd>
              </div>
              <div>
                <dt>失败次数</dt>
                <dd>{selected.todayFail}</dd>
              </div>
              <div>
                <dt>平均执行时长</dt>
                <dd>{selected.todayAvg}</dd>
              </div>
              <div>
                <dt>成功率</dt>
    ...
    return (
      <div className="scene-stack">
        <section className="today-header">
          <div>
            <h2>未完成的创建</h2>
            <p>项目列表现在只露出这一份草稿。左侧四个去向仍在，工作面跟这份草稿走。</p>
          </div>
          <button className="primary-button" type="button" onClick={onContinue}>
            继续创建
          </button>
  ...
  return (
    <div className="scene-stack">
      <section className="today-header">
        <div>
          <h2>项目列表</h2>
          <p>列举、管理、编辑当前项目。左侧四个去向不是项目名。副本不带密钥、进行中任务、对外回执、本周不再问。</p>
        </div>
        <div className="header-actions">
          <button className="primary-button" type="button" onClick={onCreate}>
            创建项目
      ...
      {projects.map((item) => (
        <section key={item.id} className="work-surface">
          ...
          <div className="project-row-actions">
            <button className="secondary-button" type="button" onClick={() => onDetail(item.id)}>
              查看项目详情
            </button>
            <button className="secondary-button" type="button" onClick={() => onMembers(item.id)}>
              查看项目成员
            </button>
            <button className="secondary-button" type="button" onClick={() => onRuns(item.id)}>
              查看项目运行
            </button>
            <button className="secondary-button" type="button" onClick={() => onOutputs(item.id)}>
              查看项目产出
            </button>
            {item.kind === "live" && item.id === "weekly" ? (
              <button className="secondary-button" type="button" onClick={() => onCopy(item.id)}>
                一键复制新增项目
              </button>
            ) : null}
          </div>
  ...
  return (
    <div className="scene-stack">
      <section className="project-header">
        <div>
          <h2>项目详情</h2>
          <p>名称、目标、周期、状态。改完回车走确认框，再在对话里确认才写回画布。</p>
        </div>
      ...
      <section className="work-surface">
        ...
        <dl className="definition-list">
          ...
          <div>
            <dt>流程环节</dt>
            <dd>{project.stages.length} 环 · 每环负责人见成员管理</dd>
          </div>
          <div>
            <dt>成员</dt>
            <dd>
...
function ProjectMembersScene({
  ...
  const [tab, setTab] = useState<MemberConfigTab>("overview");
  ...
  const ownedStages = selected
    ? project.stages.filter((stage) => OWNER_PROFILES[stage.owner]?.id === selected.id)
    : [];
  const singleSlotId =
    tab === "prompt"
      ? "prompt"
  ...
  const singleSlot = singleSlotId
    ? selected?.runtime.find((slot) => slot.id === singleSlotId)
    : undefined;
  const mcpSlot = selected?.runtime.find((slot) => slot.id === "mcp");
  const filesSlot = selected?.runtime.find((slot) => slot.id === "files");
  return (
    <div className="scene-stack">
      <section className="project-header">
        <div>
          <h2>成员管理</h2>
          <p>左侧选中一个人，右侧用标签页查看并编辑他的完整配置。改完回车 → 确认框 → 对话确认。聊天不能批准。</p>
        </div>
        <div className="header-actions">
          <ProjectSwitcher projects={switcherProjects} currentId={project.id} onChange={onSwitch} />
      ...
      {members.length === 0 || !selected ? (
        ...
        <section className="work-surface">
          <Heading
            title="成员名单与配置"
            meta="选中谁就看谁的配置。就位状态不是 daemon 权威。无安装按钮。"
          />
          <div className="people-layout">
            <div className="member-list" role="list" aria-label="成员名单">
              {members.map((member) => (
                <button
                  key={member.id}
                  type="button"
                  role="listitem"
                  aria-current={member.id === selected.id ? "page" : undefined}
            ...
            <section className="member-detail" aria-label={`${selected.name} · 配置标签页`}>
              <header className="member-detail-head">
                <h3>{selected.name}</h3>
                <div className="packet-marks">
                  <Tag tone={initStatusTone(selected.initStatus)}>
                    {initStatusLabel(selected.initStatus)}
                  </Tag>
                  {selected.model === "unselected" ? <Tag tone="warn">模型待选</Tag> : null}
                </div>
              </header>
              <Segmented
                label={`${selected.name} 配置标签页`}
                value={tab}
              ...
              {tab === "overview" ? (
                <>
                  <dl className="definition-list">
                    <div>
                      <dt>加入状态</dt>
                      <dd>{selected.joined ? "已加入本项目。" : "未加入。拒加入不会写成已加入。"}</dd>
                    </div>
                    <div>
                      <dt>负责环节</dt>
                      <dd>
                        {ownedStages.length > 0
                          ? `${ownedStages.map((stage) => stage.label).join("、")}（${ownedStages.length} 环）`
                          : "不在流程轴上。加人后还没指派环节。"}
                      </dd>
                    </div>
                  </dl>
                  <label className="field">
                    ...
                    <Select
                      ...
                      onChange={(next) => setModel(selected.id, next)}
                      options={[
                        { value: "unselected", label: "未选 · 待定" },
                        { value: "anthropic", label: "Anthropic · 平衡" },
                        { value: "openai", label: "OpenAI · 推理" },
                  ...
                  <Heading
                    title="执行方式六项"
                    meta="业务说法在前。提示词 / Skill / MCP 是第二层。选一项去查看与编辑。"
                  />
                  <ul className="runtime-slots">
                    {selected.runtime.map((slot) => (
                      <li key={slot.id}>
                        <div>
                          <strong>{slot.businessLabel}</strong>
                          <small>
                            {slot.runtimeLabel} · {slotStatusLabel(slot.status)}
                          </small>
                        </div>
                        <button
                          className="text-button"
                          type="button"
                          onClick={() => setTab(runtimeSlotTab(slot.id))}
                        >
                          去查看
                        </button>
                        <p>
              ...
              {tab === "duty" ? (
                <label className="field">
                  ...
                  <SyncedField
                    field={`member:${selected.id}:duty`}
                    label={`${selected.name} · 职责`}
                    value={selected.duty}
                    onChange={(next) => setMemberText(selected.id, "duty", next)}
                    onCommit={onCommitField}
                    onFocusField={onFocusField}
                    multiline
                    rows={3}
              ...
              {tab === "io" ? (
                <label className="field">
                  <span>
                    ...
                    <small>
                      {" "}
                      · 输出。输入来自上一环的交出物，见下方流程轴。改完回车 → 确认框 → 对话确认。
                    </small>
                  </span>
                  <SyncedField
                    field={`member:${selected.id}:handoff`}
                    label={`${selected.name} · 交出什么`}
                    value={selected.handoff}
                    onChange={(next) => setMemberText(selected.id, "handoff", next)}
                    onCommit={onCommitField}
                    onFocusField={onFocusField}
                  />
                </label>
              ...
              {singleSlot ? (
                <label className="field">
                  <span>
                    ...
                    <small>
                      {" "}
                      · {singleSlot.runtimeLabel} · {slotStatusLabel(singleSlot.status)}
                      。改完回车 → 确认框 → 对话确认。无安装按钮。
                    </small>
                  </span>
                  <SyncedField
                    field={`runtime:${selected.id}:${singleSlot.id}`}
                    label={`${selected.name} · ${singleSlot.businessLabel}`}
                    value={singleSlot.value}
                    onChange={(next) => setSlotValue(selected.id, singleSlot.id, next)}
                    onCommit={onCommitField}
                    onFocusField={onFocusField}
                    multiline
                    rows={3}
                ...
                <>
                  <label className="field">
                    <span>
                      ...
                      <small>
                        {" "}
                        · {mcpSlot.runtimeLabel} · {slotStatusLabel(mcpSlot.status)}
                        。精确授权另批。无市场安装。
                      </small>
                    </span>
                    <SyncedField
                      field={`runtime:${selected.id}:mcp`}
                      label={`${selected.name} · ${mcpSlot.businessLabel}`}
                      value={mcpSlot.value}
                      onChange={(next) => setSlotValue(selected.id, "mcp", next)}
                      onCommit={onCommitField}
                      onFocusField={onFocusField}
                      multiline
                      rows={3}
                  ...
                  <label className="field">
                    <span>
                      ...
                      <small>
                        {" "}
                        · {filesSlot.runtimeLabel} · {slotStatusLabel(filesSlot.status)}
                        。扩权要再批。
                      </small>
                    </span>
                    <SyncedField
                      field={`runtime:${selected.id}:files`}
                      label={`${selected.name} · ${filesSlot.businessLabel}`}
                      value={filesSlot.value}
                      onChange={(next) => setSlotValue(selected.id, "files", next)}
                      onCommit={onCommitField}
                      onFocusField={onFocusField}
                      multiline
                      rows={3}
      ...
      <p className="sample-caption">流程轴 · 只读。每环负责人。执行进度在运行管理。</p>
      <div className="process-axis" role="list">
  ...
  const visibleOutputs =
    sample === "packet"
      ...
      : sample === "document"
        ...
        : ready;
  const selected =
    visibleOutputs.find((item) => item.id === selectedId) ?? visibleOutputs[0] ?? null;
  ...
  return (
    <div className="scene-stack">
      <section className="project-header">
        <div>
          <h2>产出管理</h2>
          <p>无固定模板。可以是文稿、清单、链接、正文加配图/视频，或交付包。一次一种编排。</p>
        </div>
        <div className="header-actions">
          <ProjectSwitcher projects={switcherProjects} currentId={project.id} onChange={onSwitch} />
      ...
      <Segmented
        ...
        onChange={setSample}
      />
      <p className="sample-caption">原型样品 · 不是 daemon 产出目录</p>
      {sample === "working" ? (
        ...
        <section className="work-surface">
          ...
          {visibleOutputs.length > 1 ? (
            <div className="output-chooser" role="list">
        ...
        <section className="work-surface">
          <p>这份样品没有对应编排。请换一份产出，或请助手换一种展示。</p>
          <button className="secondary-button" type="button" onClick={onAskFormat}>
            请助手换一种展示
          </button>
        </section>
...
function AddMemberScene({
  ...
  return (
    <div className="scene-stack">
      ...
      <section className="work-surface">
        <Heading title="新岗位" meta="拒绝 = 不加入。没模型 = 待定，去设置。" />
        <label className="field">
          <span>岗位名</span>
          <SyncedField
        ...
        {joined ? (
          <Notice title="已加入（原型）" tone="good">
            岗位已进名单，执行方式仍要初始化。回项目画布打开「查看配置」。不是先装 MCP。
          </Notice>
        ) : null}
      </section>
...
function Conversation({
  ...
  const project =
    ...
    scene === "hitl" ||
    outputs;
  const title = setup || !project ? "个人助手" : "项目群";
...
export default function Personal20OpcE2eOptimizedV7() {
    ...
    if (active === "project-outputs") {
      return (
        <ProjectOutputsScene
          ...
          onWork={() =>
            setScene(currentProject.kind === "creating" ? "create-init" : "project-runs")
          }
          onHitl={() => setScene("hitl")}
        />
      );
  ...
  return (
    <div className="opc-e2e" style={variables}>
      <style>{`.opc-e2e {
        ...
        .opc-e2e .member-list button > span { min-width: 0; }
      ...
      <header className="prototype-bar">
        <div className="prototype-title">
          <h1>个人 2.0 · OPC 端到端原型 · v7</h1>
          <span>全中文。项目下四个去向。列表含四跳转。成员 / 运行 / 产出可切项目。</span>
        </div>
              ...
              {PROJECT_SUBNAV.map((item) => (
                <button
                  ...
                  onClick={() => {
                    ...
                    if (lifecycle === "creating") {
                      if (item.id === "project-members" || item.id === "project-runs" || item.id === "project-outputs") {
                        setScene(item.id);
                      }
                      return;
                    }
