/**
 * PERSONAL 2.0 OPC INTERACTION PROTOTYPE — optimized v8
 *
 ...
 *
 * v8 delta (baseline = unapproved v7; last owner-approved chrome remains v5):
 * 成员管理：先选人再看配置（未选人为空态，不默认第一人）。标签拆成职责 / 输入 /
 * 输出 / 技能 / 工具 / 工作说明 / 周期与触发 / 连接与权限。身份（模型、就位、
 * 负责环节）固定在详情头，不再用概览页复述六项。输入来自流程轴上一环产出，
 * 不是编造的成员私有输入字段。项目列表一行一个打开，成员/运行/产出走左侧子菜单
 * 或次要文字链。项目详情补只读流程轴与去向。运行管理的「验收回今日」只在末环。
 * 产出管理同样先选后看。加人名单用当前项目真实成员。无 Approve 在聊天。
 *
 * Design artifact:
 ...
 * C:\Users\wuron\.cursor\projects\d-agent-kernel\canvases\personal-20-opc-e2e-optimized-v8.canvas.tsx
  ...
  return (
    <div className="scene-stack">
      <section className="today-header">
        <div>
          <h2>项目列表</h2>
          <p>打开一个项目后，用左侧四个去向看详情、成员、运行、产出。副本不带密钥、进行中任务、对外回执、本周不再问。</p>
        </div>
        <div className="header-actions">
          <button className="primary-button" type="button" onClick={onCreate}>
            创建项目
...
export default function Personal20OpcE2eOptimizedV8() {
    ...
    <div className="opc-e2e" style={variables}>
      ...
      <header className="prototype-bar">
        <div className="prototype-title">
          <h1>个人 2.0 · OPC 端到端原型 · v8</h1>
          <span>先选人再看配置。项目列表一个打开。成员 / 运行 / 产出可切项目。</span>
        </div>
