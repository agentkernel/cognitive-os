/**
 * PERSONAL 2.0 OPC INTERACTION PROTOTYPE — optimized v4
 *
 ...
 *
 * v4 delta: after roster + models, initialize Members one-by-one with visible
 * progress. Execution recipe (工作说明/工具/能力包/周期与触发/外部连接/文档范围)
 * is generated as labelled samples, confirmed per Member, and inspectable on a
 * config page used in create and later maintenance. Stage test blocks until
 * the responsible Member is seated. No Install/Connect; Loop/Harness are not
 * default chrome.
 *
 ...
 *
 * Design artifact:
 * d:\agent-kernel\clients\docs\design\opc-2.0\history\2026-08-29-pre-v5-approval\personal-20-opc-e2e-optimized-v4.canvas.tsx
 * Cursor-openable archived copy (IDE detection path; not a second baseline):
 * C:\Users\wuron\.cursor\projects\d-agent-kernel\canvases\history\2026-08-29-pre-v5-approval\personal-20-opc-e2e-optimized-v4.canvas.tsx
    ...
    <div className="scene-stack">
      ...
      {active ? (
        <section className="work-surface">
          ...
          ) : active.initStatus === "confirmed" ? (
            <Notice title="此人已就位" tone="good">
              工作说明、工具、能力包、周期与触发、外部连接、文档范围已确认。可查看完整配置或下一位。
            </Notice>
          ) : (
            <Notice title="执行方式待生成或待确认" tone="info">
              目标态样品会写入画布。MCP 若待授权，确认此人即一并授权预览，无安装。
            </Notice>
          )}
          <p className="flow-end" aria-live="polite">
            本项进度 {readyCount} / {RUNTIME_SLOT_DEFS.length}
          </p>
          <ul className="runtime-slots">
            {active.runtime.map((slot) => (
              <li key={slot.id}>
                <div>
                  <strong>{slot.businessLabel}</strong>
                  <small>{slot.runtimeLabel}</small>
                </div>
                <Tag tone={slotTone(slot.status)}>{slotStatusLabel(slot.status)}</Tag>
                <p>{slot.value.trim().length > 0 ? slot.value : "还没有内容。"}</p>
              </li>
            ))}
          </ul>
          <div className="flow-actions">
            <button
              className="primary-button"
              type="button"
              disabled={!canGenerate || active.initStatus === "confirmed"}
            ...
            <button
              className="secondary-button"
              type="button"
              disabled={activeIndex < 0 || activeIndex >= members.length - 1 || !memberSeated(active)}
              onClick={() => {
...
export default function Personal20OpcE2eOptimizedV4() {
  ...
  const confirmGenerateRuntime = () => {
    ...
    const runtime = sampleRuntime(id);
    setMembers((current) =>
      current.map((item) =>
        item.id === id ? { ...item, runtime, initStatus: "ready" } : item,
      ),
    );
    for (const slot of runtime) {
      lastSynced.current[`runtime:${id}:${slot.id}`] = slot.value;
    }
    appendTurns(
      {
        author: "owner",
      ...
      },
      {
        author: "assistant",
        label: "助手 · 候选",
        text: `已写入「${name}」的目标态样品。请在画布核对后点「确认此人已就位」。没有安装。待授权的外部连接在确认此人时一并授权预览。聊天不能批准。`,
      },
    );
    setPendingRuntimeMemberId(null);
    setComposerStatus("已写入本地样品。确认就位前不算权威。");
  };
  ...
  const locationLabel = (() => {
    if (scene === "member-config") {
      return memberConfigFromLive ? "Projects / 周报与客户跟进" : "Projects / 创建中";
    }
    ...
    <div className="opc-e2e" style={variables}>
      ...
      <header className="prototype-bar">
        <div className="prototype-title">
          <h1>Personal 2.0 · OPC 端到端原型 · v4</h1>
          <span>① 项目 → ② 流程 → ③ 成员逐人就位 → ④ 测前检查就位</span>
        </div>
