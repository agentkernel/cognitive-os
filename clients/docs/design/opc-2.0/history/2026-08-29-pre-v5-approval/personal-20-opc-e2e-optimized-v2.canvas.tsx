/**
 * PERSONAL 2.0 OPC INTERACTION PROTOTYPE — optimized v2
 *
 ...
 *
 * v2 delta: create/members/process/test/joint (and add-member) use the Personal
 * Assistant as the primary create/edit entry. Canvas edits: Enter → confirm
 * dialog → Owner message in chat. Assistant proposals apply to the canvas only
 * after Owner confirms in chat. Knowledge/Settings chat still default-collapsed.
 *
 * Archived 2026-08-29. Not current product chrome. Current chrome is
 * personal-20-opc-e2e-optimized-v5.
 *
 * Design artifact:
 * d:\agent-kernel\clients\docs\design\opc-2.0\history\2026-08-29-pre-v5-approval\personal-20-opc-e2e-optimized-v2.canvas.tsx
 * Cursor-openable archived copy (IDE detection path; not a second baseline):
 * C:\Users\wuron\.cursor\projects\d-agent-kernel\canvases\history\2026-08-29-pre-v5-approval\personal-20-opc-e2e-optimized-v2.canvas.tsx
    ...
    <div className="scene-stack">
      <section className="setup-header">
        <div>
          <h2>② 确认这个班子</h2>
          <p>
            右侧助手是创建和改岗位的主入口。画布上「做什么 / 交出什么」可改：回车确认后以你的名义写入对话。每人必须显式选模型。
          </p>
        </div>
      </section>
      <section className="work-surface">
        <Heading title="岗位名单" meta="拒绝 = 不加入。没选模型 = 待定，不静默绑定。回车告知助手。" />
        <div className="staff-table-wrap" tabIndex={0} aria-label="岗位名单">
          <table>
            <caption>目标态班子样品。改动经对话同步。不是示范产品项目。</caption>
            <thead>
              <tr>
                <th scope="col">岗位</th>
                <th scope="col">做什么</th>
  ...
  return (
    <div className="scene-stack">
      <section className="setup-header">
        <div>
          <h2>③ 一条流程轴，一次只开一环</h2>
          <p>总目标：每周可打开的经营周报。总周期：周一。改输入/执行方式/权限后回车，以你的名义告诉助手。</p>
        </div>
      </section>
      ...
      <section className="work-surface">
        <Heading title={`这一环 · ${stage.label}`} meta={`负责：${stage.owner}。回车告知助手，Shift+回车换行。`} />
        <label className="field">
          <span>输入</span>
          <SyncedField
...
export default function Personal20OpcE2eOptimizedV2() {
  ...
  return (
    <div className="opc-e2e" style={variables}>
      <style>{`.opc-e2e {
        ...
        .opc-e2e .table-field textarea { min-height: 40px; }
      ...
      <header className="prototype-bar">
        <div className="prototype-title">
          <h1>Personal 2.0 · OPC 端到端原型 · v2</h1>
          <span>助手对话是创建/编辑主入口 · 画布回车同步 · Knowledge/Settings 默认收起对话</span>
        </div>
