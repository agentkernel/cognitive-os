/**
 * PERSONAL 2.0 OPC INTERACTION PROTOTYPE — optimized v6
 *
 ...
 *
 * v6 delta (baseline = owner-approved v5): Projects L1 keeps a named-project
 * submenu in Projects territory (switch updates sample canvas; empty first-run
 * invents no demo list). Live project header adds 查看产出 — a dedicated
 * assistant-composed surface (one presentation at a time; Owner confirms in
 * chat; labelled local samples). No Approve in chat; HITL stays on canvas.
 * v5 member init, seating test gate, config page, and chat protocol stay.
 *
 * Design artifact:
 * d:\agent-kernel\clients\docs\design\opc-2.0\personal-20-opc-e2e-optimized-v6.canvas.tsx
 * Cursor-openable copy (IDE detection path; not a second product baseline):
 * C:\Users\wuron\.cursor\projects\d-agent-kernel\canvases\personal-20-opc-e2e-optimized-v6.canvas.tsx
...
type SampleProjectId = "weekly" | "calendar" | "weekly-copy" | "creating-draft";
type OutputFormat = "document" | "checklist" | "packet" | "comparison";
...
const SCENES: ReadonlyArray<{ id: Scene; label: string }> = [
  ...
  { id: "project-outputs", label: "Project outputs · assistant composition" },
  { id: "add-member", label: "Add member" },
  { id: "member-config", label: "Member runtime config" },
...
const CALENDAR_STAGES: readonly ProcessStage[] = [
  {
    id: "topics",
    label: "收集选题",
    owner: "安",
...
const CALENDAR_OUTPUTS: readonly SampleOutput[] = [
  {
    id: "topic-compare",
    title: "本周选题对照",
    job: "在保留与砍掉的选题之间做一次对照",
    format: "comparison",
    accepted: true,
    needsHitl: false,
  },
...
const SAMPLE_PROJECTS: readonly SampleProject[] = [
  {
    ...
    costLine: "估计 ¥6.40 · 另有未知项不写 0",
    blurb: "前台只按业务流程。没有 CEO 六步顶栏，没有 X 英雄圈，没有示范项目。",
  ...
  },
  {
    ...
    kind: "live",
    statusLine: "已上线 · 收集选题",
    costLine: "估计 ¥3.10 · 另有未知项不写 0",
    blurb: "第二个本地样品项目。不是商店，不是默认示范。切换后工作面与产出都换成这份样品。",
    currentStageId: "topics",
    stages: CALENDAR_STAGES,
    participants: ["周 · 经理", "安 · 选题"],
...
const OUTPUT_FORMATS: readonly OutputFormat[] = ["document", "checklist", "packet", "comparison"];
...
function listedProjects(lifecycle: Lifecycle, copied: boolean): SampleProject[] {
  ...
  const live = [projectById("weekly"), projectById("calendar")];
...
function formatFromAsk(text: string, current: OutputFormat): OutputFormat {
  ...
  if (/决策包|预览|发送包/.test(text)) return "packet";
  if (/对照|比较|对比/.test(text)) return "comparison";
...
> = {
  ...
  outputs: {
    ...
    native: "一次只呈现一种编排。空态不是假画廊。换展示要聊天确认。聊天不能批。",
  },
  hitl: {
    label: "HITL 画布",
...
function isSetupChat(scene: Scene) {
  return CREATE_SCENES.includes(scene) || scene === "add-member" || scene === "member-config";
}
...
function ProjectsScene({
  ...
  copied: boolean;
  onCreate: () => void;
  onOpen: (id: SampleProjectId) => void;
  onCopy: () => void;
  onContinue: () => void;
}) {
  if (lifecycle === "empty") {
    ...
    return (
      <div className="scene-stack">
        <section className="today-header">
          <div>
            <h2>未完成的创建</h2>
            <p>Projects 现在只露出这一份草稿。左侧子菜单也只有它。</p>
          </div>
          <button className="primary-button" type="button" onClick={onContinue}>
            继续创建
          </button>
  ...
  return (
    <div className="scene-stack">
      <section className="today-header">
        <div>
          <h2>长期工作空间</h2>
          <p>左侧子菜单切换项目。列表也可打开。副本不带密钥、进行中任务、对外回执、本周不再问。</p>
        </div>
        <div className="header-actions">
    ...
    return (
      <div className="scene-stack">
        <section className="project-header">
          ...
          <div className="header-actions">
            <button className="secondary-button" type="button" onClick={onOutputs}>
              查看产出
            </button>
          </div>
        </section>
        <section className="work-surface">
          <p>这是未激活副本。不能当已上线项目跑流程。产出页现在是空的。</p>
        </section>
        <Gap>复制在真实产品里由 daemon 落草稿。这里只切换本地样品。</Gap>
      </div>
    );
  }
    ...
    <div className="scene-stack">
      ...
      <section className="work-surface">
        ...
        {stage.id === "collect" ? (
          ...
          <p>安在收集选题。进行中不是完成。可打开的选题对照在产出页。</p>
        ) : stage.id === "outline" ? (
          <p>这一环还没开始。缺：可打开的选题清单。</p>
        ) : stage.id === "review" ? (
          <p>排期要你过目。主按钮在这里。聊天不能批。</p>
        ) : (
          <p>{stage.status}</p>
        )}
        {needsOwner ? (
      ...
      <section className="work-surface">
        <Heading title="成员配置" meta="上线后维护执行方式。与创建时同一页。" />
        {members.length === 0 ? (
          <p>还没有可检查的岗位配置。加人后再初始化。</p>
        ) : (
          <ul className="seat-check">
            {members.map((member) => (
              <li key={member.id}>
                <span>
                  {member.name} · {initStatusLabel(member.initStatus)}
                </span>
                <button className="text-button" type="button" onClick={() => onViewConfig(member.id)}>
                  查看配置
                </button>
              </li>
            ))}
      ...
      <article className="output-composition" aria-label={formatLabel(format)}>
        ...
        <div className="comparison-table-wrap">
          <table>
            ...
            <tbody>
              <tr>
                <th scope="row">老客户续约口径</th>
                <td>本周保留</td>
              </tr>
              <tr>
                <th scope="row">未核过的行业传闻</th>
                <td>砍掉 · 不可信文本</td>
              </tr>
            </tbody>
          </table>
...
function ProjectOutputsScene({
  ...
  return (
    <div className="scene-stack">
      <section className="project-header">
        <div>
          <h2>{project.name} · 产出</h2>
          <p>和工作面板分开。一次只打开一种编排。不是固定仪表盘。</p>
        </div>
        <div className="header-actions">
      ...
      {sample === "working" ? (
        ...
        <section className="work-surface">
          ...
          <button className="primary-button" type="button" onClick={onWork}>
            {project.kind === "creating" ? "继续创建" : "回工作面"}
          </button>
        </section>
      ) : selected ? (
        <section className="work-surface">
          ...
          <details className="trace-fold">
            ...
            <p>
              助手按产出工作选择文稿、清单、决策包或对照。这是本地建议。你在对话里确认后画布才换。
...
function Conversation({
  ...
  const project = scene === "project" || scene === "hitl" || outputs;
...
export default function Personal20OpcE2eOptimizedV6() {
  ...
  const setMemberModel = (id: string, model: string) => {
    setMembers((current) =>
      current.map((member) => {
        ...
        if (model === "unselected") {
          return { ...member, model, initStatus: "blocked", joined: false };
        }
        const initStatus =
          member.initStatus === "confirmed"
            ? "partial"
          ...
          runtime: member.runtime.map((slot) =>
            slot.id === slotId
              ? {
                  ...slot,
                  value,
                  status: value.trim().length === 0 ? "empty" : "draft",
                }
              : slot,
          ),
        };
  ...
  const sceneLabel =
    scene === "project" || scene === "add-member" || scene === "hitl"
      ? currentProject.name
      : scene === "project-outputs"
        ? `${currentProject.name} · 产出`
        : SCENE_TITLES[scene];
  const knowledgeOk = lifecycle === "live" || (lifecycle === "creating" && createGate >= 2);
  const locationLabel = (() => {
    if (scene === "member-config") {
      return memberConfigFromLive ? `Projects / ${currentProject.name}` : "Projects / 创建中";
    }
    if (CREATE_SCENES.includes(scene)) return "Projects / 创建中";
    if (scene === "project" || scene === "add-member" || scene === "hitl" || scene === "project-outputs") {
      return `Projects / ${currentProject.name}`;
    }
  ...
  const openListedProject = (id: SampleProjectId) => {
    ...
    setScene(scene === "project-outputs" ? "project-outputs" : "project");
  };
  ...
  const applyScenario = (next: Scene) => {
    ...
    setLifecycle("live");
    if (currentProjectId === "creating-draft") setCurrentProjectId("weekly");
  };
    ...
    if (active === "projects") {
      return (
        <ProjectsScene
          ...
          onOpen={openListedProject}
          onCopy={() => {
            setCopied(true);
            setCurrentProjectId("weekly-copy");
          }}
    ...
    if (active === "project") {
      return (
        <ProjectLiveScene
          ...
          onOutputs={() => setScene("project-outputs")}
          onClose={() => setScene("today")}
          members={currentProject.id === "weekly" ? members : []}
          onViewConfig={(id) => openMemberConfig(id, true)}
        />
      );
    }
    if (active === "project-outputs") {
      return (
        <ProjectOutputsScene
          ...
          onWork={() =>
            setScene(currentProject.kind === "creating" ? "create-init" : "project")
          }
          onHitl={() => setScene("hitl")}
        />
      );
    ...
    if (active === "member-config") {
      ...
      return (
        <MemberConfigScene
          ...
          onBack={() => setScene(memberConfigFromLive ? "project" : "create-members")}
          setModel={setMemberModel}
          setMemberText={setMemberText}
    ...
    <div className="opc-e2e" style={variables}>
      ...
      <header className="prototype-bar">
        <div className="prototype-title">
          <h1>Personal 2.0 · OPC 端到端原型 · v6</h1>
          <span>Projects 子菜单切换项目 · 查看产出由助手编排</span>
        </div>
      ...
      <div className={chatHidden ? "shell chat-hidden" : "shell"}>
        <nav className="primary-nav" aria-label="Personal primary navigation">
          ...
          {projectsCurrent ? (
            <nav className="projects-submenu" aria-label="项目">
              {visibleProjectList.length === 0 ? (
                <>
                  <p className="submenu-empty">还没有项目</p>
                  <button
                    type="button"
                    onClick={() => {
                      setLifecycle("creating");
                ...
                visibleProjectList.map((item) => {
                  const open = item.id === resolvedProjectId && scene !== "projects";
                  return (
                    <button
                      key={item.id}
                      type="button"
                      aria-current={open ? "page" : undefined}
                      onClick={() => openListedProject(item.id)}
                    >
                      <span>{item.name}</span>
                      {open ? <Tag>当前</Tag> : null}
                    </button>
                  );
                })
              )}
...
function projectGroup(scene: Scene): string {
  if (scene === "project" || scene === "hitl" || scene === "project-outputs") return "项目群";
