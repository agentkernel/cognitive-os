...
- **PoC 执行骨架**（全部 not-run）：[ios-poc-runbook](docs/ios-poc-runbook.md)；共享模板 [poc-execution-record](../../shared/docs/poc-execution-record.md)。
- **边界**：remote companion（见 [mobile/README.md](../README.md)）；不承载 runtime/authority/node/Vault。
- **gate**：[Console 实现 gate](../../governance/readiness-gates.md#console-实现-gate) + [iPhone Open PoC 与 GA gates](docs/ios-product-design.md#18-open-poc-与-ga-gates)（全部 `not-run`）+ iOS 技术栈 ADR（不存在）。
