...
- Console 平台行为与实现 gate 现以 [平台设计入口](../docs/platforms/README.md) 为准（gate canonical 正文将在迁移 B6 批收纳进 [governance/readiness-gates.md](governance/readiness-gates.md)）；
- Agent Hub 行为与保证以 [Agent Hub canonical 根](../apps/cognitiveos-console/docs/agent-hub/README.md) 为准（B5 批迁入 `clients/agent-hub/docs/`）；
...
- 表内反引号包围的值都是仓库中的真实路径。尚未分配的手机代码载体只以 `app/` 保留入口表达：入口已分配但**无任何实现**，不得出现 manifest、源码或构建脚手架。
...
- 迁移进行中的行在"角色"列注明落位批次；canonical 入口链接一律指向**当前真实存在**的文件，迁移批次落地时同批改写。

## 2. PC 客户端（`clients/pc/`）

| 路径 | 平台 | 角色 | 当前状态 | owner | canonical 入口 | 上游 gate | README / 目录说明 |
|---|---|---|---|---|---|---|---|
| `clients/pc/` | PC | PC 客户端项目根：Windows 首发，macOS/Linux parity 文档域 | `planned`；implementation `not-implemented`；evidence `none`；Profile `not implemented` | Lane-CON | [pc/README.md](pc/README.md) | [Console 实现 gate](../docs/platforms/README.md#console-实现-gate) | 有 |
...
| `clients/pc/docs/` | PC | Windows v1 产品文档与 macOS/Linux 平台切片落位（B2 批迁入；canonical 正文现仍在 `apps/cognitiveos-console/docs/` 与 `docs/platforms/`） | `planned`；implementation `not-implemented`；platform tests `none` | Lane-CON | [Console README](../apps/cognitiveos-console/README.md)；[Windows v1 范围](../apps/cognitiveos-console/docs/windows-v1-scope.md) | Console 依赖组 1/2/7、M5、真实平台 PoC、技术栈 ADR | accessibility/quality/release 缺内容子目录已有薄 README |
...
| `apps/agent-shell/` | PC | TypeScript Task Shell 客户端外壳；非 authority；**不迁移**（Lane-TSC 所有） | `blocked`；仅有 M0 skeleton，M5 客户端功能 `not-implemented`；conformance `not-run` | Lane-TSC | [package.json](../apps/agent-shell/package.json)；[源码入口](../apps/agent-shell/src/index.ts) | [Lane-TSC 当前 gate](../docs/plan/PARALLEL-LANES.md)；M5 集成 | 缺（待 Lane-TSC 补） |

`apps/cognitiveos-console/` 迁移后只保留 README 与 PRODUCT-DESIGN 兼容 stub（既有 §17/§20.3 与旧章节 anchor 全部保留）。Console 目录当前只有 Markdown 产品文档，并由 [pnpm workspace](../pnpm-workspace.yaml) 排除；不得据此推断应用脚手架、运行时或发布包已经存在。`agent-shell` 的 skeleton 只登记客户端边界和计划 verbs，不提供完整 transport、CLI、持久化或 authority 能力。

## 3. 手机 companion（`clients/mobile/`）

手机是 Takeover Host / CognitiveOS 节点的 **remote companion**。手机不承载 Agent runtime、CognitiveOS authority、CognitiveOS node 或完整 Vault；高后果动作的 PC-local 边界与 Relay/配对保证只引用 [Agent Hub 平台范围](../apps/cognitiveos-console/docs/agent-hub/platforms/product-scope.md) 和 [Relay/配对/迁移](../apps/cognitiveos-console/docs/agent-hub/architecture/relay-pairing-and-migration.md)，本索引不新造保证。
...
| 路径 | 平台 | 角色 | 当前状态 | owner | canonical 入口 | 上游 gate | README / 目录说明 |
|---|---|---|---|---|---|---|---|
| `clients/mobile/` | 手机 | remote companion 文档域根与共同边界 | `planned`；implementation `not-implemented`；evidence `none` | Lane-CON | [mobile/README.md](mobile/README.md) | [Console 实现 gate](../docs/platforms/README.md#console-实现-gate) + 各平台 Open PoC/GA gate | 有 |
| `clients/mobile/shared/` | 手机 | iOS+Android 共享决策与 parity 落位（B3 批迁入 mobile-parity-matrix、mobile-platform-decision-log） | `planned` | Lane-CON | [mobile/shared/README.md](mobile/shared/README.md)；[移动决策现址](../docs/platforms/mobile-platform-decision-log.md) | 同上 | 有 |
| `clients/mobile/ios/` | 手机（iPhone） | iPhone-only 产品文档/计划/保留实现入口（B3 批迁入 ios-product-design） | `planned`；implementation `not-implemented`；evidence `none` | Lane-CON | [iPhone 产品设计现址](../docs/platforms/ios-product-design.md) | Console 实现 gate + iPhone Open PoC/GA gate | 有 |
...
| `clients/mobile/android/` | 手机（Android phone） | Android phone 产品文档/计划/保留实现入口（B3 批迁入 android-product-design） | `planned`；implementation `not-implemented`；evidence `none` | Lane-CON | [Android 产品设计现址](../docs/platforms/android-product-design.md) | Console 实现 gate + Android Open PoC/GA gate | 有 |
...
| `packages/sdk-ts/` | 共享 | PC/未来手机消费的 TypeScript SDK；**不迁移**（Lane-TSC 所有） | `blocked`；仅有 M0 skeleton，M5 transport/watch/credential 功能 `not-implemented`；conformance `not-run` | Lane-TSC | [package.json](../packages/sdk-ts/package.json)；[源码入口](../packages/sdk-ts/src/index.ts) | [Lane-TSC gate](../docs/plan/PARALLEL-LANES.md)；M5 集成 | 缺（待 Lane-TSC 补） |
| `packages/contracts-ts/` | 共享 | TypeScript 机器契约、canonical 编码、digest、projection 与生成绑定；**不迁移**（Lane-CTR 所有） | 实现已提供（包级合同能力）；不作 REQ 级实现声明；76 向量 `not-run`；Profile `not implemented` | Lane-CTR | [API barrel](../packages/contracts-ts/src/index.ts)；[代码生成 ADR](../docs/adr/0006-code-generation-policy.md) | Lane-CTR 契约流程；F-003 仍待 Lane-CFR runner 真实执行负例 | 缺（待 Lane-CTR 补） |
...
| `clients/READINESS.md` | 共享 | structure/implementation 双 readiness 判定 | structure 迁移进行中；implementation `blocked` | Lane-CON | [READINESS.md](READINESS.md) | 真实 gate 证据 | 单文件（自述） |
...
## 6. Agent Hub（`clients/agent-hub/`，B5 批迁入）

Agent Hub canonical 树当前仍在迁移前位置；下表在 B5 批迁移落地时同批改写为 `clients/agent-hub/**` 路径。产品/架构文档定义事实；source ledger 记录外部来源；template 只是格式；plan/prompt 只编排受阻断工作，四者不得互相冒充。

| 路径 | 平台 | 角色 | 当前状态 | owner | canonical 入口 | 上游 gate | README / 目录说明 |
|---|---|---|---|---|---|---|---|
| `clients/agent-hub/` | PC + 手机 | Agent Hub 项目薄入口（docs/plan/prompts 三子树 B5 迁入） | `planned`；implementation `not-implemented`；Open PoC `not-run`；Profile `not implemented` | Lane-CON + Lane-DOC（治理） | [agent-hub/README.md](agent-hub/README.md) | [Agent Hub gate](../apps/cognitiveos-console/docs/agent-hub/GOVERNANCE.md#7-实现-gate不可跳过) | 有 |
| `apps/cognitiveos-console/docs/agent-hub/` | PC + 手机 | Agent Hub canonical 产品/架构/安全/研究根（现址；B5 迁至 `clients/agent-hub/docs/`） | `planned`；implementation `not-implemented`；Open PoC `not-run`；Profile `not implemented` | Lane-CON + Lane-DOC（治理） | [Agent Hub README](../apps/cognitiveos-console/docs/agent-hub/README.md)；[GOVERNANCE](../apps/cognitiveos-console/docs/agent-hub/GOVERNANCE.md) | Agent Hub gate + Paseo/AGPL 法务 gate + 一手 provider 接口核验 | 有 |
| `docs/plan/agent-hub/` 与 [Master plan](../docs/plan/agent-hub-development-plan.md) | 共享 | 里程碑、DAG、风险、证据与各车道计划（现址；B5 迁至 `clients/agent-hub/plan/`） | 全部实现车道 `blocked` | Lane-CON | [plan README](../docs/plan/agent-hub/README.md) | Agent Hub 六类 gate | 有 |
| `docs/prompts/agent-hub/` | 共享 | 12 个宏车道与 6 个 Adapter 自包含提示词（现址；B5 迁至 `clients/agent-hub/prompts/`） | `blocked`；不得在 gate 前启动编码或 mock | Lane-CON | [prompt README](../docs/prompts/agent-hub/README.md) | 对应 plan/gate 解阻后方可执行 | 有 |
...
4. [平台设计 README](../docs/platforms/README.md) 与目标平台设计/parity。
...
2. [iPhone](../docs/platforms/ios-product-design.md) 或 [Android](../docs/platforms/android-product-design.md) 产品设计；
3. [移动 canonical 决策](../docs/platforms/mobile-platform-decision-log.md) 与 [移动 parity](../docs/platforms/mobile-parity-matrix.md)；
4. [Agent Hub 平台范围](../apps/cognitiveos-console/docs/agent-hub/platforms/product-scope.md) 与 [PC/手机旅程](../apps/cognitiveos-console/docs/agent-hub/product/journeys-and-screens.md)。
