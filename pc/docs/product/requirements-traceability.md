...
Profile 声明不属于 Evidence 档位，单独使用 `planned/experimental/implemented/unsupported`。Windows v1 当前 Profile 为 `planned`；Console `not-implemented`；全局 84 vectors 中 46 `pass` / 38 `not-run`，但产品级端到端证据仍为 `none`。
...
- Effect table 的 schema-version 冲突以及 D-001/D-006 `$id` 策略均已在 M1 闭合；行为证据状态以 PROGRESS/runner 为准。
- 影响：`BLK-001` 不再阻断 Console；剩余客户端风险由当前 M5/平台 gate 与其他开放 blocker 约束。
...
### `CONSOLE-V2-BLK-013` Governed object 双轨（已关闭）

- `docs/traceability/findings-ledger.md` 的 F-003 已由 M1 runner 负例执行闭合；该合同状态不代表 Console 实现已提供。
- runner 负例复验、codegen 对齐与 legacy `$defs` 保留/零引用决策均已完成。
- 影响：不再阻断 M1 或 M5 入口；客户端仍不得把合同闭合误写为 Console 实现或平台证据。
...
|---|---|---|---|---|
| `001` | `UNASSIGNED — Console` | Agent/local cache 伪造 commit/完成不能改变系统状态或启用动作 | future `CONSOLE-AUTHORITY-UI-001` (`none`) | `BLK-011` |
...
| `005` | `UNASSIGNED — Console` | Agent 文本/local cache 声称完成时 UI 保持 authority 状态 | future `CONSOLE-AUTHORITY-UI-002` (`none`) | `BLK-011` |
...
| `007` | `UNASSIGNED — Runtime/Console` | 五登记状态域独立显示且非法聚合状态被拒绝 | future `CONSOLE-STATE-TRACKS-001` (`none`) | `BLK-011` |
| `008` | `UNASSIGNED — Task/Console` | 缺 current Verification 或 AcceptanceDecision 时禁止显示 COMPLETED | `conformance/vectors/intent-acceptance-007.json` (`not-run`) | `BLK-011` |
...
| `011` | `UNASSIGNED — Contracts/Console` | ambiguous/stale/not-found 未固定强引用前不能提交 | `conformance/vectors/shell-target-ambiguity-001.json`（以 PROGRESS 为准） | `BLK-002/011` |
| `012` | `UNASSIGNED — Cache/Console` | revocation/tenant/channel/version 变化原子失效相关 cache/watch/preview | `context-revocation-cache-reuse` 已由 M3 kernel/cache 行为执行 `pass`；Console E2E 仍 `none` | `BLK-006/007` |
...
| `019` | `UNASSIGNED — Protocol/Console` | unknown critical extension/schema/security floor 时能力 fail closed | `conformance/vectors/schema-version-001.json`（以 PROGRESS 为准） | `BLK-003/010/011` |
...
| `024` | `UNASSIGNED — Effect/Console` | unknown 时换 key/盲重试拒绝，原 binding 对账；NOT_EXECUTED 不进入 commit | `effect-unknown-outcome` / `effect-idempotency-conflict` 已由 M4 行为执行 `pass`；Console E2E 仍 `none` | `BLK-012` |
