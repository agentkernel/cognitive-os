...
>
> **警告：本轮未取得 Hermes Agent 的官方接口一手事实。** 下列条目为待核验占位，须在 [../../sources/provider-interfaces-ledger.md](../../sources/provider-interfaces-ledger.md) 用官方仓库/文档 + 查询日 + version/commit 补齐后方可声明任何支持。禁止臆造接口、session 格式、条款或许可。
...
- gate：Hermes Adapter 在接口事实核验前保持 `blocked`；不得以占位内容启动实现。

## 产品映射

- 在接口核验前，Hermes 至多按 launch-only / observe-only 处理，不承诺高层级接管。
