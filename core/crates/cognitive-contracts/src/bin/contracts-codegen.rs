...
//! `core/specs/schemas/`, plus the error registry `core/specs/registry/errors.yaml`,
//! and emits committed, reviewable language bindings:
//!
//! - Rust modules under `core/crates/cognitive-contracts/src/generated/`
//! - TypeScript modules under `core/packages/contracts-ts/src/generated/`
//!
...
///   state machine, core/specs/transitions/task.transitions.json)
...
fn run() -> Result<(), DynError> {
    ...
    // Error registry binding (core/specs/registry/errors.yaml -> both languages).
...
// ---------------------------------------------------------------------------
// Error registry binding (core/specs/registry/errors.yaml)
// ---------------------------------------------------------------------------
...
/// `core/specs/registry/errors.yaml`. The registry digest is the spec-set
...
fn render_error_registry_rust(entries: &[RegistryEntry], registry_digest: &str) -> String {
    ...
    let _ = writeln!(out, "//! source: core/specs/registry/errors.yaml");
    let _ = writeln!(
        out,
    ...
    let _ = writeln!(
        out,
        "pub const REGISTRY_PATH: &str = \"core/specs/registry/errors.yaml\";"
    );
    let _ = writeln!(out);
    let _ = writeln!(
        out,
...
fn render_error_registry_ts(entries: &[RegistryEntry], registry_digest: &str) -> String {
    ...
    let _ = writeln!(
        out,
        "export const REGISTRY_PATH = \"core/specs/registry/errors.yaml\";"
    );
    let _ = writeln!(out);
    let _ = writeln!(out, "/**");
    let _ = writeln!(
...
impl Module {
    fn render_rust(&self, source_file: &str, digest: &str) -> String {
        ...
        let _ = writeln!(out, "//! source: core/specs/schemas/{source_file}");
        let _ = writeln!(
            out,
...
fn render_rust_mod_rs(modules: &[(String, String)], digests: &[(String, String)]) -> String {
    ...
    let _ = writeln!(
        out,
        "//! errors.yaml registry binding (ADR-0006; sources core/specs/schemas/,"
    );
    let _ = writeln!(out, "//! core/specs/registry/errors.yaml).");
    let _ = writeln!(out);
