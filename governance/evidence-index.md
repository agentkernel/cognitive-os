...
>
> 规则：证据口径以 [conformance-evidence](https://github.com/agentkernel/cognitive-os/blob/main/docs/standards/conformance-evidence.md) 为准；证据文件不入库、以 digest 引用。本索引只登记指针；**当前全部客户端证据状态为 `none` / `not-run`**，无一例外。
>
...
| conformance 向量（全局，非客户端专属） | [PROGRESS 向量分层计数](https://github.com/agentkernel/cognitive-os/blob/main/docs/plan/PROGRESS.md) | 计数以 PROGRESS 实测为准（2026-07-26 快照：85 份 60 `pass` / 25 `not-run`），本文不硬编码；不构成客户端平台证据 |
| 客户端端到端测试（tests/e2e 占位） | [tests/e2e](https://github.com/agentkernel/cognitive-os/blob/main/tests/e2e/README.md) | 占位（M4/M5）；未执行 |
