...
>
> 规则：每域有且只有一个 canonical 文件；其他文档只链接不复制（[GOVERNANCE §1](../GOVERNANCE.md#1-canonical-唯一性)）。迁移已完成，"现址"列只登记当前 canonical。
...
| 共用测试策略 / 遥测脱敏留存政策 | `clients/shared/docs/test-strategy.md`、`.../telemetry-evidence/telemetry-redaction-retention-policy.md` | [test strategy](../shared/docs/test-strategy.md) / [telemetry policy](../shared/docs/telemetry-evidence/telemetry-redaction-retention-policy.md) | B4（已落位） |
