...
| 路径 | 内容 |
|---|---|
| [core/docs/architecture/CognitiveOS-Architecture.md](core/docs/architecture/CognitiveOS-Architecture.md) | 白皮书 v1.0.2（informative，非机器合同） |
| [core/docs/architecture/RFC-0001-cognitiveos-governance-context-access.md](core/docs/architecture/RFC-0001-cognitiveos-governance-context-access.md) | 治理/Context/访问 normative companion RFC v0.2 |
| [core/specs/](core/specs) | companion 规范、registry、状态迁移表与 JSON Schema（draft 2020-12）；具体数量由一致性检查报告 |
| [core/docs/architecture/CognitiveOS-Review-Conclusions.md](core/docs/architecture/CognitiveOS-Review-Conclusions.md) · [core/docs/architecture/CognitiveOS-Architecture-Independent-Review.md](core/docs/architecture/CognitiveOS-Architecture-Independent-Review.md) | 两轮评审（V1–V17 / IMP-01~18 / F-001~F-030） |
...
| [apps/kernel-server](personal/apps/kernel-server) · [apps/admin-cli](personal/apps/admin-cli) | 单节点组合根 · 确定性管理 CLI |
| [apps/agent-shell](personal/apps/agent-shell) | Personal Task Shell 的共享客户端/会话核心（非 authority；供 Pi-hosted Shell 等入口复用） |
| [packages/pi-cognitiveos](personal/packages/pi-cognitiveos) | Pi-hosted Agent Shell Extension；Provider 流量经 daemon，Pi 工具 default-deny |
| [apps/pi-agent-adapter](personal/apps/pi-agent-adapter) | Pi compatibility/candidate adapter；不是 managed Pi lifecycle authority |
...
| [clients/legacy/cognitiveos-console/](clients/legacy/cognitiveos-console) | 兼容 stub（Console 产品文档正文迁至 [clients/pc/](https://github.com/agentkernel/cognitiveos-clients/tree/main/pc)，`planned`） |
...
| 路径 | 内容 |
|---|---|
| [core/conformance/](core/conformance) | 声明式规范向量与测试层定义；执行结果必须由 runner/evidence 报告，不由 README 固定计数 |
| [crates/cognitive-conformance](personal/crates/cognitive-conformance) | 符合性 runner；局部通过不自动构成 Personal Gate 或 Profile 结论 |
...
| [docs/plan/plan.md](docs/plan/plan.md) | 研究与任务卡细节；不是当前状态源 |
| [docs/README.md](docs/README.md) | 文档地图与分类口径 |
| [docs/research/](docs/research/README.md) | owner 分析归档；不创建任务、Gate 或当前状态 |
| [personal/docs/product/](personal/docs/product) | 愿景、资源模型、Linux 1.0 范围和用户旅程（不拥有当前状态） |
| [core/docs/architecture/](core/docs/architecture) | CognitiveOS 白皮书、RFC-0001、冻结评审 |
