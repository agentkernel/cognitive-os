...
const frontmatterSchema = JSON.parse(
  readFileSync(path.join(repoRoot, "personal", "handbook", "_meta", "handbook-frontmatter.schema.json"), "utf8"),
);
...
function greenFixture() {
  ...
  const frontmatter = {
    ...
    contracts: ["core/specs/schemas/demo.schema.json"],
  ...
  const pages = new Map([
    ["personal/handbook/en/user/demo.md", page("en")],
    ["personal/handbook/zh-CN/user/demo.md", page("zh-CN")],
    ["personal/handbook/README.md", { frontmatter: null, frontmatterError: null, body: "root nav", raw: "" }],
  ]);
  const trackedPaths = [
    "personal/handbook/README.md",
    "personal/handbook/en/user/demo.md",
    "personal/handbook/zh-CN/user/demo.md",
    "personal/handbook/_meta/manifest.json",
    "crates/demo/src/lib.rs",
    "crates/demo/tests/demo.rs",
    "core/specs/schemas/demo.schema.json",
    "Cargo.lock",
  ];
  return {
    manifest: {
      root_entry: "personal/handbook/README.md",
      locales: ["en", "zh-CN"],
      locale_roots: { en: "personal/handbook/en", "zh-CN": "personal/handbook/zh-CN" },
      documents: [
    ...
    coverage: {
      rules: [
        { glob: "personal/handbook/**", category: "handbook", docs: ["user.demo"] },
        { glob: "crates/**", category: "first-party-source", docs: ["user.demo"] },
        { glob: "core/specs/**", category: "first-party-source", docs: ["user.demo"] },
    ...
    handbookFiles: ["personal/handbook/README.md", "personal/handbook/en/user/demo.md", "personal/handbook/zh-CN/user/demo.md"],
  };
}
...
test("missing source path fails HB006", () => {
  ...
  fixture.pages.get("personal/handbook/en/user/demo.md").frontmatter.sources = [{ path: "crates/demo/src/missing.rs" }];
  assert.ok(rulesOf(runHandbookChecks(fixture)).includes("HB006"));
});

test("stale fingerprint fails HB008 with drift wording", () => {
  const fixture = greenFixture();
  const fm = fixture.pages.get("personal/handbook/en/user/demo.md").frontmatter;
...
test("broken relative link fails HB005", () => {
  const fixture = greenFixture();
  fixture.pages.get("personal/handbook/en/user/demo.md").body = "[dead](../../does/not/exist.md)";
...
test("link into History/ fails HB005", () => {
  ...
  fixture.pages.get("personal/handbook/en/user/demo.md").body = "[old](../../../History/old.md)";
  const diagnostics = runHandbookChecks(fixture);
...
test("illegal capability status fails schema validation (HB002)", () => {
  const fixture = greenFixture();
  fixture.pages.get("personal/handbook/en/user/demo.md").frontmatter.status = "done";
  assert.ok(rulesOf(runHandbookChecks(fixture)).includes("HB002"));
});

test("missing locale twin fails HB003", () => {
  const fixture = greenFixture();
  fixture.pages.delete("personal/handbook/zh-CN/user/demo.md");
  fixture.handbookFiles = fixture.handbookFiles.filter((p) => !p.startsWith("personal/handbook/zh-CN/"));
  const diagnostics = runHandbookChecks(fixture);
...
test("generated page drift fails HB010", () => {
  ...
  fixture.pages.set("personal/handbook/en/reference/demo.md", generatedPage);
  fixture.pages.set("personal/handbook/zh-CN/reference/demo.md", { ...generatedPage, frontmatter: { ...generatedPage.frontmatter, locale: "zh-CN" } });
  fixture.trackedPaths.push("personal/handbook/en/reference/demo.md", "personal/handbook/zh-CN/reference/demo.md");
  fixture.handbookFiles.push("personal/handbook/en/reference/demo.md", "personal/handbook/zh-CN/reference/demo.md");
  fixture.generatedOutputs = new Map([
    ["personal/handbook/en/reference/demo.md", "generator body"],
    ["personal/handbook/zh-CN/reference/demo.md", "generator body"],
  ]);
  const diagnostics = runHandbookChecks(fixture);
...
test("secret-shaped content fails HB012", () => {
  const fixture = greenFixture();
  fixture.pages.get("personal/handbook/en/user/demo.md").body = "example key sk-abcdefghijklmnopqrstuv";
  const diagnostics = runHandbookChecks(fixture);
...
test("copied dynamic Gate status fails HB012", () => {
  const fixture = greenFixture();
  fixture.pages.get("personal/handbook/en/user/demo.md").body = "| B08 Memory + Skill Gate | **pass** |";
  const diagnostics = runHandbookChecks(fixture);
...
test("unmanifested handbook markdown fails HB004", () => {
  const fixture = greenFixture();
  fixture.handbookFiles.push("personal/handbook/en/user/orphan.md");
  fixture.pages.set("personal/handbook/en/user/orphan.md", { frontmatter: null, frontmatterError: null, body: "", raw: "" });
  fixture.trackedPaths.push("personal/handbook/en/user/orphan.md");
  const diagnostics = runHandbookChecks(fixture);
  assert.ok(diagnostics.some((d) => d.rule === "HB004" && d.file === "personal/handbook/en/user/orphan.md"));
});
...
test("glob compiler handles ** and * boundaries", () => {
  ...
  assert.ok(compileGlob("personal/apps/kernel-server/src/personal/*doctor*.rs").test("personal/apps/kernel-server/src/personal/six_resource_doctor.rs"));
});
