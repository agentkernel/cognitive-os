/**
 * Standalone handbook consistency checker (CI gate; see personal/handbook/_meta/).
 *
...
function main() {
  ...
  if (!existsSync(repoPath("personal", "handbook", "_meta", "manifest.json"))) {
    console.error("check-handbook: personal/handbook/_meta/manifest.json not found");
    process.exit(1);
  }

  const manifest = JSON.parse(readRepoText("personal/handbook/_meta/manifest.json"));
  const coverage = JSON.parse(readRepoText("personal/handbook/_meta/source-coverage.json"));
  const sourceMap = JSON.parse(readRepoText("personal/handbook/_meta/source-map.json"));
  const frontmatterSchema = JSON.parse(readRepoText("personal/handbook/_meta/handbook-frontmatter.schema.json"));
  ...
  const handbookFiles = trackedPaths.filter((p) => p.startsWith("personal/handbook/"));
  ...
  try {
    const sourceSet = JSON.parse(readRepoText("personal/handbook/_meta/source-set.json"));
    const revision = sourceSet.implementation_baseline_revision;
    if (!/^[0-9a-f]{40}$/.test(revision ?? "")) {
      diagnostics.push({ rule: "HB013", file: "personal/handbook/_meta/source-set.json", message: "implementation_baseline_revision must be a full 40-hex commit" });
      ...
      for (const line of lsTree.split("\n")) {
        ...
        if (
          filePath.startsWith("personal/handbook/") ||
          filePath === "llms.txt" ||
      ...
      if (digest !== sourceSet.digest) {
        diagnostics.push({ rule: "HB013", file: "personal/handbook/_meta/source-set.json", message: `source-set digest is not reproducible from ${revision.slice(0, 12)} (recorded ${sourceSet.digest?.slice(0, 18)}…, computed ${digest.slice(0, 18)}…)` });
      }
    }
  } catch (err) {
    diagnostics.push({ rule: "HB013", file: "personal/handbook/_meta/source-set.json", message: `unreadable or unverifiable: ${err.message}` });
  }

  // ---- HB014 (optional): legacy docs unchanged except the allowlist --------------
  if (diffBase) {
    try {
      const allowlist = JSON.parse(readRepoText("personal/handbook/_meta/legacy-change-allowlist.json"));
      const allowed = new Set(allowlist.allowed_paths ?? []);
      const changed = git("diff", "--name-only", `${diffBase}...HEAD`).split("\n").filter(Boolean);
      for (const changedPath of changed) {
        const isNewSurface =
          changedPath.startsWith("personal/handbook/") ||
      ...
      diagnostics.push({ rule: "HB014", file: "personal/handbook/_meta/legacy-change-allowlist.json", message: `diff-base verification failed: ${err.message}` });
    }
  }
